...
/* Native Harbour types */
      ...
      typedef unsigned __int64   HB_ULONGLONG; // NOTE: HB_ULONGLONG is deprecated in Harbour++ source code
   #else
      typedef signed long long   HB_LONGLONG; // NOTE: HB_LONGLONG is deprecated in Harbour++ source code
      typedef unsigned long long HB_ULONGLONG; // NOTE: HB_ULONGLONG is deprecated in Harbour++ source code
   #endif
...
#  else
      typedef uint64_t        HB_SIZE;        /* TODO: Currently 'unsigned', to be changed 'signed' */
#  endif
   typedef int64_t         HB_ISIZ;           /* TODO: Change to HB_SIZE, after HB_SIZE has been converted to signed type. TEMPORARY type. */
   typedef uint64_t        HB_USIZ;           /* TEMPORARY type. Do not use it. */
#else
#  if defined(HB_SIZE_SIGNED)
...
#endif
typedef uint32_t HB_TYPE;

/* type of file attributes */
typedef uint32_t HB_FATTR;

/* type of reference counter */
#if defined(HB_OS_WIN_64)
   typedef uint64_t    HB_COUNTER;
...
/* type for memory pointer diff */
   ...
   typedef uint64_t HB_PTRUINT; // NOTE: HB_PTRUINT deprecated in core code
#else
...
#define HB_SWAP_UINT16( w )     ( ( uint16_t ) ( ( ( ( uint16_t ) ( w ) & 0xFF00 ) >> 8 ) | \
                                               ( ( ( uint16_t ) ( w ) & 0x00FF ) << 8 ) ) )
#define HB_SWAP_UINT32( w )     ( ( uint32_t ) ( ( ( ( uint32_t ) ( w ) & 0x000000FF ) << 24 ) | \
                                               ( ( ( uint32_t ) ( w ) & 0x0000FF00 ) <<  8 ) | \
                                               ( ( ( uint32_t ) ( w ) & 0x00FF0000 ) >>  8 ) | \
...
#endif
#if defined(HB_BIG_ENDIAN)
#  define   HB_PUT_UINT32( p, v )   HB_PUT_BE_UINT32( p, ( uint32_t ) ( v ) )
#  define   HB_GET_UINT32( p )      HB_GET_BE_UINT32( p )
#else
#  define   HB_PUT_UINT32( p, v )   HB_PUT_LE_UINT32( p, ( uint32_t ) ( v ) )
...
#  endif
   ...
   static HB_FORCEINLINE uint16_t _hb_get_std_uint16( const uint8_t * buf )
   {
      HB_U16CAST u;
   ...
   static HB_FORCEINLINE void _hb_put_std_uint16( uint8_t * buf, uint16_t val )
   {
      HB_U16CAST u;
      u.val = val;
   ...
   static HB_FORCEINLINE uint16_t _hb_get_rev_uint16( const uint8_t * buf )
   {
      HB_U16CAST u;
      u.buf[ 0 ] = buf[ 1 ];
      u.buf[ 1 ] = buf[ 0 ];
   ...
   static HB_FORCEINLINE void _hb_put_rev_uint16( uint8_t * buf, uint16_t val )
   {
      HB_U16CAST u;
      u.val = val;
      buf[ 0 ] = u.buf[ 1 ];
   ...
   static HB_FORCEINLINE uint32_t _hb_get_std_uint32( const uint8_t * buf )
   {
      HB_U32CAST u;
      memcpy( u.buf, buf, sizeof( u.buf ) );
      return u.val;
   ...
   static HB_FORCEINLINE void _hb_put_std_uint32( uint8_t * buf, uint32_t val )
   {
      HB_U32CAST u;
      u.val = val;
      memcpy( buf, u.buf, sizeof( u.buf ) );
   ...
   static HB_FORCEINLINE uint32_t _hb_get_rev_uint32( const uint8_t * buf )
   {
      HB_U32CAST u;
...
#  endif
   ...
   static HB_FORCEINLINE void _hb_put_rev_uint32( uint8_t * buf, uint32_t val )
   {
      HB_U32CAST u;
#  if defined(HB_BUILTIN_BSWAP32)
...
#  if !defined(HB_STRICT_ALIGNMENT) && defined(HB_LITTLE_ENDIAN)
   ...
   #define HB_GET_LE_UINT16( p )    ( *( const uint16_t * )( p ) )
...
#  else
   ...
   #define HB_GET_LE_UINT16( p )    ( ( uint16_t ) \
                                      ( ( ( uint16_t ) (( const uint8_t * )( p ))[ 0 ] ) | \
                                        ( ( uint16_t ) (( const uint8_t * )( p ))[ 1 ] <<  8 ) ) )
   #define HB_GET_LE_UINT32( p )    ( ( uint32_t ) \
...
#  if !defined(HB_STRICT_ALIGNMENT) && defined(HB_BIG_ENDIAN)
   ...
   #define HB_GET_BE_UINT16( p )    ( *( const uint16_t * )( p ) )
...
#  else
   ...
   #define HB_GET_BE_UINT16( p )    ( ( uint16_t ) \
                                      ( ( ( uint16_t ) (( const uint8_t * )( p ))[ 0 ] << 8 ) | \
                                        ( ( uint16_t ) (( const uint8_t * )( p ))[ 1 ] ) ) )
   #define HB_GET_BE_UINT32( p )    ( ( uint32_t ) \
