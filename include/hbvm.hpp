...
extern HB_EXPORT void hb_vmSymbolInit_RT(void);
...
extern HB_EXPORT void hb_vmPushNumInt(HB_MAXINT nNumber);
// pushes a logical value onto the stack
extern HB_EXPORT void hb_vmPushLogical(HB_BOOL bValue);
// pushes a string on to the stack
extern HB_EXPORT void hb_vmPushString(const char *szText, HB_SIZE length);
// pushes a string from pcode on to the stack
extern HB_EXPORT void hb_vmPushStringPcode(const char *szText, HB_SIZE length);
// pushes a long date onto the stack
extern HB_EXPORT void hb_vmPushDate(long lDate);
...
extern HB_EXPORT void hb_vmPushPointer(void *pPointer);
...
extern HB_EXPORT void hb_vmLock(void);
// lock VM blocking GC execution by other threads, ignore GC request
extern HB_EXPORT void hb_vmLockForce(void);
// unlock VM, allow GC execution
extern HB_EXPORT void hb_vmUnlock(void);
#ifdef _HB_API_INTERNAL_
// (try to) stop all threads except current one
extern HB_EXPORT HB_BOOL hb_vmSuspendThreads(HB_BOOL fWait);
// unblock execution of threads stopped by hb_vmSuspendThreads()
extern HB_EXPORT void hb_vmResumeThreads(void);
#endif
// Register new thread without local thread HVM stack
extern HB_EXPORT HB_BOOL hb_vmThreadRegister(void *);
// Remove registered thread which does not have local thread HVM stack yet
extern HB_EXPORT void hb_vmThreadRelease(void *);
// allocate local thread HVM stack
extern HB_EXPORT void hb_vmThreadInit(void *);
// destroy local thread HVM stack
extern HB_EXPORT void hb_vmThreadQuit(void);
// send QUIT request to given thread
extern HB_EXPORT void hb_vmThreadQuitRequest(void *);
// wait for all threads to terminate can be called only by main HVM thread
extern HB_EXPORT void hb_vmWaitForThreads(void);
// send QUIT request to all threads except current one and wait for their termination, should be called only by main HVM thread
extern HB_EXPORT void hb_vmTerminateThreads(void);
