...
const PHB_EXPR_FUNC hb_comp_ExprTable[HB_EXPR_COUNT] = {
    hb_compExprUseDummy,      hb_compExprUseNil,       hb_compExprUseNumeric,
    hb_compExprUseDate,       hb_compExprUseTimeStamp, hb_compExprUseString,
    hb_compExprUseCodeblock,  hb_compExprUseLogical,   hb_compExprUseSelf,
    hb_compExprUseArray,      hb_compExprUseHash,      hb_compExprUseFunRef,
    hb_compExprUseVarRef,     hb_compExprUseRef,       hb_compExprUseIIF,
    hb_compExprUseList,       hb_compExprUseArgList,   hb_compExprUseMacroArgList,
    hb_compExprUseArrayAt,    hb_compExprUseMacro,     hb_compExprUseFunCall,
    hb_compExprUseAliasVar,   hb_compExprUseAliasExpr, hb_compExprUseSetGet,
    hb_compExprUseSend,       hb_compExprUseFunName,   hb_compExprUseAlias,
    hb_compExprUseRTVariable, hb_compExprUseVariable,  hb_compExprUsePostInc, /* post-operators -> lowest precedence */
    hb_compExprUsePostDec,    hb_compExprUseAssign,                           /* assignments */
    hb_compExprUsePlusEq,     hb_compExprUseMinusEq,   hb_compExprUseMultEq,
    hb_compExprUseDivEq,      hb_compExprUseModEq,     hb_compExprUseExpEq,
    hb_compExprUseOr,                                                       /* logical operators */
    hb_compExprUseAnd,        hb_compExprUseNot,       hb_compExprUseEqual, /* relational operators */
    hb_compExprUseEQ,         hb_compExprUseNE,        hb_compExprUseIN,
    hb_compExprUseLT,         hb_compExprUseGT,        hb_compExprUseLE,
    hb_compExprUseGE,         hb_compExprUsePlus, /* addition */
    hb_compExprUseMinus,      hb_compExprUseMult, /* multiple */
    hb_compExprUseDiv,        hb_compExprUseMod,       hb_compExprUsePower,
    hb_compExprUseNegate,                          /* sign operator */
    hb_compExprUsePreInc,     hb_compExprUsePreDec /* highest precedence */
};
...
static HB_EXPR_FUNC(hb_compExprUseTimeStamp)
{
  ...
  case HB_EA_LVALUE:
    ...
    break;
  case HB_EA_PUSH_PCODE:
    HB_GEN_FUNC2(PushTimeStamp, pSelf->value.asDate.lDate, pSelf->value.asDate.lTime);
    break;
  case HB_EA_POP_PCODE:
    break;
  case HB_EA_PUSH_POP:
...
static HB_EXPR_FUNC(hb_compExprUseString)
{
  ...
  case HB_EA_PUSH_PCODE:
    ...
    if (!HB_SUPPORT_MACROTEXT)
      HB_GEN_FUNC2(PushString, pSelf->value.asString.string, pSelf->nLength + 1);
    else
      hb_compPushMacroText(HB_COMP_PARAM, pSelf->value.asString.string, pSelf->nLength, HB_FALSE);
#else
    HB_GEN_FUNC2(PushString, pSelf->value.asString.string, pSelf->nLength + 1);
    if (hb_macroIsValidMacroText(pSelf->value.asString.string, pSelf->nLength))
    {
...
static HB_EXPR_FUNC(hb_compExprUseArray)
{
  ...
  case HB_EA_PUSH_PCODE: {
    uint16_t usItems = (uint16_t)hb_compExprParamListCheck(HB_COMP_PARAM, pSelf);
    ...
    else
    {
      ...
      if (!fArgsList)
      {
        ...
        HB_GEN_FUNC3(PCode3, HB_P_ARRAYGEN, HB_LOBYTE(usItems), HB_HIBYTE(usItems));
      }
      else
      {
        HB_GEN_FUNC3(PCode3, HB_P_MACROARRAYGEN, HB_LOBYTE(usItems), HB_HIBYTE(usItems));
      }
    }
    break;
  }
...
static HB_EXPR_FUNC(hb_compExprUseHash)
{
  ...
  case HB_EA_PUSH_PCODE: {
    uint16_t usItems = (uint16_t)(pSelf->nLength >> 1);
    /* NOTE: direct type change */
    pSelf->ExprType = HB_ET_ARGLIST;
    HB_EXPR_USE(pSelf, HB_EA_PUSH_PCODE);
    /* NOTE: restore original expression type */
...
static HB_EXPR_FUNC(hb_compExprUseRef)
{
  ...
  case HB_EA_PUSH_PCODE: {
    ...
    {
      if (pExp->value.asAlias.pVar->ExprType == HB_ET_VARIABLE && pExp->value.asAlias.pAlias->ExprType == HB_ET_ALIAS &&
    ...
    else if (pExp->ExprType == HB_ET_VARREF || pExp->ExprType == HB_ET_REFERENCE)
    {
      HB_EXPR_USE(pExp, HB_EA_PUSH_PCODE);
      break;
    }
...
#endif
      {
        hb_compErrorVParams(HB_COMP_PARAM, HB_COMP_PARAM->functions.pLast->szName ? "Function" : "Codeblock");
      }
...
static HB_EXPR_FUNC(hb_compExprUseMacroArgList)
{
  ...
  case HB_EA_PUSH_PCODE: {
    ...
    while (pExpr)
    {
      if ((pExpr->ExprType == HB_ET_MACRO && (pExpr->value.asMacro.SubType & HB_ET_MACRO_LIST)) ||
          (pExpr->ExprType == HB_ET_ARGLIST && pExpr->value.asList.reference) ||
          (pExpr->ExprType == HB_ET_FUNCALL && pExpr->value.asFunCall.pFunName->ExprType == HB_ET_FUNNAME &&
...
static HB_EXPR_FUNC(hb_compExprUseArrayAt)
{
  ...
  case HB_EA_REDUCE: {
    ...
    if (pSelf->value.asList.pExprList->ExprType == HB_ET_VARIABLE)
    {
      ...
      {
#endif
        pSelf->value.asList.pExprList = hb_compExprNewAliasVar(hb_compExprNewAlias("MEMVAR", HB_COMP_PARAM),
        ...
        else
        {
          ...
          if (pExpr) /* found ? */
          {
            ...
            else if( pExpr->ExprType == HB_ET_STRING && HB_SUPPORT_ARRSTR )   /* is it a literal string */
            {
               ...
               {
                  uint8_t ucValue = ( uint8_t ) pExpr->value.asString.string[ nIndex - 1 ];
  ...
  case HB_EA_PUSH_PCODE: {
    ...
    else if (pSelf->value.asList.pIndex->ExprType == HB_ET_ARGLIST)
    {
      fMacroIndex = pSelf->value.asList.pIndex->value.asList.reference;
             ...
             pSelf->value.asList.pIndex->value.asFunCall.pFunName->value.asSymbol.funcid == HB_F_ARRAYTOPARAMS)
    {
      pSelf->value.asList.pIndex->value.asFunCall.pFunName->value.asSymbol.flags |= HB_FN_MULTIARG;
      fMacroIndex = HB_TRUE;
    }
    if (pSelf->value.asList.reference && HB_SUPPORT_ARRSTR)
    {
      ...
      else if (pList->ExprType == HB_ET_ALIASVAR && pList->value.asAlias.pVar->ExprType == HB_ET_VARIABLE &&
      ...
      else if (pList->ExprType == HB_ET_ARRAYAT && !pList->value.asList.reference)
      {
        pList->value.asList.reference = HB_TRUE;
        HB_EXPR_USE(pList, HB_EA_PUSH_PCODE);
        pList->value.asList.reference = HB_FALSE;
      }
      else if (pList->ExprType == HB_ET_MACRO && pList->value.asMacro.SubType & HB_ET_MACRO_VAR)
      {
  ...
  case HB_EA_PUSH_PCODE:
    ...
    else
    {
      ...
      else
      {
         ...
         */
#if !defined(HB_MACRO_SUPPORT)
        hb_compPushMacroText(HB_COMP_PARAM, pSelf->value.asMacro.szMacro, strlen(pSelf->value.asMacro.szMacro),
                             HB_TRUE);
#else
    ...
    else
    {
      ...
      HB_GEN_FUNC1(PCode1, (uint8_t)HB_MACRO_GENFLAGS);
    }
    break;
...
static HB_EXPR_FUNC(hb_compExprUseFunCall)
{
    ...
    if (pSelf->value.asFunCall.pFunName->ExprType == HB_ET_FUNNAME)
    {
...
#endif
      {
        ...
        case HB_F_I18N_NGETTEXT_STRICT: {
          ...
          fStrict = funcID == HB_F_I18N_GETTEXT_STRICT || funcID == HB_F_I18N_NGETTEXT_STRICT;
          fNoop = funcID == HB_F_I18N_GETTEXT_NOOP || funcID == HB_F_I18N_NGETTEXT_NOOP;
          fPlural =
          ...
          if (fPlural && usCount)
          {
            ...
            pArg = pArg->pNext;
            --usCount;
            if (pCount->ExprType <= HB_ET_FUNREF && pCount->ExprType != HB_ET_NUMERIC)
            {
              ...
              if (pArg->ExprType == HB_ET_STRING)
              {
                if (HB_COMP_PARAM->fI18n && pArg->nLength > 0)
                {
                  if (pCount)
                  {
                    ...
                    hb_compI18nAddPlural(HB_COMP_PARAM, szPlurals, 1, szContext, HB_COMP_PARAM->currModule,
                                         HB_COMP_PARAM->currLine);
                  }
                  else
                    hb_compI18nAdd(HB_COMP_PARAM, pArg->value.asString.string, szContext, HB_COMP_PARAM->currModule,
                                   HB_COMP_PARAM->currLine);
                }
              }
              else if (pCount && pArg->ExprType == HB_ET_ARRAY && hb_compExprListTypeCheck(pArg, HB_ET_STRING))
              {
                if (HB_COMP_PARAM->fI18n)
                {
                  ...
                  hb_compI18nAddPlural(HB_COMP_PARAM, szPlurals, ulLen, szContext, HB_COMP_PARAM->currModule,
                                       HB_COMP_PARAM->currLine);
                }
              }
          ...
          if (iWarning != 0)
          {
            ...
            if (iWarning == HB_COMP_WARN_PARAM_TYPE)
              hb_compGenWarning(HB_COMP_PARAM, hb_comp_szWarnings, 'W', HB_COMP_WARN_PARAM_TYPE,
                                pBadParam && pBadParam->ExprType > HB_ET_NONE && pBadParam->ExprType <= HB_ET_FUNREF
                                    ? hb_compExprDescription(pBadParam)
                                    : "Unknown",
                                szExpect);
            else
            {
  ...
  case HB_EA_PUSH_PCODE: {
    ...
    if (pSelf->value.asFunCall.pParms)
    {
      usCount = (uint16_t)hb_compExprParamListCheck(HB_COMP_PARAM, pSelf->value.asFunCall.pParms);
    ...
    if (pSelf->value.asFunCall.pFunName->ExprType == HB_ET_FUNNAME)
    {
      if (!fArgsList)
      {
        if (pSelf->value.asFunCall.pFunName->value.asSymbol.funcid == HB_F_ARRAYTOPARAMS && usCount == 1 &&
        ...
        else if (pSelf->value.asFunCall.pFunName->value.asSymbol.funcid == HB_F_ARRAY && HB_SUPPORT_EXTOPT)
        {
          if (usCount)
            HB_EXPR_USE(pSelf->value.asFunCall.pParms, HB_EA_PUSH_PCODE);
                 ...
                 pSelf->value.asFunCall.pParms->value.asList.pExprList->ExprType == HB_ET_VARREF)
        {
          ...
          if (pFunc->wParamCount && (pFunc->funFlags & HB_FUNF_USES_LOCAL_PARAMS) != 0 && iScope == HB_VS_LOCAL_VAR &&
              (uint16_t)iVar <= pFunc->wParamCount)
          {
            HB_COMP_EXPR_FREE(pSelf->value.asFunCall.pParms);
            pSelf->value.asFunCall.pParms = hb_compExprNewLong(iVar, HB_COMP_PARAM);
          }
          else
            hb_compErrorRefer(HB_COMP_PARAM, NULL,
                              pSelf->value.asFunCall.pParms->value.asList.pExprList->value.asSymbol.name);
        }
#endif
      }
    ...
    if (fArgsList)
    {
      ...
      HB_GEN_FUNC2(PCode2, HB_P_FUNCTIONSHORT, (uint8_t)usCount);
    break;
  }
  case HB_EA_POP_PCODE:
    break;
  ...
  case HB_EA_STATEMENT: {
    ...
    if (fArgsList)
    {
      ...
      HB_GEN_FUNC2(PCode2, HB_P_DOSHORT, (uint8_t)usCount);
    break;
  }
  case HB_EA_DELETE:
    if (pSelf->value.asFunCall.pParms)
...
static HB_EXPR_FUNC(hb_compExprUseAliasVar)
{
  ...
  case HB_EA_REDUCE:
    ...
    if (pSelf->value.asAlias.pAlias->ExprType == HB_ET_LIST)
    {
      ...
      if (HB_SUPPORT_EXTOPT && pSelf->value.asAlias.pAlias->value.asList.pExprList->ExprType == HB_ET_STRING &&
          pSelf->value.asAlias.pAlias->value.asList.pExprList->pNext == NULL)
      {
        pSelf->value.asAlias.pAlias = hb_compExprReduceAliasString(
            pSelf->value.asAlias.pAlias, pSelf->value.asAlias.pAlias->value.asList.pExprList, HB_COMP_PARAM);
      }
    }
    else
  ...
  case HB_EA_PUSH_PCODE: {
    ...
    else if (pAlias->ExprType == HB_ET_ALIAS)
    {
       ...
       */
      HB_GEN_FUNC4(PushAliasedVar, pSelf->value.asAlias.pVar->value.asSymbol.name, HB_TRUE, pAlias->value.asSymbol.name,
                   0);
    }
    else if (pAlias->ExprType == HB_ET_NUMERIC)
    {
      ...
      if (pAlias->value.asNum.NumType == HB_ET_LONG)
        HB_GEN_FUNC4(PushAliasedVar, pSelf->value.asAlias.pVar->value.asSymbol.name, HB_TRUE, NULL,
                     pAlias->value.asNum.val.l);
      else
  ...
  case HB_EA_POP_PCODE: {
    ...
    else if (pAlias->ExprType == HB_ET_ALIAS)
    {
       ...
       * MEMVAR->var
       */
      HB_GEN_FUNC4(PopAliasedVar, pSelf->value.asAlias.pVar->value.asSymbol.name, HB_TRUE, pAlias->value.asSymbol.name,
                   0);
    }
    else if (pAlias->ExprType == HB_ET_NUMERIC)
    {
       ...
       */
      if (pAlias->value.asNum.NumType == HB_ET_LONG)
        HB_GEN_FUNC4(PopAliasedVar, pSelf->value.asAlias.pVar->value.asSymbol.name, HB_TRUE, NULL,
                     pAlias->value.asNum.val.l);
      else
        hb_compErrorAlias(HB_COMP_PARAM, pAlias);
    }
  ...
  case HB_EA_REDUCE:
    ...
    if (HB_SUPPORT_EXTOPT && pSelf->value.asAlias.pAlias->ExprType == HB_ET_STRING)
      pSelf->value.asAlias.pAlias =
          hb_compExprReduceAliasString(pSelf->value.asAlias.pAlias, pSelf->value.asAlias.pAlias, HB_COMP_PARAM);
    break;
...
static HB_EXPR_FUNC(hb_compExprUseFunName)
{
  ...
  case HB_EA_PUSH_PCODE:
    HB_GEN_FUNC2(PushFunSym, pSelf->value.asSymbol.name, pSelf->value.asSymbol.flags);
    break;
...
static HB_EXPR_FUNC(hb_compExprUseSend)
{
  ...
  case HB_EA_REDUCE:
    ...
    if (pSelf->value.asMessage.pObject && (HB_SUPPORT_HARBOUR || pSelf->nLength == 1))
  ...
  case HB_EA_PUSH_PCODE:
    if (pSelf->value.asMessage.pParms) /* Is it a method call ? */
    {
      ...
      if (fArgsList)
      {
        ...
        HB_GEN_FUNC2(PCode2, HB_P_SENDSHORT, (uint8_t)iParms);
    }
    else
    {
...
static HB_EXPR_FUNC(hb_compExprUseAssign)
{
  ...
  case HB_EA_REDUCE: {
    ...
    if (HB_SUPPORT_HARBOUR && pSelf->value.asOperator.pLeft->ExprType == HB_ET_VARIABLE &&
        (pExpr->ExprType == HB_EO_PLUS || pExpr->ExprType == HB_EO_MINUS || pExpr->ExprType == HB_EO_MULT ||
         pExpr->ExprType == HB_EO_DIV || pExpr->ExprType == HB_EO_MOD || pExpr->ExprType == HB_EO_POWER) &&
...
static HB_EXPR_FUNC(hb_compExprUsePlus)
{
  ...
  case HB_EA_PUSH_PCODE:
    if (HB_SUPPORT_EXTOPT)
    {
      ...
      {
        if (pLeft->value.asNum.NumType == HB_ET_LONG ? pLeft->value.asNum.val.l == 1 : pLeft->value.asNum.val.d == 1)
        {
          HB_EXPR_USE(pRight, HB_EA_PUSH_PCODE);
          HB_GEN_FUNC1(PCode1, HB_P_INC);
          break;
        }
        else if (pLeft->value.asNum.NumType == HB_ET_LONG ? pLeft->value.asNum.val.l == -1
      ...
      else if (pRight->ExprType == HB_ET_NUMERIC)
      {
        ...
        else if (pRight->value.asNum.NumType == HB_ET_LONG ? pRight->value.asNum.val.l == -1
...
#endif
{
  ...
  HB_PCODE_DATA->fVParams = (pSelf->value.asCodeblock.flags & HB_BLOCK_VPARAMS) != 0;
#else
  hb_compCodeBlockStart(HB_COMP_PARAM, iEarlyEvalPass);
  HB_COMP_PARAM->functions.pLast->fVParams = (pSelf->value.asCodeblock.flags & HB_BLOCK_VPARAMS) != 0;
  ...
  while (pExpr)
  {
    ...
    if ((pSelf->value.asCodeblock.flags & HB_BLOCK_REDUCE) != 0 || HB_SUPPORT_HARBOUR)
    {
...
static void hb_compExprCodeblockEarly(PHB_EXPR pSelf, HB_COMP_DECL)
{
  ...
  pExpr = pSelf->value.asCodeblock.pExprList;
  if (pExpr->ExprType == HB_ET_MACRO && pExpr->value.asMacro.cMacroOp && pExpr->pNext == NULL)
  {
    /* simple macro variable expansion: &variable
     * 'szMacro' is a variable name
     * {|| &variable} => &( '{||' + variable +'}' )
    ...
    pNew = hb_compExprSetOperand(hb_compExprNewPlus(pNew, HB_COMP_PARAM),
                                 hb_compExprNewString("}", 1, HB_FALSE, HB_COMP_PARAM), HB_COMP_PARAM);
  ...
  else
  {
    ...
    if (!hb_compExprCodeblockPush(pSelf, 1, HB_COMP_PARAM))
    {
      ...
      pExpr = hb_compExprNewMacro(
          hb_compExprNewString(pSelf->value.asCodeblock.string, pSelf->nLength, HB_FALSE, HB_COMP_PARAM), 0, NULL,
          HB_COMP_PARAM);
      HB_EXPR_USE(pExpr, HB_EA_PUSH_PCODE);
      HB_COMP_EXPR_FREE(pExpr);
...
static void hb_compExprPushOperEq(PHB_EXPR pSelf, uint8_t bOpEq, HB_COMP_DECL)
{
...
#endif
    {
      hb_compExprPushSendPopPush(pSelf->value.asOperator.pLeft, pSelf->value.asOperator.pRight, HB_FALSE, bOpEq,
                                 HB_COMP_PARAM);
    }
    return;
  }
  else if (bOpEq != bNewOp)
  {
    ...
    {
      uint16_t usType = pSelf->value.asOperator.pLeft->value.asMacro.SubType;
    ...
    else if (pSelf->value.asOperator.pLeft->ExprType == HB_ET_VARIABLE)
    {
      ...
      {
        if (iScope == HB_VS_LOCAL_VAR && pSelf->value.asOperator.pRight->ExprType == HB_ET_NUMERIC &&
...
static void hb_compExprPushPreOp(PHB_EXPR pSelf, uint8_t bOper, HB_COMP_DECL)
{
  ...
  if (pSelf->value.asOperator.pLeft->ExprType == HB_ET_SEND)
  {
    ...
    if (HB_SUPPORT_EXTOPT)
    {
      ...
      HB_GEN_FUNC1(PCode1, (uint8_t)((bOper == HB_P_INC) ? HB_P_INCEQ : HB_P_DECEQ));
    }
    else
#endif
    {
      hb_compExprPushSendPopPush(pSelf->value.asOperator.pLeft, NULL, HB_FALSE, bOper, HB_COMP_PARAM);
    }
    return;
  }
...
static void hb_compExprPushPostOp(PHB_EXPR pSelf, uint8_t bOper, HB_COMP_DECL)
{
  ...
  if (pSelf->value.asOperator.pLeft->ExprType == HB_ET_SEND)
  {
    ...
    if (HB_SUPPORT_EXTOPT)
    {
      ...
      HB_GEN_FUNC1(PCode1, (uint8_t)((bOper == HB_P_INC) ? HB_P_INCEQPOP : HB_P_DECEQPOP));
    }
    else
#endif
    {
      hb_compExprPushSendPopPush(pSelf->value.asOperator.pLeft, NULL, HB_TRUE, bOper, HB_COMP_PARAM);
    }
    return;
  }
  else if (HB_SUPPORT_HARBOUR)
    ...
    else if (pSelf->value.asOperator.pLeft->ExprType == HB_ET_VARIABLE)
    {
      ...
      if (iScope != HB_VS_LOCAL_FIELD && iScope != HB_VS_GLOBAL_FIELD && iScope != HB_VS_UNDECLARED)
      {
        if (iScope == HB_VS_LOCAL_VAR)
        {
          /* Push current value */
          HB_EXPR_USE(pSelf->value.asOperator.pLeft, HB_EA_PUSH_PCODE);
          HB_GEN_FUNC3(PCode3, (uint8_t)(bOper == HB_P_INC ? HB_P_LOCALINC : HB_P_LOCALDEC), HB_LOBYTE(iVar),
                       HB_HIBYTE(iVar));
        }
        else
        {
