...
static int32_t mxml_refil_getc(MXML_REFIL *ref);
#define mxml_refil_ungetc(ref, ch) ref->sparechar = ch

/* Output routines */
/* Currently not used */
#if 0
static MXML_OUTPUT * mxml_output_new(MXML_OUTPUT_FUNC func, int32_t node_count);
static void mxml_output_destroy(MXML_OUTPUT * out);
static MXML_STATUS mxml_output_string(MXML_OUTPUT * out, const char * s);
#endif

static MXML_STATUS mxml_output_setup(MXML_OUTPUT *out, MXML_OUTPUT_FUNC func, int32_t node_count);
static MXML_STATUS mxml_output_char(MXML_OUTPUT *out, int32_t c);
                                       ...
                                       int32_t style)
{
  ...
  while (iStatus < 6) {
    ...
    case 0:
      ...
      default:
        if (HB_ISALPHA(chr)) {
          if (mxml_sgs_append_char(buf_name, static_cast<char>(chr)) != MXML_STATUS_OK) {
            hbxml_set_doc_status(ref, pDoc, pNode, MXML_STATUS_MALFORMED, MXML_ERROR_NAMETOOLONG);
    ...
    case 4:
      if (chr == quotechr) {
        ...
        mxml_sgs_append_char(buf_attrib, static_cast<char>(chr)); /* we'll need it */
      } else if (chr == MXML_LINE_TERMINATOR) {
        hbxml_doc_new_line(pDoc);
        if (mxml_sgs_append_char(buf_attrib, static_cast<char>(chr)) != MXML_STATUS_OK) {
    ...
    case 5:
      if (chr == ';') {
        ...
        mxml_sgs_append_char(buf_attrib, static_cast<char>(chr));
...
static MXML_STATUS mxml_attribute_write(MXML_OUTPUT *out, PHBXML_ATTRIBUTE pAttr, int32_t style)
{
  ...
  if (!(style & MXML_STYLE_NOESCAPE)) {
    ...
    mxml_output_string_len(out, hb_itemGetCPtr(pAttr->pValue), hb_itemGetCLen(pAttr->pValue));
  }
...
static void mxml_node_unlink(PHB_ITEM pNode)
{
  ...
  auto pPrev = hb_itemNew(hb_param(-1, Harbour::Item::ANY));

  hb_objSendMsg(pNode, "ONEXT", 0);
  auto pNext = hb_itemNew(hb_param(-1, Harbour::Item::ANY));
  ...
  if (!pParent->isNil()) {
    ...
    if (pPrev->isNil()) { /* was I the first node? */
...
static void mxml_node_read_data(MXML_REFIL *ref, PHB_ITEM pNode, PHB_ITEM doc, int32_t iStyle)
{
  ...
  while (chr != MXML_EOF) {
    ...
    if (chr != '<') {
      ...
      if (chr == ';' && iStatus == 1) {
        int32_t iAmpLen = iPos - iPosAmper - 2;
        char *bp = buf + iPosAmper + 1;

        if (iAmpLen <= 0) {
          /* error! - we have "&;" */
          MXML_DELETOR(buf);
          hbxml_set_doc_status(ref, doc, pNode, MXML_STATUS_MALFORMED, MXML_ERROR_WRONGENTITY);
          return;
        }
  ...
  while (iPos > 1 && HB_ISSPACE(static_cast<uint8_t>(buf[iPos - 1]))) {
    iPos--;
  }
  ...
  while (iStatus < 2) {
    ...
    case 1:
      if (HB_ISALNUM(chr) || chr == '_' || chr == '-' || chr == '.' || chr == ':') {
        /* cannot cause reallocations */
        buf[iPos++] = static_cast<char>(chr);
      } else if (chr == '>' || chr == ' ' || chr == '/' || chr == '\r' || chr == '\t' || chr == '\n') {
        mxml_refil_ungetc(ref, chr);
        iStatus = 2;
      } else {
        MXML_DELETOR(buf);
...
static void mxml_node_read_tag(MXML_REFIL *ref, PHB_ITEM pNode, PHB_ITEM doc, int32_t style)
{
  ...
  auto chr = static_cast<char>(mxml_refil_getc(ref));
...
static void mxml_node_read_cdata(MXML_REFIL *ref, PHB_ITEM pNode, PHB_ITEM pDoc)
{
  ...
  if (iStatus == 100) {
    hbxml_set_doc_status(ref, pDoc, pNode, MXML_STATUS_MALFORMED, MXML_ERROR_INVNODE);
  } else {
    auto iPos = 0;
    int32_t iAllocated = MXML_ALLOC_BLOCK;
...
static void mxml_node_file_indent(MXML_OUTPUT *out, int32_t depth, int32_t style)
{
    ...
    if (pItem->isNil() && pChild->isNil()) {
      ...
      if (!pItem->isNil()) {
        ...
        if (!(style & MXML_STYLE_NOESCAPE)) {
          ...
          mxml_output_string_len(out, hb_itemGetCPtr(pItem), hb_itemGetCLen(pItem));
        }
      }
...
static MXML_STATUS mxml_refil_setup(MXML_REFIL *ref, MXML_REFIL_FUNC func, char *buf, HB_ISIZ buflen, HB_ISIZ bufsize)
{
  ...
  ref->refil_func = func;
  ref->buffer = reinterpret_cast<uint8_t *>(buf);
...
#if 0
void mxml_refil_ungetc(MXML_REFIL * ref, int32_t chr)
{
   ref->sparechar = chr;
}
#endif
...
{
  auto fh = static_cast<HB_FHANDLE>(ref->u.hFile);
  HB_ISIZ len;
...
static const char *edesc[] = {"Input/output error",
                              "Not enough memory",
...
static const char *mxml_error_desc(MXML_ERROR_CODE code)
{
  int32_t iCode = (static_cast<int32_t>(code)) - 1;
...
HB_FUNC(HBXML_DATAREAD)
{
  ...
  auto pRoot = hb_itemNew(hb_param(-1, Harbour::Item::ANY));
...
HB_FUNC(HBXML_NODE_WRITE)
{
  ...
  out.u.hFile = static_cast<HB_FHANDLE>(hb_itemGetNInt(pHandle));
