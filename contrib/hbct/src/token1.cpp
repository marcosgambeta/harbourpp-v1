  ...
  if (iParamCheck) {
    ...
    while (nToken < nTokenCounter) {
      ...
      case DO_TOKEN1_TOKENLOWER:
        if (pcSubStr != pc) { // letters can be tokenizers, too, but they should not be lowercase'd
          *(pcRet + (pcSubStr - pcString)) = static_cast<char>(hb_charLower(static_cast<uint8_t>(*pcSubStr)));
        }
        break;

      case DO_TOKEN1_TOKENUPPER:
        if (pcSubStr != pc) { // letters can be tokenizers, too, but they should not be uppercase'd
          *(pcRet + (pcSubStr - pcString)) = static_cast<char>(hb_charUpper(static_cast<uint8_t>(*pcSubStr)));
        }
        break;
    ...
    case DO_TOKEN1_TOKEN: {
      ...
      if (iArgErrorMode != CT_ARGERR_IGNORE) {
        pSubst = ct_error_subst(static_cast<uint16_t>(iArgErrorMode), EG_ARG, CT_ERROR_TOKEN, nullptr, HB_ERR_FUNCNAME,
