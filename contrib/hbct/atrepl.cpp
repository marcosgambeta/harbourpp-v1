...
HB_FUNC( ATREPL )
{
   if( HB_ISCHAR( 1 ) && HB_ISCHAR( 2 ) )
   {
         ...
         while( nMatchCounter < nCounter )
         {
            switch( iAtLike )
            {
               case CT_SETATLIKE_EXACT:
                  pc = static_cast< char * >( HB_UNCONST( ct_at_exact_forward( pcRetSubStr, sRetSubStrLen, pcStringToMatch,
                                                                   nStrToMatchLen, &nMatchStrLen ) ) );
                  break;

               case CT_SETATLIKE_WILDCARD:
                  pc = static_cast< char * >( HB_UNCONST( ct_at_wildcard_forward( pcRetSubStr, sRetSubStrLen, pcStringToMatch,
      ...
      else
      {
         ...
         {
            case CT_SETATLIKE_EXACT:
               pc = static_cast< char * >( HB_UNCONST( ct_at_exact_backward( pcRetStr + nIgnore, nRetStrLen - nIgnore,
                                                                 pcStringToMatch, nStrToMatchLen, &nMatchStrLen ) ) );
               break;

            case CT_SETATLIKE_WILDCARD:
               pc = static_cast< char * >( HB_UNCONST( ct_at_wildcard_backward( pcRetStr + nIgnore, nRetStrLen - nIgnore,
