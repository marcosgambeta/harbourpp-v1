   ...
   if( _s )
   {
      ...
      if( iIndex < ( int ) HB_SIZEOFARRAY(_s->hDlgModeless) )
      {
         ...
         if( hbwapi_is_HANDLE(3) )
            ...
            hDlg = CreateDialogIndirect(GetModuleHandle(nullptr), ( LPCDLGTEMPLATE ) hb_parc(1), hb_parl(2) ? _s->hWnd : nullptr, hbwapi_par_raw_DLGPROC(3));
         else
         {
            switch( iResource )
            {
               ...
               {
                  void * hTemplate;
                  hDlg = CreateDialog(GetModuleHandle(nullptr), HB_PARSTR(1, &hTemplate, nullptr), hb_parl(2) ? _s->hWnd : nullptr, ( DLGPROC ) hb_wvt_gtDlgProcMLess);
                  hb_strfree(hTemplate);
               }
               break;

               case 1:
                  hDlg = CreateDialog(GetModuleHandle(nullptr), MAKEINTRESOURCE(hb_parni(1)), hb_parl(2) ? _s->hWnd : nullptr, ( DLGPROC ) hb_wvt_gtDlgProcMLess);
                  break;

               case 2:
                  /* argument 1 is already unicode compliant, so no conversion */
                  hDlg = CreateDialogIndirect(GetModuleHandle(nullptr), ( LPCDLGTEMPLATE ) hb_parc(1), hb_parl(2) ? _s->hWnd : nullptr, ( DLGPROC ) hb_wvt_gtDlgProcMLess);
                  break;
            }
         }
   ...
   if( _s )
   {
      ...
      if( iIndex < ( int ) HB_SIZEOFARRAY(_s->hDlgModal) )
      {
         ...
         switch( iResource )
         {
            ...
            {
               void * hTemplate;
               iResult = DialogBoxParam(GetModuleHandle(nullptr), HB_PARSTR(1, &hTemplate, nullptr), hParent, ( DLGPROC ) hb_wvt_gtDlgProcModal, ( LPARAM ) ( DWORD ) iIndex + 1);
               hb_strfree(hTemplate);
            }
            break;

            case 1:
               iResult = DialogBoxParam(GetModuleHandle(nullptr), MAKEINTRESOURCE(hb_parni(1)), hParent, ( DLGPROC ) hb_wvt_gtDlgProcModal, ( LPARAM ) ( DWORD ) iIndex + 1);
               break;

            case 2:
               /* argument 1 is already unicode compliant, so no conversion */
               iResult = DialogBoxIndirectParam(GetModuleHandle(nullptr), ( LPCDLGTEMPLATE ) hb_parc(1), hParent, ( DLGPROC ) hb_wvt_gtDlgProcModal, ( LPARAM ) ( DWORD ) iIndex + 1);
               break;
         }
      }
   }
