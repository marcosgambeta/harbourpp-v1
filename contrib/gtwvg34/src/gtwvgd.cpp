...
static void hb_gt_wvt_Free(PHB_GTWVT pWVT)
{
  ...
  for (iIndex = 0; iIndex < (int32_t)HB_SIZEOFARRAY(pWVT->pFunc); iIndex++) {
    if (pWVT->pFunc[iIndex] != nullptr && pWVT->iType[iIndex] == 2) {
      hb_itemRelease((HB_ITEM *)pWVT->pFunc[iIndex]); // TODO: C++ cast
      pWVT->pFunc[iIndex] = nullptr;
    }
    if (pWVT->pcbFunc[iIndex] != nullptr) {
      hb_itemRelease((HB_ITEM *)pWVT->pcbFunc[iIndex]); // TODO: C++ cast
...
static LRESULT hb_gt_wvt_FireEvent(PHB_GTWVT pWVT, int32_t nEvent, HB_ITEM *pParams)
{
    ...
    if (hb_vmRequestReenter()) {
      ...
      if (pWVT->pGT->pNotifierBlock) {
        ...
        if (pResult->isPointer()) {
          nResult = (uintptr_t)hb_itemGetPtr(pResult);
        } else {
      ...
      if (pWVT->pNotifierGUI) {
        HB_ITEM *pResult = hb_vmEvalBlockV(pWVT->pNotifierGUI, 2, pEvent, pParams);
...
{
  HB_ITEM *pEvParams = hb_itemArrayNew(2);
...
static int32_t hb_gt_wvt_key_ansi_to_oem(int32_t c)
{
  ...
  if (MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, (LPCSTR)pszSrc, 1, (LPWSTR)pszWide, 1) &&
      WideCharToMultiByte(CP_OEMCP, 0, (LPCWSTR)pszWide, 1, (LPSTR)pszDst, 1, nullptr, nullptr)) {
    return pszDst[0];
  } else {
    return c;
  }
...
static void hb_gt_wvt_MouseEvent(PHB_GTWVT pWVT, UINT message, WPARAM wParam, LPARAM lParam)
{
...
#if defined(UNICODE)
        if (j > 0) {
          HB_ITEM *pItem = hb_itemPutStrLenU16(nullptr, HB_CDP_ENDIAN_NATIVE, sBuffer, j);
...
#else
        if (j > 0) {
          HB_ITEM *pItem = hb_itemPutCLPtr(nullptr, sBuffer, j);
  ...
  if (keyCode != 0) {
    HB_ITEM *pEvParams = hb_itemArrayNew(6);
...
static LRESULT CALLBACK hb_gt_wvt_WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  if (pWVT) {
    ...
    case WM_PAINT:
      if (pWVT->pPP->iWndType == HB_WNDTYPE_CRT) {
        ...
        { /* To evoke if that portion is covered by GUI control */
          HB_ITEM *pEvParams = hbwapi_itemPut_HANDLE(nullptr, hWnd);
    ...
    case WM_HSCROLL: {
      ...
      hbwapi_arraySet_HANDLE(pEvParams, 3, (HANDLE)lParam);
    ...
    case WM_SETFOCUS:
      ...
      { /* For mixing GUI/CUI items */
        ...
        hb_arraySetNInt(pEvParams, 3, (uintptr_t)lParam); /* unused */
    ...
    case WM_CLOSE: /* Clicked 'X' on system menu */
      if (pWVT->CloseMode == 0) {
        HB_ITEM *pEvParams = hb_itemNew(nullptr);
    ...
    case WM_COMMAND:
      if ((HIWORD(wParam) == 0 && !IsWindow((HWND)lParam))) {
        ...
        hbwapi_arraySet_HANDLE(pEvParams, 3, (HWND)lParam); /* Controls hWnd */
    ...
    case WM_CTLCOLORSTATIC: {
      ...
      hbwapi_arraySet_HANDLE(pEvParams, 1, (HDC)wParam);
      hbwapi_arraySet_HANDLE(pEvParams, 2, (HWND)lParam);
...
static HWND hb_gt_wvt_CreateWindow(PHB_GTWVT pWVT, HB_BOOL bResizable)
{
  ...
  hWnd = CreateWindowEx(pWVT->pPP->exStyle,  /* extended style */
...
static HB_BOOL hb_gt_wvt_Info(PHB_GT pGT, int32_t iType, PHB_GT_INFO pInfo)
{
  ...
  case HB_GTI_ICONFILE:
    if (hb_itemType(pInfo->pNewVal) & Harbour::Item::STRING) {
      ...
      pWVT->hIcon = (HICON)LoadImage(nullptr, HB_ITEMGETSTR(pInfo->pNewVal, &hImageName, nullptr), IMAGE_ICON, 0, 0,
    ...
    pInfo->pResult = hb_itemPutPtr(pInfo->pResult, (void *)(uintptr_t)pWVT->hIcon);
    break;
  ...
  case HB_GTI_SPEC: {
    ...
    case HB_GTS_SYSTRAYICON:
      if (pWVT->hWnd && (hb_itemType(pInfo->pNewVal2) & Harbour::Item::ARRAY)) {
        ...
        if (iIconType == 0) {
          hIcon = (HICON)LoadImage(nullptr, HB_ARRAYGETSTR(pInfo->pNewVal2, 3, &hIconName, nullptr), IMAGE_ICON, 0, 0,
                                   LR_LOADFROMFILE);
          hb_strfree(hIconName);
        } else if (iIconType == 1) {
