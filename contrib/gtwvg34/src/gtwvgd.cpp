...
static HB_BOOL hb_gt_wvt_TextOut( PHB_GTWVT pWVT, HDC hdc, int col, int row, int iColor, LPCTSTR lpString, UINT cbString )
{
   ...
   return ExtTextOut( hdc, xy.x, xy.y, ETO_CLIPPED | ETO_OPAQUE, &rClip,
                      lpString, cbString, pWVT->FixedFont ? nullptr : pWVT->FixedSize );
}
...
#if defined( HB_OS_WIN_CE )
            pWVT->hIcon = ( HICON ) LoadImage( nullptr,
            ...
            case HB_GTS_FACTOR:
               if( pWVT->hWnd )
               {
                  ...
                  if( s_guiData->pfnLayered )
                  {
                     ...
                     s_guiData->pfnLayered(pWVT->hWnd, RGB(255, 255, 255), ( BYTE ) hb_itemGetNI(pInfo->pNewVal2), /* LWA_COLORKEY | */ LWA_ALPHA);
                  }
#endif
               }
               break;
