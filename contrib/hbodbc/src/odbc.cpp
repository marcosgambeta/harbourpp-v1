...
{
   auto pHEnv = static_cast<PHB_SQLHENV>(Cargo);
...
static HB_GARBAGE_FUNC(hb_SQLHDBC_Destructor)
{
   ...
   auto pHDbc = static_cast<PHB_SQLHDBC>(Cargo);
...
{
   /* Retrieve image pointer holder */
   auto pHStmt = static_cast<PHB_SQLHSTMT>(Cargo);
...
HB_FUNC( SQLDRIVERCONNECT )  /* hDbc, @cConnectString --> nRetCode */
{
   ...
   if( hDbc != nullptr ) {
      ...
      void *      hConnStr;
      HB_SIZE     nConnStr;
      auto cConnStr = static_cast<SQLTCHAR*>(HB_UNCONST(O_HB_PARSTRDEF(2, &hConnStr, &nConnStr)));
...
HB_FUNC( SQLCONNECT )  /* hDbc, cDSN, cUseName, cPassword --> nRetCode */
{
   ...
   if( hDbc != nullptr ) {
      ...
      auto cDSN  = static_cast<SQLTCHAR*>(HB_UNCONST(O_HB_PARSTRDEF(2, &hDSN, &nDSN)));
      auto cUser = static_cast<SQLTCHAR*>(HB_UNCONST(O_HB_PARSTRDEF(3, &hUser, &nUser)));
      auto cPass = static_cast<SQLTCHAR*>(HB_UNCONST(O_HB_PARSTRDEF(4, &hPass, &nPass)));

      ret = SQLConnect(hDbc, cDSN, static_cast<SQLSMALLINT>(nDSN), cUser, static_cast<SQLSMALLINT>(nUser), cPass, static_cast<SQLSMALLINT>(nPass));
...
HB_FUNC( SQLEXECDIRECT )  /* hStmt, cStatement --> nRetCode */
{
   ...
   if( hStmt != nullptr ) {
      void *     hStatement;
      HB_SIZE    nStatement;
      auto cStatement = static_cast<SQLTCHAR*>(HB_UNCONST(O_HB_PARSTRDEF(2, &hStatement, &nStatement)));
      ...
      switch( iType ) {
         ...
         case SQL_WLONGVARCHAR:
         {
            ...
            if( SQL_SUCCEEDED(res = SQLGetData(hStmt, uiField, iTargetType, buffer, 0, &nLen)) ) {
               if( nLen > 0 ) {
                  auto nMaxLen = static_cast<SQLLEN>(hb_parnint(4));
...
HB_FUNC( SQLDESCRIBECOL )  /* hStmt, nCol, @cName, nLen, @nBufferLen, @nDataType, @nColSize, @nDec, @nNull --> nRetCode */
{
   ...
   if( hStmt != nullptr ) {
      ...
      hb_retni(SQLDescribeCol(hStmt,
                              ...
                              static_cast<SQLSMALLINT*>(&iDataType),
                              static_cast<SQLULEN*>(&nColSize),
...
HB_FUNC( SQLCOLATTRIBUTE )  /* hStmt, nCol, nField, @cName, nLen, @nBufferLen, @nAttribute --> nRetCode */
{
   ...
   if( hStmt != nullptr ) {
...
#else
      auto nNumPtr = static_cast<SQLINTEGER>(hb_parnl(7));
#endif
      if( iLen <= 0 ) {
         iLen = 64;
      }
      ...
      hb_retni(SQLColAttributes(hStmt,
                                ...
                                iLen,
                                static_cast<SQLSMALLINT*>(&iBufLen),
                                static_cast<SQLINTEGER*>(&nNumPtr)));
#endif
...
{
   auto hEnv = hb_SQLHENV_par(1);
   auto hDbc = hb_SQLHDBC_par(2);
   auto hStmt = hb_SQLHSTMT_par(3);

   if( (hEnv != nullptr) || (hDbc != nullptr) || (hStmt != nullptr) ) {
      SQLINTEGER  lError;
      SQLSMALLINT iLen;
      SQLTCHAR    buffer[256];
...
HB_FUNC( SQLROWCOUNT )
{
   ...
   if( hStmt != nullptr ) {
      auto iRowCountPtr = static_cast<SQLLEN>(hb_parnint(2));
...
HB_FUNC( SQLSETCONNECTATTR )  /* hDbc, nOption, uOption */
{
   ...
   if( hDbc != nullptr ) {
      ...
      hb_retni(SQLSetConnectOption(hDbc,
                                   static_cast<SQLUSMALLINT>(hb_parni(2)),
                                   HB_ISCHAR(3) ? static_cast<SQLULEN>(static_cast<HB_PTRUINT>(hb_parc(3))) :
...
HB_FUNC( SQLGETCONNECTATTR )  /* hDbc, nOption, @cOption */
{
   ...
   if( hDbc != nullptr ) {
      ...
      hb_retni(SQLGetConnectAttr(hDbc,
                                 static_cast<SQLINTEGER>(hb_parnl(2)),
                                 static_cast<SQLPOINTER>(buffer),
                                 static_cast<SQLINTEGER>(sizeof(buffer)),
...
HB_FUNC( SQLPREPARE )  /* hStmt, cStatement --> nRetCode */
{
   ...
   if( hStmt != nullptr ) {
      ...
      hb_retni(SQLPrepare(hStmt,
                          static_cast<SQLTCHAR*>(HB_UNCONST(O_HB_PARSTRDEF(2, &hStatement, nullptr))),
