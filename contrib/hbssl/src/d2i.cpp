...
HB_FUNC(D2I_PUBKEY)
{
  ...
  if (pszKeyDer) {
    EVP_PKEY *pKey = d2i_PUBKEY(nullptr, &pszKeyDer, (long)hb_parclen(1));
    if (pKey)
...
HB_FUNC(D2I_RSAPUBLICKEY)
{
  ...
  if (pszKeyDer) {
    RSA *pKey = d2i_RSAPublicKey(nullptr, &pszKeyDer, (long)hb_parclen(1));
    if (pKey)
      hb_RSA_ret(pKey);
    else
      hb_retptr(nullptr);
...
HB_FUNC(D2I_X509)
{
  ...
  if (pszCrtDer) {
    X509 *x509 = d2i_X509(nullptr, &pszCrtDer, (long)hb_parclen(1));
    if (x509)
      hb_X509_ret(x509);
    else
      hb_retptr(nullptr);
...
#define EC_P256_LEN 32
    ...
    ECDSA_SIG *sig = d2i_ECDSA_SIG(nullptr, &pszSigDer, (long)hb_parclen(1));
