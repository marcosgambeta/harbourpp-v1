...
{
  HB_ITEM *pItem = hb_param(1, HB_IT_POINTER);
  ...
  if (sd != HB_NO_SOCKET) {
    if (hb_SSL_is(2)) {
      ...
      if (ssl) {
        ...
        if (pStream) {
          if (!hb_znetInetInitialize(pItem, (PHB_ZNETSTREAM)pStream, hb_inetReadSSL, hb_inetWriteSSL, hb_inetFlushSSL,
                                     hb_inetCloseSSL, hb_inetErrorSSL, hb_inetErrStrSSL)) {
