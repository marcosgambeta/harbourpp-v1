...
/* Routine to parse a buffer for a EOL
...
static long _ft_skip( long iRecs )
{
   ...
   if( ft_text->handles[ft_text->area] )
   {
      ...
      auto cBuff = static_cast<char*>(hb_xgrab(BUFFSIZE));
...
HB_FUNC(FT_FUSE)
{
   ...
   if( pszFileName )
   {
      ft_text->handles[ft_text->area] = hb_fileExtOpen(pszFileName, nullptr, static_cast<HB_FATTR>(hb_parnidef(2, FO_READWRITE | FO_DENYNONE) & 0xFF), nullptr, nullptr);
      ft_text->offset[ft_text->area]   = 0;
      ft_text->recno[ft_text->area]    = 1;
      if( ft_text->handles[ft_text->area] == nullptr )
      {
...
HB_FUNC(FT_FREADLN)
{
   ...
   if( ft_text->handles[ft_text->area] )
   {
      ...
      HB_ISIZ iBytesRead;
      HB_ISIZ eol_len;
      auto cPtr = static_cast<char*>(hb_xgrab(BUFFSIZE));
...
HB_FUNC(FT_FDELETE)
{
   ...
   if( ft_text->handles[ft_text->area] )
   {
      ...
      auto Buff = static_cast<char*>(hb_xgrab(BUFFSIZE));
...
HB_FUNC(FT_FWRITELN)
{
   ...
   if( ft_text->handles[ft_text->area] )
   {
      ...
      else
      {
         ...
         auto buffer = static_cast<char*>(hb_xgrab(BUFFSIZE));
