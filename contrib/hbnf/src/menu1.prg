...
FUNCTION ft_Menu1(aBar, aOptions, aColors, nTopRow, lShadow)
   ...
   // inkey codes for A - Z
   LOCAL aKeyCodes := {K_ALT_A, K_ALT_B, K_ALT_C, K_ALT_D, K_ALT_E, K_ALT_F, K_ALT_G, K_ALT_H, K_ALT_I, K_ALT_J, K_ALT_K, K_ALT_L, K_ALT_M, ;
                       K_ALT_N, K_ALT_O, K_ALT_P, K_ALT_Q, K_ALT_R, K_ALT_S, K_ALT_T, K_ALT_U, K_ALT_V, K_ALT_W, K_ALT_X, K_ALT_Y, K_ALT_Z}
   ...
   AEval(aBar, {|x, i|HB_SYMBOL_UNUSED(x), aBarcol[i] := nTtlUsed, nTtlUsed += (Len(aBar[i]) + 1)}, 2, Len(aBar) - 1)
   ...
   AEval(t_aChoices, {|x, i|HB_SYMBOL_UNUSED(x), AAdd(t_aValidKeys, ""), _ftValKeys(i, t_aChoices, @t_aValidKeys)})
   ...
   AEval(aBarKeys, {|x, i|HB_SYMBOL_UNUSED(x), aBarKeys[i] := aKeyCodes[Asc(Upper(LTrim(aBar[i]))) - Asc("@")]})
...
STATIC FUNCTION _ftWidest(i, t_aChoices, aBarWidth)
   ...
   AEval(t_aChoices[i, 1], {|a, b|HB_SYMBOL_UNUSED(a), aBarWidth[i] := Max(aBarWidth[i], Len(t_aChoices[i, 1, b]))})
