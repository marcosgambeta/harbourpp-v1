...
static HBITMAP hPrepareBitmap(LPCTSTR szBitmap, UINT uiBitmap, int iExpWidth, int iExpHeight, bool bMap3Dcolors,
                              HWND hCtrl, int iMode)
{
        ...
        if (pPackedDib || bMap3Dcolors)
        {
          ...
          if (iExpWidth != iWidth || iExpHeight != iHeight)
          {
            ...
            */
            ...
            bool bResult =
                StretchBlt(hdcTarget, 0, 0, iExpWidth, iExpHeight, hdcSource, 0, 0, iWidth, iHeight, SRCCOPY);
  ...
  case 1: {
    ...
    hBitmap = static_cast<HBITMAP>(
        LoadImage(static_cast<HINSTANCE>(wvg_hInstance()), szBitmap, IMAGE_BITMAP, iExpWidth, iExpHeight, uiOptions));
...
HB_FUNC(WVG_PREPAREBITMAPFROMFILE)
{
  void *hText;
  auto hBitmap =
      hPrepareBitmap(HB_PARSTR(1, &hText, nullptr), 0, hb_parni(2), hb_parni(3), hb_parl(4), wvg_par_HWND(5), 0);
  hb_strfree(hText);
...
{
  auto hBitmap = hPrepareBitmap(nullptr, hb_parni(1), hb_parni(2), hb_parni(3), hb_parl(4), wvg_par_HWND(5), 2);
  hb_retptr(static_cast<void *>(hBitmap));
}
...
HB_FUNC(WVG_PREPAREBITMAPFROMRESOURCENAME)
{
  void *hText;
  auto hBitmap =
      hPrepareBitmap(HB_PARSTR(1, &hText, nullptr), 0, hb_parni(2), hb_parni(3), hb_parl(4), wvg_par_HWND(5), 1);
  hb_strfree(hText);
  hb_retptr(static_cast<void *>(hBitmap));
}
...
HB_FUNC(WVG_STATUSBARCREATEPANEL)
{
  ...
  case 0: {
    ...
    auto iParts = static_cast<int>(SendMessage(hWndSB, SB_GETPARTS, static_cast<WPARAM>(WIN_STATUSBAR_MAX_PARTS),
                                               reinterpret_cast<LPARAM>(static_cast<LPINT>(ptArray))));
    RECT rc{};
    ...
    ptArray[iParts] = -1;
    if (SendMessage(hWndSB, SB_SETPARTS, static_cast<WPARAM>(iParts) + 1,
                    reinterpret_cast<LPARAM>(static_cast<LPINT>(ptArray))))
    {
      hb_retl(true);
      return;
...
HB_FUNC(WVG_STATUSBARSETTEXT)
{
  ...
  if (hWndSB && IsWindow(hWndSB))
  {
    ...
    void *hCaption;
    iPart -= 1; /* Zero based */
    auto iFlags = static_cast<int>(
        HIWORD(SendMessage(hWndSB, SB_GETTEXT, static_cast<WPARAM>(iPart), reinterpret_cast<LPARAM>(szText))));
    SendMessage(hWndSB, SB_SETTEXT, static_cast<WPARAM>(iPart) | iFlags,
...
INT_PTR CALLBACK WvgDialogProcChooseFont(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  auto bret = false;
  auto binit = false;
  ...
  if (msg == WM_INITDIALOG)
  {
    ...
    auto pBlock =
        static_cast<PHB_ITEM>(hb_itemNew(reinterpret_cast<PHB_ITEM>(cf->lCustData))); // TODO: unnecessary cast
...
HB_FUNC(WVG_ADDTOOLBARBUTTON)
{
  ...
  case 1: { /* button from image */
    /* set string */
    void *hCaption;
    auto iNewString = static_cast<int>(
        SendMessage(hWndTB, TB_ADDSTRING, 0, reinterpret_cast<LPARAM>(HB_PARSTR(3, &hCaption, nullptr))));
    hb_strfree(hCaption);
...
HB_FUNC(WVG_SETWINDOWPROCBLOCK)
{
  ...
  auto oldProc = reinterpret_cast<WNDPROC>(
      SetWindowLongPtr(hWnd, GWLP_WNDPROC, reinterpret_cast<HB_PTRUINT>(ControlWindowProcedure)));
#endif
...
{
  auto hwndTip = CreateWindowEx(0, TOOLTIPS_CLASS, 0, WS_POPUP | TTS_ALWAYSTIP, /* | TTS_BALLOON, */
                                CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, wvg_par_HWND(1), nullptr,
