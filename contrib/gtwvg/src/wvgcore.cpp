...
IPicture *hb_wvt_gtLoadPictureFromResource(LPCTSTR resource, LPCTSTR section)
{
  ...
  if (res) {
    ...
    LONG nFileSize = (LONG)SizeofResource(GetModuleHandle(nullptr), res);
...
HB_BOOL hb_wvt_gtRenderPicture(int32_t x, int32_t y, int32_t wd, int32_t ht, IPicture *iPicture, HB_BOOL bDoNotScale)
{
  ...
  if (iPicture) {
    ...
    if (bDoNotScale) {
      ...
      if (lHeight > lWidth) {
        iWd = (int32_t)((double)ht * lWidth / lHeight);
        iWd = HB_MIN(iWd, wd);
        iHt = (int32_t)((double)iWd * lHeight / lWidth);
      } else {
        iHt = (int32_t)((double)wd * lHeight / lWidth);
        iHt = HB_MIN(iHt, ht);
        iWd = (int32_t)((double)iHt * lWidth / lHeight);
      }
      x += abs((iWd - wd) / 2);
      y += abs((iHt - ht) / 2);
      wd = iWd;
...
HB_BOOL hb_wvt_DrawImage(HDC hdc, int32_t x, int32_t y, int32_t wd, int32_t ht, LPCTSTR lpImage, HB_BOOL bDoNotScale)
{
  HGLOBAL hGlobal;
  HANDLE hFile;
  DWORD nFileSize;
...
HB_FUNC(WVT_DRAWIMAGE)
{
  ...
  if (HB_ISNUM(5)) {
    ...
    void *hImage;
    hb_wvt_DrawImage(_s->hdc, iLeft, iTop, (iRight - iLeft) + 1, (iBottom - iTop) + 1, HB_PARSTR(5, &hImage, nullptr),
                     hb_parl(7));
    hb_strfree(hImage);
#if defined(__SETGUI__)
    if (_s->bGui) {
      hb_wvt_DrawImage(_s->hGuiDC, iLeft, iTop, (iRight - iLeft) + 1, (iBottom - iTop) + 1,
                       HB_PARSTR(5, &hImage, nullptr), hb_parl(7));
      hb_strfree(hImage);
    }
#endif
  }
...
HB_FUNC(WVT_DRAWLABEL)
{
  ...
  HB_STRNCPY(logfont.lfFaceName, (HB_ISCHAR(8) ? HB_PARSTR(8, &hText, nullptr) : _s->fontFace),
             HB_SIZEOFARRAY(logfont.lfFaceName) - 1);
  hb_strfree(hText);
...
HB_FUNC(WVT_DRAWBUTTON)
{
  ...
  int32_t iTop, iLeft, iBottom, iRight;
  int32_t iAlign;
  int32_t iTextHeight /*, iTextWidth */;
  LOGBRUSH lb = {0, 0, 0};
  HBRUSH hBrush;
  ...
  if (bImage) {
    ...
    if (HB_ISNUM(6)) {
      ...
      hb_strfree(hImage);
      if (_s->bGui) {
        hb_wvt_DrawImage(_s->hGuiDC, iLeft + 4, iTop + 4, iImageWidth, iImageHeight, HB_PARSTR(6, &hImage, nullptr),
                         FALSE);
        hb_strfree(hImage);
      }
    }
...
HB_FUNC(WVT_DRAWPICTUREEX)
{
  ...
  if (HB_ISNUM(5)) {
    ...
    hb_retl(hb_wvt_gtRenderPicture(iLeft, iTop, iRight - iLeft + 1, iBottom - iTop + 1,
                                   (IPicture *)(uintptr_t)hb_parnint(5), hb_parl(7)));
  }
}
...
HB_FUNC(WVT_DRAWLABELOBJ)
{
  ...
  SetTextColor(_s->hdc, fgClr);
  SetBkColor(_s->hdc, bgClr);
  SelectObject(_s->hdc, (HFONT)(uintptr_t)hb_parnint(10));
  ...
  if (_s->bGui) {
    SetTextColor(_s->hGuiDC, fgClr);
    SetBkColor(_s->hGuiDC, bgClr);
    SelectObject(_s->hGuiDC, (HFONT)(uintptr_t)hb_parnint(10));
...
HB_FUNC(WVT_DRAWTEXTBOX)
{
  ...
  SelectObject(_s->hdc, (HFONT)(uintptr_t)hb_parnint(12));
  ...
  if (_s->bGui) {
    ...
    SelectObject(_s->hGuiDC, (HFONT)(uintptr_t)hb_parnint(12));
...
HB_FUNC(WVT_DRAWPROGRESSBAR)
{
  ...
  if (bImage) {
    void *hImage;
    hb_wvt_DrawImage(_s->hdc, rc.left, rc.top, rc.right - rc.left + 1, rc.bottom - rc.top + 1,
                     HB_PARSTR(9, &hImage, nullptr), FALSE);
    hb_strfree(hImage);
#if defined(__SETGUI__)
    if (_s->bGui) {
      hb_wvt_DrawImage(_s->hGuiDC, rc.left, rc.top, rc.right - rc.left + 1, rc.bottom - rc.top + 1,
                       HB_PARSTR(9, &hImage, nullptr), FALSE);
      hb_strfree(hImage);
    }
#endif
...
HB_FUNC(WVT_CREATEFONT)
{
  ...
  HB_STRNCPY(logfont.lfFaceName, (!HB_ISCHAR(1) ? _s->fontFace : HB_PARSTR(1, &hText, nullptr)),
             HB_SIZEOFARRAY(logfont.lfFaceName) - 1);
  hb_strfree(hText);

  hb_retnint((uintptr_t)CreateFontIndirect(&logfont));
}
...
{
  IPicture *iPicture = (IPicture *)(uintptr_t)hb_parnl(1);
...
HB_FUNC(WVT_LOADFONT)
{
  ...
  HB_STRNCPY(logfont.lfFaceName, (HB_ISCHAR(2) ? HB_PARSTR(2, &hF, nullptr) : _s->fontFace),
             HB_SIZEOFARRAY(logfont.lfFaceName) - 1);
  hb_strfree(hF);
...
HB_FUNC(WVT_SAVESCREEN)
{
  ...
  hb_arraySetNInt(info, 3, (uintptr_t)hBmp);
...
HB_FUNC(WVT_RESTSCREEN)
{
  ...
  hCompDC = CreateCompatibleDC(_s->hdc);
  hBmp = (HBITMAP)SelectObject(hCompDC, (HBITMAP)(uintptr_t)hb_parvnint(5, 3));
  if (hBmp) {
  ...
  if (!bDoNotDestroyBMP) {
    ...
    DeleteObject((HBITMAP)(uintptr_t)hb_parvnint(5, 3));
  }
  hb_retl(bResult);
}
