...
static SDDNODE s_ocidd = {nullptr,
                          "OCILIB",
...
static HB_ERRCODE ocilibConnect(SQLDDCONNECTION *pConnection, PHB_ITEM pItem)
{
  ...
  OCI_Connection *cn = OCI_ConnectionCreate(
      static_cast<mtext *>(const_cast<char *>(M_HB_ARRAYGETSTR(pItem, 2, &hConn, nullptr))),
...
{
  HB_ERRCODE errCode =
      OCI_ConnectionFree((static_cast<SDDCONN *>(pConnection->pSDDConn))->pConn) ? Harbour::SUCCESS : Harbour::FAILURE;
      ...
      case Harbour::DB::Field::STRING:
        if (OCI_IsNull(rs, ui)) {
          ...
          if ((val = OCI_GetString(rs, ui)) != nullptr) {
            // pItem = D_HB_ITEMPUTSTRLEN(pItem, val, static_cast<HB_SIZE>(dtslen(val)));  /* TODO: Pad it to
            // pField->uiLen size with spaces? */
