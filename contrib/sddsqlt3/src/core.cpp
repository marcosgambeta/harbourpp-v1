...
{
   PHB_ITEM pError = hb_errRT_New(ES_ERROR, "SDDSQLITE3", errGenCode, errSubCode, szDescription, szOperation, errOsCode, EF_NONE);
...
static HB_ERRCODE sqlite3Disconnect(SQLDDCONNECTION * pConnection)
{
   HB_ERRCODE errCode = sqlite3_close((static_cast<SDDCONN*>(pConnection->pSDDConn))->pDb) == SQLITE_OK ? Harbour::SUCCESS : Harbour::FAILURE;
...
static HB_ERRCODE sqlite3Execute(SQLDDCONNECTION * pConnection, PHB_ITEM pItem)
{
   ...
   if( sqlite3_get_table(pDb, S_HB_ITEMGETSTR(pItem, &hStatement, nullptr), &pResult, &iRow, &iCol, &pszErrMsg) != SQLITE_OK ) {
