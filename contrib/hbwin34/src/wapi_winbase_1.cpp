...
HB_FUNC(WAPI_SETPROCESSWORKINGSETSIZE)
{
  BOOL bResult = SetProcessWorkingSetSize(hbwapi_par_raw_HANDLE(1), static_cast<SIZE_T>(hb_parnint(2)),
                                          static_cast<SIZE_T>(hb_parnint(3)));
  DWORD dwLastError = GetLastError();
  hbwapi_SetLastError(dwLastError);
...
{
  FARPROC pProc =
      GetProcAddress(static_cast<HMODULE>(hb_parptr(1)),
...
HB_FUNC(WAPI_GETVOLUMEINFORMATION)
{
  ...
  void *hRootPath;
  BOOL bResult = GetVolumeInformation(HB_PARSTR(1, &hRootPath, nullptr), lpVolNameBuf, dwVolNameSize, &dwSerialNumber,
