...
HB_FUNC( WAPI_SETWINDOWPOS )
{
   ...
   BOOL bResult = SetWindowPos(hbwapi_par_raw_HWND(1), hWndInsertAfter, hb_parni(3), hb_parni(4), hb_parni(5), hb_parni(6), hbwapi_par_UINT(7));
...
static int s_MessageBoxTimeout(IN HWND hWnd, IN LPCTSTR lpText, IN LPCTSTR lpCaption, IN UINT uType, IN WORD wLanguageId, IN DWORD dwMilliseconds)
{
   ...
   return s_pMessageBoxTimeout ?
            s_pMessageBoxTimeout(hWnd, lpText, lpCaption, uType, wLanguageId, dwMilliseconds) :
...
HB_FUNC( WAPI_MESSAGEBOXTIMEOUT )
{
   void * hStr1;
   void * hStr2;
   int iResult = s_MessageBoxTimeout(hbwapi_par_raw_HWND(1), HB_PARSTR(2, &hStr1, nullptr), HB_PARSTR(3, &hStr2, nullptr), hbwapi_par_UINT(4), hbwapi_par_WORD(5), hbwapi_par_DWORD(6));
   hbwapi_SetLastError(GetLastError());
   hbwapi_ret_NI(iResult);
   hb_strfree(hStr1);
   hb_strfree(hStr2);
...
HB_FUNC( WAPI_CREATEWINDOWEX )
{
   ...
   HWND hResult = CreateWindowEx(
      ...
      HB_ISNUM(10) ? reinterpret_cast<HMENU>(static_cast<HB_PTRUINT>(hb_parnint(10))) : hbwapi_par_raw_HMENU(10),  /* hMenu */
...
HB_FUNC( WAPI_SETSCROLLINFO )
{
   hbwapi_ret_NI(SetScrollInfo(hbwapi_par_raw_HWND(1),
                               hbwapi_par_INT(2),
                               hb_parclen(3) == sizeof(SCROLLINFO) ? reinterpret_cast<LPSCROLLINFO>(const_cast<char*>(hbwapi_par_raw_STRUCT(3))) : nullptr,
...
HB_FUNC( WAPI_LOADIMAGE )
{
   ...
   HANDLE hImage = LoadImage(hbwapi_par_raw_HINSTANCE(1),
                             ...
                             static_cast<UINT>(hb_parnidef(3, IMAGE_BITMAP)),
...
HB_FUNC( WAPI_LOADMENU )
{
   ...
   HMENU hMenu = LoadMenu(hbwapi_par_raw_HINSTANCE(1), HB_ISNUM(2) ? MAKEINTRESOURCE(hbwapi_par_INT(2)) : HB_PARSTRDEF(2, &hMenuName, nullptr));
   hbwapi_SetLastError(GetLastError());
...
{
   BOOL fResult = CheckMenuRadioItem(hbwapi_par_raw_HMENU(1), hbwapi_par_UINT(2), hbwapi_par_UINT(3), hbwapi_par_UINT(4), hbwapi_par_UINT(5));
   hbwapi_SetLastError(GetLastError());
   hbwapi_ret_L(fResult);
}
...
{
   BOOL fResult = SetMenuDefaultItem(hbwapi_par_raw_HMENU(1), hbwapi_par_UINT(2), HB_ISNUM(3) ? hbwapi_par_INT(3) : hbwapi_par_BOOL(3));
   hbwapi_SetLastError(GetLastError());
   hbwapi_ret_L(fResult);
}
...
{
   UINT uiResult = GetMenuDefaultItem(hbwapi_par_raw_HMENU(1), HB_ISNUM(2) ? hbwapi_par_INT(2) : hbwapi_par_BOOL(2), hbwapi_par_UINT(3));
   hbwapi_SetLastError(GetLastError());
   if( uiResult == static_cast<UINT>(-1) )
   {
      hbwapi_ret_NI(-1);
...
HB_FUNC( WAPI_POSTMESSAGE )  /* NOTE: unsafe function, may write past buffer */
{
   ...
   bool bResult = PostMessage(hbwapi_par_raw_HWND(1), hbwapi_par_UINT(2), hbwapi_par_WPARAM(3), szText ? reinterpret_cast<LPARAM>(szText) : hbwapi_par_LPARAM(4));
   hbwapi_SetLastError(GetLastError());
   hbwapi_ret_L(bResult);
...
HB_FUNC( WAPI_SENDNOTIFYMESSAGE )  /* NOTE: unsafe function, may write past buffer */
{
   ...
   bool bResult = SendNotifyMessage(hbwapi_par_raw_HWND(1), hbwapi_par_UINT(2), hbwapi_par_WPARAM(3), szText ? reinterpret_cast<LPARAM>(szText) : hbwapi_par_LPARAM(4));
   hbwapi_SetLastError(GetLastError());
   hbwapi_ret_L(bResult);
...
HB_FUNC( WAPI_SENDMESSAGE )  /* NOTE: unsafe function, may write past buffer */
{
   ...
   LRESULT result = SendMessage(hbwapi_par_raw_HWND(1), hbwapi_par_UINT(2), hbwapi_par_WPARAM(3), szText ? reinterpret_cast<LPARAM>(szText) : hbwapi_par_LPARAM(4));
   hbwapi_SetLastError(GetLastError());
   hb_retnint(result);
...
HB_FUNC( WAPI_SENDMESSAGETIMEOUT )  /* NOTE: unsafe function, may write past buffer */
{
   ...
   LRESULT result = SendMessageTimeout(hbwapi_par_raw_HWND(1),
                                hbwapi_par_UINT(2),
                                hbwapi_par_WPARAM(3),
...
{
   void * hName = nullptr;
   hbwapi_ret_raw_HANDLE(LoadBitmap(hbwapi_par_raw_HINSTANCE(1), HB_ISNUM(2) ? MAKEINTRESOURCE(hbwapi_par_INT(2)) : HB_PARSTR(2, &hName, nullptr)));
   hb_strfree(hName);
}
...
{
   void * hName = nullptr;
   HICON h = LoadIcon(hbwapi_par_raw_HINSTANCE(1), HB_ISNUM(2) ? MAKEINTRESOURCE(hbwapi_par_INT(2)) : HB_PARSTR(2, &hName, nullptr));
   hbwapi_SetLastError(GetLastError());
...
{
   RECT rc;
   hbwapi_ret_L(RedrawWindow(hbwapi_par_raw_HWND(1), hbwapi_par_RECT(&rc, 2, false), static_cast<HRGN>(hbwapi_par_raw_HANDLE(3)), hbwapi_par_UINT(4)));
}
...
{
   hbwapi_ret_LRESULT(DefWindowProc(hbwapi_par_raw_HWND(1), hbwapi_par_UINT(2), hbwapi_par_WPARAM(3), hbwapi_par_LPARAM(4)));
}

HB_FUNC( WAPI_CALLWINDOWPROC )
{
   hbwapi_ret_LRESULT(CallWindowProc(hbwapi_par_raw_WNDPROC(1), hbwapi_par_raw_HWND(2), hbwapi_par_UINT(3), hbwapi_par_WPARAM(4), hbwapi_par_LPARAM(5)));
}
