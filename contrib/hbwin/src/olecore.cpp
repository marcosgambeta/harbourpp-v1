...
static HB_GARBAGE_FUNC(hb_ole_destructor)
{
  ...
  if (pDisp) {
    ...
    if (pOle->pCallBack && *pOle->pCallBack) {
      HB_ITEM *pCallBack = *pOle->pCallBack;
...
IDispatch *hb_oleItemGet(HB_ITEM *pItem)
{
...
HB_ITEM *hb_oleItemPut(HB_ITEM *pItem, IDispatch *pDisp)
{
...
HB_ITEM *hb_oleItemGetCallBack(HB_ITEM *pItem)
{
  auto pOle = static_cast<HB_OLE *>(hb_itemGetPtrGC(pItem, &s_gcOleFuncs));
...
void hb_oleItemSetCallBack(HB_ITEM *pItem, HB_ITEM **pCallBack)
{
  auto pOle = static_cast<HB_OLE *>(hb_itemGetPtrGC(pItem, &s_gcOleFuncs));

  if (pOle) {
    if (pOle->pCallBack && *pOle->pCallBack) {
      HB_ITEM *pCallBackPrev = *pOle->pCallBack;
      *pOle->pCallBack = nullptr;
      pOle->pCallBack = nullptr;
...
void hb_oleItemSetDestructor(HB_ITEM *pItem, HB_OLE_DESTRUCTOR_FUNC pFunc, void *cargo)
{
  auto pOle = static_cast<HB_OLE *>(hb_itemGetPtrGC(pItem, &s_gcOleFuncs));
...
VARIANT *hb_oleItemGetVariant(HB_ITEM *pItem)
{
  return static_cast<VARIANT *>(hb_itemGetPtrGC(pItem, &s_gcVariantFuncs));
}

HB_ITEM *hb_oleItemPutVariant(HB_ITEM *pItem, VARIANT *pVariant, HB_BOOL fMove)
{
...
static BSTR hb_oleItemToString(HB_ITEM *pItem)
{
...
static void hb_oleStringToItem(BSTR strVal, HB_ITEM *pItem)
{
                                   ...
                                   long *plIndex)
{
  ...
  for (uiPos = 1; uiPos <= cElements; ++uiPos) {
    ...
    case VT_I1:
      ...
      break;
    case VT_UI1:
      V_UI1(&v) = pStr ? static_cast<uint8_t>(static_cast<unsigned char>(pStr[uiPos - 1]))
                       : static_cast<uint8_t>(hb_arrayGetNI(pItem, uiPos));
      ptr = &V_UI1(&v);
      break;
    case VT_I2:
      ...
      break;
    case VT_UI2:
      V_UI2(&v) = pStr ? static_cast<uint16_t>(static_cast<unsigned char>(pStr[uiPos - 1]))
                       : static_cast<uint16_t>(hb_arrayGetNI(pItem, uiPos));
      ptr = &V_UI2(&v);
      break;
    case VT_I4:
      V_I4(&v) = pStr ? static_cast<int32_t>(static_cast<unsigned char>(pStr[uiPos - 1]))
                      : static_cast<int32_t>(hb_arrayGetNL(pItem, uiPos));
      ptr = &V_I4(&v);
      break;
    case VT_UI4:
      V_UI4(&v) = pStr ? static_cast<uint32_t>(static_cast<unsigned char>(pStr[uiPos - 1]))
                       : static_cast<uint32_t>(hb_arrayGetNL(pItem, uiPos));
      ptr = &V_UI4(&v);
      break;
...
static HB_BOOL hb_oleSafeArrayToString(HB_ITEM *pItem, SAFEARRAY *pSafeArray)
{
  long lFrom, lTo;
  VARTYPE vt = VT_UI1;
...
IDispatch *hb_oleItemGetDispatch(HB_ITEM *pItem)
{
  if (pItem->isObject()) {
...
static void hb_oleItemToVariantRef(VARIANT *pVariant, HB_ITEM *pItem, VARIANT *pVarRef, HB_OLEOBJ_FUNC pObjFunc)
{
...
#if defined(HB_OLE_NO_LL)
    // workaround for wrong OLE variant structure definition
    *(static_cast<int64_t *>(&V_I4(pVariant))) = hb_itemGetNInt(pItem);
#else
...
void hb_oleItemToVariant(VARIANT *pVariant, HB_ITEM *pItem)
{
  hb_oleItemToVariantRef(pVariant, pItem, nullptr, nullptr);
}

void hb_oleItemToVariantEx(VARIANT *pVariant, HB_ITEM *pItem, HB_OLEOBJ_FUNC pObjFunc)
{
...
void hb_oleDispatchToItem(HB_ITEM *pItem, IDispatch *pdispVal, uint16_t uiClass)
{
  if (pdispVal) {
...
void hb_oleVariantToItemEx(HB_ITEM *pItem, VARIANT *pVariant, uint16_t uiClass)
{
  ...
  case VT_I8:
    ...
    // workaround for wrong OLE variant structure definition
    hb_itemPutNInt(pItem, *(static_cast<int64_t *>(&V_I4(pVariant))));
#else
  ...
  case VT_I8 | VT_BYREF:
    ...
    // workaround for wrong OLE variant structure definition
    hb_itemPutNInt(pItem, *reinterpret_cast<int64_t *>(V_R8REF(pVariant)));
#else
...
#elif defined(HB_OLE_NO_LL)
    // workaround for wrong OLE variant structure definition
    hb_itemPutNInt(pItem, *(static_cast<int64_t *>(&V_UI4(pVariant))));
#else
...
void hb_oleVariantToItem(HB_ITEM *pItem, VARIANT *pVariant)
{
  hb_oleVariantToItemEx(pItem, pVariant, 0);
}

void hb_oleVariantUpdate(VARIANT *pVariant, HB_ITEM *pItem, HB_OLEOBJ_FUNC pObjFunc)
{
  switch (V_VT(pVariant)) {
  case VT_DISPATCH | VT_BYREF: {
    IDispatch *pDisp = hb_oleItemGetDispatch(pItem);
...
#elif defined(HB_OLE_NO_LLREF)
    // workaround for wrong OLE variant structure definition
    *reinterpret_cast<int64_t *>(V_R8REF(pVariant)) = static_cast<int64_t>(hb_itemGetNInt(pItem));
#else
    *V_I8REF(pVariant) = static_cast<int64_t>(hb_itemGetNInt(pItem));
#endif
    break;
  ...
  case VT_UI2 | VT_BYREF:
    *V_UI2REF(pVariant) = static_cast<uint16_t>(hb_itemGetNI(pItem));
    break;
...
HB_BOOL hb_oleDispInvoke(HB_SYMB *pSym, HB_ITEM *pObject, HB_ITEM *pParam, DISPPARAMS *pParams, VARIANT *pVarResult,
                         HB_OLEOBJ_FUNC pObjFunc, uint16_t uiClass)
{
  ...
  if ((pSym || pObject) && hb_vmRequestReenter()) {
    ...
    if (pObject && !pObject->isHash()) {
      hb_vmSend(static_cast<uint16_t>(iParams));
    } else {
      hb_vmProc(static_cast<uint16_t>(iParams));
    }
...
static void GetParams(DISPPARAMS *dispparam, uint32_t uiOffset, HB_BOOL fUseRef, UINT uiNamedArgs, HB_ITEM **pNamedArgs,
                      DISPID *pDispIds)
{
...
static HRESULT GetNamedParams(IDispatch *pDisp, OLECHAR *szMethodName, HB_ITEM *pHash, UINT *puiNamedArgs,
                              HB_ITEM **pArgs, DISPID *pDispIds)
{
  ...
  for (nPos = 1; nPos <= nLen; ++nPos) {
    HB_ITEM *pKey = hb_hashGetKeyAt(pHash, nPos);
    if (pKey->isString()) {
      pNames[iArgs + 1] =
...
static void PutParams(DISPPARAMS *dispparam, uint32_t uiOffset, uint16_t uiClass)
{
  ...
  for (uiArg = dispparam->cNamedArgs; uiArg < dispparam->cArgs; uiArg++) {
    auto uiParam = static_cast<uint16_t>(uiOffset + dispparam->cArgs - uiArg);
...
HB_FUNC(WIN_OLEAUTO___ONERROR)
{
  ...
  if (szMethod[0] == '_' && iPCount >= 1) {
    ...
    if (lOleError == S_OK) {
      ...
      lOleError = HB_VTBL(pDisp)->Invoke(HB_THIS_(pDisp) dispid, HB_ID_REF(IID_NULL), LOCALE_USER_DEFAULT, wFlags,
                                         &dispparam, nullptr, &excep, &uiArgErr);
  ...
  if (iPCount >= 1 && HB_ISHASH(1)) {
    ...
    HB_ITEM *pArgs[HB_OLE_MAX_NAMEDARGS];
...
HB_FUNC(__OLEVARIANTNEW)
{
...
#if defined(HB_OLE_NO_LL)
      // workaround for wrong OLE variant structure definition
      *(static_cast<int64_t *>(&V_I4(&variant))) = hb_itemGetNInt(pInit);
#else
  ...
  case VT_UI2:
    if (pInit == nullptr || pInit->isNumeric()) {
      V_VT(&variant) = VT_UI2;
      V_UI2(&variant) = static_cast<uint16_t>(hb_itemGetNI(pInit));
    }
    break;
