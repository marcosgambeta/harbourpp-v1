...
{
   HB_OLE * pOle = static_cast<HB_OLE*>(Cargo);
...
{
   IEnumVARIANT** ppEnum = static_cast<IEnumVARIANT**>(Cargo);
...
{
   VARIANT * pVariant = static_cast<VARIANT*>(Cargo);
...
static void hb_oleExcepDescription(EXCEPINFO * pExcep, char ** pszDescription, char ** pszSource)
{
   ...
   {
      int iStrLen = static_cast<int>(SysStringLen(pExcep->bstrSource));
   ...
   {
      int iStrLen = static_cast<int>(SysStringLen(pExcep->bstrDescription));
...
{
   HB_OLE * pOle = static_cast<HB_OLE*>(hb_parptrGC(&s_gcOleFuncs, iParam));
...
{
   HB_OLE * pOle = static_cast<HB_OLE*>(hb_itemGetPtrGC(pItem, &s_gcOleFuncs));
...
{
   HB_OLE * pOle = static_cast<HB_OLE*>(hb_gcAllocate(sizeof(HB_OLE), &s_gcOleFuncs));
...
{
   IEnumVARIANT ** ppEnum = static_cast<IEnumVARIANT**>(hb_parptrGC(&s_gcOleenumFuncs, iParam));
...
{
   VARIANT * pVariant = static_cast<VARIANT*>(hb_parptrGC(&s_gcVariantFuncs, iParam));
...
{
   VARIANT * pDestVariant = static_cast<VARIANT*>(hb_gcAllocate(sizeof(VARIANT), &s_gcVariantFuncs));
...
{
   UINT uiStrLen = static_cast<UINT>(hb_itemCopyStrU16(pItem, HB_CDP_ENDIAN_NATIVE, nullptr, UINT_MAX));
...
static HB_BOOL hb_oleSafeArrayFill(SAFEARRAY * pSafeArray, VARTYPE vt, PHB_ITEM pItem, int iDims, int iDim, long * plSize, long * plIndex)
{
   const char * pStr;
   void * pData;
   ...
   cElements = static_cast<UINT>(plSize[iDim - 1]);
   if( uiPos < cElements )
   {
      cElements = uiPos;
   }
   ...
   for( uiPos = 1; uiPos <= cElements; ++uiPos )
   {
      ...
      {
         case VT_I1:
            V_I1(&v) = pStr ? static_cast<HB_I8>(static_cast<signed char>(pStr[uiPos - 1])) : static_cast<HB_I8>(hb_arrayGetNI(pItem, uiPos));
            ptr = &V_I1(&v);
            break;
         case VT_UI1:
            V_UI1(&v) = pStr ? static_cast<HB_U8>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_U8>(hb_arrayGetNI(pItem, uiPos));
            ptr = &V_UI1(&v);
            break;
         case VT_I2:
            V_I2(&v) = pStr ? static_cast<HB_I16>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_I16>(hb_arrayGetNI(pItem, uiPos));
            ptr = &V_I2(&v);
            break;
         case VT_UI2:
            V_UI2(&v) = pStr ? static_cast<HB_U16>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_U16>(hb_arrayGetNI(pItem, uiPos));
            ptr = &V_UI2(&v);
            break;
         case VT_I4:
            V_I4(&v) = pStr ? static_cast<HB_I32>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_I32>(hb_arrayGetNL(pItem, uiPos));
            ptr = &V_I4(&v);
            break;
         case VT_UI4:
            V_UI4(&v) = pStr ? static_cast<HB_U32>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_U32>(hb_arrayGetNL(pItem, uiPos));
            ptr = &V_UI4(&v);
            break;
...
#  if defined(HB_OLE_NO_LL)
            /* workaround for wrong OLE variant structure definition */
            ptr = &V_I4(&v);
            *(static_cast<HB_LONGLONG*>(ptr)) = pStr ? static_cast<HB_I64>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_I64>(hb_arrayGetNInt(pItem, uiPos));
#  else
            V_I8(&v) = pStr ? static_cast<HB_I64>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_I64>(hb_arrayGetNInt(pItem, uiPos));
            ptr = &V_I8(&v);
#  endif
            break;
         case VT_UI8:
#  if defined(HB_OLE_NO_LL)
            /* workaround for wrong OLE variant structure definition */
            ptr = &V_UI4(&v);
            *(static_cast<HB_ULONGLONG*>(ptr)) = pStr ? static_cast<HB_U64>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_U64>(hb_arrayGetNInt(pItem, uiPos));
#  else
            V_UI8(&v) = pStr ? static_cast<HB_U64>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_U64>(hb_arrayGetNInt(pItem, uiPos));
            ptr = &V_UI8(&v);
#  endif
            break;
#endif
         case VT_INT:
            V_INT(&v) = pStr ? static_cast<HB_INT>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_INT>(hb_arrayGetNI(pItem, uiPos));
            ptr = &V_INT(&v);
            break;
         case VT_UINT:
            V_UINT(&v) = pStr ? static_cast<HB_UINT>(static_cast<unsigned char>(pStr[uiPos - 1])) : static_cast<HB_UINT>(hb_arrayGetNI(pItem, uiPos));
            ptr = &V_UINT(&v);
            break;
         case VT_ERROR:
            if( pStr == nullptr )
...
static SAFEARRAY * hb_oleSafeArrayFromItem(PHB_ITEM pItem, VARTYPE vt, int iDims, long * plSize)
{
   ...
   sabound = iDims > static_cast<int>(HB_SIZEOFARRAY(boundbuf)) ? static_cast<SAFEARRAYBOUND*>(hb_xgrab(sizeof(SAFEARRAYBOUND) * iDims)) : boundbuf;
...
static void hb_oleSafeArrayToItem(PHB_ITEM pItem, SAFEARRAY * pSafeArray, int iDims, int iDim, long * plIndex, VARTYPE vt, HB_USHORT uiClass)
{
   long lFrom, lTo;
      ...
      if( iDim == iDims )
      {
         ...
         do
         {
            ...
            if( SafeArrayGetElement(pSafeArray, plIndex,
                                    vt == VT_VARIANT ? static_cast<void*>(&vItem) :
...
void hb_oleVariantToItemEx(PHB_ITEM pItem, VARIANT * pVariant, HB_USHORT uiClass)
{
   ...
   switch( V_VT(pVariant) )
   {
      ...
      case VT_UNKNOWN | VT_BYREF:
      {
         ...
         hb_itemClear(pItem);
         if( punkVal && HB_VTBL(punkVal)->QueryInterface(HB_THIS_(punkVal) HB_ID_REF(IID_IDispatch), static_cast<void**>(static_cast<void*>(&pdispVal))) == S_OK )
         {
      ...
      case VT_DISPATCH | VT_BYREF:
         hb_itemClear(pItem);
         hb_oleDispatchToItem(pItem, V_VT(pVariant) == VT_DISPATCH ? V_DISPATCH(pVariant) : *V_DISPATCHREF(pVariant), uiClass);
         break;
      ...
      {
         double dblVal;
         if( VarR8FromDec(V_VT(pVariant) == VT_DECIMAL ? &HB_WIN_U1(pVariant, decVal) : V_DECIMALREF(pVariant), &dblVal) != S_OK )
         {
            dblVal = 0;
         }
         hb_itemPutND(pItem, dblVal);
   ...
   switch( V_VT(pVariant) )
   {
      ...
      case VT_UNKNOWN | VT_BYREF:
      {
         ...
         if( pDisp )
         {
            ...
            if( HB_VTBL(pDisp)->QueryInterface(HB_THIS_(pDisp) HB_ID_REF(IID_IEnumVARIANT), static_cast<void**>(static_cast<void*>(&pUnk))) == S_OK )
            {
...
HB_BOOL hb_oleDispInvoke(PHB_SYMB pSym, PHB_ITEM pObject, PHB_ITEM pParam, DISPPARAMS * pParams, VARIANT * pVarResult, HB_OLEOBJ_FUNC pObjFunc, HB_USHORT uiClass)
{
...
static void GetParams(DISPPARAMS * dispparam, HB_UINT uiOffset, HB_BOOL fUseRef, UINT uiNamedArgs, PHB_ITEM * pNamedArgs, DISPID * pDispIds)
{
   VARIANTARG * pArgs = nullptr;
   UINT         uiArgCount;

   uiArgCount = static_cast<UINT>(hb_pcount());
...
static HRESULT GetNamedParams(IDispatch * pDisp, OLECHAR * szMethodName, PHB_ITEM pHash, UINT * puiNamedArgs, PHB_ITEM * pArgs, DISPID * pDispIds)
{
...
HB_FUNC( __OLECREATEOBJECT ) /* (cOleName | cCLSID  [, cIID ]) */
{
   ...
   if( cOleName )
   {
      ...
      if( lOleError == S_OK )
      {
         lOleError = CoCreateInstance(HB_ID_REF(ClassID), nullptr, CLSCTX_SERVER, HB_ID_REF(iid), static_cast<void**>(static_cast<void*>(&pDisp)));
      }
   }
   else
   {
...
HB_FUNC( __OLEGETACTIVEOBJECT ) /* (cOleName | cCLSID  [, cIID ]) */
{
   ...
   if( cOleName )
   {
      wCLSID = static_cast<BSTR>(AnsiToWide(const_cast<LPSTR>(cOleName)));
      if( cOleName[0] == '{' )
      {
      ...
      if( cID )
      {
         if( cID[0] == '{' )
         {
            wCLSID = static_cast<BSTR>(AnsiToWide(const_cast<LPSTR>(cID)));
      ...
      if( lOleError == S_OK )
      {
         ...
         if( lOleError == S_OK )
         {
            lOleError = HB_VTBL(pUnk)->QueryInterface(HB_THIS_(pUnk) HB_ID_REF(iid), static_cast<void**>(static_cast<void*>(&pDisp)));
   ...
   if( lOleError == S_OK )
   {
      if( V_VT(&variant) == VT_UNKNOWN )
      {
         lOleError = HB_VTBL(V_UNKNOWN(&variant))->QueryInterface(
            ...
            HB_ID_REF(IID_IEnumVARIANT), static_cast<void**>(static_cast<void*>(&pEnum)));
      }
      ...
      if( lOleError == S_OK )
      {
         ...
         ppEnum = static_cast<IEnumVARIANT**>(hb_gcAllocate(sizeof(IEnumVARIANT*), &s_gcOleenumFuncs));
         *ppEnum = pEnum;
         hb_retptrGC(ppEnum);
         return;
      }
...
HB_FUNC( WIN_OLEAUTO___ONERROR )
{
   ...
   if( szMethod[0] == '_' && iPCount >= 1 )
   {
      ...
      lOleError = HB_VTBL(pDisp)->GetIDsOfNames(HB_THIS_(pDisp) HB_ID_REF(IID_NULL), &pMemberArray, 1, LOCALE_USER_DEFAULT, &dispid);
...
HB_FUNC( __OLEGETNAMEID )
{
   ...
   if( pDisp )
   {
      ...
      pwszMethod = const_cast<HB_WCHAR*>(hb_parstr_u16(2, HB_CDP_ENDIAN_NATIVE, &hMethod, nullptr));
      lOleError = HB_VTBL(pDisp)->GetIDsOfNames(HB_THIS_(pDisp) HB_ID_REF(IID_NULL), &pwszMethod, 1, LOCALE_USER_DEFAULT, &dispid);
      hb_strfree(hMethod);
      if( lOleError == S_OK )
      {
         hb_retnint(dispid);
...
static void hb_oleInvokeCall(WORD wFlags)
{
   ...
   if( pDisp )
   {
      ...
      ++uiOffset;
      pwszMethod = const_cast<HB_WCHAR*>(hb_parstr_u16(uiOffset, HB_CDP_ENDIAN_NATIVE, &hMethod, nullptr));
      if( pwszMethod )
      {
         lOleError = HB_VTBL(pDisp)->GetIDsOfNames(HB_THIS_(pDisp) HB_ID_REF(IID_NULL), &pwszMethod, 1, LOCALE_USER_DEFAULT, &dispid);
         hb_strfree(hMethod);
      ...
      if( lOleError == S_OK )
      {
         ...
         lOleError = HB_VTBL(pDisp)->Invoke(HB_THIS_(pDisp) dispid, HB_ID_REF(IID_NULL), LOCALE_USER_DEFAULT, wFlags, &dispparam, &variant, &excep, &uiArgErr);
         ...
         if( lOleError != S_OK )
         {
            ...
            hb_errRT_OLE(EG_ARG, 1007, static_cast<HB_ERRCODE>(lOleError), szExcepDescription, HB_ERR_FUNCNAME, szExcepSource);
...
HB_FUNC( __OLEVARIANTNEW )
{
   ...
   switch( iType )
   {
      ...
      case VT_UNKNOWN:
         ...
         if( pDisp )
         {
            if( HB_VTBL(pDisp)->QueryInterface(HB_THIS_(pDisp) HB_ID_REF(IID_IEnumVARIANT), static_cast<void**>(static_cast<void*>(&V_UNKNOWN(&variant)))) == S_OK )
            {
      ...
      default:
         if( iType & VT_ARRAY )
         {
            ...
            if( iType == (iType & VT_TYPEMASK) )
            {
               ...
               plSize = iDims < static_cast<int>(HB_SIZEOFARRAY(plBuf)) ? plBuf : static_cast<long*>(hb_xgrab(sizeof(long) * iDims));
