...
static HB_ERRCODE pgsqlConnect(SQLDDCONNECTION *pConnection, PHB_ITEM pItem)
{
  ...
  if (pszHost && (strncmp(pszHost, "postgresql://", 13) == 0 || strchr(pszHost, '='))) {
    ...
    pConn = PQsetdbLogin(pszHost, hb_arrayGetCPtr(pItem, 6), hb_arrayGetCPtr(pItem, 7), hb_arrayGetCPtr(pItem, 8),
                         hb_arrayGetCPtr(pItem, 5), hb_arrayGetCPtr(pItem, 3), hb_arrayGetCPtr(pItem, 4));
  }
...
static HB_ERRCODE pgsqlOpen(SQLBASEAREAP pArea)
{
  ...
  ExecStatusType status = PQresultStatus(pResult);
  if (status != PGRES_TUPLES_OK && status != PGRES_COMMAND_OK) {
    hb_errRT_PostgreSQLDD(EG_OPEN, ESQLDD_INVALIDQUERY, PQresultErrorMessage(pResult), pArea->szQuery,
                          static_cast<HB_ERRCODE>(status));
    PQclear(pResult);
    return Harbour::FAILURE;
  }
  ...
  pArea->pRowFlags = static_cast<uint8_t *>(hb_xgrabz((pArea->ulRecCount + 1) * sizeof(uint8_t)));
