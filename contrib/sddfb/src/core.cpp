...
static SDDNODE s_firebirddd = {nullptr,
                               "FIREBIRD",
...
static HB_ERRCODE fbConnect(SQLDDCONNECTION *pConnection, PHB_ITEM pItem)
{
  ...
  if (isc_attach_database(status, static_cast<short>(hb_arrayGetCLen(pItem, 5)), hb_arrayGetCPtr(pItem, 5), &hDb,
                          static_cast<short>(i), parambuf)) {
...
static HB_ERRCODE fbOpen(SQLBASEAREAP pArea)
{
  ...
  if (isc_dsql_allocate_statement(status, phDb, &hStmt)) {
    hb_errRT_FirebirdDD(EG_OPEN, ESQLDD_STMTALLOC, "Allocate statement failed", nullptr,
                        static_cast<HB_ERRCODE>(isc_sqlcode(status)));
  ...
  if (isc_dsql_prepare(status, &hTrans, &hStmt, 0, pArea->szQuery, SQL_DIALECT_V5, pSqlda)) {
    hb_errRT_FirebirdDD(EG_OPEN, ESQLDD_INVALIDQUERY, "Prepare statement failed", pArea->szQuery,
                        static_cast<HB_ERRCODE>(isc_sqlcode(status)));
  ...
  if (isc_dsql_execute(status, &hTrans, &hStmt, 1, nullptr)) {
    hb_errRT_FirebirdDD(EG_OPEN, ESQLDD_EXECUTE, "Execute statement failed", pArea->szQuery,
                        static_cast<HB_ERRCODE>(isc_sqlcode(status)));
    isc_dsql_free_statement(status, &hStmt, DSQL_drop);
    isc_rollback_transaction(status, &hTrans);
    hb_xfree(pSqlda);
  ...
  if (pSqlda->sqld > pSqlda->sqln) {
    ...
    if (isc_dsql_describe(status, &hStmt, SQL_DIALECT_V5, pSqlda)) {
      hb_errRT_FirebirdDD(EG_OPEN, ESQLDD_STMTDESCR, "Describe statement failed", nullptr,
                          static_cast<HB_ERRCODE>(isc_sqlcode(status)));
      isc_dsql_free_statement(status, &hStmt, DSQL_drop);
      isc_rollback_transaction(status, &hTrans);
      hb_xfree(pSqlda);
  ...
  while (ulRecNo > pArea->ulRecCount && !pArea->fFetched) {
    ...
    if (lErr == 0) {
      ...
      for (ui = 0; ui < pArea->area.uiFieldCount; ui++) {
        ...
        case SQL_TIMESTAMP:
          pItem = hb_itemPutTDT(pItem, (reinterpret_cast<ISC_TIMESTAMP *>(pVar->sqldata))->timestamp_date + 2400001,
                                (reinterpret_cast<ISC_TIMESTAMP *>(pVar->sqldata))->timestamp_time / 10);
          break;
      ...
      if (isc_dsql_free_statement(status, phStmt, DSQL_drop)) {
        hb_errRT_FirebirdDD(EG_OPEN, ESQLDD_STMTFREE, "Statement free error", nullptr,
                            static_cast<HB_ERRCODE>(isc_sqlcode(status)));
        return Harbour::FAILURE;
      }
      ...
      if (isc_commit_transaction(status, phTr)) {
        hb_errRT_FirebirdDD(EG_OPEN, ESQLDD_COMMIT, "Transaction commit error", nullptr,
                            static_cast<HB_ERRCODE>(isc_sqlcode(status)));
        return Harbour::FAILURE;
      }
