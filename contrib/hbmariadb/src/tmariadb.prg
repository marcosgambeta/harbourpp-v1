...
METHOD GetRow(nRow) CLASS TMariaDBQuery
   ...
   IF ::nResultHandle != NIL
      ...
      IF ::aRow != NIL
         ...
         oRow := TMariaDBRow():New(::aRow, ::aFieldStruct)
      ENDIF
   ENDIF
...
CREATE CLASS TMariaDBTable INHERIT TMariaDBQuery
...
METHOD Query(cQuery) CLASS TMariaDBServer
   ...
   IF nNumTables == 1
      oQuery := TMariaDBTable():New(::nSocket, cQuery, cTableName)
   ELSE
      oQuery := TMariaDBQuery():New(::nSocket, cQuery)
   ENDIF
