...
PROCEDURE wvw_NoSysMenu(nWin, lRemoveClose)
   ...
   IF !Empty(hWnd := wvw_Get_hnd_Window(nWin)) .AND. !Empty(hMenu := wapi_GetSystemMenu(hWnd, .F.))
...
FUNCTION wvw_TrackPopupMenu(nWin, hMenu)
   ...
   RETURN wapi_TrackPopupMenu(hMenu, hb_bitOr(WIN_TPM_CENTERALIGN, WIN_TPM_RETURNCMD, WIN_TPM_RECURSE), xy["x"], xy["y"], 0, hWnd)
...
PROCEDURE wvw_SetParent(nWin1, nWin2)
   ...
   IF nWin2 != 0 .AND. !Empty(hWnd1 := wvw_Get_hnd_Window(nWin1)) .AND. !Empty(hWnd2 := wvw_Get_hnd_Window(nWin2))
...
PROCEDURE wvw_SetWindowPos(nWin, x, y)  /* position in pixel */
   ...
   IF !Empty(hWnd := wvw_Get_hnd_Window(nWin))
      ...
      wapi_SetWindowPos(hWnd,, x, y, (rc["right"] - rc["left"]) + 1, (rc["bottom"] - rc["top"]) + 1, WIN_SWP_NOZORDER)
   ENDIF
...
FUNCTION wvw_SetOnTop(nWin)
   ...
   RETURN wapi_SetWindowPos(hWnd, WIN_HWND_TOPMOST, rc["left"], rc["top"],,, hb_bitOr(WIN_SWP_NOSIZE, WIN_SWP_NOMOVE, WIN_SWP_NOACTIVATE))
...
FUNCTION wvw_SetAsNormal(nWin)
   ...
   RETURN wapi_SetWindowPos(hWnd, WIN_HWND_NOTOPMOST, rc["left"], rc["top"],,, hb_bitOr(WIN_SWP_NOSIZE, WIN_SWP_NOMOVE, WIN_SWP_NOACTIVATE))
