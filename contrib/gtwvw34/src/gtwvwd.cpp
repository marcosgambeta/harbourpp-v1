...
static PWVW_GLO s_wvw = nullptr; /* FIXME: MT compatibility */
static auto s_fInit = false;
static auto s_fIsReady = false;
static auto s_fSizeIsReady = false;

static COLORREF s_COLORS[] = {BLACK,      BLUE,           GREEN,  CYAN,        RED,          MAGENTA,
                              BROWN,      LIGHT_GRAY,     GRAY,   BRIGHT_BLUE, BRIGHT_GREEN, BRIGHT_CYAN,
                              BRIGHT_RED, BRIGHT_MAGENTA, YELLOW, WHITE};

#ifdef WVW_DEBUG
static auto s_nCountPuts = 0;
static auto s_nCountScroll = 0;
static auto s_nCountPaint = 0;
static auto s_nSetFocus = 0;
static auto s_nKillFocus = 0;
#endif
...
static void hb_gtInitStatics(int32_t nWin, LPCTSTR szWinName, int32_t iRow1, int32_t iCol1, int32_t iRow2, int32_t iCol2);
static HWND hb_gt_wvwCreateWindow(HINSTANCE hInstance, HINSTANCE hPrevInstance, int32_t iCmdShow);
static bool hb_gt_wvwInitWindow(PWVW_WIN wvw_win, HWND hWnd, int32_t iCol, int32_t iRow);
...
static void hb_gt_wvwHandleMenuSelection(int32_t iMenuIndex);

static void hb_gt_wvwValidateCol(PWVW_WIN wvw_win);
static void hb_gt_wvwValidateRow(PWVW_WIN wvw_win);

static int32_t hb_gt_wvwCalcPixelHeight(PWVW_WIN wvw_win);
static int32_t hb_gt_wvwCalcPixelWidth(PWVW_WIN wvw_win);
static bool hb_gt_wvw_SetColors(PWVW_WIN wvw_win, HDC hdc, int32_t iColor);

static bool hb_gt_wvwTextOut(PWVW_WIN wvw_win, HDC hdc, int32_t iCol, int32_t iRow, HB_SIZE index, HB_SIZE nLen);
static HB_SIZE hb_gt_wvw_GetIndexForTextBuffer(PWVW_WIN wvw_win, int32_t iCol, int32_t iRow);

static void hb_gt_wvwCreateObjects(int32_t nWin);
...
static void hb_gt_wvwInputNotAllowed(int32_t nWin, UINT message, WPARAM wParam, LPARAM lParam);

static bool hb_gt_wvwInWindow(int32_t nWin, int32_t iRow, int32_t iCol);
static int32_t hb_gt_wvwFindWindow(int32_t iRow, int32_t iCol);

static void hb_gt_wvw_FUNCPrologue(int32_t iNumCoord, int32_t *iRow1, int32_t *iCol1, int32_t *iRow2, int32_t *iCol2);
static void hb_gt_wvw_FUNCEpilogue(void);

/* functions created in order to allow us operating MainCoord Mode: */
static void hb_gt_wvw_mouse_vSetPos(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol);
static int32_t hb_gt_wvw_vDispCount(PWVW_WIN wvw_win);
static void hb_gt_wvw_vDispBegin(PWVW_WIN wvw_win);
static void hb_gt_wvw_vDispEnd(PWVW_WIN wvw_win);
static void hb_gt_wvw_vReplicate(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol, int32_t iColor, uint8_t bAttr, uint16_t usChar,
                                 HB_SIZE nLen);
static bool hb_gt_wvw_vSetMode(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol);
static void hb_gt_wvw_vSetPos(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol);
static bool hb_gt_wvw_vGetChar(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol, int32_t *piColor, uint8_t *pbAttr, uint16_t *pusChar);
static void hb_gt_wvw_vPutCharX(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol, int32_t iColor, uint8_t bAttr, uint16_t usChar);
static bool hb_gt_wvw_vPutChar(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol, int32_t iColor, uint8_t bAttr, uint16_t usChar);
static bool hb_gt_wvw_vCheckPos(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol, long *plIndex);
static void hb_gt_wvw_vGetSize(PWVW_WIN wvw_win, int32_t *piRows, int32_t *piCols);

static void hb_wvw_InitPendingRect(PWVW_WIN wvw_win);
static void hb_wvw_UpdatePendingRect(PWVW_WIN wvw_win, int32_t iRow1, int32_t iCol1, int32_t iRow2, int32_t iCol2);

static void hb_gt_wvwFillLineSpace(PWVW_WIN wvw_win, HDC hdc, int32_t startCol, int32_t iRow, HB_SIZE len, int32_t iColor);
...
static void s_RunControlBlock(PWVW_WIN wvw_win, int32_t nClass, HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam,
                              int32_t iEventType);
static void s_ReposControls(PWVW_WIN wvw_win, int32_t nClass);
...
static void hb_gt_wvw_Init(HB_GT *pGT, HB_FHANDLE hFilenoStdin, HB_FHANDLE hFilenoStdout, HB_FHANDLE hFilenoStderr)
{
  ...
  if (!s_fInit) {
    s_wvw = static_cast<PWVW_GLO>(hb_xgrabz(sizeof(WVW_GLO)));

    s_wvw->iPaintRefresh = 100;
    s_wvw->fMainCoordMode = false;
    s_wvw->fVertCaret = false;
    s_wvw->fNOSTARTUPSUBWINDOW = false;
    s_wvw->fDevCentreWindow = false;
    s_wvw->fDevHCentreWindow = false;
    s_wvw->fDevVCentreWindow = false;
    s_wvw->iDefLineSpacing = 0;
    s_wvw->iDefLSpaceColor = -1;
    s_wvw->fAllowNonTop = false;
    s_wvw->fRecurseCBlock = false;
    s_wvw->hWndTT = 0;
    s_wvw->fQuickSetMode = false;
    s_wvw->fFlashingWindow = false;
    s_wvw->iScrolling = 0;
  ...
  s_wvw->a.OriginalPen = static_cast<HPEN>(SelectObject(s_wvw->pWin[0]->hdc, s_wvw->a.penWhite));
  s_wvw->a.OriginalBrush = static_cast<HBRUSH>(SelectObject(s_wvw->pWin[0]->hdc, s_wvw->a.currentBrush));
...
void hb_gt_wvw_SetPos(HB_GT *pGT, int32_t iRow, int32_t iCol)
{
  ...
  hb_gt_wvw_FUNCPrologue(2, &iRow, &iCol, nullptr, nullptr);
...
static void hb_gt_wvw_SetCursorStyle(HB_GT *pGT, int32_t iStyle)
{
  ...
  if (fCursorOn) {
    if (!s_wvw->fVertCaret) {
      s_wvw->a.CaretExist =
          static_cast<bool>(CreateCaret(wvw_win->hWnd, nullptr, wvw_win->PTEXTSIZE.x, wvw_win->CaretSize));
    } else {
      s_wvw->a.CaretExist =
          static_cast<bool>(CreateCaret(wvw_win->hWnd, nullptr, wvw_win->CaretSize, wvw_win->PTEXTSIZE.y));
    }
  }
...
static void hb_gt_wvw_Replicate(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t iColor, uint8_t bAttr, uint16_t usChar,
                                HB_SIZE nLen)
{
#if 0
...
static void hb_gt_wvw_vPutText(PWVW_WIN wvw_win, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, const TCHAR *pbyBuffer,
                               int32_t iColor)
{
  HB_SIZE j = 0;
  ...
  for (int32_t iRow = iTop; iRow <= iBottom; iRow++) {
    ...
    for (int32_t iCol = iLeft; iCol <= iRight; iCol++) {
...
static int32_t hb_gt_wvw_PutText(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t iColor, const char *szText, HB_SIZE nLen)
{
#if 0
  ...
  /* Very experimental and half-done */
  {
    ...
    if (cdp) {
      ...
      for (HB_SIZE pos = 0; pos < nLen; ++pos) {
...
static void hb_gt_wvw_SetAttribute(HB_GT *pGT, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, int32_t iColor)
{
#if 0
...
static HB_BOOL hb_gt_wvw_SetMode(HB_GT *pGT, int32_t iRow, int32_t iCol)
{
#if 0
...
static void hb_gt_wvw_WriteAt(HB_GT *pGT, int32_t iRow, int32_t iCol, const char *pText, HB_SIZE nLen)
{
...
static void hb_gt_wvw_usBox(HB_GT *pGT, PWVW_WIN wvw_win, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight,
                            const char *szFrame, int32_t iColor)
{
  int32_t iMaxRow, iMaxCol, i;
  ...
  if (iTop <= iMaxRow && iLeft <= iMaxCol && iBottom >= 0 && iRight >= 0) {
    ...
    auto bPadCh = static_cast<BYTE>(HB_GTSELF_GETCLEARCHAR(hb_gt_Base())); /* FIXME */
    ...
    if (iTop == iBottom) {
      ...
      int32_t iRows = (iBottom > iMaxRow ? iMaxRow + 1 : iBottom) - (iTop < 0 ? -1 : iTop) - 1;
      int32_t iCols = (iRight > iMaxCol ? iMaxCol + 1 : iRight) - (iLeft < 0 ? -1 : iLeft) - 1;
      int32_t iFirst = iLeft < 0 ? 0 : iLeft + 1;
...
static void hb_gt_wvw_Box(HB_GT *pGT, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, const char *szFrame, int32_t iColor)
{
  iTop = iTop < 0 ? 0 : iTop;
...
static void hb_gt_wvw_HorizLine(HB_GT *pGT, int32_t iRow, int32_t iLeft, int32_t iRight, uint16_t usChar, int32_t iColor)
{
  int32_t iWidth;
  ...
  if (s_wvw->fMainCoordMode) {
    ...
    hb_gt_wvw_FUNCPrologue(4, &iRow, &iLeft, nullptr, &iRight);
  }
  ...
  if (iRow < iWidth) {
    ...
    if (iLeft < iRight) {
      hb_gt_wvw_vReplicate(s_wvw->pWin[s_wvw->iCurWindow], iRow, iLeft, iColor, HB_GT_ATTR_BOX, usChar,
                           iRight - iLeft + 1);
    } else {
      hb_gt_wvw_vReplicate(s_wvw->pWin[s_wvw->iCurWindow], iRow, iRight, iColor, HB_GT_ATTR_BOX, usChar,
                           iLeft - iRight + 1);
    }
  }
...
static void hb_gt_wvw_VertLine(HB_GT *pGT, int32_t iCol, int32_t iTop, int32_t iBottom, uint16_t usChar, int32_t iColor)
{
  int32_t iWidth;
  int32_t iHeight;
  ...
  if (s_wvw->fMainCoordMode) {
    ...
    hb_gt_wvw_FUNCPrologue(3, &iTop, &iCol, &iBottom, nullptr);
  }
...
static void hb_gt_wvw_OutStd(HB_GT *pGT, const char *pbyStr, HB_SIZE nLen)
{
  HB_SYMBOL_UNUSED(pGT);
...
static void hb_gt_wvw_OutErr(HB_GT *pGT, const char *pbyStr, HB_SIZE nLen)
{
  HB_SYMBOL_UNUSED(pGT);
...
static HB_BOOL hb_gt_wvw_GetCharFromInputQueue(int32_t *c)
{
...
static HB_BOOL hb_gt_wvw_mouse_IsPresent(HB_GT *pGT)
{
  HB_SYMBOL_UNUSED(pGT);
...
static int32_t hb_gt_wvw_mouse_Col(HB_GT *pGT)
{
  ...
  if (s_wvw->fMainCoordMode) {
    return hb_gt_wvw_GetMouseX(s_wvw->pWin[s_wvw->iNumWindows - 1]) +
           hb_gt_wvw_ColOfs(s_wvw->pWin[s_wvw->iNumWindows - 1]);
  } else {
...
static int32_t hb_gt_wvw_mouse_Row(HB_GT *pGT)
{
  ...
  if (s_wvw->fMainCoordMode) {
    return hb_gt_wvw_GetMouseY(s_wvw->pWin[s_wvw->iNumWindows - 1]) +
           hb_gt_wvw_RowOfs(s_wvw->pWin[s_wvw->iNumWindows - 1]);
  } else {
...
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_gt_wvw_mouse_GetPos(%p,%p,%p)", static_cast<void*>(pGT), piRow, piCol));
#endif
...
static HB_BOOL hb_gt_wvw_mouse_ButtonState(HB_GT *pGT, int32_t iButton)
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_gt_wvw_mouse_ButtonState(%p,%i)", static_cast<void*>(pGT), iButton));
#endif
...
static HB_BOOL hb_gt_wvw_Info(HB_GT *pGT, int32_t iType, HB_GT_INFO *pInfo)
{
  ...
  case HB_GTI_WINTITLE:
    ...
    if (hb_itemType(pInfo->pNewVal) & Harbour::Item::STRING) {
      void *hWindowTitle;
      hb_gt_wvw_SetWindowTitle(wvw_win, HB_ITEMGETSTR(pInfo->pNewVal, &hWindowTitle, nullptr));
      hb_strfree(hWindowTitle);
    }
    break;
...
#else
      hb_gt_winapi_setClipboard(wvw_win->CodePage == OEM_CHARSET ? CF_OEMTEXT : CF_TEXT, pInfo->pNewVal);
#endif
    else {
...
#else
      hb_gt_winapi_getClipboard(wvw_win->CodePage == OEM_CHARSET ? CF_OEMTEXT : CF_TEXT, pInfo->pResult);
#endif
    }
    break;
    ...
    if (hb_itemType(pInfo->pNewVal) & Harbour::Item::NUMERIC) {
      ...
      if (iIndex >= 0 && iIndex < 16) {
        ...
        if (hb_itemType(pInfo->pNewVal2) & Harbour::Item::NUMERIC) {
          s_COLORS[iIndex] = static_cast<COLORREF>(hb_itemGetNL(pInfo->pNewVal2));
        }
      }
    } else {
      if (!pInfo->pResult) {
...
{
  auto wvw_win = hb_gt_wvw_win(s_wvw->iCurWindow);
  RECT r;
  auto iRet = 0;
...
BOOL CALLBACK hb_gt_wvw_DlgProcMLess(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  if (pFunc) {
    ...
    case WM_NCDESTROY:
      if (s_wvw->a.pFunc[iIndex] != nullptr && s_wvw->a.iType[iIndex] == 2) {
...
BOOL CALLBACK hb_gt_wvw_DlgProcModal(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  if (pFunc) {
    ...
    case WM_NCDESTROY:
      if (s_wvw->a.pFuncModal[iIndex] != nullptr && s_wvw->a.iTypeModal[iIndex] == 2) {
...
static int32_t hb_gt_wvwCalcPixelHeight(PWVW_WIN wvw_win)
{
  return hb_gt_wvw_LineHeight(wvw_win) * wvw_win->ROWS + wvw_win->iSBHeight + wvw_win->iTBHeight;
}
...
static bool hb_gt_wvwAllocSpBuffer(PWVW_WIN wvw_win, int32_t iCol, int32_t iRow)
{
  int32_t iColor = wvw_win->nWinId == 0 ? 0x07 : hb_gtGetCurrColor();

  wvw_win->COLS = iCol;
  wvw_win->ROWS = iRow;
  wvw_win->BUFFERSIZE = static_cast<HB_SIZE>(iCol * iRow);
...
bool hb_gt_wvw_ValidWindowSize(PWVW_WIN wvw_win, int32_t iRows, int32_t iCols, HFONT hFont, int32_t iWidth, int32_t *piMaxRows,
                               int32_t *piMaxCols)
{
  RECT rcWorkArea{};
...
void hb_gt_wvw_ResetWindowSize(PWVW_WIN wvw_win, HWND hWnd)
{
  ...
  PWVW_WIN wvw_zer = s_wvw->pWin[0];

  /* set the font and get it's size to determine the size of the client area
     for the required number of rows and columns */
  auto hdc = GetDC(hWnd);
  auto hFont = hb_gt_wvw_GetFont(wvw_win->fontFace, wvw_win->fontHeight, wvw_win->fontWidth, wvw_win->fontWeight,
                                 wvw_win->fontQuality, wvw_win->CodePage);
  ...
  if (wvw_win->fontWidth < 0 || s_wvw->a.Win9X || (tm.tmPitchAndFamily & TMPF_FIXED_PITCH) ||
      wvw_win->PTEXTSIZE.x != tm.tmMaxCharWidth) {
    wvw_win->FixedFont = false;
  } else {
    wvw_win->FixedFont = true;
  }

  for (auto n = 0; n < wvw_win->COLS;
       n++) { /* wvw_win->FixedSize[] is used by ExtTextOut() to emulate */ /*          fixed font when a proportional
                                                                               font is used */
  ...
  if (IsZoomed(wvw_win->hWnd)) {
    ...
    height += diffHeight;
      ...
      /* Centre the window within the CLIENT area on the screen
         but only if wvw_win->CentreWindow == true */
  ...
  if (wvw_win->hStatusBar != nullptr) {
    SetWindowPos(wvw_win->hStatusBar, nullptr, wi.left, wi.bottom - wvw_win->iSBHeight, width, wvw_win->iSBHeight,
                 SWP_NOZORDER);
  }

  if (wvw_win->hToolBar != nullptr) {
    SetWindowPos(wvw_win->hToolBar, nullptr, wi.left, wi.top - wvw_win->iTBHeight, width, wvw_win->iTBHeight,
                 SWP_NOZORDER);
  }

  if (wvw_win->ctlList != nullptr) {
...
int32_t hb_gt_wvw_key_ansi_to_oem(int32_t c)
{
#if !defined(UNICODE)
  BYTE pszSrc[2];
  wchar_t pszWide[1];
...
static void xUserTimerNow(int32_t nWin, HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  static auto s_fRunning = false;
...
static LRESULT CALLBACK hb_gt_wvwWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_COMMAND: { /* handle menu items */
    ...
    if (iId < WVW_ID_BASE_PUSHBUTTON) {
      ...
      if (fTopMost || s_wvw->fAllowNonTop) {
        ...
        int32_t nCtrlId = hb_gt_wvw_FindControlId(wvw_win, WVW_CONTROL_PUSHBUTTON, hCtrlWnd, nullptr);
        ...
        hb_gt_wvwInputNotAllowed(nWin, message, wParam, lParam);
            ...
            TraceLog(nullptr, "  lowordwParam (control id)=%i\n", lowordwParam);
            TraceLog(nullptr, "  hiwordwParam (notification)=%i\n", hiwordwParam);
            TraceLog(nullptr, "  lowordlParam=%i\n", lowordlParam);
            TraceLog(nullptr, "  hiwordlParam=%i\n", hiwordlParam);
#endif
      ...
      case CBN_KILLFOCUS:
        ...
        if (iEvent == CBN_KILLFOCUS || fTopMost || s_wvw->fAllowNonTop) {
          auto hCtrlWnd = reinterpret_cast<HWND>(lParam);
          int32_t nCtrlId = hb_gt_wvw_FindControlId(wvw_win, WVW_CONTROL_COMBOBOX, hCtrlWnd, nullptr);
      ...
      case EN_CHANGE:
        ...
        if (iEvent == EN_KILLFOCUS || fTopMost || s_wvw->fAllowNonTop) {
          auto hCtrlWnd = reinterpret_cast<HWND>(lParam);
          int32_t nCtrlId = hb_gt_wvw_FindControlId(wvw_win, WVW_CONTROL_EDITBOX, hCtrlWnd, nullptr);
  ...
  case WM_PAINT: {
    ...
    auto hOldFont = static_cast<HFONT>(SelectObject(hdc, wvw_win->hFont));
...
#ifdef WVW_DEBUG
    printf("\nPuts(%d), Scroll(%d), Paint(%d), SetFocus(%d), KillFocus(%d) ", s_nCountPuts, s_nCountScroll,
           ++s_nCountPaint, s_nSetFocus, s_nKillFocus);
#endif
    return 0;
  }
  ...
  case WM_SETFOCUS:
    ...
    if (nWin == s_wvw->iNumWindows - 1) {
      if (!s_wvw->fMainCoordMode) {
        ...
        hb_gtSetPos(wvw_win->caretPos.y + hb_gt_wvw_RowOfs(wvw_win), wvw_win->caretPos.x + hb_gt_wvw_ColOfs(wvw_win));
      }
  ...
  case WM_SYSKEYDOWN:
    ...
    /* case WM_SYSCHAR: */
    {
      ...
      wvw_win->fIgnoreWM_SYSCHAR = false;
      switch (wParam) {
      case VK_LEFT:
      ...
      default: {
        ...
        if (bCtrl && iScanCode == 76) { /* CTRL_VK_NUMPAD5 ) */
          ...
          if (bCtrl) {                                    /* Not scroll lock */
            ...
            wvw_win->fIgnoreWM_SYSCHAR = true;
          } else {
  ...
  case WM_VSCROLL: {
    ...
    nCtrlId = hb_gt_wvw_FindControlId(wvw_win, WVW_CONTROL_SCROLLBAR, hCtrlWnd, nullptr);
    if (nCtrlId != 0) {
  ...
  case WM_CTLCOLOREDIT: {
    if (s_wvw->a.pSymWVW_ONCTLCOLOR) {
      ...
      hb_vmPushNumInt(lParam);
      hb_vmDo(2);
      auto res = static_cast<LRESULT>(hbwapi_par_RESULT(-1));
      if (res != -1) {
        return res;
      }
    }
  ...
  case WM_DRAWITEM:
    ...
    if (wvw_win->fSBPaint) {
      ...
      ExtTextOut(lpDIS->hDC, rectCorner.top, rectCorner.left + 3, 0, &lpDIS->rcItem, ptStr, static_cast<UINT>(stLen),
                 nullptr);
...
static HWND hb_gt_wvwCreateWindow(HINSTANCE hInstance, HINSTANCE hPrevInstance, int32_t iCmdShow)
{
  ...
  if (!RegisterClass(&wndclass)) {
    if (GetLastError() != ERROR_CLASS_ALREADY_EXISTS) {
      hb_errInternal(10001, "Failed to register WVW window class", nullptr, nullptr);
    }
    return nullptr;
  }
  ...
  if (hWnd == nullptr) {
    hb_errInternal(10001, "Failed to create WVW window", nullptr, nullptr);
    return nullptr;
  }
  ...
  if (s_wvw->a.pSymWVW_PAINT && s_wvw->iPaintRefresh > 0) {
    SetTimer(hWnd, WVW_ID_SYSTEM_TIMER, static_cast<UINT>(s_wvw->iPaintRefresh), nullptr);
  }
...
void hb_gt_wvw_CreateToolTipWindow(PWVW_WIN wvw_win)
{
  ...
  if (InitCommonControlsEx(&icex)) {
     ...
     */
    HWND hWndTT = CreateWindow(TOOLTIPS_CLASS, TEXT(""), WS_POPUP | TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT,
                               CW_USEDEFAULT, CW_USEDEFAULT, nullptr, nullptr, s_wvw->hInstance, nullptr);
    ...
    if (SendMessage(s_wvw->hWndTT, TTM_ADDTOOL, 0, reinterpret_cast<LPARAM>(&ti))) {
...
WPARAM hb_gt_wvw_ProcessMessages(PWVW_WIN wvw_win)
{
  ...
  while (PeekMessage(&msg, nullptr, 0, 0, PM_NOREMOVE)) {
    ...
    if (s_wvw->iScrolling && msg.message == WM_LBUTTONUP) {
      ...
      s_wvw->iWrongButtonUp = 0;
      (void)PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE); // TODO: C++ cast
    }
...
POINT hb_gt_wvw_GetXYFromColRow(PWVW_WIN wvw_win, int32_t iCol, int32_t iRow)
{
  POINT xy;
...
POINT hb_gt_wvw_GetColRowFromXY(PWVW_WIN wvw_win, int32_t x, int32_t y)
{
  POINT colrow;
...
{
  RECT colrow;
  int32_t iLineSpacing = wvw_win->iLineSpacing; /* TODO: pls improve efficiency */
  ...
  colrow.bottom =
      xy.bottom / (wvw_win->PTEXTSIZE.y + iLineSpacing) - ((xy.bottom % (wvw_win->PTEXTSIZE.y + iLineSpacing)) ? 0 : 1);
...
static HB_SIZE hb_gt_wvw_GetIndexForTextBuffer(PWVW_WIN wvw_win, int32_t iCol, int32_t iRow)
{
  return static_cast<HB_SIZE>(iRow * wvw_win->COLS + iCol);
}
...
static bool hb_gt_wvwTextOut(PWVW_WIN wvw_win, HDC hdc, int32_t iCol, int32_t iRow, HB_SIZE index, HB_SIZE nLen)
{
  ...
  if (nLen > static_cast<HB_SIZE>(wvw_win->COLS)) {
    nLen = static_cast<HB_SIZE>(wvw_win->COLS);
  }
  ...
  if (wvw_win->FixedFont) {
    return static_cast<bool>(ExtTextOut(hdc, xy.x, xy.y, ETO_CLIPPED | ETO_OPAQUE, &mClip, wvw_win->TextLine,
                                        static_cast<UINT>(nLen), nullptr));
  } else {
    return static_cast<bool>(ExtTextOut(hdc, xy.x, xy.y, ETO_CLIPPED | ETO_OPAQUE, &mClip, wvw_win->TextLine,
                                        static_cast<UINT>(nLen), wvw_win->FixedSize));
  }
}
...
static bool hb_gt_wvw_SetColors(PWVW_WIN wvw_win, HDC hdc, int32_t iColor)
{
  int32_t fore = iColor & 0x000F;
  int32_t back = (iColor & 0x00F0) >> 4;
...
void hb_gt_wvw_SetInvalidRect(PWVW_WIN wvw_win, int32_t iLeft, int32_t iTop, int32_t iRight, int32_t iBottom)
{
...
int32_t hb_gt_wvw_JustTranslateKey(int32_t key, int32_t shiftkey, int32_t altkey, int32_t controlkey)
{
...
HFONT hb_gt_wvw_GetFont(LPCTSTR pszFace, int32_t iHeight, int32_t iWidth, int32_t iWeight, int32_t iQuality, int32_t iCodePage)
{
  if (iHeight > 0) {
    LOGFONT lf{};
    ...
    lf.lfUnderline = 0;
    lf.lfStrikeOut = 0;
    lf.lfCharSet = static_cast<BYTE>(iCodePage); /* OEM_CHARSET */
    lf.lfOutPrecision = 0;
    lf.lfClipPrecision = 0;
    lf.lfQuality = static_cast<BYTE>(iQuality);    /* DEFAULT_QUALITY, DRAFT_QUALITY or PROOF_QUALITY */
...
static void hb_gtInitStatics(int32_t nWin, LPCTSTR szWinName, int32_t iRow1, int32_t iCol1, int32_t iRow2, int32_t iCol2)
{
  ...
  if (nWin == 0) {
    ...
    wvw_win->CentreWindow = true; /* Default is to always display window in centre of screen */

    /* two following parameters are meaningful only if CentreWindow is false */
    wvw_win->HCentreWindow = false; /* horizontally */
    wvw_win->VCentreWindow = false; /* vertically */

    wvw_win->CodePage = GetACP(); /* Set code page to default system */

    s_wvw->a.Win9X = hb_iswin9x();
    s_wvw->a.AltF4Close = false;

    wvw_win->InvalidateWindow = true;
    wvw_win->EnableShortCuts = false;

    wvw_win->fToolTipActive = false;
    wvw_win->hWndTT = nullptr;
    wvw_win->hPopup = nullptr;
    ...
    s_wvw->a.pbhBitmapList = nullptr;
    s_wvw->a.pphPictureList = nullptr;

    s_wvw->a.pbhUserBitmap = nullptr;
    s_wvw->a.iBMcache = 0;
  ...
  wvw_win->fIgnoreWM_SYSCHAR = false;
  wvw_win->fPaint = false;
  wvw_win->fGetFocus = false;
  ...
  wvw_win->ctlList = nullptr;

  wvw_win->hPBfont = nullptr; /* will be created on first creation of pushbutton, if ever */
  wvw_win->hCBfont = nullptr; /* will be created on first creation of combobox, if ever */
  wvw_win->hEBfont = nullptr; /* will be created on first creation of editbox, if ever */
  wvw_win->hCXfont = nullptr; /* will be created on first creation of checkbox, if ever */
  wvw_win->hSBfont = nullptr; /* will be created on first creation of statusbar, if ever */
  wvw_win->hSTfont = nullptr; /* will be created on first creation of static control, if ever */
...
void hb_gt_wvw_AddCharToInputQueue(int32_t iKey)
{
  int32_t nWin = s_wvw->iNumWindows - 1;
  int32_t iNextPos = s_wvw->pWin[nWin]->keyPointerIn;

  if (iKey == K_MOUSEMOVE || iKey == K_NCMOUSEMOVE) {
    if (s_wvw->pWin[nWin]->keyLast == iKey &&
        s_wvw->pWin[s_wvw->iNumWindows - 1]->keyPointerIn != s_wvw->pWin[nWin]->keyPointerOut) {
      return;
    }
  }
...
int32_t hb_gt_wvw_GetMouseX(PWVW_WIN wvw_win)
{
  return wvw_win->mousePos.x;
}

int32_t hb_gt_wvw_GetMouseY(PWVW_WIN wvw_win)
{
  return wvw_win->mousePos.y;
}

void hb_gt_wvw_SetMouseX(PWVW_WIN wvw_win, int32_t ix)
{
  wvw_win->mousePos.x = ix;
}

void hb_gt_wvw_SetMouseY(PWVW_WIN wvw_win, int32_t iy)
{
...
int32_t hb_gt_wvw_OpenWindow(LPCTSTR szWinName, int32_t iRow1, int32_t iCol1, int32_t iRow2, int32_t iCol2, DWORD dwStyle, HWND hWndParent)
{
#if 0
  ...
  WNDCLASS wndclass;
   ...
   if( s_wvw->fMainCoordMode && (!hb_gt_wvwInWindow(0, iRow1, iCol1) || !hb_gt_wvwInWindow(0, iRow2, iCol2) ) ) {
      MessageBox(nullptr, TEXT("TRACE: Invalid (Row,Col)" ), szWinName, MB_ICONERROR);
      return 0;
   }
#endif
  ...
  hWnd = CreateWindow(s_wvw->szSubWinName, szWinName, /* window name */
                      dwStyle,
  ...
  s_wvw->pWin[s_wvw->iNumWindows - 1]->hIcon = nullptr;
  ...
  hb_gt_wvwWindowEpilogue();
   ...
   if( s_wvw->iNumWindows == 1 ) {
      if( !UnregisterClass(s_wvw->szSubWinName, s_wvw->hInstance) ) {
         MessageBox(nullptr, TEXT("TRACE: Failed UnregisterClass()"), hb_gt_wvw_GetAppName(), MB_ICONERROR);
      }
   }
#endif
...
bool hb_gt_wvw_BufferedKey(int32_t iKey)
{
  return iKey != VK_SHIFT && iKey != VK_MENU && iKey != VK_CONTROL && iKey != VK_PAUSE && iKey != VK_CAPITAL &&
         iKey != VK_NUMLOCK && iKey != VK_SCROLL;
}
...
int32_t hb_gt_wvw_RowOfs(PWVW_WIN wvw_win)
{
  return wvw_win->iRowOfs;
}

/* returns col offset of window */
int32_t hb_gt_wvw_ColOfs(PWVW_WIN wvw_win)
{
...
void hb_gt_wvw_HBFUNCPrologue(PWVW_WIN wvw_win, int32_t *piRow1, int32_t *piCol1, int32_t *piRow2, int32_t *piCol2)
{
...
{
  if (s_wvw->fMainCoordMode) {
    s_wvw->pWin[0]->caretPos.y =
        s_wvw->pWin[s_wvw->iCurWindow]->caretPos.y + hb_gt_wvw_RowOfs(s_wvw->pWin[s_wvw->iCurWindow]);
    s_wvw->pWin[0]->caretPos.x =
        s_wvw->pWin[s_wvw->iCurWindow]->caretPos.x + hb_gt_wvw_ColOfs(s_wvw->pWin[s_wvw->iCurWindow]);
 ...
 */
int32_t hb_gt_wvw_SetCurWindow(int32_t nWin)
{
  int32_t nWinOld = s_wvw->iCurWindow;
  bool fMainCoordMode;
  ...
  hb_gtSetPos(s_wvw->pWin[s_wvw->iCurWindow]->caretPos.y, s_wvw->pWin[s_wvw->iCurWindow]->caretPos.x);
...
static HB_BOOL hb_gt_wvw_GetChar(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t *piColor, uint8_t *pbAttr, uint16_t *pusChar)
{
  PWVW_WIN wvw_win = s_wvw->pWin[s_wvw->iNumWindows - 1];
...
static HB_BOOL hb_gt_wvw_PutChar(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t iColor, uint8_t bAttr, uint16_t usChar)
{
  PWVW_WIN wvw_win = s_wvw->pWin[s_wvw->iNumWindows - 1];
...
static bool hb_gt_wvw_vCheckPos(PWVW_WIN wvw_win, int32_t iRow, int32_t iCol, long *plIndex)
{
  if (iRow >= 0 && iCol >= 0) {
    ...
    if (iRow < iHeight && iCol < iWidth) {
      if (plIndex) {
        *plIndex = static_cast<long>(hb_gt_wvw_GetIndexForTextBuffer(wvw_win, iCol, iRow));
      }
      return true;
    }
  }
...
static HB_BOOL hb_gt_wvw_CheckPos(HB_GT *pGT, int32_t iRow, int32_t iCol, long *plIndex)
{
  PWVW_WIN wvw_win = s_wvw->pWin[s_wvw->iNumWindows - 1];
  ...
  if (iRow <= WVW_MAX_ROWS && iCol <= WVW_MAX_COLS) {
    if (wvw_win->hWnd) {
      ...
      if (hFont) {
         ...
         */
        if (hb_gt_wvw_ValidWindowSize(wvw_win, iRow, iCol, hFont, wvw_win->fontWidth, nullptr, nullptr)) {
...
/* called during init static, or after user paint
   This function must be called only when fPaintPending == false */
static void hb_wvw_InitPendingRect(PWVW_WIN wvw_win)
{
...
int32_t hb_gt_wvw_LineHeight(PWVW_WIN wvw_win)
{
...
static void hb_gt_wvwFillLineSpace(PWVW_WIN wvw_win, HDC hdc, int32_t startCol, int32_t iRow, HB_SIZE len, int32_t iColor)
{
  ...
  int32_t byColorIndex = wvw_win->iLSpaceColor < 0 ? ((iColor & 0x00F0) >> 4) : wvw_win->iLSpaceColor;
...
int32_t hb_gt_wvw_SetMenuKeyEvent(PWVW_WIN wvw_win, int32_t iMenuKeyEvent)
{
  int32_t iOldEvent = wvw_win->MenuKeyEvent;
...
int32_t hb_gt_wvw_SetCodePage(PWVW_WIN wvw_win, int32_t iCodePage)
{
  int32_t iOldCodePage = wvw_win->CodePage;
...
HICON hb_gt_wvw_SetWindowIcon(PWVW_WIN wvw_win, int32_t nIcon, LPCTSTR szIconName)
{
  HICON hIcon;

  if (szIconName == nullptr) {
    hIcon = LoadIcon(GetModuleHandle(nullptr), MAKEINTRESOURCE(nIcon));
  } else {
    hIcon = LoadIcon(GetModuleHandle(nullptr), szIconName);
  }

  if (hIcon) {
    SendMessage(wvw_win->hWnd, WM_SETICON, ICON_SMALL, reinterpret_cast<LPARAM>(hIcon)); /* Set Title Bar ICON */
    SendMessage(wvw_win->hWnd, WM_SETICON, ICON_BIG, reinterpret_cast<LPARAM>(hIcon));   /* Set Task List Icon */
    wvw_win->hIcon = hIcon;
  }
...
{
  auto hIcon = static_cast<HICON>(LoadImage(nullptr, szIconName, IMAGE_ICON, 0, 0, LR_LOADFROMFILE));
...
bool hb_gt_wvw_GetIPictDimension(IPicture *pPicture, int32_t *piWidth, int32_t *piHeight)
{
...
bool hb_gt_wvw_GetImageDimension(const char *szImage, int32_t *piWidth, int32_t *piHeight)
{
  auto fResult = true;

  *piWidth = *piHeight = 0;

  auto hBitmap = hb_gt_wvw_FindUserBitmapHandle(szImage, piWidth, piHeight);

  if (!hBitmap) {
    auto pPicture = hb_gt_wvw_LoadPicture(szImage);
...
IPicture *hb_gt_wvw_LoadPicture(const char *szImage)
{
  ...
  if (szImage) {
    PHB_FILE pFile = hb_fileExtOpen(szImage, nullptr, FO_READ | FO_SHARED | FO_PRIVATE | FXO_DEFAULTS | FXO_SHARELOCK,
                                    nullptr, nullptr);
        ...
        if (hGlobal) {
          if (hb_fileRead(pFile, hGlobal, nFileSize, -1) == nFileSize) {
            ...
            if (CreateStreamOnHGlobal(hGlobal, FALSE, &pStream) == S_OK && pStream) {
              OleLoadPicture(pStream, static_cast<LONG>(nFileSize), TRUE, HB_ID_REF(IID_IPicture),
...
COLORREF hb_gt_wvw_GetColorData(int32_t iIndex)
{
...
PWVW_WIN hb_gt_wvw_win(int32_t nWin)
{
...
{
  if (s_wvw) {
    int32_t nWin = s_wvw->iNumWindows > 0 ? s_wvw->iNumWindows - 1 : 0;
...
static BITMAPINFO *PackedDibLoad(const char *szFileName)
{
  ...
  PHB_FILE fhnd = hb_fileExtOpen(szFileName, nullptr, FO_READ | FO_SHARED | FO_PRIVATE | FXO_SHARELOCK | FXO_NOSEEKPOS,
                                 nullptr, nullptr);

  if (fhnd) {
    BITMAPFILEHEADER bmfh;

    if (static_cast<size_t>(hb_fileRead(fhnd, &bmfh, sizeof(bmfh), -1)) == sizeof(bmfh) &&
        bmfh.bfType == 0x4d42 /* "BM" */ && bmfh.bfSize > sizeof(bmfh) &&
      ...
      if (static_cast<DWORD>(hb_fileRead(fhnd, pbmi, dwPackedDibSize, -1)) != dwPackedDibSize) {
        hb_xfree(pbmi);
        pbmi = nullptr;
      }
    }
...
HBITMAP hb_gt_wvw_FindBitmapHandle(const char *szFileName, int32_t *piWidth, int32_t *piHeight)
{
  ...
  while (pbh) {
    if (strcmp(szFileName, pbh->szFilename) == 0 &&
        (!fStrictDimension || (*piWidth == pbh->iWidth && *piHeight == pbh->iHeight))) {
      if (!fStrictDimension) {
        *piWidth = pbh->iWidth;
...
void hb_gt_wvw_AddBitmapHandle(const char *szFileName, HBITMAP hBitmap, int32_t iWidth, int32_t iHeight)
{
 ...
 */
HBITMAP hb_gt_wvw_FindUserBitmapHandle(const char *szFileName, int32_t *piWidth, int32_t *piHeight)
{
...
void hb_gt_wvw_AddUserBitmapHandle(const char *szFileName, HBITMAP hBitmap, int32_t iWidth, int32_t iHeight)
{
  auto pbhNew = static_cast<WVW_BMP *>(hb_xgrabz(sizeof(WVW_BMP)));
                                ...
                                bool fMap3Dcolors, HWND hCtrl)
{
  ...
  if (szBitmap) {
    ...
    if (!hBitmap) {
      ...
      hBitmap = static_cast<HBITMAP>(LoadImage(GetModuleHandle(nullptr), HB_FSNAMECONV(szBitmap, &lpFree), IMAGE_BITMAP,
    ...
    if (!hBitmap) {
      hBitmap = static_cast<HBITMAP>(LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(uiBitmap), IMAGE_BITMAP,
                                               iExpWidth, iExpHeight, uiOptions));
    ...
    if (!hBitmap) {
      ...
      if (pPackedDib || fMap3Dcolors) {
        ...
        if (!fMap3Dcolors) {
          ...
          hBitmap = static_cast<HBITMAP>(LoadImage(nullptr, HB_FSNAMECONV(szBitmap, &lpFree), IMAGE_BITMAP, iExpWidth,
        ...
        if (iExpWidth != iWidth || iExpHeight != iHeight) {
          ...
          fResult = StretchBlt(hdcTarget,  /* handle to destination DC */
...
HWND hb_gt_wvw_FindControlHandle(PWVW_WIN wvw_win, int32_t nClass, int32_t nId, int32_t *pnStyle)
{
  if (wvw_win) {
...
int32_t hb_gt_wvw_FindControlId(PWVW_WIN wvw_win, int32_t nClass, HWND hWnd, int32_t *pnStyle)
{
  if (wvw_win) {
    auto wvw_ctl = wvw_win->ctlList;
...
int32_t hb_gt_wvw_LastControlId(PWVW_WIN wvw_win, int32_t nClass)
{
  if (wvw_win) {
    auto wvw_ctl = wvw_win->ctlList;
                                ...
                                int32_t nStyle)
{
  if (wvw_win) {
    ...
    wvw_ctl->hWnd = hWnd;
    wvw_ctl->nId = nId;
    wvw_ctl->pBlock = pBlock ? hb_itemNew(pBlock) : nullptr;
    wvw_ctl->fBusy = false;
    wvw_ctl->nBusy = 0;
...
PWVW_CTL hb_gt_wvw_ctl(PWVW_WIN wvw_win, int32_t nClass, HWND hWnd, int32_t nId)
{
  if (wvw_win) {
    auto wvw_ctl = wvw_win->ctlList;

    while (wvw_ctl) {
      if (wvw_ctl->nClass == nClass && ((hWnd && wvw_ctl->hWnd == hWnd) || (nId && wvw_ctl->nId == nId))) {
        return wvw_ctl;
      }
      wvw_ctl = wvw_ctl->pNext;
    }
...
bool hb_gt_wvw_StoreControlProc(PWVW_WIN wvw_win, int32_t nClass, HWND hWnd, WNDPROC OldProc)
{
  if (wvw_win) {
    auto wvw_ctl = wvw_win->ctlList;
...
WNDPROC hb_gt_wvw_GetControlProc(PWVW_WIN wvw_win, int32_t nClass, HWND hWnd)
{
  if (wvw_win) {
    auto wvw_ctl = wvw_win->ctlList;
...
int32_t hb_gt_wvw_GetControlClass(PWVW_WIN wvw_win, HWND hWnd)
{
  if (wvw_win) {
    auto wvw_ctl = wvw_win->ctlList;
      ...
      case CBN_KILLFOCUS: {
        ...
        if (iCurSel == CB_ERR) {
          break;
               ...
               iTextLen       = SendMessage(wvw_ctl->hWnd, CB_GETLBTEXTLEN, static_cast<WPARAM>(iCurSel), 0);
               lptstrSelected = static_cast<LPTSTR>(hb_xgrab((iTextLen + 1) * sizeof(TCHAR)));

               SendMessage(wvw_ctl->hWnd, CB_GETLBTEXT, static_cast<WPARAM>(iCurSel), static_cast<LPARAM>(lptstrSelected));
...
static void s_ReposControls(PWVW_WIN wvw_win, int32_t nClass)
{
  ...
  while (wvw_ctl) {
    if (nClass == 0 || nClass == wvw_ctl->nClass) {
      ...
      SetWindowPos(wvw_ctl->hWnd, nullptr, iLeft, iTop, iRight - iLeft + 1, iBottom - iTop + 1, SWP_NOZORDER);
    }
...
static LRESULT CALLBACK hb_gt_wvw_BtnProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  if (OldProc == nullptr) {
    hb_errInternal(10011, "Failed hb_gt_wvw_GetControlProc()", nullptr, nullptr);
                           ...
                           int32_t iOffBottom, int32_t iOffRight, double dStretch, bool fMap3Dcolors, int32_t iStyle, HWND *phWnd)
{
  ...
  *phWnd = hWnd;
  ...
  if (hWnd) {
    ...
    hb_gt_wvw_StoreControlProc(
        wvw_win, WVW_CONTROL_PUSHBUTTON, hWnd,
        reinterpret_cast<WNDPROC>(
            SetWindowLongPtr(hWnd, GWLP_WNDPROC, reinterpret_cast<LPARAM>(static_cast<WNDPROC>(hb_gt_wvw_BtnProc)))));

    SendMessage(hWnd, WM_SETFONT, reinterpret_cast<WPARAM>(wvw_win->hPBfont), static_cast<LPARAM>(TRUE));
...
HB_FUNC(WVW_ADDROWS)
{
  ...
  if (wvw && wvw_win) {
    ...
    if (/* iRows < 0 || */
        ...
        wvw_win->ROWS + iRows > WVW_MAX_ROWS || wvw_win->ROWS + iRows < 1) {
      hb_retl(false);
      return;
    }

    auto nNumChars = static_cast<HB_SIZE>(iRows * wvw_win->COLS);
    ...
    if (iRows > 0) {
      ...
      HB_SIZE nBufStop = nBufStart + nNumChars;

      int32_t iColor = wvw_win->screenBuffer[nBufLastRow].c.bColor;
    ...
    if (!hb_gt_wvw_GetMainCoordMode()) {
      int32_t iCurWindow = wvw->iCurWindow;
    ...
    SetWindowPos(wvw_win->hWnd, nullptr, wi.left, wi.top, width, height, SWP_NOZORDER);
    ...
    if (wvw_win->hStatusBar != nullptr) {
      ...
      if( wvw_win->hToolBar != nullptr ) {
         SetWindowPos(wvw_win->hToolBar, nullptr, wi.left, wi.top - wvw_win->iTBHeight, width, wvw_win->iTBHeight, SWP_NOZORDER);
      }
    ...
    if (iRows > 0) {
      ...
      wvw_win->InvalidateWindow = true;
      hb_gt_wvw_SetInvalidRect(wvw_win, 0, wvw_win->ROWS - iRows, wvw_win->COLS - 1, wvw_win->ROWS - 1);
    }
...
HB_FUNC(WVW_SETLINESPACING)
{
  ...
  if (wvw_win) {
    int32_t iOldLineSpacing = wvw_win->iLineSpacing;
...
{
  hb_retl(hb_wvw_Move_Ready(false));
...
{
  hb_retl(hb_wvw_Size_Ready(false));
