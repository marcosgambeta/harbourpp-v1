...
{
   PHB_FILE fhnd = hb_fileExtOpen(hb_parcx(1), nullptr, FO_READ | FO_SHARED | FO_PRIVATE | FXO_SHARELOCK | FXO_NOSEEKPOS, nullptr, nullptr);
...
{
   void * hText;
   SendMessage(GetDlgItem(hbwapi_par_raw_HWND(1), hb_parni(2)), LB_ADDSTRING, 0, reinterpret_cast<LPARAM>(HB_PARSTRDEF(3, &hText, nullptr)));
   hb_strfree(hText);
}
...
{
   void * hText;
   SendMessage(GetDlgItem(hbwapi_par_raw_HWND(1), hb_parni(2)), CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(HB_PARSTRDEF(3, &hText, nullptr)));
   hb_strfree(hText);
}
