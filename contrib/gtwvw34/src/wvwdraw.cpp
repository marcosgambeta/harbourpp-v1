...
static bool hb_gt_wvw_DrawImage(HWND hWnd, int x1, int y1, int wd, int ht, const char * image, bool fTransparent)
{
   ...
   if( !hBitmap ) {
      ...
      if( HB_VTBL(pPicture)->get_Handle(HB_THIS_(pPicture) reinterpret_cast<OLE_HANDLE*>(&hBitmapTemp)) == S_OK && hBitmapTemp ) {
   ...
   if( fTransparent ) {
      ...
      fResult = static_cast<bool>(StretchBlt(
...
static bool hb_gt_wvw_RenderPicture(PWVW_WIN wvw_win, int x1, int y1, int wd, int ht, IPicture * pPicture, bool bTransp)
{
   ...
   if( pPicture ) {
      ...
      if( nWidth && nHeight ) {
         ...
         while( x < xe ) {
            while( y < ye ) {
               HB_VTBL(pPicture)->Render(HB_THIS_(pPicture) wvw_win->hdc, x, y, dc, dr, 0, static_cast<OLE_YPOS_HIMETRIC>(nHeight), nWidth, -nHeight, &rc_dummy);
               y += dr;
            }
            y  = r;
            x += dc;
...
static IPicture * s_FindPictureHandle(PWVW_GLO wvw, const char * szFileName, int * piWidth, int * piHeight)
{
   ...
   while( pph ) {
      if( strcmp(szFileName, pph->szFilename) == 0 && (!fStrictDimension || (*piWidth == pph->iWidth && *piHeight == pph->iHeight)) ) {
         if( !fStrictDimension ) {
            *piWidth  = pph->iWidth;
...
static IPicture * hb_gt_wvw_rr_LoadPictureFromResource(PWVW_GLO wvw, const char * resname, HB_UINT iresimage, int * piWidth, int * piHeight)
{
   ...
   if( resname ) {
      ...
      if( !hbmpx ) {
         LPTSTR lpFree;
         hbmpx = static_cast<HBITMAP>(LoadImage(GetModuleHandle(nullptr), HB_FSNAMECONV(resname, &lpFree), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR));
         if( lpFree )
         {
            hb_xfree(lpFree);
         }
      ...
      if( !hbmpx ) {
         hbmpx = static_cast<HBITMAP>(LoadImage(GetModuleHandle(nullptr), MAKEINTRESOURCE(iresimage), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR));
...
HB_FUNC( WVW_DRAWIMAGE )
{
   ...
   if( wvw && wvw_win ) {
      ...
      if( ( fActBottom || fActRight ) && !(fActBottom && fActRight) ) {
         if( fActRight ) {
            iRightNew = iLeft + static_cast<int>(static_cast<float>(iImgWidth) / iImgHeight * (iBottomNew - iTop + 1)) - 1;  /* right corner (width) must be proportional to height */
         } else {
            iBottomNew = iTop + static_cast<int>(static_cast<float>(iImgHeight) / iImgWidth * (iRightNew - iLeft + 1)) - 1;  /* bottom corner (height) must be proportional to width */
         }
      }
...
HB_FUNC( WVW_DRAWLABELEX )
{
   ...
   if( wvw && wvw_win && iSlot >= 0 && iSlot < static_cast<int>(HB_SIZEOFARRAY(wvw->a.hUserFonts)) && wvw->a.hUserFonts[iSlot] ) {
      int iTop  = hb_parni(2);
   ...
   if( wvw && wvw_win && wvw_zer ) {
      ...
      if( fImage ) {
         ...
         if( HB_ISNUM(7) ) {
            ...
            if( iSlot >= 0 && iSlot < static_cast<int>(HB_SIZEOFARRAY(wvw->a.pPicture)) ) {
               hb_gt_wvw_RenderPicture(wvw_win, iLeft + 4, iTop + 4, iImageWidth, iImageHeight, wvw->a.pPicture[iSlot], false);
            }
         } else {
...
HB_FUNC( WVW_DRAWPICTURE )
{
   ...
   if( wvw && wvw_win && iSlot >= 0 && iSlot < static_cast<int>(HB_SIZEOFARRAY(wvw->a.pPicture)) && wvw->a.pPicture[iSlot] ) {
      int iTop    = hb_parni(2),
          iLeft   = hb_parni(3),
          iBottom = hb_parni(4),
          iRight  = hb_parni(5);
      ...
      hb_retl(hb_gt_wvw_RenderPicture(wvw_win, iLeft, iTop, iRight - iLeft + 1, iBottom - iTop + 1, wvw->a.pPicture[iSlot], false));
   } else {
      hb_retl(false);
   }
}
