...
#define SubclassWindow(hwnd, lpfn)                                                                                     \
  (reinterpret_cast<WNDPROC>(                                                                                          \
      SetWindowLongPtr((hwnd), GWLP_WNDPROC, reinterpret_cast<LPARAM>(static_cast<WNDPROC>(lpfn)))))
...
static int32_t s_byDefLineSpacing = 0;       /* default line spacing */
static int32_t s_iDefLSpaceColor = -1;       /* if >= 0 this will be the color index for spacing between lines */
static BOOL s_bAllowNonTop = FALSE;      /* allow non-topmost window's control to accept input */
static BOOL s_bRecurseCBlock = FALSE;    /* allow control's codeblock to recurse */
...
static BOOL s_bFlashingWindow = FALSE; /* topmost window is flashing due to invalid input on other window */
static int32_t s_iScrolling        = 0;    /* scrollbar is scrolling */
static int32_t s_iWrongButtonUp    = 0;    /* number of consecutive scrollbar's WM_LBUTTONUP encountered by gtProcessMessages */
static int32_t s_iMaxWrongButtonUp = 500;  /* max number of s_iWrongButtonUp. If it goes higher than this number, the scrollbar is forced to stop */
...
#ifdef WVW_DEBUG
static int32_t nCountPuts = 0, nCountScroll = 0, nCountPaint = 0, nSetFocus = 0, nKillFocus = 0;
#endif

static int32_t K_Ctrl[] = {K_CTRL_A, K_CTRL_B, K_CTRL_C, K_CTRL_D, K_CTRL_E, K_CTRL_F, K_CTRL_G, K_CTRL_H, K_CTRL_I,
...
/*                                                                   */
HB_EXTERN_BEGIN
static void hb_gtInitStatics(UINT usWinNum, LPCTSTR lpszWinName, uint16_t usRow1, uint16_t usCol1, uint16_t usRow2,
                             uint16_t usCol2);
static void hb_gt_wvwAddCharToInputQueue(int32_t data);
static HWND hb_gt_wvwCreateWindow(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int32_t iCmdShow);
static BOOL hb_gt_wvwInitWindow(WIN_DATA *pWindowData, HWND hWnd, uint16_t col, uint16_t row);
static void hb_gt_wvwResetWindowSize(WIN_DATA *pWindowData, HWND hWnd);
static BOOL hb_gt_wvwSetCodePage(UINT usWinNum, int32_t iCodePage);
...
static BOOL hb_gt_wvwValidWindowSize(WIN_DATA *pWindowData, int32_t rows, int32_t cols, HFONT hFont, int32_t iWidth, int32_t *pmaxrows,
                                     int32_t *pmaxcols);
...
static void hb_gt_wvwSetMouseX(WIN_DATA *pWindowData, uint16_t ix);
static void hb_gt_wvwSetMouseY(WIN_DATA *pWindowData, uint16_t iy);
static int32_t hb_gt_wvwJustTranslateKey(int32_t key, int32_t shiftkey, int32_t altkey, int32_t controlkey);
static void hb_gt_wvwTranslateKey(int32_t key, int32_t shiftkey, int32_t altkey, int32_t controlkey);
static void hb_gt_wvwSetInvalidRect(WIN_DATA *pWindowData, uint16_t left, uint16_t top, uint16_t right, uint16_t bottom);
static void hb_gt_wvwDoInvalidateRect(WIN_DATA *pWindowData);
static void hb_gt_wvwHandleMenuSelection(int32_t);
static void hb_gt_wvwUnreachedXY(WIN_DATA *pWindowData, int32_t *cols, int32_t *rows);
static POINT hb_gt_wvwGetColRowFromXY(WIN_DATA *pWindowData, uint16_t x, uint16_t y);
static POINT hb_gt_wvwTBGetColRowFromXY(WIN_DATA *pWindowData, uint16_t x, uint16_t y);
static POINT hb_gt_wvwGetColRowForTextBuffer(WIN_DATA *pWindowData, uint16_t index);
static void hb_gt_wvwValidateCol(WIN_DATA *pWindowData);
static void hb_gt_wvwValidateRow(WIN_DATA *pWindowData);
static uint16_t hb_gt_wvwCalcPixelHeight(WIN_DATA *pWindowData);
static uint16_t hb_gt_wvwCalcPixelWidth(WIN_DATA *pWindowData);
static BOOL hb_gt_wvwSetColors(WIN_DATA *pWindowData, HDC hdc, BYTE attr);
static BOOL hb_gt_wvwTextOut(WIN_DATA *pWindowData, HDC hdc, uint16_t col, uint16_t row, LPCTSTR lpString, uint16_t cbString);
static void hb_gt_wvwSetStringInTextBuffer(WIN_DATA *pWindowData, int32_t col, int32_t row, BYTE color, BYTE attr,
                                           BYTE *sBuffer, ULONG length);
static uint16_t hb_gt_wvwGetIndexForTextBuffer(WIN_DATA *pWindowData, uint16_t col, uint16_t row);
...
static UINT hb_gt_wvwOpenWindow(LPCTSTR lpszWinName, int32_t usRow1, int32_t usCol1, int32_t usRow2, int32_t usCol2, DWORD dwStyle,
                                int32_t iParentWin);
...
static BOOL hb_gt_wvwInWindow(UINT usWinNum, uint16_t usrow, uint16_t uscol);
static UINT hb_gt_wvwFindWindow(uint16_t usRow, uint16_t usCol);
...
static void hb_wvw_vmouse_SetPos(WIN_DATA *pWindowData, uint16_t usRow, uint16_t usCol);
static int32_t hb_gt_wvw_usDispCount(WIN_DATA *pWindowData);
static void hb_gt_wvw_vDispBegin(WIN_DATA *pWindowData);
static void hb_gt_wvw_vDispEnd(WIN_DATA *pWindowData);
#if 0
static void hb_gt_wvw_vGetText(WIN_DATA * pWindowData, uint16_t top, uint16_t left, uint16_t bottom, uint16_t right, BYTE * sBuffer);
static void hb_gt_wvw_vPuts(WIN_DATA * pWindowData, int32_t iRow, int32_t iCol, BYTE byColor, BYTE byAttr, BYTE * pbyStr, ULONG ulLen);
#endif
static void hb_gt_wvw_vReplicate(WIN_DATA *pWindowData, int32_t iRow, int32_t iCol, int32_t bColor, BYTE bAttr, uint16_t usChar,
                                 ULONG ulLen);
static void hb_gt_wvw_vPutText(WIN_DATA *pWindowData, uint16_t top, uint16_t left, uint16_t bottom, uint16_t right,
                               const char *sBuffer, int32_t bColor);
static void hb_gt_wvw_vSetAttribute(WIN_DATA *pWindowData, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, int32_t bColor);
static BOOL hb_gt_wvw_bSetMode(WIN_DATA *pWindowData, uint16_t row, uint16_t col);
static void hb_gt_wvw_vxPutch(WIN_DATA *pWindowData, uint16_t iRow, uint16_t iCol, int32_t bColor, BYTE bChar);
static void hb_gt_wvw_usBox(WIN_DATA *pWindowData, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, const char *pbyFrame,
                            int32_t bColor);
static void hb_gt_wvw_vSetPos(WIN_DATA *pWindowData, int32_t iRow, int32_t iCol);
static void hb_wvw_InitPendingRect(WIN_DATA *pWindowData);
static void hb_wvw_UpdatePendingRect(WIN_DATA *pWindowData, uint16_t usRow1, uint16_t usCol1, uint16_t usRow2, uint16_t usCol2);
static void hb_gt_wvwFillLineSpace(WIN_DATA *pWindowData, HDC hdc, uint16_t startCol, uint16_t irow, uint16_t len,
                                   BYTE byAttrib);
...
static IPicture *FindPictureHandle(const char *szFileName, int32_t *piWidth, int32_t *piHeight);
static void AddPictureHandle(const char *szFileName, IPicture *iPicture, int32_t iWidth, int32_t iHeight);
/* bitmap caching functions for user drawn bitmaps (wvw_drawimage) */
static HBITMAP FindUserBitmapHandle(const char *szFileName, int32_t *piWidth, int32_t *piHeight);
static void AddUserBitmapHandle(const char *szFileName, HBITMAP hBitmap, int32_t iWidth, int32_t iHeight);
static int32_t GetControlClass(UINT usWinNum, HWND hWndCtrl);
static void RunControlBlock(UINT usWinNum, BYTE byCtrlClass, HWND hWndCtrl, UINT message, WPARAM wParam, LPARAM lParam,
                            int32_t iEventType);
...
static void hb_gt_wvw_Init(HB_GT *pGT, HB_FHANDLE hFilenoStdin, HB_FHANDLE hFilenoStdout, HB_FHANDLE hFilenoStderr)
{
#if 0
  ...
  if (bStartMode) {
    s_pWvwData = new WVW_DATA;
    s_pWvwData->s_sApp = new APP_DATA;
  ...
  for (uint16_t i = 0; i < WVW_MAXWINDOWS; i++) {
    s_pWvwData->s_pWindows[i] = nullptr;
  }

  hb_gt_wvwWindowPrologue();

  hb_gtInitStatics(0, static_cast<LPCTSTR>(s_pWvwData->szAppName), 0, 0, WVW_DEFAULT_ROWS - 1, WVW_DEFAULT_COLS - 1);

  s_pWvwData->hInstance = static_cast<HINSTANCE>(hInstance);

  s_pWvwData->s_pWindows[0]->hWnd = hb_gt_wvwCreateWindow(static_cast<HINSTANCE>(hInstance),
                                                          static_cast<HINSTANCE>(hPrevInstance), nullptr, iCmdShow);
   ...
   */
  s_pWvwData->s_sApp->OriginalPen =
      static_cast<HPEN>(SelectObject(s_pWvwData->s_pWindows[0]->hdc, static_cast<HPEN>(s_pWvwData->s_sApp->penWhite)));
  s_pWvwData->s_sApp->OriginalBrush = static_cast<HBRUSH>(
      SelectObject(s_pWvwData->s_pWindows[0]->hdc, static_cast<HBRUSH>(s_pWvwData->s_sApp->currentBrush)));
  /*
   ...
   */
  SelectObject(s_pWvwData->s_pWindows[0]->hdc, static_cast<HPEN>(s_pWvwData->s_sApp->OriginalPen));
  SelectObject(s_pWvwData->s_pWindows[0]->hdc, static_cast<HBRUSH>(s_pWvwData->s_sApp->OriginalBrush));
...
static void hb_gt_wvw_Exit(HB_GT *pGT)
{
#if 0
  ...
  for (j = static_cast<int32_t>(s_pWvwData->s_usNumWindows - 1); j >= 0; j--) {
    pWindowData = static_cast<WIN_DATA *>(s_pWvwData->s_pWindows[j]);
    ...
    if (pWindowData->hWnd) {
      ...
      if (j == 0) {
        ...
        if (s_pWvwData->s_sApp->penWhite) {
          DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->penWhite));
        }
        if (s_pWvwData->s_sApp->penWhiteDim) {
          DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->penWhiteDim));
        }
        if (s_pWvwData->s_sApp->penBlack) {
          DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->penBlack));
        }
        if (s_pWvwData->s_sApp->penDarkGray) {
          DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->penDarkGray));
        }
        if (s_pWvwData->s_sApp->penGray) {
          DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->penGray));
        }
        if (s_pWvwData->s_sApp->penNull) {
          DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->penNull));
        }
        if (s_pWvwData->s_sApp->currentPen) {
          DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->currentPen));
        }
        if (s_pWvwData->s_sApp->currentBrush) {
          DeleteObject(static_cast<HBRUSH>(s_pWvwData->s_sApp->currentBrush));
        }
        if (s_pWvwData->s_sApp->diagonalBrush) {
          DeleteObject(static_cast<HBRUSH>(s_pWvwData->s_sApp->diagonalBrush));
        }
        if (s_pWvwData->s_sApp->solidBrush) {
          DeleteObject(static_cast<HBRUSH>(s_pWvwData->s_sApp->solidBrush));
        }
        if (s_pWvwData->s_sApp->wvwWhiteBrush) {
          DeleteObject(static_cast<HBRUSH>(s_pWvwData->s_sApp->wvwWhiteBrush));
        }
        if (s_pWvwData->s_sApp->gridPen) {
          DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->gridPen));
        }
      }

      if (pWindowData->hIcon) {
        DestroyIcon(static_cast<HICON>(pWindowData->hIcon));
      }
      ...
      if (pWindowData->hPBfont) {
        DeleteObject(static_cast<HFONT>(pWindowData->hPBfont));
      }

      if (pWindowData->hCBfont) {
        DeleteObject(static_cast<HFONT>(pWindowData->hCBfont));
      }

      if (pWindowData->hCXfont) {
        DeleteObject(static_cast<HFONT>(pWindowData->hCXfont));
      }
      ...
      if (pWindowData->hSTfont) {
        DeleteObject(static_cast<HFONT>(pWindowData->hSTfont));
      }
    }
  ...
  for (i = 0; i < WVW_FONTS_MAX; i++) {
    if (s_pWvwData->s_sApp->hUserFonts[i]) {
      DeleteObject(static_cast<HFONT>(s_pWvwData->s_sApp->hUserFonts[i]));
    }
  }

  for (i = 0; i < WVW_PENS_MAX; i++) {
    if (s_pWvwData->s_sApp->hUserPens[i]) {
      DeleteObject(static_cast<HPEN>(s_pWvwData->s_sApp->hUserPens[i]));
    }
  }
...
void hb_gt_wvw_SetPos(HB_GT *pGT, int32_t iRow, int32_t iCol)
{
#if 0
  ...
  if (s_pWvwData->s_bMainCoordMode) {
    hb_gt_wvwFUNCPrologue(2, &i_Row, &i_Col, nullptr, nullptr);
  }
...
static int32_t hb_gt_wvw_GetCursorStyle(HB_GT *pGT)
{
#if 0
...
static void hb_gt_wvw_SetCursorStyle(HB_GT *pGT, int32_t iStyle)
{
#if 0
  ...
  auto usFullSize = static_cast<uint16_t>(s_pWvwData->s_bVertCaret ? pWindowData->PTEXTSIZE.x : pWindowData->PTEXTSIZE.y);
  ...
  if (bCursorOn) {
    if (!s_pWvwData->s_bVertCaret) {
      s_pWvwData->s_sApp->CaretExist = CreateCaret(pWindowData->hWnd, static_cast<HBITMAP>(nullptr),
                                                   pWindowData->PTEXTSIZE.x, pWindowData->CaretSize);
    } else {
      s_pWvwData->s_sApp->CaretExist = CreateCaret(pWindowData->hWnd, static_cast<HBITMAP>(nullptr),
                                                   pWindowData->CaretSize, pWindowData->PTEXTSIZE.y);
    }
  }
  hb_gt_wvwSetCaretOn(pWindowData, bCursorOn);
}

static void hb_gt_wvw_DispBegin(HB_GT *pGT)
{
#if 0
...
static void hb_gt_wvw_DispEnd(HB_GT *pGT)
{
#if 0
...
static int32_t hb_gt_wvw_DispCount(HB_GT *pGT)
{
#if 0
...
static void hb_gt_wvw_Replicate(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t bColor, BYTE bAttr, uint16_t usChar, ULONG ulLen)
{
#if 0
  ...
  if (s_pWvwData->s_bMainCoordMode) {
    hb_gt_wvwFUNCPrologue(2, &iRow, &iCol, nullptr, nullptr);
  }
...
// static void hb_gt_wvw_PutText(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t bColor, const char * pText, ULONG ulLen)
static int32_t hb_gt_wvw_PutText(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t bColor, const char *pText, ULONG ulLen)
{
#if 0
  ...
  hb_gt_wvw_vPutText(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], static_cast<uint16_t>(iRow),
                     static_cast<uint16_t>(iCol), static_cast<uint16_t>(iRow),
                     static_cast<uint16_t>(ulLen) == 0
                         ? 0
                         : static_cast<uint16_t>(iCol) + (static_cast<uint16_t>(static_cast<ULONG>(ulLen))) - 1,
                     pText, bColor);
  ...
  return iCol; /* TODO: return correct value */
}

static void hb_gt_wvw_SetAttribute(HB_GT *pGT, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, int32_t bColor)
{
#if 0
...
#if 0
static void hb_gt_wvw_Scroll(HB_GT *pGT, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, int32_t bColor, uint16_t bChar, int32_t iRows, int32_t iCols)
{
#if 0
   ...
   BYTE * fpBlank;
   BYTE * fpBuff;
   int32_t    iLength = (iRight - iLeft) + 1;
   int32_t    iCount, iColOld, iColNew, iColSize;
   BOOL   bMalloc = FALSE;
   ...
   for( iCount = (iRows >= 0 ? iTop : iBottom); (iRows >= 0 ? iCount <= iBottom : iCount >= iTop); (iRows >= 0 ? iCount++ : iCount--) ) {
      int32_t iRowPos = iCount + iRows;

      /* Read the text to be scrolled into the current row */
      if( (iRows || iCols) && iRowPos <= iBottom && iRowPos >= iTop ) {
         hb_gt_wvw_vGetText(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], static_cast<uint16_t>(iRowPos), static_cast<uint16_t>(iColOld), static_cast<uint16_t>(iRowPos), static_cast<uint16_t>(iColOld) + static_cast<uint16_t>(iColSize), fpBuff);
      }
      ...
      if( (iRows || iCols) && iRowPos <= iBottom && iRowPos >= iTop ) {
         hb_gt_wvw_vPutText(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], static_cast<uint16_t>(iCount), static_cast<uint16_t>(iColNew), static_cast<uint16_t>(iCount), static_cast<uint16_t>(iColNew) + static_cast<uint16_t>(iColSize), reinterpret_cast<const char*>(fpBuff), 0);
      }
   }
   ...
   if( s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->InvalidateWindow ) {
      ...
      ScrollWindowEx(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->hWnd, -iCols * s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->PTEXTSIZE.x, -iRows * hb_wvw_LineHeight(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]), &cr, nullptr, nullptr, &crInvalid, 0);
...
static BOOL hb_gt_wvw_SetMode(HB_GT *pGT, int32_t iRow, int32_t iCol)
{
#if 0
  ...
  return hb_gt_wvw_bSetMode(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], static_cast<uint16_t>(iRow),
                            static_cast<uint16_t>(iCol));
}

static void hb_gt_wvw_WriteAt(HB_GT *pGT, int32_t iRow, int32_t iCol, const char *pText, ULONG ulLength)
{
  HB_GTSELF_PUTTEXT(pGT, iRow, iCol, static_cast<BYTE>(HB_GTSELF_GETCOLOR(pGT)), pText, ulLength);

  /* Finally, save the new cursor position, even if off-screen */
  HB_GTSELF_SETPOS(pGT, iRow, iCol + static_cast<int32_t>(ulLength));
}

static BOOL hb_gt_wvw_GetBlink(HB_GT *pGT)
{
#if 0
...
static void hb_gt_wvw_SetBlink(HB_GT *pGT, BOOL bBlink)
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_gt_wvw_SetBlink( %d )", static_cast<int32_t>(bBlink)));
#endif
...
static void hb_gt_wvw_Box(HB_GT *pGT, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, const char *pbyFrame, int32_t bColor)
{
  int32_t i_Top = (iTop < 0 ? 0 : iTop);
  int32_t i_Left = (iLeft < 0 ? 0 : iLeft);
  int32_t i_Bottom = (iBottom < 0 ? 0 : iBottom);
  int32_t i_Right = (iRight < 0 ? 0 : iRight);
  ...
  hb_gt_wvw_usBox(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], i_Top, i_Left, i_Bottom, i_Right, pbyFrame,
                  bColor);
...
static void hb_gt_wvw_HorizLine(HB_GT *pGT, int32_t iRow, int32_t iLeft, int32_t iRight, uint16_t bChar, int32_t bColor)
{
  int32_t iWidth;
  int32_t i_Row = (iRow < 0 ? 0 : iRow);
  int32_t i_Left = (iLeft < 0 ? 0 : iLeft);
  int32_t i_Right = (iRight < 0 ? 0 : iRight);
  ...
  if (s_pWvwData->s_bMainCoordMode) {
    ...
    hb_gt_wvwFUNCPrologue(4, &i_Row, &i_Left, nullptr, &i_Right);
  }
  ...
  if (i_Row < iWidth) {
    ...
    if (i_Left < i_Right) {
      hb_gt_wvw_vReplicate(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], i_Row, i_Left, bColor, HB_GT_ATTR_BOX,
                           bChar, i_Right - i_Left + 1);
    } else {
      hb_gt_wvw_vReplicate(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], i_Row, i_Right, bColor, HB_GT_ATTR_BOX,
                           bChar, i_Left - i_Right + 1);
    }
  }
...
static void hb_gt_wvw_VertLine(HB_GT *pGT, int32_t iCol, int32_t iTop, int32_t iBottom, uint16_t bChar, int32_t bColor)
{
  int32_t i_Width;
  int32_t i_Height;
  int32_t i_Row;
  int32_t i_Col = (iCol < 0 ? 0 : iCol);
  int32_t i_Top = (iTop < 0 ? 0 : iTop);
  int32_t i_Bottom = (iBottom < 0 ? 0 : iBottom);
  ...
  if (s_pWvwData->s_bMainCoordMode) {
    ...
    hb_gt_wvwFUNCPrologue(3, &i_Top, &i_Col, &i_Bottom, nullptr);
  }
  ...
  if (i_Col < i_Width) {
    ...
    while (i_Row <= i_Bottom) {
      hb_gt_wvw_vxPutch(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], static_cast<uint16_t>(i_Row++),
                        static_cast<uint16_t>(i_Col), bColor, bChar);
    }
...
static void hb_gt_wvw_OutStd(HB_GT *pGT, const char *pbyStr, HB_SIZE ulLen)
{
  HB_SYMBOL_UNUSED(pGT);
  hb_fsWriteLarge(s_iStdOut, reinterpret_cast<BYTE *>(const_cast<char *>(pbyStr)), ulLen);
}

static void hb_gt_wvw_OutErr(HB_GT *pGT, const char *pbyStr, HB_SIZE ulLen)
{
  HB_SYMBOL_UNUSED(pGT);
  hb_fsWriteLarge(s_iStdErr, reinterpret_cast<BYTE *>(const_cast<char *>(pbyStr)), ulLen);
}

static BOOL hb_gt_wvwGetCharFromInputQueue(int32_t *c)
{
   ...
   * c = 0;
     ...
     iNextPos = ( s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows-1]->keyPointerOut >= WVW_CHAR_QUEUE_SIZE - 1 ) ? 0 :
   s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows-1]->keyPointerOut+1 ; if( iNextPos !=
   s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows-1]->keyPointerIn ) {
...
static int32_t hb_gt_wvw_ReadKey(HB_GT *pGT, int32_t eventmask)
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_gt_wvw_ReadKey( %d )", static_cast<int32_t>(eventmask)));
#endif
...
/*                                                                   */
static void hb_gt_wvw_Tone(HB_GT *pGT, double dFrequency, double dDuration)
{
#if 0
...
static void hb_gt_wvw_mouse_Init(HB_GT *pGT)
{
  HB_SYMBOL_UNUSED(pGT);
  hb_wvw_vmouse_Init();
...
static void hb_gt_wvw_mouse_Exit(HB_GT *pGT)
{
  HB_SYMBOL_UNUSED(pGT);
  hb_wvw_vmouse_Exit();
}

static BOOL hb_gt_wvw_mouse_IsPresent(HB_GT *pGT)
{
  HB_SYMBOL_UNUSED(pGT);
  return TRUE;
}

static int32_t hb_gt_wvw_mouse_Col(HB_GT *pGT)
{
  HB_SYMBOL_UNUSED(pGT);
  if (s_pWvwData->s_bMainCoordMode) {
    return hb_gt_wvwGetMouseX(s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]) +
           hb_gt_wvwColOfs(s_pWvwData->s_usNumWindows - 1);
  } else {
    return hb_gt_wvwGetMouseX(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]);
  }
}

static int32_t hb_gt_wvw_mouse_Row(HB_GT *pGT)
{
  HB_SYMBOL_UNUSED(pGT);
  if (s_pWvwData->s_bMainCoordMode) {
    return hb_gt_wvwGetMouseY(s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]) +
           hb_gt_wvwRowOfs(s_pWvwData->s_usNumWindows - 1);
  } else {
    return hb_gt_wvwGetMouseY(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]);
  }
}

static void hb_gt_wvw_mouse_SetPos(HB_GT *pGT, int32_t iRow, int32_t iCol)
{
  int32_t i_Row = (iRow < 0 ? 0 : iRow);
  int32_t i_Col = (iCol < 0 ? 0 : iCol);
  ...
  hb_wvw_vmouse_SetPos(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow], static_cast<uint16_t>(i_Row),
                       static_cast<uint16_t>(i_Col));
...
static void hb_gt_wvw_mouse_GetPos(HB_GT *pGT, int32_t *piRow, int32_t *piCol)
{
#if 0
...
static BOOL hb_gt_wvw_mouse_ButtonState(HB_GT *pGT, int32_t iButton)
{
#if 0
...
static int32_t hb_gt_wvw_mouse_CountButton(HB_GT *pGT)
{
  HB_SYMBOL_UNUSED(pGT);
 ...
 */
/* int32_t gt_info(int32_t iMsgType, BOOL bUpdate, int32_t iParam, void *vpParam ) */
static BOOL hb_gt_wvw_Info(HB_GT *pGT, int32_t iType, HB_GT_INFO *pInfo)
{
#if 0
  ...
  case HB_GTI_INPUTFD:
    pInfo->pResult = hb_itemPutNInt(pInfo->pResult, reinterpret_cast<UINT_PTR>(GetStdHandle(STD_INPUT_HANDLE)));
    break;

  case HB_GTI_OUTPUTFD:
    pInfo->pResult = hb_itemPutNInt(pInfo->pResult, reinterpret_cast<UINT_PTR>(GetStdHandle(STD_OUTPUT_HANDLE)));
    break;

  case HB_GTI_ERRORFD:
    pInfo->pResult = hb_itemPutNInt(pInfo->pResult, reinterpret_cast<UINT_PTR>(GetStdHandle(STD_ERROR_HANDLE)));
    break;
  ...
  case HB_GTI_ICONFILE: {
    ...
    if (hb_itemType(pInfo->pNewVal) & Harbour::Item::STRING) {
      void *hImageName;
      hIcon = static_cast<HICON>(hb_gt_wvwSetWindowIconFromFile(s_pWvwData->s_usCurWindow,
...
#endif
      hb_gt_wvw_bSetMode(pWindowData, static_cast<uint16_t>(iY / pWindowData->PTEXTSIZE.y),
                         static_cast<uint16_t>(iX / pWindowData->PTEXTSIZE.x));
    }
...
#define SetGFXContext()                                                                                                \
  ...
  hBrush = static_cast<HBRUSH>(CreateSolidBrush(color));                                                               \
...
static int32_t hb_gt_wvw_gfxPrimitive(HB_GT *pGT, int32_t iType, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, int32_t iColor)
{
  COLORREF color;
  ...
  case HB_GFX_MAKECOLOR:
    ReleaseDC(pWindowData->hWnd, hdc);
    return static_cast<int32_t>(iTop << 16 | iLeft << 8 | iBottom);
...
void gt_gfxText(int32_t iTop, int32_t iLeft, char *cBuf, int32_t iColor, int32_t iSize, int32_t iWidth)
{
...
BOOL CALLBACK hb_gt_wvwDlgProcMLess(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  for (iIndex = 0; iIndex < WVW_DLGML_MAX; iIndex++) {
    if ((s_pWvwData->s_sApp->hDlgModeless[iIndex] != nullptr) && (s_pWvwData->s_sApp->hDlgModeless[iIndex] == hDlg)) {
      if (s_pWvwData->s_sApp->pFunc[iIndex] != nullptr) {
  ...
  case WM_NCDESTROY:
    if (s_pWvwData->s_sApp->pFunc[iIndex] != nullptr && s_pWvwData->s_sApp->iType[iIndex] == 2) {
...
BOOL CALLBACK hb_gt_wvwDlgProcModal(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  for (iIndex = 0; iIndex < WVW_DLGMD_MAX; iIndex++) {
    if ((s_pWvwData->s_sApp->hDlgModal[iIndex] != nullptr) && (s_pWvwData->s_sApp->hDlgModal[iIndex] == hDlg)) {
      if (s_pWvwData->s_sApp->pFuncModal[iIndex] != nullptr) {
  ...
  case WM_NCDESTROY:
    if (s_pWvwData->s_sApp->pFuncModal[iIndex] != nullptr && s_pWvwData->s_sApp->iTypeModal[iIndex] == 2) {
...
static void hb_gt_wvwCreateObjects(UINT usWinNum)
{
  ...
  s_pWvwData->s_sApp->penWhite = CreatePen(PS_SOLID, 0, static_cast<COLORREF>(RGB(255, 255, 255)));
  s_pWvwData->s_sApp->penBlack = CreatePen(PS_SOLID, 0, static_cast<COLORREF>(RGB(0, 0, 0)));
  s_pWvwData->s_sApp->penWhiteDim = CreatePen(PS_SOLID, 0, static_cast<COLORREF>(RGB(205, 205, 205)));
  s_pWvwData->s_sApp->penDarkGray = CreatePen(PS_SOLID, 0, static_cast<COLORREF>(RGB(150, 150, 150)));
  s_pWvwData->s_sApp->penGray = CreatePen(PS_SOLID, 0, static_cast<COLORREF>(_COLORS[7]));
  s_pWvwData->s_sApp->penNull = CreatePen(PS_NULL, 0, static_cast<COLORREF>(_COLORS[7]));
  s_pWvwData->s_sApp->currentPen = CreatePen(PS_SOLID, 0, static_cast<COLORREF>(RGB(0, 0, 0)));
...
static uint16_t hb_gt_wvwCalcPixelHeight(WIN_DATA *pWindowData)
{
  return hb_wvw_LineHeight(pWindowData) * pWindowData->ROWS + pWindowData->usSBHeight + pWindowData->usTBHeight;
}

static uint16_t hb_gt_wvwCalcPixelWidth(WIN_DATA *pWindowData)
{
  return static_cast<uint16_t>(pWindowData->PTEXTSIZE.x) * pWindowData->COLS;
}
                                     ...
                                     int32_t *pmaxcols)
{
  ...
  width = (iWidth < 0 ? static_cast<uint16_t>(-iWidth)
                      : static_cast<uint16_t>(static_cast<uint16_t>(static_cast<LONG>(tm.tmAveCharWidth)))) *
          static_cast<uint16_t>(cols); /* Total pixel width this setting would take */
  height = static_cast<uint16_t>(static_cast<uint16_t>(static_cast<LONG>(tm.tmHeight))) *
           static_cast<uint16_t>(rows); /* Total pixel height this setting would take */

  GetWindowRect(pWindowData->hWnd, &wi);
  GetClientRect(pWindowData->hWnd, &ci);

  diffWidth = static_cast<SHORT>((wi.right - wi.left) - (ci.right));
  diffHeight = static_cast<SHORT>((wi.bottom - wi.top) - (ci.bottom));
  width += diffWidth;
  height += diffHeight;

  height += static_cast<uint16_t>(pWindowData->byLineSpacing * rows);
  ...
  if (pmaxrows) {
    (*pmaxrows) =
        (maxHeight - diffHeight - pWindowData->usTBHeight - pWindowData->usSBHeight) / hb_wvw_LineHeight(pWindowData);
  }
  if (pmaxcols) {
...
static void hb_gt_wvwResetWindowSize(WIN_DATA *pWindowData, HWND hWnd)
{
   ...
   */
  auto hdc = GetDC(hWnd);
  auto hFont = hb_gt_wvwGetFont(pWindowData->fontFace, pWindowData->fontHeight, pWindowData->fontWidth,
                                pWindowData->fontWeight, pWindowData->fontQuality, pWindowData->CodePage);
  ...
  pWindowData->PTEXTSIZE.x = pWindowData->fontWidth < 0
                                 ? -pWindowData->fontWidth
...
#endif
  if (pWindowData->fontWidth < 0 || (tm.tmPitchAndFamily & TMPF_FIXED_PITCH) ||
      (pWindowData->PTEXTSIZE.x != tm.tmMaxCharWidth)) {
    pWindowData->FixedFont = FALSE;
  } else {
    pWindowData->FixedFont = TRUE;
  }

  for (auto n = 0; n < pWindowData->COLS; n++) { /* pWindowData->FixedSize[] is used by ExtTextOut() to emulate fixed
                                                    font when a proportional font is used */
  ...
  if (IsZoomed(pWindowData->hWnd)) {
    ...
    height = (static_cast<uint16_t>(static_cast<LONG>(wi.bottom))) - (static_cast<uint16_t>(static_cast<LONG>(wi.top))) + 1;
    width = (static_cast<uint16_t>(static_cast<LONG>(wi.right))) - (static_cast<uint16_t>(static_cast<LONG>(wi.left))) + 1;
  ...
  if (pWindowData->hStatusBar != nullptr) {
    SetWindowPos(pWindowData->hStatusBar, nullptr, wi.left, wi.bottom - pWindowData->usSBHeight, width,
                 pWindowData->usSBHeight, SWP_NOZORDER);
  }

  if (pWindowData->hToolBar != nullptr) {
    SetWindowPos(pWindowData->hToolBar, nullptr, wi.left, wi.top - pWindowData->usTBHeight, width,
                 pWindowData->usTBHeight, SWP_NOZORDER);
  }

  if (pWindowData->pcdCtrlList != nullptr) {
...
static int32_t hb_wvw_key_ansi_to_oem(int32_t c)
{
  char pszAnsi[4];
  char pszOem[4];

  sprintf(pszAnsi, "%c", c);
  CharToOemBuff(static_cast<LPCSTR>(pszAnsi), static_cast<LPTSTR>(pszOem), 1);
  c = static_cast<BYTE>(*pszOem);
...
static void xUserPaintNow(UINT usWinNum)
{
  ...
  if (s_pWvwData->s_sApp->pSymWVW_PAINT) {
    if (hb_vmRequestReenter()) {
      ...
      hb_vmPushNil();
      hb_vmPushInteger(static_cast<int32_t>(usWinNum));

      /* follow WVT convention to not passing coordinates anymore
         hb_vmPushInteger(static_cast<int32_t>(rpaint.top));
         hb_vmPushInteger(static_cast<int32_t>(rpaint.left));
         hb_vmPushInteger(static_cast<int32_t>(rpaint.bottom));
         hb_vmPushInteger(static_cast<int32_t>(rpaint.right));
         hb_vmDo(5);
       */
...
static LRESULT CALLBACK hb_gt_wvwWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_COMMAND: { /* handle menu items */
    ...
    if (iId < WVW_ID_BASE_PUSHBUTTON) {
      ...
      /*
         int32_t lowordwParam = (int32_t) LOWORD(wParam);
         int32_t hiwordwParam = (int32_t) HIWORD(wParam);
         int32_t lowordlParam = (int32_t) LOWORD(lParam);
         int32_t hiwordlParam = (int32_t) HIWORD(lParam);
      ...
      case CBN_KILLFOCUS:
        ...
        if ((iEvent == CBN_KILLFOCUS) || bTopMost || s_pWvwData->s_bAllowNonTop) {
          ...
          RunControlBlock(usWinNum, WVW_CONTROL_COMBOBOX, hWndCtrl, message, wParam, lParam, static_cast<int32_t>(iEvent));
      ...
      case EN_CHANGE:
        ...
        if ((iEvent == EN_KILLFOCUS) || bTopMost || s_pWvwData->s_bAllowNonTop) {
          ...
          RunControlBlock(usWinNum, WVW_CONTROL_EDITBOX, hWndCtrl, message, wParam, lParam, static_cast<int32_t>(iEvent));
  ...
  case WM_PAINT: {
    ...
    int32_t colStart = 0, colStop = 0, rowStart = 0, rowStop = 0;
    ...
    iybeyond = hb_wvw_LineHeight(pWindowData) * pWindowData->ROWS + pWindowData->usTBHeight;

    if (updateRect.left > ixbeyond || updateRect.top > iybeyond) {
      /* do nothing now, will be handled later */
    } else if (pWindowData->pBuffer != nullptr && pWindowData->pColors != nullptr) {
      /*
      ...
      for (irow = static_cast<uint16_t>(rowStart); irow <= static_cast<uint16_t>(rowStop); irow++) {
        uint16_t index, startIndex, startCol, len;
        BYTE oldColor, color;

        auto icol = static_cast<uint16_t>(colStart);
        ...
        while (icol <= colStop) {
          ...
          if (color != oldColor) {
            ...
            hb_gt_wvwTextOut(pWindowData, hdc, startCol, irow,
                             reinterpret_cast<char const *>(pWindowData->pBuffer) + startIndex, len);
    ...
    if (pWindowData->bPaint) {
      if (s_pWvwData->s_sApp->pSymWVW_PAINT) {
        ...
        hb_wvw_UpdatePendingRect(pWindowData, static_cast<uint16_t>(rowStart), static_cast<uint16_t>(colStart),
                                 static_cast<uint16_t>(rowStop), static_cast<uint16_t>(colStop));
...
#ifdef WVW_DEBUG
    printf("\nPuts( %d ), Scroll( %d ), Paint( %d ), SetFocus( %d ), KillFocus( %d ) ", nCountPuts, nCountScroll,
           ++nCountPaint, nSetFocus, nKillFocus);
#endif
    return 0;
  }
  ...
  case WM_SETFOCUS:
    ...
    if (usWinNum == s_pWvwData->s_usNumWindows - 1) {
      if (!s_pWvwData->s_bMainCoordMode) {
        hb_gtSetPos(static_cast<SHORT>(pWindowData->caretPos.y), static_cast<SHORT>(pWindowData->caretPos.x));
      } else {
        hb_gtSetPos((static_cast<uint16_t>(static_cast<LONG>(pWindowData->caretPos.y))) + hb_gt_wvwRowOfs(usWinNum),
                    (static_cast<uint16_t>(static_cast<LONG>(pWindowData->caretPos.x))) + hb_gt_wvwColOfs(usWinNum));
      }
  ...
  case WM_SYSCHAR:
    ...
    if (!pWindowData->bIgnoreWM_SYSCHAR) {
      int32_t c, iScanCode = HIWORD(lParam) & 0xFF;
      switch (iScanCode) {
      case 2:
        c = K_ALT_1;
        break;
  ...
  case WM_DRAWITEM:
    if (pWindowData->bSBPaint) {
      ...
      ExtTextOut(lpDIS->hDC, rectCorner.top, rectCorner.left + 3, 0, &lpDIS->rcItem, ptStr, stLen, nullptr);
...
static HWND hb_gt_wvwCreateWindow(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int32_t iCmdShow)
{
  ...
  if (!RegisterClass(&wndclass)) {
    MessageBox(nullptr, TEXT("Failed to register class."), s_pWvwData->szAppName, MB_ICONERROR);
    return 0;
  }
  ...
  if (hWnd == nullptr) {
    MessageBox(nullptr, TEXT("Failed to create window."), TEXT("HARBOUR_WVW"), MB_ICONERROR);
    return nullptr;
  }
  ...
  if (s_pWvwData->s_sApp->pSymWVW_PAINT && s_pWvwData->s_uiPaintRefresh > 0) {
    SetTimer(hWnd, WVW_ID_SYSTEM_TIMER, static_cast<UINT>(s_pWvwData->s_uiPaintRefresh), nullptr);
  }
   ...
   */
  if (hb_dynsymFind("HB_NOSTARTUPWINDOW") != nullptr) {
    iCmdShow = SW_HIDE;
  }
...
static void hb_gt_wvwCreateToolTipWindow(WIN_DATA *pWindowData)
{
   ...
   */
  hWndTT = CreateWindow(TOOLTIPS_CLASS, TEXT(""), WS_POPUP | TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                        CW_USEDEFAULT, nullptr, static_cast<HMENU>(nullptr), s_pWvwData->hInstance, nullptr);
   ...
   */
  if (!SendMessage(s_pWvwData->hWndTT, TTM_ADDTOOL, 0, reinterpret_cast<LPARAM>(&ti))) {
    return;
  }
...
POINT hb_gt_wvwGetXYFromColRow(WIN_DATA *pWindowData, uint16_t col, uint16_t row)
{
  POINT xy{};
...
RECT hb_gt_wvwGetColRowFromXYRect(WIN_DATA *pWindowData, RECT xy)
{
  ...
  colrow.bottom = (xy.bottom / (pWindowData->PTEXTSIZE.y + usLineSpaces) -
                   (xy.bottom % (pWindowData->PTEXTSIZE.y + usLineSpaces) ? 0 : 1));
...
RECT hb_gt_wvwGetXYFromColRowRect(WIN_DATA *pWindowData, RECT colrow)
{
  ...
  xy.bottom = (colrow.bottom + 1) * hb_wvw_LineHeight(pWindowData) - (pWindowData->byLineSpacing / 2);
...
static BOOL hb_gt_wvwSetCaretPos(WIN_DATA *pWindowData)
{
  ...
  xy = hb_gt_wvwGetXYFromColRow(pWindowData, static_cast<SHORT>(pWindowData->caretPos.x),
                                static_cast<SHORT>(pWindowData->caretPos.y));
...
static BOOL hb_gt_wvwTextOut(WIN_DATA *pWindowData, HDC hdc, uint16_t col, uint16_t row, LPCTSTR lpString, uint16_t cbString)
{
  ...
  if (pWindowData->FixedFont) {
    Result = ExtTextOut(hdc, xy.x, xy.y, ETO_CLIPPED | ETO_OPAQUE, &mClip, lpString, cbString, nullptr);
  } else {
    Result = ExtTextOut(hdc, xy.x, xy.y, ETO_CLIPPED | ETO_OPAQUE, &mClip, lpString, cbString, pWindowData->FixedSize);
  }
...
static BOOL hb_gt_wvwSetColors(WIN_DATA *pWindowData, HDC hdc, BYTE attr)
{
  int32_t fore = attr & 0x000F;
  int32_t back = (attr & 0x00F0) >> 4;
...
static void hb_gt_wvwTranslateKey(int32_t key, int32_t shiftkey, int32_t altkey, int32_t controlkey)
{
  int32_t iKey = hb_gt_wvwJustTranslateKey(key, shiftkey, altkey, controlkey);
...
HFONT hb_gt_wvwGetFont(const char *pszFace, int32_t iHeight, int32_t iWidth, int32_t iWeight, int32_t iQuality, int32_t iCodePage)
{
  HFONT hFont;
                             ...
                             uint16_t usCol2)
{
  ...
  if (usWinNum == 0) {
    pWindowData->byWinId = static_cast<int32_t>(usWinNum);
    ...
    pWindowData->pColors = nullptr;
    pWindowData->pBuffer = nullptr;
    pWindowData->caretPos.x = 0;
    pWindowData->caretPos.y = 0;
    ...
    pWindowData->hWndTT = nullptr;
    pWindowData->hPopup = nullptr;
    ...
    s_pWvwData->s_sApp->pphPictureList = nullptr;

    s_pWvwData->s_sApp->pbhUserBitmap = nullptr;
  ...
  pWindowData->pcdCtrlList = nullptr;

  pWindowData->hPBfont = nullptr; /* will be created on first creation of pushbutton, if ever */

  pWindowData->hCBfont = nullptr; /* will be created on first creation of combobox, if ever */

  pWindowData->hEBfont = nullptr; /* will be created on first creation of editbox, if ever */

  pWindowData->hCXfont = nullptr; /* will be created on first creation of checkbox, if ever */

  pWindowData->hSBfont = nullptr; /* will be created on first creation of statusbar, if ever */

  pWindowData->hSTfont = nullptr; /* will be created on first creation of static control, if ever */
...
static void hb_gt_wvwAddCharToInputQueue(int32_t iKey)
{
  UINT uiWinNum = s_pWvwData->s_usNumWindows - 1;
  int32_t iNextPos = s_pWvwData->s_pWindows[uiWinNum]->keyPointerIn;

  if (iKey == K_MOUSEMOVE || iKey == K_NCMOUSEMOVE) {
    if (s_pWvwData->s_pWindows[uiWinNum]->keyLast == iKey &&
        s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->keyPointerIn !=
            s_pWvwData->s_pWindows[uiWinNum]->keyPointerOut) {
      return;
    }
  }
...
uint16_t hb_gt_wvwGetMouseX(WIN_DATA *pWindowData)
{
  return static_cast<SHORT>(pWindowData->mousePos.x);
}

uint16_t hb_gt_wvwGetMouseY(WIN_DATA *pWindowData)
{
  return static_cast<SHORT>(pWindowData->mousePos.y);
}
                                           ...
                                           BYTE *sBuffer, ULONG length)
{
   ...
   */
  index = hb_gt_wvwGetIndexForTextBuffer(pWindowData, static_cast<uint16_t>(col), static_cast<uint16_t>(row));

  if ((length + index) <= static_cast<ULONG>(pWindowData->BUFFERSIZE)) {
     ...
     */
    end = hb_gt_wvwGetColRowForTextBuffer(pWindowData, index + ((static_cast<uint16_t>(static_cast<ULONG>(length))) - 1));
    hb_gt_wvwSetInvalidRect(pWindowData, static_cast<uint16_t>(col), static_cast<uint16_t>(row), static_cast<uint16_t>(end.x),
                            static_cast<uint16_t>(end.y));
  }
}
...
static void hb_gt_wvwHandleMenuSelection(int32_t menuIndex)
{
...
static void hb_gt_wvwMouseEvent(WIN_DATA *pWindowData, HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  colrow = hb_gt_wvwGetColRowFromXY(pWindowData, static_cast<SHORT>(xy.x), static_cast<SHORT>(xy.y));

  hb_gt_wvwSetMouseX(pWindowData, static_cast<SHORT>(colrow.x));
  hb_gt_wvwSetMouseY(pWindowData, static_cast<SHORT>(colrow.y));
  ...
  case WM_RBUTTONUP:
    if (pWindowData->hPopup) {
      GetCursorPos(&xy);
      lPopupRet = TrackPopupMenu(pWindowData->hPopup, TPM_CENTERALIGN + TPM_RETURNCMD, xy.x, xy.y, 0, hWnd, nullptr);
      if (lPopupRet) {
  ...
  if (s_pWvwData->s_sApp->pSymWVW_MOUSE && keyCode != 0) {
    if (hb_vmRequestReenter()) {
      ...
      hb_vmPushNil();
      hb_vmPushInteger(static_cast<int32_t>(pWindowData->byWinId));
      hb_vmPushLong(static_cast<SHORT>(keyCode));
      hb_vmPushLong(static_cast<SHORT>(colrow.y));
      hb_vmPushLong(static_cast<SHORT>(colrow.x));
      hb_vmPushLong(static_cast<SHORT>(keyState));
      hb_vmDo(5);
...
static void hb_gt_wvwTBMouseEvent(WIN_DATA *pWindowData, HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  colrow = hb_gt_wvwTBGetColRowFromXY(pWindowData, static_cast<SHORT>(xy.x), static_cast<SHORT>(xy.y));
...
static void hb_gt_wvwWindowPrologue(void)
{
  ...
  s_pWvwData->s_pWindows[uiWindow - 1] = new WIN_DATA;
}
...
static void hb_gt_wvwWindowEpilogue(void)
{
  ...
  delete s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1];
  s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1] = nullptr;
  s_pWvwData->s_usNumWindows--;
...
static UINT hb_gt_wvwOpenWindow(LPCTSTR lpszWinName, int32_t iRow1, int32_t iCol1, int32_t iRow2, int32_t iCol2, DWORD dwStyle,
                                int32_t iParentWin)
 ...
 */
{
  ...
  /* in MainCoord Mode make sure that usRowx and usColx are within Main Window's bound! */
   ...
   if( s_pWvwData->s_bMainCoordMode && (!hb_gt_wvwInWindow(0, iRow1, iCol1) || !hb_gt_wvwInWindow(0, iRow2, iCol2)) ) {
      MessageBox(nullptr, TEXT("Invalid (Row,Col)"), lpszWinName, MB_ICONERROR);
      return 0;
   }
#endif
  ...
  if (iParentWin < 0) {
    ...
    pParentWindow = s_pWvwData->s_pWindows[static_cast<uint16_t>(iParentWin)];
  }
  ...
  if (!s_pWvwData->s_bSWRegistered && (s_pWvwData->s_usNumWindows == 1)) {
    ...
    if (!RegisterClass(&wndclass)) {
      ...
      MessageBox(nullptr, TEXT("Failed to register class."), s_pWvwData->szSubWinName, MB_ICONERROR);
      return 0;
    }
  ...
  hb_gt_wvwWindowPrologue();
  hb_gtInitStatics(s_pWvwData->s_usNumWindows - 1, lpszWinName, static_cast<uint16_t>(iRow1), static_cast<uint16_t>(iCol1),
                   static_cast<uint16_t>(iRow2), static_cast<uint16_t>(iCol2));

  hWnd = CreateWindow(s_pWvwData->szSubWinName, lpszWinName, /* window name */
                      dwStyle,
                      ...
                      CW_USEDEFAULT,                                              /*width           */
                      CW_USEDEFAULT,                                              /*height          */
                      (pParentWindow == nullptr ? nullptr : pParentWindow->hWnd), /*x parent window */
                      nullptr,                                                    /*menu            */
  ...
  if (hWnd == nullptr) {
    ...
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr,
                  GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), reinterpret_cast<LPTSTR>(&lpMsgBuf), 0,
                  nullptr);
  ...
  s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->hdc =
      GetDC(s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->hWnd);
  s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->hCompDC =
      CreateCompatibleDC(s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->hdc);
  s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->hIcon = nullptr;
  ...
  /* E, logo apos, restaura aos valores originais mantendo em s_pWvwData->s_sApp os valores salvos para restauracao
     quando for utilizar DeleteObject()
   */
  SelectObject(s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->hdc,
               static_cast<HPEN>(s_pWvwData->s_sApp->OriginalPen));
  SelectObject(s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->hdc,
               static_cast<HBRUSH>(s_pWvwData->s_sApp->OriginalBrush));
...
static BOOL hb_gt_wvwBufferedKey(LONG lKey)
{
  return lKey != VK_SHIFT && lKey != VK_MENU && lKey != VK_CONTROL && lKey != VK_PAUSE && lKey != VK_CAPITAL &&
         lKey != VK_NUMLOCK && lKey != VK_SCROLL;
}
...
static void hb_gt_wvwInputNotAllowed(UINT usWinNum, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  if (!s_pWvwData->s_bFlashingWindow) {
    ...
    SetTimer(nullptr, 0, 50, static_cast<TIMERPROC>(hb_gt_wvwFlashWindow));
  }
}
 ...
 */
void hb_gt_wvwFUNCPrologue(BYTE byNumCoord, int32_t *iRow1, int32_t *iCol1, int32_t *iRow2, int32_t *iCol2)
{
  UINT usWinNum;

  if (byNumCoord < 2) {
    (*iCol1) = static_cast<uint16_t>(s_pWvwData->s_pWindows[0]->caretPos.x);
  }
  if (byNumCoord < 1) {
    (*iRow1) = static_cast<uint16_t>(s_pWvwData->s_pWindows[0]->caretPos.y);
  }

  usWinNum = hb_gt_wvwFindWindow(static_cast<uint16_t>(*iRow1), static_cast<uint16_t>(*iCol1));
...
{
  s_pWvwData->s_pWindows[0]->caretPos.y =
      s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->caretPos.y + hb_gt_wvwRowOfs(s_pWvwData->s_usCurWindow);
  s_pWvwData->s_pWindows[0]->caretPos.x =
      s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->caretPos.x + hb_gt_wvwColOfs(s_pWvwData->s_usCurWindow);
...
void hb_wvw_HBFUNCPrologue(UINT usWinNum, uint16_t *pusRow1, uint16_t *pusCol1, uint16_t *pusRow2, uint16_t *pusCol2)
{
  if (pusRow1) {
...
static UINT hb_gt_wvwSetCurWindow(UINT usWinNum)
{
  ...
  hb_gtSetMode(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->ROWS,
               s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->COLS);

  s_pWvwData->s_bQuickSetMode = FALSE;

  /* tell GTAPI about the new Row(), Col() */

  hb_gtSetPos(static_cast<SHORT>(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->caretPos.y),
              static_cast<SHORT>(s_pWvwData->s_pWindows[s_pWvwData->s_usCurWindow]->caretPos.x));
                                 ...
                                 ULONG ulLen)
{
  ...
  for (ULONG i = 0; i < ulLen; i++) {
    *(byChars + i) = static_cast<BYTE>(usChar);
  }
...
static BOOL hb_gt_wvw_GetChar(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t *pbColor, BYTE *pbAttr, uint16_t *pusChar)
{
  long lIndex;
...
static BOOL hb_gt_wvw_PutChar(HB_GT *pGT, int32_t iRow, int32_t iCol, int32_t bColor, BYTE bAttr, uint16_t usChar)
{
  long lIndex;
  ...
  if (lIndex < pWindowData->BUFFERSIZE) {
    pWindowData->pBuffer[lIndex] = static_cast<BYTE>(usChar);
...
static BOOL hb_gt_wvw_CheckPos(HB_GT *pGT, int32_t iRow, int32_t iCol, long *plIndex)
{
  HB_SYMBOL_UNUSED(pGT);

  auto pWindowData = static_cast<WIN_DATA *>(s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]);
  *plIndex = static_cast<long>(
      hb_gt_wvwGetIndexForTextBuffer(pWindowData, static_cast<uint16_t>(iCol), static_cast<uint16_t>(iRow)));

  return TRUE;
}

static void hb_gt_wvw_GetSize(HB_GT *pGT, int32_t *piRows, int32_t *piCols)
{
  HB_SYMBOL_UNUSED(pGT);
...
static void hb_gt_wvw_Save(HB_GT *pGT, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, void *pBuffer)
{
  ...
  while (i_Top <= i_Bottom) {
    ...
    for (int32_t iCol = i_Left; iCol <= i_Right; ++iCol) {
      if (!HB_GTSELF_GETCHAR(pGT, i_Top, iCol, &iColor, nullptr, &usChar)) {
        usChar = static_cast<uint16_t>(HB_GTSELF_GETCLEARCHAR(pGT));
        iColor = HB_GTSELF_GETCLEARCOLOR(pGT);
      }
      *pbyBuffer++ = static_cast<BYTE>(usChar);
      *pbyBuffer++ = static_cast<BYTE>(iColor); /* TOFIX */
    }
    ++i_Top;
  }
...
static void hb_gt_wvw_Rest(HB_GT *pGT, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, const void *pBuffer)
{
  ...
  pWindowData->InvalidateWindow = TRUE;
  hb_gt_wvwSetInvalidRect(pWindowData, static_cast<uint16_t>(i_Left), static_cast<uint16_t>(i_Top),
                          static_cast<uint16_t>(i_Right), static_cast<uint16_t>(i_Bottom));
...
static void hb_gt_wvw_ExposeArea(HB_GT *pGT, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight)
{
  HB_SYMBOL_UNUSED(pGT);
  HB_SYMBOL_UNUSED(iTop);
  HB_SYMBOL_UNUSED(iLeft);
...
static void hb_gt_wvw_vPutText(WIN_DATA *pWindowData, uint16_t top, uint16_t left, uint16_t bottom, uint16_t right,
                               const char *sBuffer, int32_t bColor)
{
  uint16_t icol, index, j = 0;

  for (uint16_t irow = top; irow <= bottom; irow++) {
    index = hb_gt_wvwGetIndexForTextBuffer(pWindowData, left, irow);
    for (icol = left; icol <= right; icol++) {
      if (index >= pWindowData->BUFFERSIZE) {
        break;
  ...
  hb_gt_wvwSetInvalidRect(pWindowData, static_cast<uint16_t>(left), static_cast<uint16_t>(top), static_cast<uint16_t>(right),
...
static void hb_gt_wvw_vSetAttribute(WIN_DATA *pWindowData, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, int32_t bColor)
{
  ...
  for (int32_t irow = iTop; irow <= iBottom; irow++) {
    index = hb_gt_wvwGetIndexForTextBuffer(pWindowData, static_cast<uint16_t>(iLeft), static_cast<uint16_t>(irow));
  ...
  hb_gt_wvwSetInvalidRect(pWindowData, static_cast<uint16_t>(iLeft), static_cast<uint16_t>(iTop),
                          static_cast<uint16_t>(iRight), static_cast<uint16_t>(iBottom));
}
...
static BOOL hb_gt_wvw_bSetMode(WIN_DATA *pWindowData, uint16_t row, uint16_t col)
{
  ...
  if (row <= WVW_MAX_ROWS && col <= WVW_MAX_COLS) {
    if (pWindowData->hWnd) {
      hFont = hb_gt_wvwGetFont(pWindowData->fontFace, pWindowData->fontHeight, pWindowData->fontWidth,
                               pWindowData->fontWeight, pWindowData->fontQuality, pWindowData->CodePage);
      if (hFont) {
         ...
         */
        if (hb_gt_wvwValidWindowSize(pWindowData, row, col, hFont, pWindowData->fontWidth, nullptr, nullptr)) {
                            ...
                            int32_t bColor)
{
  ...
  int32_t iHeight;
  int32_t iWidth;
  uint16_t sWidth = pWindowData->COLS;
  uint16_t sHeight = pWindowData->ROWS;
  BYTE szBox[10];
  auto bPadCh = static_cast<BYTE>(HB_GTSELF_GETCLEARCHAR(hb_gt_Base()));

  if ((iLeft >= 0 && iLeft < sWidth) || (iRight >= 0 && iRight < sWidth) || (iTop >= 0 && iTop < sHeight) ||
    ...
    if (iHeight > 1 && iWidth > 1 && iTop >= 0 && iTop < sHeight && iLeft >= 0 && iLeft < sWidth) {
      hb_gt_wvw_vxPutch(pWindowData, static_cast<uint16_t>(iTop), static_cast<uint16_t>(iLeft), bColor,
    ...
    if (iCol < iRight && iCol < sWidth && iTop >= 0 && iTop < sHeight) {
      hb_gt_wvw_vReplicate(pWindowData, iTop, iCol, bColor, HB_GT_ATTR_BOX, szBox[1],
                           iWidth + ((iRight - iLeft) > 1 ? -2 : 0)); /* iTop line */
    }
    if (iHeight > 1 && (iRight - iLeft) > 0 && iRight < sWidth && iTop >= 0 && iTop < sHeight) {
      hb_gt_wvw_vxPutch(pWindowData, static_cast<uint16_t>(iTop), static_cast<uint16_t>(iRight), bColor,
                        szBox[2]); /* Upper Right corner */
    }
      ...
      for (iRow = iTop + 1; iRow < iBottom; iRow++) {
        if (iRow >= 0 && iRow < sHeight) {
          ...
          if (iCol < 0) {
            ...
            hb_gt_wvw_vxPutch(pWindowData, static_cast<uint16_t>(iRow), static_cast<uint16_t>(iCol++), bColor,
                              szBox[7]); /* Left side */
          }

          hb_gt_wvw_vReplicate(pWindowData, static_cast<uint16_t>(iRow), static_cast<uint16_t>(iCol), bColor,
                               HB_GT_ATTR_BOX, szBox[8], iWidth - 2); /* Fill */
          if (iRight < sWidth) {
            hb_gt_wvw_vxPutch(pWindowData, static_cast<uint16_t>(iRow), static_cast<uint16_t>(iRight), bColor,
                              szBox[3]); /* Right side */
          }
        }
      }
      ...
      for (iRow = (iWidth > 1 ? iTop + 1 : iTop); iRow < ((iRight - iLeft) > 1 ? iBottom : iBottom + 1); iRow++) {
        if (iRow >= 0 && iRow < sHeight) {
          if (iLeft >= 0 && iLeft < sWidth) {
            hb_gt_wvw_vxPutch(pWindowData, static_cast<uint16_t>(iRow), static_cast<uint16_t>(iLeft), bColor,
                              szBox[7]); /* Left side */
          }
    ...
    if (iHeight > 1 && iWidth > 1) {
      if (iLeft >= 0 && iBottom < sHeight) {
        hb_gt_wvw_vxPutch(pWindowData, static_cast<uint16_t>(iBottom), static_cast<uint16_t>(iLeft), bColor,
                          szBox[6]); /* Bottom iLeft corner */
      }
      iCol = iLeft + 1;
      if (iCol < 0) {
        iCol = 0; /* The width was corrected earlier. */
      ...
      if (iCol <= iRight && iBottom < sHeight) {
        hb_gt_wvw_vReplicate(pWindowData, iBottom, iCol, bColor, HB_GT_ATTR_BOX, szBox[5],
                             iWidth - 2); /* Bottom line */
      }
      if (iRight < sWidth && iBottom < sHeight) {
        hb_gt_wvw_vxPutch(pWindowData, static_cast<uint16_t>(iBottom), static_cast<uint16_t>(iRight), bColor,
                          szBox[4]); /* Bottom Right corner */
      }
    }
...
{
  return (static_cast<BYTE>(static_cast<LONG>(pWindowData->PTEXTSIZE.y))) +
         static_cast<BYTE>(pWindowData->byLineSpacing);
}
                                   ...
                                   BYTE byAttrib)
{
  ...
  int32_t byColorIndex = pWindowData->iLSpaceColor < 0 ? ((byAttrib & 0x00F0) >> 4) : pWindowData->iLSpaceColor;
...
BOOL hb_gt_wvwSetMenuKeyEvent(UINT usWinNum, int32_t iMenuKeyEvent)
{
  int32_t iOldEvent;
...
int32_t hb_gt_wvwGetLastMenuEvent(UINT usWinNum)
{
  return s_pWvwData->s_pWindows[usWinNum]->LastMenuEvent;
}

int32_t hb_gt_wvwSetLastMenuEvent(UINT usWinNum, int32_t iLastMenuEvent)
{
  int32_t iRetval = s_pWvwData->s_pWindows[usWinNum]->LastMenuEvent;
...
static HICON hb_gt_wvwSetWindowIcon(UINT usWinNum, int32_t icon, const char *lpIconName)
{
  ...
  if (hIcon) {
    SendMessage(s_pWvwData->s_pWindows[usWinNum]->hWnd, WM_SETICON, ICON_SMALL,
                reinterpret_cast<LPARAM>(hIcon)); /* Set Title Bar ICON */
    SendMessage(s_pWvwData->s_pWindows[usWinNum]->hWnd, WM_SETICON, ICON_BIG,
...
int32_t hb_gt_wvwGetWindowTitle(UINT usWinNum, char *title, int32_t length)
{
  return GetWindowText(s_pWvwData->s_pWindows[usWinNum]->hWnd, title, length);
}

BOOL hb_gt_wvwSetFont(UINT usWinNum, const char *fontFace, int32_t height, int32_t width, int32_t Bold, int32_t Quality)
{
  int32_t size;
  BOOL bResult = FALSE;
  auto hFont = hb_gt_wvwGetFont(fontFace, height, width, Bold, Quality, s_pWvwData->s_pWindows[usWinNum]->CodePage);
  ...
  if (hFont) {
    ...
    if (hb_gt_wvwValidWindowSize(pWindowData, pWindowData->ROWS, pWindowData->COLS, hFont, width, nullptr, nullptr)) {
...
void hb_gt_wvwPostMessage(UINT usWinNum, int32_t message)
{
  SendMessage(s_pWvwData->s_pWindows[usWinNum]->hWnd, WM_CHAR, message, 0);
}

BOOL hb_gt_wvwSetWindowPos(UINT usWinNum, int32_t left, int32_t top)
{
  RECT wi{};

  GetWindowRect(s_pWvwData->s_pWindows[usWinNum]->hWnd, &wi);
  return SetWindowPos(s_pWvwData->s_pWindows[usWinNum]->hWnd, nullptr, left, top, (wi.right - wi.left) + 1,
...
BOOL GetIPictDimension(IPicture *pPic, int32_t *pWidth, int32_t *pHeight)
{
  OLE_HANDLE oHtemp;
  BITMAP bmTemp;
  ...
  GetObject(reinterpret_cast<HBITMAP>(oHtemp), sizeof(BITMAP), reinterpret_cast<LPSTR>(&bmTemp));
...
BOOL GetImageDimension(const char *image, int32_t *pWidth, int32_t *pHeight)
{
  HBITMAP hBitmap;
  BOOL bResult = TRUE;
...
static void DrawTransparentBitmap(HDC hdc, HBITMAP hBitmap, short xStart, short yStart, int32_t iDestWidth, int32_t iDestHeight)
{
  BITMAP bm;
  COLORREF cColor;
  POINT ptSize;
 ...
 */
BOOL hb_gt_wvwDrawImage(UINT usWinNum, int32_t x1, int32_t y1, int32_t wd, int32_t ht, const char *image, BOOL bTransparent)
{
  HBITMAP hBitmap;
  BOOL bResult;
  ...
  if (!hBitmap) {
    ...
    /* 2006-07-24 canNOT do it this way:
       ...
       iWidth = (int32_t) lWidth;
       iHeight = (int32_t) lHeight;
     */

    pPic->lpVtbl->get_Handle(pPic, &oHtemp);

    hBitmap = static_cast<HBITMAP>(CopyImage(reinterpret_cast<HBITMAP>(oHtemp), IMAGE_BITMAP, 0, 0, LR_COPYRETURNORG));
    ...
    GetObject(hBitmap, sizeof(BITMAP), reinterpret_cast<LPSTR>(&bmTemp));
  ...
  if (bTransparent) {
    ...
    bResult = StretchBlt(hdc,      /* handle to destination DC */
    ...
    if (nFileSize != INVALID_FILE_SIZE) {
      ...
      if (hGlobal) {
        if (ReadFile(hFile, hGlobal, nFileSize, &nReadByte, nullptr)) {
          ...
          OleLoadPicture(iStream, nFileSize, TRUE, IID_IPicture,
                         reinterpret_cast<LPVOID *>(&iPicture)); /* &IID_IPicture -> IID_IPicture */
        }
        GlobalFree(hGlobal);
      }
    }
...
BOOL hb_gt_wvwRenderPicture(UINT usWinNum, int32_t x1, int32_t y1, int32_t wd, int32_t ht, IPicture *iPicture, BOOL bTransp)
{
  LONG lWidth, lHeight;
  int32_t x, y, xe, ye;
  int32_t c = x1;
  ...
  if (iPicture) {
    ...
    if (bTransp) {
      ...
      if (oHtemp) {
        auto hdc = GetDC(pWindowData->hWnd);
        DrawTransparentBitmap(hdc, reinterpret_cast<HBITMAP>(oHtemp), static_cast<short>(x1), static_cast<short>(y1),
                              wd, ht);
        ReleaseDC(pWindowData->hWnd, hdc);
    ...
    if (dc == 0) {
      dc = static_cast<int32_t>(static_cast<float>(dr) * lWidth / lHeight);
    }
    if (dr == 0) {
      dr = static_cast<int32_t>(static_cast<float>(dc) * lHeight / lWidth);
    }
    if (tor == 0) {
      tor = dr;
    }
    ...
    while (x < xe) {
      while (y < ye) {
        iPicture->lpVtbl->Render(iPicture, pWindowData->hdc, x, y, dc, dr, 0, lHeight, lWidth, -lHeight, nullptr);
        y += dr;
      }
      y = r;
      x += dc;
    ...
    SelectClipRgn(pWindowData->hdc, nullptr);
    DeleteObject(hrgn1);
...
COLORREF hb_gt_wvwGetColorData(int32_t iIndex)
{
  return _COLORS[iIndex];
}

BOOL hb_gt_wvwSetColorData(int32_t iIndex, COLORREF ulCr)
{
  BOOL bResult = FALSE;
...
void hb_gt_wvwDrawBoxRaised(UINT usWinNum, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight,
                            BOOL bTight) /* <-- none in gtwvt */
{
  ...
  MoveToEx(pWindowData->hdc, iRight, iBottom, nullptr); /*  Right Inner   */
  LineTo(pWindowData->hdc, iRight, iTop);
  ...
  if (!bTight) {
    ...
    MoveToEx(pWindowData->hdc, iRight + 1, iTop - 1, nullptr); /*  Right Outer  */
...
void hb_gt_wvwDrawBoxRecessed(UINT usWinNum, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight, BOOL bTight)
{
  WIN_DATA *pWindowData = s_pWvwData->s_pWindows[usWinNum];
...
void hb_gt_wvwDrawOutline(UINT usWinNum, int32_t iTop, int32_t iLeft, int32_t iBottom, int32_t iRight)
{
  WIN_DATA *pWindowData = s_pWvwData->s_pWindows[usWinNum];
...
{
  return static_cast<WIN_DATA *>(s_pWvwData->s_pWindows[iWin]);
}
...
HB_FUNC(WVW_NOPENWINDOW)
{
  ...
  WIN_DATA *pWindowData;
  int32_t irow1, icol1, irow2, icol2;
  RECT wi{};
  RECT rcWorkArea{};
  UINT usWinNum;

  DWORD dwStyle = (HB_ISNIL(6) ? (static_cast<DWORD>(WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_CLIPCHILDREN))
  ...
  if (s_pWvwData->s_usNumWindows == WVW_MAXWINDOWS) {
    MessageBox(nullptr, TEXT("Too many Windows to open"), TEXT("Error"), MB_ICONERROR);
    hb_retni(0);
    return;
  }

  if (iParentWin > static_cast<INT>(s_pWvwData->s_usNumWindows) - 1) {
    MessageBox(nullptr, TEXT("Invalid Parent Window"), TEXT("Error"), MB_ICONERROR);
    hb_retni(0);
    return;
  }
  ...
  if (HB_ISCHAR(1)) {
    ...
    if (iLen > WVW_MAXWINNAMELENGTH - 1) {
      MessageBox(nullptr, TEXT("Windows name too long"), TEXT("Error"), MB_ICONERROR);
      hb_retni(0);
      return;
    }
    pFileName = nullptr;
...
HB_FUNC(WVW_LCLOSEWINDOW)
{
  ...
  if (!s_pWvwData->s_bMainCoordMode) {
    ...
    hb_gtSetMode(s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->ROWS,
                 s_pWvwData->s_pWindows[s_pWvwData->s_usNumWindows - 1]->COLS);
...
{
  hb_retni(static_cast<int32_t>(s_pWvwData->s_usNumWindows));
}
...
HB_FUNC(WVW_XREPOSWINDOW)
{
  ...
  for (UINT i = 1; i < s_pWvwData->s_usNumWindows; i++) {
    if (bAnchored) {
...
HB_FUNC(WVW_NROWOFS)
{
  ...
  hb_retni(static_cast<int32_t>(hb_gt_wvwRowOfs(usWinNum)));
}
...
HB_FUNC(WVW_NCOLOFS)
{
  ...
  hb_retni(static_cast<int32_t>(hb_gt_wvwColOfs(usWinNum)));
}
...
HB_FUNC(WVW_MAXMAXROW)
{
  ...
  hb_gt_wvwValidWindowSize(s_pWvwData->s_pWindows[usWinNum], 10, 10, s_pWvwData->s_pWindows[usWinNum]->hFont,
                           s_pWvwData->s_pWindows[usWinNum]->fontWidth, &maxrows, nullptr);
  hb_retni(maxrows - 1);
}
...
HB_FUNC(WVW_MAXMAXCOL)
{
  ...
  /* rows and cols passed are dummy ones */
  hb_gt_wvwValidWindowSize(s_pWvwData->s_pWindows[usWinNum], 10, 10, s_pWvwData->s_pWindows[usWinNum]->hFont,
                           s_pWvwData->s_pWindows[usWinNum]->fontWidth, nullptr, &maxcols);
  hb_retni(maxcols - 1);
}
...
HB_FUNC(WVW_ADDROWS)
{
  ...
  if (/* (iRows < 0) || */
      (s_pWvwData->s_bMainCoordMode &&
       (pWindowData->usRowOfs + pWindowData->ROWS + iRows > s_pWvwData->s_pWindows[0]->ROWS)) ||
      (pWindowData->ROWS + iRows > WVW_MAX_ROWS) || (pWindowData->ROWS + iRows < 1)) {
    hb_retl(false);
    return;
  }

  usNumChars = static_cast<uint16_t>(iRows) * pWindowData->COLS;

  if (iRows > 0) {
    /* initialize chars and attributes */
    uint16_t usBufLastRow = hb_gt_wvwGetIndexForTextBuffer(pWindowData, 0, pWindowData->ROWS - 1);
    uint16_t usBufStart = hb_gt_wvwGetIndexForTextBuffer(pWindowData, 0, pWindowData->ROWS);

    memset(&(pWindowData->byBuffer[usBufStart]), ' ', usNumChars);

    memset(&(pWindowData->byColors[usBufStart]), static_cast<char>(pWindowData->byColors[usBufLastRow]), usNumChars);
  }

  /* update vars */
  pWindowData->ROWS += static_cast<uint16_t>(iRows);
  ...
  GetWindowRect(pWindowData->hWnd, &wi);
  GetClientRect(pWindowData->hWnd, &ci);
  auto diffheight = static_cast<uint16_t>((wi.bottom - wi.top) - (ci.bottom - ci.top));
  auto diffwidth = static_cast<uint16_t>((wi.right - wi.left) - (ci.right - ci.left));

  height += diffheight;
  width += diffwidth;

  SetWindowPos(pWindowData->hWnd, nullptr, wi.left, wi.top, width, height, SWP_NOZORDER);
  ...
  if (pWindowData->hStatusBar != nullptr) {
   ...
   if( pWindowData->hToolBar != nullptr ) {
      SetWindowPos(pWindowData->hToolBar, nullptr, wi.left, wi.top - pWindowData->usTBHeight, width, pWindowData->usTBHeight, SWP_NOZORDER);
   }
  ...
  if (iRows > 0) {
    ...
    pWindowData->InvalidateWindow = TRUE;
    hb_gt_wvwSetInvalidRect(pWindowData, 0, static_cast<uint16_t>(pWindowData->ROWS) - static_cast<uint16_t>(iRows),
                            static_cast<uint16_t>(pWindowData->COLS) - 1, static_cast<uint16_t>(pWindowData->ROWS) - 1);
  }
...
HB_FUNC(WVW_SETWINSTYLE)
{
  ...
  if (HB_ISNUM(2)) {
    lpStyle = SetWindowLongPtr(s_pWvwData->s_pWindows[usWinNum]->hWnd, GWL_STYLE, static_cast<LONG_PTR>(hb_parnl(2)));
    SetWindowPos(s_pWvwData->s_pWindows[usWinNum]->hWnd, nullptr, 0, 0, 0, 0,
...
HB_FUNC(WVW_ENABLEMAXIMIZE)
{
  ...
  if (HB_ISLOG(2)) {
    if (hb_parl(2)) {
      ...
      lpStyle |= static_cast<LONG_PTR>(WS_MAXIMIZEBOX);
    } else {
      if (!bState) {
        return; /* no need */
      }
  ...
  if (HB_ISNUM(1) && (hb_parni(1) >= 50 || hb_parni(1) == 0)) {
    ...
    if (s_pWvwData->s_sApp->pSymWVW_PAINT) {
      for (UINT i = 0; i < s_pWvwData->s_usNumWindows; i++) {
        if (s_pWvwData->s_uiPaintRefresh > 0) {
          SetTimer(s_pWvwData->s_pWindows[i]->hWnd, WVW_ID_SYSTEM_TIMER,
                   static_cast<UINT>(s_pWvwData->s_uiPaintRefresh), nullptr);
        } else {
 ...
 * returns old setting of s_pWvwData->s_bDefHCentreWindow (default horizontal CentreWindow setting for newly opened
 * subwindow) then assigns s_pWvwData->s_bDefHCentreWindow := lCentre (if supplied) NOTES:
 * - lCentre will be the default CentreWindow for all subwindow opens
 */
 ...
 * returns old setting of s_pWvwData->s_bDefVCentreWindow (default horizontal CentreWindow setting for newly opened
 * subwindow) then assigns s_pWvwData->s_bDefVCentreWindow := lCentre (if supplied) NOTES:
 * - lCentre will be the default CentreWindow for all subwindow opens
 */
...
HB_FUNC(WVW_GETTITLE)
{
  ...
  hb_gt_wvwGetWindowTitle(usWinNum, reinterpret_cast<char *>(ucText), 1023);

  hb_retc(reinterpret_cast<char *>(ucText));
}
...
HB_FUNC(WVW_SETCLIPBOARD)
{
  ...
  lptstrCopy[nLen + 1] = static_cast<TCHAR>(0);
  ...
  if (hglb != nullptr) {
    ...
    if (lptstr != nullptr) {
      ...
      for (ul = 0; ul < GlobalSize(hglb); ul++) {
        hb_gt_wvwAddCharToInputQueue(static_cast<int32_t>(lptstr[ul]));
      }
...
HB_FUNC(WVW_GETWINDOWHANDLE)
{
  ...
  hb_retnl(reinterpret_cast<LONG>(hb_gt_wvwGetWindowHandle(usWinNum)));
}
...
HB_FUNC(WVW_CENTERWINDOW)
{
  ...
  hb_retl(hb_gt_wvwSetCentreWindow(usWinNum, HB_ISNIL(2) ? TRUE : hb_parl(2), HB_ISNIL(3) ? FALSE : hb_parl(3)));
}
...
int32_t nCopyAnsiToWideChar(LPWORD lpWCStr, LPSTR lpAnsiIn)
{
  int32_t nChar = 0;
...
IPicture *rr_LoadPictureFromResource(const char *resname, UINT iresimage, LONG *lwidth, LONG *lheight)
{
  ...
  if (resname == nullptr) {
    ...
    if (!hbmpx) {
      hbmpx = static_cast<HBITMAP>(LoadImage(s_pWvwData->hInstance,
                                             static_cast<LPCTSTR>(MAKEINTRESOURCE(static_cast<WORD>(iresimage))),
    ...
    if (!hbmpx) {
      hbmpx = static_cast<HBITMAP>(LoadImage(s_pWvwData->hInstance, resname, IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR));
  ...
  *lheight = iHeight;
  ...
  if (hbmpx != nullptr) {
    ...
    if (iPicture == nullptr) {
      ...
      OleCreatePictureIndirect(&picd, IID_IPicture, TRUE,
                               reinterpret_cast<LPVOID *>(&iPicture)); /* &IID_IPicture -> IID_IPicture */
...
static BITMAPINFO *PackedDibLoad(PTSTR szFileName)
{
  ...
  if (!bSuccess || (dwBytesRead != sizeof(BITMAPFILEHEADER)) ||
      (bmfh.bfType != *reinterpret_cast<WORD *>(const_cast<char *>("BM")))) {
    CloseHandle(hFile);
    return nullptr;
  }
...
static BYTE *PackedDibGetBitsPtr(BITMAPINFO *pPackedDib)
{
  return (reinterpret_cast<BYTE *>(pPackedDib)) + PackedDibGetInfoHeaderSize(pPackedDib) +
...
HBITMAP FindBitmapHandle(const char *szFileName, int32_t *piWidth, int32_t *piHeight)
{
  ...
  while (pbh) {
    ...
    if (strcmp(szFileName, pbh->szFilename) == 0 &&
        (!bStrictDimension || (*piWidth == pbh->iWidth && *piHeight == pbh->iHeight))) {
      if (!bStrictDimension) {
        *piWidth = pbh->iWidth;
...
void AddBitmapHandle(const char *szFileName, HBITMAP hBitmap, int32_t iWidth, int32_t iHeight)
{
...
static IPicture *FindPictureHandle(const char *szFileName, int32_t *piWidth, int32_t *piHeight)
{
  ...
  while (pph) {
    ...
    if (strcmp(szFileName, pph->szFilename) == 0 &&
        (!bStrictDimension || (*piWidth == pph->iWidth && *piHeight == pph->iHeight))) {
      if (!bStrictDimension) {
        *piWidth = pph->iWidth;
...
static HBITMAP hPrepareBitmap(char *szBitmap, UINT uiBitmap, int32_t iExpWidth, int32_t iExpHeight, BOOL bMap3Dcolors,
                              HWND hCtrl)
{
  HBITMAP hBitmap;
  ...
  if (szBitmap) {
    ...
    if (!hBitmap) {
      hBitmap = static_cast<HBITMAP>(
          LoadImage(s_pWvwData->hInstance, szBitmap, IMAGE_BITMAP, iExpWidth, iExpHeight, uiOptions));
      if (hBitmap) {
    ...
    if (!hBitmap) {
      ...
      if (pPackedDib || bMap3Dcolors) {
        ...
        if (!bMap3Dcolors) {
          ...
          hBitmap = CreateDIBitmap(hdc, reinterpret_cast<PBITMAPINFOHEADER>(pPackedDib), CBM_INIT,
                                   PackedDibGetBitsPtr(pPackedDib), pPackedDib, DIB_RGB_COLORS);
          ...
          hBitmap = static_cast<HBITMAP>(LoadImage(static_cast<HINSTANCE>(nullptr), szBitmap, IMAGE_BITMAP, iExpWidth,
                                                   iExpHeight, LR_LOADFROMFILE | LR_LOADMAP3DCOLORS));
        ...
        if (iExpWidth != iWidth || iExpHeight != iHeight) {
          ...
          if (!bResult) {
            ...
            MessageBox(nullptr, TEXT("Cannot shrink/stretch bitmap for WVW Control"), s_pWvwData->szAppName,
                       MB_ICONERROR);
...
BOOL AddTBButton(HWND hWndToolbar, char *szBitmap, UINT uiBitmap, char *pszLabel, int32_t iCommand, int32_t iBitmapType,
                 BOOL bMap3Dcolors, WIN_DATA *pWindowData, BOOL bDropdown)
{
  TBBUTTON tbb;
  TBADDBITMAP tbab;
  char szBuffer[WVW_TB_LABELMAXLENGTH + 2];
  int32_t iNewBitmap, iNewString;
  int32_t iOffset;
  BOOL bSuccess;
  HBITMAP hBitmap;
  ...
  if (iCommand == 0) {
    ...
    bSuccess = static_cast<BOOL>(SendMessage(hWndToolbar, TB_ADDBUTTONS, static_cast<WPARAM>(1),
  ...
  if (iBitmapType == 0) {
    int32_t iExpWidth, iExpHeight;

    iExpWidth = pWindowData->iTBImgWidth;
    iExpHeight = pWindowData->iTBImgHeight;

    hBitmap = hPrepareBitmap(szBitmap, uiBitmap, iExpWidth, iExpHeight, bMap3Dcolors, hWndToolbar);
    ...
    iNewBitmap = SendMessage(hWndToolbar, TB_ADDBITMAP, static_cast<WPARAM>(1),
                             reinterpret_cast<WPARAM>(&tbab)); /* TODO: WPARAM ou LPARAM ? */
  } else {                                                     /* system bitmap */
    iNewBitmap = static_cast<int32_t>(uiBitmap) + iOffset;
  }

  szBuffer[0] = static_cast<char>(0);
  strcat(szBuffer, pszLabel);
  szBuffer[strlen(szBuffer) + 1] = static_cast<char>(0);
  iNewString = SendMessage(hWndToolbar, TB_ADDSTRING, 0, reinterpret_cast<LPARAM>(szBuffer));
  ...
  bSuccess = SendMessage(hWndToolbar, TB_ADDBUTTONS, static_cast<WPARAM>(1),
                         reinterpret_cast<LPARAM>(static_cast<LPTBBUTTON>(&tbb)));

  return bSuccess;
}

int32_t IndexToCommand(HWND hWndTB, int32_t iIndex)
{
  TBBUTTON tbb;
...
int32_t CommandToIndex(HWND hWndTB, int32_t iCommand)
{
  return SendMessage(hWndTB, TB_COMMANDTOINDEX, static_cast<WPARAM>(iCommand), 0);
}
...
void hb_gt_wvwTBinitSize(WIN_DATA *pWindowData, HWND hWndTB)
{
  ...
  if (GetClientRect(hWndTB, &rTB)) {
    auto usTBHeight = static_cast<uint16_t>(rTB.bottom);
...
LRESULT CALLBACK hb_gt_wvwTBProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  if (hWndParent == nullptr) {
    ...
    MessageBox(nullptr, TEXT("hb_gt_wvwTBProc(): parent of toolbar is missing"), s_pWvwData->szAppName, MB_ICONERROR);
  ...
  if (usWinNum >= s_pWvwData->s_usNumWindows) {
    /* TODO: runtime/internal error is better */
    MessageBox(nullptr, TEXT("hb_gt_wvwTBProc(): invalid handle of tollbar's parent"), s_pWvwData->szAppName,
               MB_ICONERROR);
  ...
  return CallWindowProc(static_cast<WNDPROC>(pWindowData->tbOldProc), hWnd, message, wParam, lParam);
}
                      ...
                      RECT rOffCtrl, byte bStyle)
{
  ...
  pcdNew->phiCodeBlock = nullptr;

  if (phiCodeBlock != nullptr) {
  ...
  pcdNew->OldProc = static_cast<WNDPROC>(nullptr);
...
CONTROL_DATA *GetControlData(UINT usWinNum, BYTE byCtrlClass, HWND hWndCtrl, UINT uiCtrlid)
{
  ...
  while (pcd) {
    if (byCtrlClass == pcd->byCtrlClass &&
        ((hWndCtrl && hWndCtrl == pcd->hWndCtrl) || (uiCtrlid && uiCtrlid == pcd->uiCtrlid))) {
      return pcd;
    }
    pcd = pcd->pNext;
  }
...
WNDPROC GetControlProc(UINT usWinNum, BYTE byCtrlClass, HWND hWndCtrl)
{
  ...
  return static_cast<WNDPROC>(nullptr);
}
      ...
      case CBN_KILLFOCUS:
        ...
        if (iCurSel == CB_ERR) {
          break;
               ...
               SendMessage(static_cast<HWND>(pcd->hWndCtrl), CB_GETLBTEXT, static_cast<WPARAM>(iCurSel), static_cast<LPARAM>(lptstrSelected));
...
static void ReposControls(UINT usWinNum, BYTE byCtrlClass)
{
  ...
  while (pcd) {
    if (byCtrlClass == 0 || byCtrlClass == pcd->byCtrlClass) {
      ...
      xy = hb_gt_wvwGetXYFromColRow(pWindowData, static_cast<uint16_t>(pcd->rCtrl.left),
                                    static_cast<uint16_t>(pcd->rCtrl.top));
      iTop = xy.y + pcd->rOffCtrl.top;
      iLeft = xy.x + pcd->rOffCtrl.left;

      xy = hb_gt_wvwGetXYFromColRow(pWindowData, (static_cast<uint16_t>(static_cast<LONG>(pcd->rCtrl.right))) + 1,
                                    (static_cast<uint16_t>(static_cast<LONG>(pcd->rCtrl.bottom))) + 1);
      ...
      SetWindowPos(pcd->hWndCtrl, nullptr, iLeft, iTop, iRight - iLeft + 1, iBottom - iTop + 1, SWP_NOZORDER);
    }
...
LRESULT CALLBACK hb_gt_wvwXBProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  if (OldProc == nullptr) {
    MessageBox(nullptr, TEXT("Failed GetControlProc of Scrollbar"), s_pWvwData->szAppName, MB_ICONERROR);
...
LRESULT CALLBACK hb_gt_wvwBtnProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SYSKEYDOWN: {
    ...
    int32_t c = static_cast<int32_t>(wParam);
  ...
  return CallWindowProc(static_cast<WNDPROC>(OldProc), hWnd, message, wParam, lParam);
}
                  ...
                  int32_t iOffRight, double dStretch, BOOL bMap3Dcolors, int32_t iStyle)
{
  ...
  if (pWindowData->hPBfont == nullptr) {
  ...
  hWndButton = CreateWindowEx(0L,                             /* no extended styles */
                              "BUTTON",                       /* pushbutton/checkbox control class */
                              const_cast<LPSTR>(lpszCaption), /* text for caption */
                              WS_CHILD | WS_VISIBLE | static_cast<DWORD>(iStyle), /* button styles */
  ...
  if (hWndButton) {
    ...
    if (szBitmap || uiBitmap) {
      ...
      hBitmap = hPrepareBitmap(szBitmap, uiBitmap, static_cast<int32_t>(dStretch) * iExpWidth,
                               static_cast<int32_t>(dStretch) * iExpHeight, bMap3Dcolors, hWndButton);

      if (hBitmap) {
        SendMessage(hWndButton,                         /* handle to destination window */
                    BM_SETIMAGE,                        /* message to send */
                    static_cast<WPARAM>(IMAGE_BITMAP),  /* image type */
                    reinterpret_cast<LPARAM>(hBitmap)); /* handle to the image (HANDLE) */
      }
    }
    ...
    SendMessage(hWndButton, WM_SETFONT, reinterpret_cast<WPARAM>(pWindowData->hPBfont), static_cast<LPARAM>(TRUE));
    hb_stornl(reinterpret_cast<LONG>(hWndButton), 12);
...
LRESULT CALLBACK hb_gt_wvwCBProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SYSKEYDOWN: {
    ...
    bDropped = SendMessage(static_cast<HWND>(hWnd), CB_GETDROPPEDSTATE, 0, 0);
    ...
    if (bKbdType == WVW_CB_KBD_STANDARD) {
      ...
      case VK_NEXT:
        ...
        if (bDropped || bAlt || bShift || bCtrl) {
          ...
          if (!bDropped) {
            SendMessage(static_cast<HWND>(hWnd), CB_SHOWDROPDOWN, static_cast<WPARAM>(TRUE), 0);
            return 0;
          } else {
...
LRESULT CALLBACK hb_gt_wvwEBProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SYSKEYDOWN: {
    ...
    BOOL bMultiline = ((bEBType & WVW_EB_MULTILINE) == WVW_EB_MULTILINE);
