...
static BOOL hb_gt_wvw_Info(HB_GT *pGT, int32_t iType, PHB_GT_INFO pInfo)
{
#if 0
...
LRESULT CALLBACK hb_gt_wvwBtnProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SYSKEYDOWN: {
    ...
    int32_t c = static_cast<int>(wParam);
                  ...
                  int32_t iOffRight, double dStretch, BOOL bMap3Dcolors, int32_t iStyle)
{
  ...
  if (hWndButton) {
    ...
    if (szBitmap || uiBitmap) {
      ...
      hBitmap = hPrepareBitmap(szBitmap, uiBitmap, static_cast<int>(dStretch) * iExpWidth,
                               static_cast<int>(dStretch) * iExpHeight, bMap3Dcolors, hWndButton);
