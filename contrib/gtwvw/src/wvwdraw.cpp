...
HB_FUNC(WVW_DRAWLABELOBJ)
{
  ...
  RECT rect{};
  int32_t oldTextAlign, iAlignHorz, iAlignVert, iAlignH = 0, iAlignV;
...
HB_FUNC(WVW_DRAWBOXRAISED)
{
  ...
  int32_t iOLeft, iOTop, iORight, iOBottom;
...
HB_FUNC(WVW_DRAWIMAGE)
{
  ...
  auto s_sApp = hb_gt_wvwGetAppData();
  POINT xy{};
  int32_t iLeft, iTop, iRight = 0, iBottom = 0;
  ...
  if ((bActBottom || bActRight) && !(bActBottom && bActRight)) {
    int32_t iDispWidth, iDispHeight;
    if (bActRight) {
...
HB_FUNC(WVW_DRAWGRIDVERT)
{
  ...
  for (auto i = 1; i <= iTabs; i++) {
    usCol = static_cast<uint16_t>(hb_parvni(4, i));
    if (hb_gt_wvw_GetMainCoordMode()) {
...
HB_FUNC(WVW_DRAWBUTTON)
{
  ...
  RECT rc{};
  int32_t iTop, iLeft, iBottom, iRight;
  int32_t iAlign, oldTextAlign, oldBkMode;
  int32_t iTextHeight /*, iTextWidth */;
  int32_t iImageWidth, iImageHeight;
  COLORREF /* oldBkColor, */ oldTextColor;
  LOGBRUSH lb{};
  IPicture *iPicture;
...
HB_FUNC(WVW_DRAWSTATUSBAR)
{
  ...
  int32_t iTop, iLeft, iBottom, iRight;
  POINT xy{};
  uint16_t usTop, usLeft, usBottom, usRight;

  iNext = 0;

  for (auto i = 0; i < iPanels; i++) {
    usTop = static_cast<uint16_t>(hb_parvni(3, iNext + 1));
    usLeft = static_cast<uint16_t>(hb_parvni(3, iNext + 2));
    usBottom = static_cast<uint16_t>(hb_parvni(3, iNext + 3));
    usRight = static_cast<uint16_t>(hb_parvni(3, iNext + 4));
  ...
  usTop = static_cast<uint16_t>(hb_parvni(3, (4 * iPanels) - 1));
  usLeft = static_cast<uint16_t>(hb_parvni(3, 4 * iPanels));
