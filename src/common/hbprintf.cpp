...
#if defined(HB_NO_MODFL) || defined(__MINGW32CE__) || defined(HB_OS_CYGWIN) || defined(HB_OS_BEOS) ||                  \
    defined(__OpenBSD__) || defined(__NetBSD__) || defined(__DragonFly__) ||                                           \
    (defined(__FreeBSD_version) && __FreeBSD_version < 603000) || defined(HB_OS_ANDROID) || defined(HB_OS_MINIX)
  ...
  if (plst->repeat) {
    ...
    if (iArg > plst->size) {
      ...
      if (prev_size == _ARGBUF_SIZE) {
        plst->arglst = static_cast<v_param *>(
            memcpy(hb_xgrab(plst->size * sizeof(v_param)), plst->arglst, _ARGBUF_SIZE * sizeof(v_param)));
      } else {
        ...
        if (c != 0 && c != '%') {
          ...
          case 'F': // double decimal notation
            if (length == _L_LONGDOUBLE_) {
              ...
              argval.value.as_x_long_dbl =
                  static_cast<_x_long_dbl>((value & (_HB_NUM_NAN | _HB_NUM_PINF | _HB_NUM_NINF)) == 0 ? d : 0);
            }
            if (value & _HB_NUM_NAN) {
              size = put_str(buffer, bufsize, size,
                             c == 'f' ? ((flags & _F_SIGN) ? "+nan" : "nan") : ((flags & _F_SIGN) ? "+NAN" : "NAN"),
                             flags, width, -1);
            } else if (value & _HB_NUM_PINF) {
              size = put_str(buffer, bufsize, size,
                             c == 'f' ? ((flags & _F_SIGN) ? "+inf" : "inf") : ((flags & _F_SIGN) ? "+INF" : "INF"),
                             flags, width, -1);
          ...
          case 'X': // unsigned int hexadecimal conversion
            ...
            if (c == 'o') {
              ...
              size = put_dec(buffer, bufsize, size, argval.value.as_x_uintmax_t, flags & ~(_F_SPACE | _F_SIGN), width,
                             precision, 0);
            } else {
