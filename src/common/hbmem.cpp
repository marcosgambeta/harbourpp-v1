...
void *hb_xmemcpy(void *pDestArg, const void *pSourceArg, HB_SIZE nLen)
{
  ...
  pDest = static_cast<uint8_t *>(pDestArg);
  pSource = static_cast<const uint8_t *>(pSourceArg);
  nRemaining = nLen;
  ...
  while (nRemaining) {
    ...
    if (nRemaining > UINT_MAX) {
      ...
      iCopySize = static_cast<int32_t>(nRemaining);
      nRemaining = 0;
    }
...
#ifndef hb_xmemset
void *hb_xmemset(void *pDestArg, int32_t iFill, HB_SIZE nLen)
{
#if 0
  ...
  while (nRemaining) {
    ...
    if (nRemaining > UINT_MAX) {
      ...
      iSetSize = static_cast<int32_t>(nRemaining);
      nRemaining = 0;
    }
