...
HB_MAXUINT hb_dateMilliSeconds(void)
{
...
#if defined(HB_OS_WIN)
   {
      SYSTEMTIME st;
      GetSystemTime(&st);
      return static_cast<HB_MAXUINT>(hb_dateEncode(st.wYear, st.wMonth, st.wDay)) * HB_MILLISECS_PER_DAY + hb_timeEncode(st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
   }
#elif defined(HB_OS_UNIX)
   {
      struct timeval tv;
      gettimeofday(&tv, nullptr);
      return (static_cast<HB_MAXUINT>(tv.tv_sec) + static_cast<HB_MAXUINT>(HB_SYS_DATE_BASE) * HB_SECONDS_PER_DAY) * 1000 + tv.tv_usec / 1000;
   }
#else
   {
      struct timeb tb;
      ftime(&tb);
      return (static_cast<HB_MAXUINT>(tb.time) + static_cast<HB_MAXUINT>(HB_SYS_DATE_BASE) * HB_SECONDS_PER_DAY ) * 1000 + tb.millitm;
   }
#endif
}
   ...
   if( iHour >= 0 && iHour < 24 &&
      ...
      if( lJulian != 0 || (iYear == 0 && iMonth == 0 && iDay == 0) ) {
         dTimeStamp = static_cast<double>(lJulian) +
                      ...
                      iSeconds) * 1000 + iMSec) / HB_MILLISECS_PER_DAY;
      }
   }
   return dTimeStamp;
}
...
double hb_timeStampPackD(int iYear, int iMonth, int iDay, int iHour, int iMinutes, double dSeconds)
{
   ...
   if( iHour >= 0 && iHour < 24 && iMinutes >= 0 && iMinutes < 60 && dSeconds >= 0 && dSeconds < 60 ) {
      ...
      if( lJulian != 0 || (iYear == 0 && iMonth == 0 && iDay == 0) ) {
         dTimeStamp = static_cast<double>(lJulian) + static_cast<double>(((iHour * 60 + iMinutes) * 60) + dSeconds) / HB_SECONDS_PER_DAY;
      }
   }
   return dTimeStamp;
}
