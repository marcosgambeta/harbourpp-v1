...
void hb_put_ieee754(uint8_t *ptr, double d)
{
  ...
  if (d == 0.0) {
    ...
    l2 = static_cast<HB_U32>(ldexp(df, HB_MANTISSA_BITS + 1 - 32)) &
         ((static_cast<HB_U32>(1) << (HB_MANTISSA_BITS - 32)) - 1);
...
double hb_get_ieee754(const uint8_t *ptr)
{
...
#if defined(HB_LONG_LONG_OFF)
  HB_U32 l1 = HB_GET_LE_UINT32(ptr);
  HB_U32 l2 = HB_GET_LE_UINT32(ptr + 4);
  iSig = static_cast<int>(l2 >> (HB_MANTISSA_BITS + HB_EXPONENT_BITS - 32)) & 1;
  auto iExp = static_cast<int>((l2 >> (HB_MANTISSA_BITS - 32)) & HB_EXPONENT_MASK);
  ...
  auto iExp = static_cast<int>((ll >> HB_MANTISSA_BITS) & HB_EXPONENT_MASK);
  ll &= HB_MANTISSA_MASK;
  if ((ll | iExp) != 0) {
...
double hb_get_ord_ieee754(const uint8_t *ptr)
{
  ...
  HB_U32 l1 = HB_GET_BE_UINT32(ptr + 4);
  HB_U32 l2 = HB_GET_BE_UINT32(ptr);
...
double hb_get_le_int64(const uint8_t *ptr)
{
  ...
  HB_U32 l1 = HB_GET_LE_UINT32(ptr);
  HB_U32 l2 = HB_GET_LE_INT32(ptr + 4);
  return ldexp(static_cast<double>(l2), 32) + static_cast<double>(l1);
}
...
void hb_put_le_uint64(const uint8_t *ptr, double d)
{
  ...
  HB_U32 l1 = static_cast<HB_U32>(d);
  HB_U32 l2 = static_cast<HB_U32>(d / 4294967296.0);
  HB_PUT_LE_UINT32(ptr, l1);
  HB_PUT_LE_UINT32(ptr + 4, l2);
}
