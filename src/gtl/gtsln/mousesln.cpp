...
{
  uint16_t usNewButtons = (s_usMouseState & ~s_usLastMouseState) & M_BUTTON_KEYMASK;
...
int hb_gt_sln_mouse_Inkey(int iEventMask, HB_BOOL fCheckNew)
{
  if (s_usMouseState != s_usLastMouseState) {
    if (s_usMouseState & M_CURSOR_MOVE) {
      ...
      uint16_t usKeyDiff = (s_usMouseState ^ s_usLastMouseState);
  ...
  if ((iEventMask & Mask) && (Evt.buttons & GpmBtn)) {                                                                 \
    ...
    if (GetGpmEvent(&Evt)) {
      ...
      if ((Evt.type & GPM_MOVE) && (iEventMask & INKEY_MOVE)) {
        ...
        else CHECK_BUTTON_DOWN(INKEY_RDOWN, GPM_B_RIGHT, K_RBUTTONDOWN,
                               K_RDBLCLK) else CHECK_BUTTON_DOWN(INKEY_MMIDDLE, GPM_B_MIDDLE, K_MBUTTONDOWN, K_MDBLCLK)
