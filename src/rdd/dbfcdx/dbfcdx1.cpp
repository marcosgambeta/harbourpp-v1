...
#define hb_cdxPageKeyBufPtr(P) ((reinterpret_cast<uint8_t *>(&(P)->node)) + (P)->TagParent->pIndex->uiPageLen)
#define hb_cdxPageIntKeyPool(P) ((reinterpret_cast<uint8_t *>(&(P)->node)) + CDX_INT_HEADSIZE)
#define hb_cdxPageExtKeyPool(P) ((reinterpret_cast<uint8_t *>(&(P)->node)) + CDX_EXT_HEADSIZE)
...
static void hb_cdxTagPoolFree(LPCDXTAG pTag, int32_t nPagesLeft);
...
static void hb_cdxIndexPoolFree(LPCDXINDEX pIndex, int32_t nPagesLeft);

// split Root Page
static int32_t hb_cdxPageRootSplit(LPCDXPAGE pPage);
...
static int16_t cdxStackSize = 0;
static int16_t cdxTmpStackSize = 0;
#endif
...
static LPCDXKEY hb_cdxKeyNew(uint16_t uiLen)
{
...
static LPCDXKEY hb_cdxKeyPut(LPCDXKEY pKey, const uint8_t *pbVal, uint16_t uiLen, HB_ULONG ulRec)
{
  if (pbVal == nullptr) {
    uiLen = 0;
  }
...
static LPCDXKEY hb_cdxKeyPutCL(LPCDXKEY pKey, const char *pText, HB_SIZE nLen, HB_ULONG ulRec, uint16_t uiKeyLen,
                               int32_t iMode)
{
  if (!pKey) {
  ...
  pKey->mode = static_cast<uint16_t>(iMode);
  pKey->rec = ulRec;
...
static int32_t hb_cdxValCompare(LPCDXTAG pTag, const uint8_t *val1, int32_t len1, const uint8_t *val2, int32_t len2, int32_t iMode)
{
...
static uint8_t hb_cdxItemTypeCmp(uint8_t bType)
{
...
static LPCDXKEY hb_cdxKeyPutItem(LPCDXKEY pKey, HB_ITEM *pItem, HB_ULONG ulRec, LPCDXTAG pTag, int32_t iMode)
{
  ...
  case 'C': {
    ...
    pKey = hb_cdxKeyPutCL(pKey, pText, nLen, ulRec, static_cast<uint16_t>(nDestLen), iMode);
    if (pFree) {
      hb_xfree(pFree);
    }
    return pKey;
  ...
  case 'T':
    ...
    break;
  case 'L':
    *buf = static_cast<uint8_t>(pItem->getL() ? 'T' : 'F');
    nLen = 1;
    break;
  default:
    ptr = nullptr;
  ...
  pKey = hb_cdxKeyPut(pKey, ptr, static_cast<uint16_t>(nLen), ulRec);
  pKey->mode = static_cast<uint16_t>(iMode);
...
static void hb_cdxTagClearScope(LPCDXTAG pTag, uint16_t nScope)
{
#if 0
...
static HB_ULONG hb_cdxIndexGetAvailPage(LPCDXINDEX pIndex, bool fHeader)
{
  ...
  if (pIndex->freePage != 0 && pIndex->freePage != CDX_DUMMYNODE && !fHeader) {
    ...
    if (fHeader) {
      ...
      auto byPageBuf = static_cast<uint8_t *>(hb_xgrabz(nSize));
...
static void hb_cdxIndexFlushAvailPage(LPCDXINDEX pIndex)
{
  ...
  ulPage = pIndex->freePage;
  if (pLst && pLst->fStat) {
    auto byPageBuf = static_cast<uint8_t *>(hb_xgrabz(pIndex->uiPageLen));
...
// write index page
static void hb_cdxIndexPageWrite(LPCDXINDEX pIndex, HB_ULONG ulPage, const uint8_t *pBuffer, HB_SIZE nSize)
{
  if (pIndex->fReadonly) {
...
// read index page
static void hb_cdxIndexPageRead(LPCDXINDEX pIndex, HB_ULONG ulPage, uint8_t *pBuffer, HB_SIZE nSize)
{
...
static uint8_t *hb_cdxPageGetKeyVal(LPCDXPAGE pPage, int32_t iKey)
{
#ifdef HB_CDX_DBGCODE
  ...
  if (pPage->pKeyBuf) {
    ...
    if (pPage->bufKeyNum == 0) {
      pPage->bufKeyPos = static_cast<int16_t>(pPage->TagParent->pIndex->uiPageLen - CDX_EXT_HEADSIZE);
      pPage->bufKeyLen = static_cast<int16_t>(iLen);
    }
    while (pPage->bufKeyNum <= iKey) {
      int32_t iPos, iTmp, iTrl, iDup;
      ...
      if ((iTmp = iLen - iDup - iTrl) > 0) {
        pPage->bufKeyPos -= static_cast<int16_t>(iTmp);
      ...
      pPage->bufKeyLen = static_cast<int16_t>(iLen - iTrl);
      pPage->bufKeyNum++;
#if 0
...
static HB_ULONG hb_cdxPageGetKeyRec(LPCDXPAGE pPage, int32_t iKey)
{
#ifdef HB_CDX_DBGCODE
  if (iKey < 0 || iKey >= pPage->iKeys) {
...
#endif
  if (pPage->pKeyBuf) {
    uint8_t *ptr = &pPage->pKeyBuf[(iKey + 1) * (pPage->TagParent->uiLen + 8) - 8];
    return HB_GET_LE_UINT32(ptr);
  } else if (pPage->PageType & CDX_NODE_LEAF) {
    uint8_t *ptr = &hb_cdxPageExtKeyPool(pPage)[iKey * pPage->ReqByte];
    return HB_GET_LE_UINT32(ptr) & pPage->RNMask;
  } else {
    uint8_t *ptr = &hb_cdxPageIntKeyPool(pPage)[(iKey + 1) * (pPage->TagParent->uiLen + 8) - 8];
    return HB_GET_BE_UINT32(ptr);
  }
}

// get child page number from interior index page
static HB_ULONG hb_cdxPageGetKeyPage(LPCDXPAGE pPage, int32_t iKey)
{
  uint8_t *ptr;
...
static int32_t hb_cdxPageGetKeyTrl(LPCDXPAGE pPage, int32_t iKey)
{
#ifdef HB_CDX_DBGCODE_EXT
  if (iKey < 0 || iKey >= pPage->iKeys) {
...
#endif
  if (pPage->pKeyBuf) {
    uint8_t *ptr = &pPage->pKeyBuf[(iKey + 1) * (pPage->TagParent->uiLen + 8) - 2];
    return HB_GET_LE_UINT16(ptr);
  } else {
    uint8_t *ptr = &hb_cdxPageExtKeyPool(pPage)[(iKey + 1) * pPage->ReqByte - 4];
...
{
  if (pPage->iKeys > 1) {
    int32_t K, iLen = pPage->TagParent->uiLen;
    HB_ULONG ulRec, ulRecPrev;
    uint8_t *pbVal;
    auto pbValPrev = static_cast<uint8_t *>(hb_xgrab(iLen));
...
static void hb_cdxPageCheckDupTrl(LPCDXPAGE pPage, uint8_t *pKeyBuf, int32_t iKeys, bool fSpc)
{
  int32_t iNum = pPage->TagParent->uiLen, iPos, iDup, iTrl, iFree = pPage->TagParent->pIndex->uiPageLen - CDX_EXT_HEADSIZE;
  int32_t iLen = iNum + 8;
  uint8_t bTrail = pPage->TagParent->bTrail;
  auto bErr = false;
...
static void hb_cdxPageLeafDecode(LPCDXPAGE pPage, uint8_t *pKeyBuf);
static void hb_cdxPageCheckDupTrlRaw(LPCDXPAGE pPage)
{
  auto pKeyBuf = static_cast<uint8_t *>(hb_xgrab(pPage->iKeys * (pPage->TagParent->uiLen + 8)));
...
static void hb_cdxChkLeafRecord(const uint8_t *pSrc, HB_ULONG ulRec, int32_t iDup, int32_t iTrl, LPCDXPAGE pPage)
{
  int32_t iTmp = HB_GET_LE_UINT32(pSrc + pPage->ReqByte - 4) >> (32 - pPage->TCBits - pPage->DCBits);
  HB_ULONG ulRec2 = HB_GET_LE_UINT32(pSrc) & pPage->RNMask;
  int32_t iDup2 = iTmp & pPage->DCMask, iTrl2 = (iTmp >> pPage->DCBits) & pPage->TCMask;
...
static void hb_cdxSetLeafRecord(uint8_t *pDst, HB_ULONG ulRec, int32_t iDup, int32_t iTrl, int32_t iReq, int32_t iDCbits, int32_t iTCbits)
{
  int32_t iBits, iFrom;
  ...
  for (auto i = 0; i < iReq; i++, ulRec >>= 8) {
    pDst[i] = static_cast<uint8_t>(ulRec & 0xff);
    if (i >= iFrom) {
      pDst[i] |= static_cast<uint8_t>(iBits & 0xff);
      iBits >>= 8;
    }
  }
}
...
static void hb_cdxPageLeafEncode(LPCDXPAGE pPage, uint8_t *pKeyBuf, int32_t iKeys)
{
  int32_t iReq, iNum, iLen;
  uint8_t *pKeyPos, *pRecPos, *pSrc;
  ...
  pKeyPos = reinterpret_cast<uint8_t *>(&pPage->node.extNode) + pPage->TagParent->pIndex->uiPageLen;
  pSrc = &pKeyBuf[0];
  for (auto iKey = 0; iKey < iKeys; iKey++, pSrc += iLen, pRecPos += iReq) {
    int32_t iTrl, iDup, iTmp;
    HB_ULONG ulRec;
...
static void hb_cdxPageLeafDecode(LPCDXPAGE pPage, uint8_t *pKeyBuf)
{
  int32_t iBits, iReq, iLen = pPage->TagParent->uiLen;
  uint8_t *pDst, *pSrc, *pRec, bTrail = pPage->TagParent->bTrail;
  ...
  pSrc = reinterpret_cast<uint8_t *>(&pPage->node.extNode) + pPage->TagParent->pIndex->uiPageLen;
  iReq = pPage->ReqByte;
  for (auto iKey = 0; iKey < pPage->iKeys; iKey++, pRec += iReq) {
    int32_t iTmp, iDup, iTrl, iNew;
    HB_ULONG ulRec;
    uint8_t *pTmp;
...
{
  int32_t iLen = pPage->TagParent->uiLen;
  uint8_t bBits;
  ...
  pPage->DCMask = pPage->TCMask = static_cast<uint16_t>(HB_CDXBITMASK(bBits));
...
static void hb_cdxPageCalcLeafSpace(LPCDXPAGE pPage, uint8_t *pKeyBuf, int32_t iKeys)
{
  int32_t iNum = pPage->TagParent->uiLen, iSize;
  int32_t iLen = iNum + 8;
  uint8_t ReqByte;
  ...
  for (auto iKey = 0; iKey < iKeys; iKey++) {
    ...
    pPage->iFree -= static_cast<int16_t>(iSize);
    pPage->iKeys++;
  }
}

// remove key from page
static int32_t hb_cdxPageLeafDelKey(LPCDXPAGE pPage)
{
  int32_t iKey = pPage->iCurKey, iLen = pPage->TagParent->uiLen + 8, iSpc;
  int32_t iRet = 0;
...
#endif
  if (!pPage->pKeyBuf) {
    auto pKeyBuf = static_cast<uint8_t *>(hb_xgrab((pPage->iKeys) * iLen));
    hb_cdxPageLeafDecode(pPage, pKeyBuf);
  ...
  if (iKey < pPage->iKeys - 1) {
    int32_t iPos = (iKey + 2) * iLen - 4, iDup = 0, iDupNext;
    iDupNext = HB_GET_LE_UINT16(&pPage->pKeyBuf[iPos]);
    iSpc -= iDupNext;
    if (iKey > 0) {
      int32_t iPrev = (iKey - 1) * iLen, iNext = (iKey + 1) * iLen, iNum = pPage->TagParent->uiLen, iTrlNext, iDupCurr;
...
#else
      {
        int32_t iTrlPrev = HB_GET_LE_UINT16(&pPage->pKeyBuf[iPrev + iLen - 2]);
  ...
  pPage->iFree += static_cast<int16_t>(iSpc);
  if (--pPage->iKeys > iKey) {
...
static int32_t hb_cdxPageLeafAddKey(LPCDXPAGE pPage, LPCDXKEY pKey)
{
  int32_t iKey, iNum = pPage->TagParent->uiLen;
  int32_t iLen = iNum + 8, iSpc, iTrl, iDup, iMax, iPos;
  uint8_t bTrail = pPage->TagParent->bTrail;
  int32_t iRet = 0;
...
#endif
  if (!pPage->pKeyBuf) {
    auto pKeyBuf = static_cast<uint8_t *>(hb_xgrab((pPage->iKeys + 1) * iLen));
    hb_cdxPageLeafDecode(pPage, pKeyBuf);
    pPage->pKeyBuf = pKeyBuf;
  } else {
    pPage->pKeyBuf = static_cast<uint8_t *>(hb_xrealloc(pPage->pKeyBuf, (pPage->iKeys + 1) * iLen));
  }
...
static void hb_cdxPageIntSetKey(LPCDXPAGE pPage, int32_t iKey, bool fIns, uint8_t *pbVal, HB_ULONG ulRec, HB_ULONG ulPag)
{
  int32_t iLen = pPage->TagParent->uiLen;
  int32_t iPos = iKey * (iLen + 8);
  uint8_t *pKeyPool = hb_cdxPageIntKeyPool(pPage);
...
static void hb_cdxPageIntDelKey(LPCDXPAGE pPage, int32_t iKey)
{
  int32_t iLen = pPage->TagParent->uiLen + 8;
  uint8_t *pKeyPool = hb_cdxPageIntKeyPool(pPage);
...
static void hb_cdxPageLoad(LPCDXPAGE pPage)
{
  ...
  hb_cdxIndexPageRead(pPage->TagParent->pIndex, pPage->Page, reinterpret_cast<uint8_t *>(&pPage->node),
                      pPage->TagParent->pIndex->uiPageLen);
  pPage->PageType = static_cast<uint8_t>(HB_GET_LE_UINT16(pPage->node.intNode.attr));
  ...
  if ((pPage->PageType & CDX_NODE_LEAF) != 0) {
    ...
    if (pPage->DCBits <= 8) {
      ...
      pPage->DCMask = static_cast<uint16_t>(HB_CDXBITMASK(pPage->DCBits));
      pPage->TCMask = static_cast<uint16_t>(HB_CDXBITMASK(pPage->TCBits));
    }
...
#if 0
      if( !pPage->pKeyBuf ) {
         auto pKeyBuf = static_cast<uint8_t*>(hb_xgrab((pPage->iKeys + 1) * ( pPage->TagParent->uiLen + 6 )));
         hb_cdxPageLeafDecode(pPage, pKeyBuf);
         pPage->pKeyBuf = pKeyBuf;
      }
#endif
...
static void hb_cdxPageStore(LPCDXPAGE pPage)
{
  ...
  if ((pPage->PageType & CDX_NODE_LEAF) != 0) {
    ...
    if (pPage->DCBits <= 8) {
      pPage->node.extNode.dupMask = static_cast<uint8_t>(pPage->DCMask);
      pPage->node.extNode.trlMask = static_cast<uint8_t>(pPage->TCMask);
    } else {
  ...
  hb_cdxIndexPageWrite(pPage->TagParent->pIndex, pPage->Page, reinterpret_cast<const uint8_t *>(&pPage->node),
                       pPage->TagParent->pIndex->uiPageLen);
#ifdef HB_CDX_DBGCODE_EXT
  hb_cdxPageCheckKeys(pPage);
#endif
...
static int32_t hb_cdxPageKeyLeafBalance(LPCDXPAGE pPage, int32_t iChildRet)
{
  LPCDXPAGE childs[CDX_BALANCE_LEAFPAGES + 2], lpTmpPage;
  int32_t iChKeys[CDX_BALANCE_LEAFPAGES + 2], iChFree[CDX_BALANCE_LEAFPAGES + 2];
  int32_t iFirstKey, iBlncKeys = CDX_BALANCE_LEAFPAGES;
  int32_t iLen = pPage->TagParent->uiLen + 8, iKeys = 0, iFree = 0, iSkip = 0, iBufSize = 0;
  uint8_t *pKeyPool = nullptr, *pPtr;
  auto fIns = false;
  int32_t iRet = 0, iDup, iMax, i;
  ...
  if (iKeys > 0) {
    iBufSize = iKeys;
    pPtr = pKeyPool = static_cast<uint8_t *>(hb_xgrab(iBufSize * iLen));
      ...
      if (!fIns && lpTmpPage != nullptr) {
        ...
        if (iSize < 0) {
          ...
          if (iBufSize >= iKeys + lpTmpPage->iKeys) {
            ...
            auto pTmp = static_cast<uint8_t *>(hb_xgrab(iBufSize * iLen));
...
static int32_t hb_cdxPageKeyIntBalance(LPCDXPAGE pPage, int32_t iChildRet)
{
  LPCDXPAGE childs[CDX_BALANCE_INTPAGES + 2], lpTmpPage;
  int32_t iFirstKey, iBlncKeys = CDX_BALANCE_INTPAGES;
  int32_t iLen = pPage->TagParent->uiLen + 8, iKeys = 0, iNeedKeys, iMin = pPage->TagParent->MaxKeys, iMax = 0, iDiv;
  uint8_t *pKeyPool = nullptr, *pPtr;
  ...
  if (iKeys > 0) {
    pPtr = pKeyPool = static_cast<uint8_t *>(hb_xgrab(iKeys * iLen));
    for (i = 0; i < iBlncKeys; i++) {
      if (childs[i]->iKeys > 0) {
  ...
  if (iKeys > 0) {
    ...
    pPtr = pKeyPool;
    for (i = 0; i < iBlncKeys; i++) {
      int32_t iNodeKeys = (fForce && i == 1) ? 1 : ((iKeys + iBlncKeys - i - 1) / (iBlncKeys - i));
#ifdef HB_CDX_DBGCODE
...
static int32_t hb_cdxPageBalance(LPCDXPAGE pPage, int32_t iChildRet)
{
  int32_t iRet = 0;
...
static int32_t hb_cdxPageKeyRemove(LPCDXPAGE pPage)
{
  int32_t iChildRet;
...
static int32_t hb_cdxPageKeyInsert(LPCDXPAGE pPage, LPCDXKEY pKey)
{
  int32_t iChildRet;
...
static void hb_cdxTagHeaderStore(LPCDXTAG pTag)
{
  ...
  uiKeyLen = pTag->KeyExpr == nullptr ? 0 : static_cast<uint16_t>(strlen(pTag->KeyExpr));
  uiForLen = pTag->ForExpr == nullptr ? 0 : static_cast<uint16_t>(strlen(pTag->ForExpr));
  ...
  hb_cdxIndexPageWrite(pTag->pIndex, pTag->TagBlock, reinterpret_cast<const uint8_t *>(&tagHeader), sizeof(tagHeader));
}
...
static bool hb_cdxSetPageSize(LPCDXINDEX pIndex, bool fLargeFile, uint16_t uiPageSize, uint16_t uiHeaderSize)
{
...
{
  CDXTAGHEADER tagHeader;
  uint16_t uiForPos, uiForLen, uiKeyPos, uiKeyLen;
  HB_ULONG ulRecNo;

  // read the page from a file
  hb_cdxIndexPageRead(pTag->pIndex, pTag->TagBlock, reinterpret_cast<uint8_t *>(&tagHeader), sizeof(tagHeader));
...
static int32_t hb_cdxPageSeekKey(LPCDXPAGE pPage, LPCDXKEY pKey, HB_ULONG ulKeyRec)
{
  int32_t l, r, n, k;
...
static void hb_cdxTagKeyRead(LPCDXTAG pTag, uint8_t bTypRead)
{
  auto fAfter = false;
  auto fBof = false;
  auto fEof = false;
...
static void hb_cdxIndexReindex(LPCDXINDEX pIndex)
{
  ...
  while (pTagList) {
    pTag = pTagList;
    hb_cdxIndexAddTag(pIndex, pTag->szName, pTag->KeyExpr, pTag->pKeyItem, static_cast<uint8_t>(pTag->uiType),
...
{
  uint16_t uiPageSize = DBFAREA_DATA(&pIndex->pArea->dbfarea)->uiIndexPageSize;
  auto fLargeFile = false;
...
static void hb_cdxOrdListClear(CDXAREAP pArea, bool fAll, LPCDXINDEX pKeepInd)
{
  ...
  if (pArea->lpIndexes) {
    ...
    if (!fAll) {
      ...
      auto pFileNameDbf = hb_fsFNameSplit(pArea->dbfarea.szDataFileName);
      auto pFileNameCdx = hb_fsFNameSplit(pArea->lpIndexes->szFileName);
...
static LPCDXINDEX hb_cdxFindBag(CDXAREAP pArea, const char *szBagName)
{
  ...
  auto pFileName = hb_fsFNameSplit(szBagName);
  pIndex = pArea->lpIndexes;
  while (pIndex) {
    auto pIndexName = hb_fsFNameSplit(pIndex->szFileName);
...
// get Tag by number
static LPCDXTAG hb_cdxGetTagByNumber(CDXAREAP pArea, uint16_t uiTag)
{
  LPCDXTAG pTag = nullptr;
...
// get Tag number
static uint16_t hb_cdxGetTagNumber(CDXAREAP pArea, LPCDXTAG pFindTag)
{
  uint16_t uiTag = 0;
  LPCDXTAG pTag = nullptr;
  LPCDXINDEX pIndex = pArea->lpIndexes;
...
static LPCDXTAG hb_cdxFindTag(CDXAREAP pArea, HB_ITEM *pTagItem, HB_ITEM *pBagItem, uint16_t *puiTag)
{
  ...
  if (puiTag) {
    if (!pTag) {
      ...
      *puiTag = static_cast<uint16_t>(iTag);
    }
  }
...
static bool hb_cdxDBOISkipWild(CDXAREAP pArea, LPCDXTAG pTag, bool fForward, HB_ITEM *pWildItm)
{
  ...
  if (iFixed && !pTag->TagEOF && pTag->CurKey->rec != 0 &&
      hb_cdxValCompare(pTag, reinterpret_cast<const uint8_t *>(szPattern), iFixed, pTag->CurKey->val, iFixed,
                       CDX_CMP_PREFIX) == -iStop) {
    LPCDXKEY pKey;

    pKey = hb_cdxKeyPut(nullptr, reinterpret_cast<const uint8_t *>(szPattern), static_cast<uint16_t>(iFixed),
  ...
  if (fForward) {
    ...
    while (!pTag->TagEOF) {
      ...
      if (iFixed && hb_cdxValCompare(pTag, reinterpret_cast<const uint8_t *>(szPattern), iFixed, pTag->CurKey->val,
...
static HB_ERRCODE hb_cdxSkip(CDXAREAP pArea, HB_LONG lToSkip)
{
  ...
  if (pTag != nullptr) {
    if (ulPos &&
        (pTag->logKeyPos != ulPos || pTag->logKeyRec != ulRec || (pTag->curKeyState & CDX_CURKEY_LOGPOS) == 0)) {
      ulPos = 0;
    }
...
static HB_ERRCODE hb_cdxStructSize(CDXAREAP pArea, uint16_t *uiSize)
{
#if 0
...
static HB_ERRCODE hb_cdxOrderInfo(CDXAREAP pArea, uint16_t uiIndex, LPDBORDERINFO pInfo)
{
#if 0
  ...
  case DBOI_BAGNAME:
    if (pTag != nullptr) {
      auto pFileName = hb_fsFNameSplit(pTag->pIndex->szFileName);
...
static int32_t hb_cdxQuickSortCompare(LPCDXSORTINFO pSort, uint8_t *pKey1, uint8_t *pKey2)
{
  int32_t i, iLen = pSort->keyLen;
...
static bool hb_cdxQSort(LPCDXSORTINFO pSort, uint8_t *pSrc, uint8_t *pBuf, HB_LONG lKeys)
{
  if (lKeys > 1) {
...
static void hb_cdxSortAddNodeKey(LPCDXSORTINFO pSort, int32_t iLevel, uint8_t *pKeyVal, HB_ULONG ulRec, HB_ULONG ulPage)
{
  LPCDXPAGE pPage;
  auto fNew = false;
  int32_t iLen = pSort->keyLen, iDup = 0, iTrl = 0, iTmp, iPos;
  uint8_t *pTmp;
...
#ifdef HB_CDX_PACKTRAIL
      int32_t iMax = iLen - iTrl;
#else
      int32_t iMax = iLen - HB_MAX(iTrl, pSort->iLastTrl);
#endif
  ...
  if (iLevel == 0) {
    ...
    pPage->iFree -= static_cast<int16_t>(iTmp + pPage->ReqByte);
    pPage->iKeys++;
#ifdef HB_CDX_DBGCODE_EXT
    hb_cdxPageCheckDupTrlRaw(pSort->NodeList[iLevel]);
#endif
...
static void hb_cdxSortGetPageKey(LPCDXSORTINFO pSort, HB_ULONG ulPage, uint8_t **pKeyVal, HB_ULONG *pulRec)
{
...
static bool hb_cdxSortKeyGet(LPCDXSORTINFO pSort, uint8_t **pKeyVal, HB_ULONG *pulRec)
{
...
static void hb_cdxSortKeyAdd(LPCDXSORTINFO pSort, HB_ULONG ulRec, const uint8_t *pKeyVal, int32_t iKeyLen)
{
  int32_t iLen = pSort->keyLen;
  uint8_t *pDst;
...
static LPCDXSORTINFO hb_cdxSortNew(LPCDXTAG pTag, HB_ULONG ulRecCount)
{
  ...
  pSort->pLastKey = static_cast<uint8_t *>(hb_xgrabz(iLen + 1));
...
#ifdef HB_CDX_DBGCODE_EXT
    if (ulKey != 0) {
      int32_t i = hb_cdxValCompare(pSort->pTag, pSort->pLastKey, iLen, pKeyVal, iLen, CDX_CMP_EXACT);
      if (i == 0) {
...
static void hb_cdxTagDoIndex(LPCDXTAG pTag, bool fReindex)
{
  ...
  if (ulRecCount == 0) {
    ...
    uint16_t uiSaveTag = pArea->uiTag;
    ...
    if (fDirectRead) {
      pSort->pRecBuff = static_cast<uint8_t *>(hb_xgrab(pArea->dbfarea.uiRecordLen * iRecBufSize));
    }
    ...
    while (!pArea->dbfarea.area.fEof) {
      ...
      if (ulRecNo <= ulRecCount && (pForItem == nullptr || hb_cdxEvalCond(pArea, pForItem, false))) {
        ...
        case Harbour::Item::LOGICAL:
          cTemp[0] = static_cast<uint8_t>(pItem->getL() ? 'T' : 'F');
