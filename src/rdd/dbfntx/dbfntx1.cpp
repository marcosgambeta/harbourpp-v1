...
static bool hb_ntxTagKeyFind(LPTAGINFO pTag, LPKEYINFO pKey, HB_USHORT uiLen)
{
  ...
  else if (pKey->Xtra == NTX_MAX_REC_NUM)
  { /* for seek last */
    fNext = fPrev = true;
  }
  else if (pKey->Xtra != NTX_IGNORE_REC_NUM)
  { /* for key del and current key */
    ulRecNo = pKey->Xtra;
  }
  /* else -> normal seek */
  ...
  if (ulRecNo && !pTag->fSortRec)
  { /* small hack - should speedup in some cases */
  ...
  else
  {
    ...
    while (errCode == Harbour::SUCCESS && !pArea->dbfarea.area.fEof)
    {
      ...
      if (ulRecNo <= ulRecCount && (pForItem == nullptr || hb_ntxEvalCond(pArea, pForItem, false)))
      {
        ...
        case Harbour::Item::STRING:
        case Harbour::Item::MEMO:
          hb_ntxSortKeyAdd(pSort, pArea->dbfarea.ulRecNo, hb_itemGetCPtr(pItem),
...
static HB_ERRCODE hb_ntxOrderInfo(NTXAREAP pArea, HB_USHORT uiIndex, LPDBORDERINFO pInfo)
{
  ...
  if (pTag != nullptr)
  {
    ...
    case DBOI_READLOCK:
      ...
      {
        pInfo->itmResult =
            hb_itemPutL(pInfo->itmResult, hb_itemGetL(pInfo->itmNewVal) ? hb_ntxIndexLockRead(pTag->pIndex)
                                                                        : hb_ntxIndexUnLockRead(pTag->pIndex));
      }
      else
      {
    ...
    case DBOI_WRITELOCK:
      ...
      {
        pInfo->itmResult =
            hb_itemPutL(pInfo->itmResult, hb_itemGetL(pInfo->itmNewVal) ? hb_ntxIndexLockWrite(pTag->pIndex, true)
                                                                        : hb_ntxIndexUnLockWrite(pTag->pIndex));
      }
      else
      {
