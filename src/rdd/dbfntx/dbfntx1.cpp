...
//#define HB_SHORTCAST int16_t (deprecated)
 ...
 */
static char *hb_ntxNumToStr(HB_ITEM *pItem, char *szBuffer, uint16_t length, uint16_t dec)
{
  char *ptr = szBuffer;
 ...
 */
static HB_ITEM *hb_ntxStrToNum(HB_ITEM *pItem, const char *szKeyVal, uint16_t length, uint16_t dec)
{
 ...
 */
static LPKEYINFO hb_ntxKeyNew(LPKEYINFO pKeyFrom, int32_t keylen)
{
 ...
 */
static LPKEYINFO hb_ntxKeyCopy(LPKEYINFO pKeyDest, LPKEYINFO pKey, int32_t keylen)
{
  if (!pKeyDest) {
 ...
 */
static uint8_t hb_ntxItemType(HB_ITEM *pItem)
{
 ...
 */
static uint8_t hb_ntxItemTypeCmp(uint8_t bType)
{
 ...
 */
static LPKEYINFO hb_ntxKeyPutItem(LPKEYINFO pKey, HB_ITEM *pItem, HB_ULONG ulRecNo, LPTAGINFO pTag, bool fTrans,
                                  uint16_t *puiLen)
{
  HB_SIZE len;
 ...
 */
static HB_ITEM *hb_ntxKeyGetItem(HB_ITEM *pItem, LPKEYINFO pKey, LPTAGINFO pTag, bool fTrans)
{
  if (pKey) {
    switch (pTag->KeyType) {
    case 'C':
 ...
 */
static bool hb_ntxEvalCond(NTXAREAP pArea, HB_ITEM *pCondItem, bool fSetWA)
{
  auto iCurrArea = 0;
  auto fRet = false;
 ...
 */
static bool hb_ntxEvalSeekCond(LPTAGINFO pTag, HB_ITEM *pCondItem)
{
  auto fRet = false;
  HB_ITEM *pKeyVal;
 ...
 */
static uint8_t hb_ntxGetKeyType(LPTAGINFO pTag)
{
  uint8_t bType;
 ...
 */
static int32_t hb_ntxValCompare(LPTAGINFO pTag, const char *val1, int32_t len1, const char *val2, int32_t len2, bool fExact)
{
...
static bool hb_ntxInTopScope(LPTAGINFO pTag, const char *key)
{
  ...
  if (pScope->scopeKeyLen) {
    int32_t i = hb_ntxValCompare(pTag, pScope->scopeKey->key, pScope->scopeKeyLen, key, pTag->KeyLength, false);
 ...
 */
static void hb_ntxTagSetScope(LPTAGINFO pTag, uint16_t nScope, HB_ITEM *pItem)
{
  NTXAREAP pArea = pTag->pIndex->pArea;
  HB_ITEM *pScopeVal;
 ...
 */
static void hb_ntxTagGetScope(LPTAGINFO pTag, uint16_t nScope, HB_ITEM *pItem)
{
  NTXAREAP pArea = pTag->pIndex->pArea;
  PHB_NTXSCOPE pScope;
...
static void hb_ntxPageCheckKeys(LPPAGEINFO pPage, LPTAGINFO pTag, int32_t iPos, int32_t iType)
{
  int32_t i;
 ...
 */
static bool hb_ntxBlockRead(LPNTXINDEX pIndex, HB_ULONG ulBlock, void *buffer, int32_t iSize)
{
 ...
 */
static bool hb_ntxBlockWrite(LPNTXINDEX pIndex, HB_ULONG ulBlock, const void *buffer, int32_t iSize)
{
  if (!pIndex->lockWrite) {
...
static LPTAGINFO hb_ntxTagNew(LPNTXINDEX pIndex, const char *szTagName, bool fTagName, const char *szKeyExpr,
                              HB_ITEM *pKeyExpr, uint8_t bKeyType, uint16_t uiKeyLen, uint16_t uiKeyDec,
                              const char *szForExpr, HB_ITEM *pForExpr, bool fAscendKey, bool fUnique, bool fCustom,
                              bool fSortRec)
{
 ...
 */
static LPTAGINFO hb_ntxTagLoad(LPNTXINDEX pIndex, HB_ULONG ulBlock, const char *szTagName, uint8_t *buffer)
{
...
static void hb_ntxIndexTagAdd(LPNTXINDEX pIndex, LPTAGINFO pTag)
{
  ...
  int32_t iTags = HB_GET_LE_UINT16(lpCTX->ntags), iLen, i;
...
{
  LPCTXHEADER lpCTX = reinterpret_cast<LPCTXHEADER>(pIndex->HeaderBuff);
  int32_t iTags = HB_GET_LE_UINT16(lpCTX->ntags);
  LPCTXTAGITEM pTagItem = lpCTX->tags;
...
static HB_ERRCODE hb_ntxTagHeaderSave(LPTAGINFO pTag)
{
  LPNTXINDEX pIndex = pTag->pIndex;
  NTXHEADER Header;
  int32_t iSize = NTX_ROOTHEAD_HEADSIZE, type, version = 0, iLen;
  HB_ULONG next = 0;
  ...
  if (pIndex->Update) {
    memset(reinterpret_cast<uint8_t *>(&Header) + NTX_ROOTHEAD_HEADSIZE, 0, sizeof(NTXHEADER) - NTX_ROOTHEAD_HEADSIZE);
...
static HB_ERRCODE hb_ntxIndexHeaderSave(LPNTXINDEX pIndex)
{
  if (pIndex->Compound) {
    LPCTXHEADER lpCTX = reinterpret_cast<LPCTXHEADER>(pIndex->HeaderBuff);
    int32_t iSize = pIndex->Update ? NTXBLOCKSIZE : 16;
    uint16_t type;
...
static HB_ERRCODE hb_ntxIndexLoad(LPNTXINDEX pIndex, const char *szTagName)
{
  ...
  if (!pIndex->fValidHeader) {
    if (!pIndex->HeaderBuff) {
      pIndex->HeaderBuff = static_cast<uint8_t *>(hb_xgrab(NTXBLOCKSIZE));
    }
  ...
  if (pIndex->Compound) {
    uint8_t tagbuffer[NTXBLOCKSIZE];
    LPCTXHEADER lpCTX = reinterpret_cast<LPCTXHEADER>(pIndex->HeaderBuff);
    LPCTXTAGITEM pTagItem = lpCTX->tags;
    int32_t iTags;
 ...
 */
static int32_t hb_ntxPageKeyFind(LPTAGINFO pTag, LPPAGEINFO pPage, const char *key, int16_t keylen, bool fNext,
                             HB_ULONG ulRecNo, bool *fStop)
{
  int32_t iLast = -1, iBegin = 0, iEnd = pPage->uiKeys - 1;
 ...
 */
static bool hb_ntxPageFindRecNo(LPPAGEINFO pPage, int32_t *iStart, HB_ULONG ulRecno)
{
  int32_t iKey = *iStart;
...
static bool hb_ntxTagKeyFind(LPTAGINFO pTag, LPKEYINFO pKey, uint16_t uiLen)
{
  ...
  do {
    ...
    hb_ntxTagSetPageStack(pTag, pPage->Page, static_cast<uint16_t>(iKey));
  ...
  if (ulRecNo && !pTag->fSortRec) { /* small hack - should speedup in some cases */
    if (hb_ntxPageFindRecNo(pPage, &iKey, ulRecNo)) {
      pTag->stack[pTag->stackLevel - 1].ikey = static_cast<int16_t>(iKey);
    }
  }

  hb_ntxPageGetKey(pPage, static_cast<uint16_t>(iKey), pTag->CurKeyInfo, pTag->KeyLength);
  hb_ntxPageRelease(pTag, pPage);
                               ...
                               LPPAGEINFO pLast)
{
  ...
  int32_t i, j, iMove = ((pFirst->uiKeys + pLast->uiKeys + 1) >> 1) - pFirst->uiKeys;
  ...
  if (iMove > 0) {
    ...
    i++;
    pLast->uiKeys -= static_cast<uint16_t>(i);
    /* TODO?: update to keep last key pointer fixed */
    ...
    pLast->uiKeys -= static_cast<uint16_t>(iMove);
    pFirst->uiKeys--;
...
static bool hb_ntxTagKeyAdd(LPTAGINFO pTag, LPKEYINFO pKey)
{
  ...
  while (iLevel >= 0 && pKey) {
    ...
    if (pPage->uiKeys < pTag->MaxKeys) {
      hb_ntxPageKeyAdd(pTag, pPage, static_cast<uint16_t>(iKey), pKey->Tag, pKey->Xtra, pKey->key);
      pKey = nullptr;
    } else {
      pTag->stackLevel = 0;
      ...
      if (iLevel > 0) {
        ...
        if (uiFirst != uiLast) {
          ...
          if (uiFirst == uiBaseKey) {
            ...
            uiBaseKey = static_cast<uint16_t>(iKey);
          } else {
            pLast = pPage;
            ...
            uiBaseKey = pFirst->uiKeys + static_cast<uint16_t>(iKey) + 1;
          }
...
#endif
      pKey = hb_ntxPageSplit(pTag, pPage, pKey, static_cast<uint16_t>(iKey));
      if (pNewKey) {
        hb_ntxKeyFree(pNewKey);
      }
      pNewKey = pKey;
...
static bool hb_ntxTagKeyDel(LPTAGINFO pTag, LPKEYINFO pKey)
{
  ...
  hb_ntxPageKeyDel(pPage, static_cast<uint16_t>(iKey));
 ...
 */
static int32_t hb_ntxFindTagByName(LPNTXINDEX pIndex, const char *szTag)
{
  for (auto i = 0; i < pIndex->iTags; i++) {
 ...
 */
static LPTAGINFO hb_ntxFindTag(NTXAREAP pArea, HB_ITEM *pTagItem, HB_ITEM *pBagItem)
{
  LPNTXINDEX pIndex;
  auto fBag = false;
 ...
 */
static int32_t hb_ntxFindTagNum(NTXAREAP pArea, LPTAGINFO pTag)
{
 ...
 */
static int32_t hb_ntxTagCount(NTXAREAP pArea)
{
  LPNTXINDEX pIndex = pArea->lpIndexes;
  int32_t i = 0;
  ...
  if (hb_ntxTagLockRead(pTag)) {
    ...
    if (hb_ntxCurKeyRefresh(pTag)) {
      if (pTag->top.scopeKeyLen || pTag->bottom.scopeKeyLen || pTag->pIndex->pArea->dbfarea.area.dbfi.fFilter) {
        ...
        int32_t iLevel = pTag->stackLevel, iKey, iFirst = 1;
 ...
 */
static bool hb_ntxOrdSkipEval(LPTAGINFO pTag, bool fForward, HB_ITEM *pEval)
{
#if 0
 ...
 * skip while code block doesn't return true
 */
static bool hb_ntxOrdSkipWild(LPTAGINFO pTag, bool fForward, HB_ITEM *pWildItm)
{
#if 0
  ...
  if (hb_ntxTagLockRead(pTag)) {
    ...
    if (hb_ntxCurKeyRefresh(pTag)) {
      ...
      if (iFixed && hb_ntxValCompare(pTag, szPattern, iFixed, pTag->CurKeyInfo->key, iFixed, false) == -iStop) {
        ...
        if (!hb_ntxTagKeyFind(pTag, pKey, static_cast<uint16_t>(iFixed))) {
          if (fForward) {
            pTag->TagEOF = true;
          } else {
            pTag->TagBOF = true;
 ...
 */
static bool hb_ntxOrdSkipRegEx(LPTAGINFO pTag, bool fForward, HB_ITEM *pRegExItm)
{
#if 0
 ...
 */
static bool hb_ntxOrdKeyAdd(LPTAGINFO pTag, HB_ITEM *pItem)
{
  NTXAREAP pArea = pTag->pIndex->pArea;
  auto fResult = false;
  LPKEYINFO pKey;
 ...
 * user key value is not implemented
 */
static bool hb_ntxOrdKeyDel(LPTAGINFO pTag, HB_ITEM *pItem)
{
  NTXAREAP pArea = pTag->pIndex->pArea;
  auto fResult = false;
  LPKEYINFO pKey = nullptr;
 ...
 */
static HB_ULONG hb_ntxOrdScopeEval(LPTAGINFO pTag, HB_EVALSCOPE_FUNC pFunc, void *pParam, HB_ITEM *pItemLo,
                                   HB_ITEM *pItemHi)
{
  ...
  if (hb_ntxTagLockRead(pTag)) {
    hb_ntxTagGoTop(pTag);
    while (!pTag->TagEOF) {
      pFunc(pTag->CurKeyInfo->Xtra, reinterpret_cast<uint8_t *>(pTag->CurKeyInfo->key), ulLen, pParam);
      ulCount++;
      hb_ntxTagSkipNext(pTag);
    }
    hb_ntxTagUnLockRead(pTag);
...
static int32_t hb_ntxQuickSortCompare(LPNTXSORTINFO pSort, uint8_t *pKey1, uint8_t *pKey2)
{
...
static bool hb_ntxQSort(LPNTXSORTINFO pSort, uint8_t *pSrc, uint8_t *pBuf, HB_LONG lKeys)
{
  if (lKeys > 1) {
...
static void hb_ntxSortAddNodeKey(LPNTXSORTINFO pSort, uint8_t *pKeyVal, HB_ULONG ulRec)
{
  LPPAGEINFO pPage;
  HB_ULONG ulPage = 0;
  int32_t iLevel = 0;
...
static void hb_ntxSortGetPageKey(LPNTXSORTINFO pSort, HB_ULONG ulPage, uint8_t **pKeyVal, HB_ULONG *pulRec)
{
...
static bool hb_ntxSortKeyGet(LPNTXSORTINFO pSort, uint8_t **pKeyVal, HB_ULONG *pulRec)
{
...
static void hb_ntxSortKeyAdd(LPNTXSORTINFO pSort, HB_ULONG ulRec, const char *pKeyVal, int32_t iKeyLen)
{
  int32_t iLen = pSort->keyLen;
  uint8_t *pDst;
...
#ifdef HB_NTX_DEBUG_EXT
    if (ulKey != 0) {
      int32_t i = hb_ntxValCompare(pTag, static_cast<const char *>(pSort->pLastKey), iLen,
...
static HB_ERRCODE hb_ntxTagCreate(LPTAGINFO pTag, bool fReindex)
{
  ...
  if (ulRecCount == 0) {
    ...
    pSort->pBuffIO = static_cast<uint8_t *>(hb_xgrab(pSort->ulSizeIO * NTXBLOCKSIZE));
...
static HB_ERRCODE hb_ntxSeek(NTXAREAP pArea, HB_BOOL fSoftSeek, HB_ITEM *pItem, HB_BOOL fFindLast)
{
#if 0
...
static HB_ERRCODE hb_ntxRddInfo(LPRDDNODE pRDD, uint16_t uiIndex, HB_ULONG ulConnect, HB_ITEM *pItem)
{
#if 0
