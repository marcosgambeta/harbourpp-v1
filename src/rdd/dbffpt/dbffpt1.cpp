...
static uint16_t s_uiRddIdBLOB = static_cast<uint16_t>(-1);
static uint16_t s_uiRddIdFPT = static_cast<uint16_t>(-1);
...
static HB_ERRCODE hb_memoErrorRT(FPTAREAP pArea, HB_ERRCODE uiGenCode, HB_ERRCODE uiSubCode, const char *szFileName,
                                 HB_ERRCODE uiOsCode, uint16_t uiFlags)
{
...
static const char *hb_memoDefaultFileExt(int32_t iType, uint16_t uiRdd)
{
...
static int32_t hb_memoDefaultType(LPRDDNODE pRDD, HB_ULONG ulConnect)
{
  int32_t iType = DB_MEMO_FPT;
...
static void hb_fptPackGCitems(LPMEMOGCTABLE pGCtable)
{
  ...
  pGCtable->usItems = static_cast<uint16_t>(j);
}

// Write proper header into modified GC free memo blocks.
static HB_ERRCODE hb_fptWriteGCitems(FPTAREAP pArea, LPMEMOGCTABLE pGCtable, uint16_t usItem)
{
  FPTBLOCK fptBlock;
  HB_ERRCODE errCode = Harbour::SUCCESS;
#if 0
   int32_t iStart, iStop;
#endif
                                    ...
                                    bool fRaw)
{
  ...
  if (pArea->bMemoType == DB_MEMO_DBT) {
    ...
    if (ulByteSize == 0) {
      ...
      if (hb_fileReadAt(pArea->pMemoFile, &fptBlock, sizeof(FPTBLOCK), FPT_BLOCK_OFFSET(ulOffset)) ==
          sizeof(FPTBLOCK)) {
  ...
  if (nRead >= 512 && nRead != static_cast<HB_SIZE>(FS_ERROR)) {
    ...
    if (pArea->uiMemoVersion == DB_MEMOVER_SIX || pArea->bMemoType == DB_MEMO_SMT) {
      ...
      if (pGCtable->ulDirPage) {
        ...
        bPageBuf = static_cast<uint8_t *>(hb_xgrab(pGCtable->ulSize));
        if (hb_fileReadAt(pArea->pMemoFile, bPageBuf, pGCtable->ulSize, pGCtable->ulDirPage + sizeof(FPTBLOCK)) !=
            pGCtable->ulSize) {
          hb_xfree(bPageBuf);
          return EDBF_CORRUPT;
        }
        pGCtable->usMaxItem = static_cast<uint16_t>((pGCtable->ulSize - 2) >> 3);
...
static HB_ERRCODE hb_fptWriteGCdata(FPTAREAP pArea, LPMEMOGCTABLE pGCtable)
{
  ...
  if (pGCtable->bChanged > 0) {
    ...
    if (pGCtable->bType == DB_MEMOVER_SIX) {
      uint16_t usItems = HB_MIN(pGCtable->usItems, pGCtable->usMaxItem);
      ...
      for (i = j; i < pGCtable->usItems; i++) {
        HB_PUT_LE_UINT16(&pGCtable->fptHeader.reserved2[(i - j) * 6],
                         (static_cast<uint16_t>(pGCtable->pGCitems[i].ulSize)));
      ...
      if (pGCtable->ulDirPage && pGCtable->bChanged > 1) {
        ...
        auto bPageBuf = static_cast<uint8_t *>(hb_xgrab(pGCtable->ulSize));
        memset(bPageBuf, 0xAD, pGCtable->ulSize);
        HB_PUT_LE_UINT16(bPageBuf, (static_cast<uint16_t>(usItems) << 2) + 3);
        j = pGCtable->usItems - usItems;
        for (i = j; i < pGCtable->usItems; i++) {
        ...
        if (hb_fileWriteAt(pArea->pMemoFile, &fptBlock, sizeof(FPTBLOCK), pGCtable->ulDirPage) != sizeof(FPTBLOCK) ||
          ...
          for (i = j; i < pGCtable->usItems; i++) {
            HB_PUT_LE_UINT32(&bPageBuf[(i - j) * 8 + 2],
                             (static_cast<uint16_t>(pGCtable->pGCitems[i].ulSize) * pArea->ulMemoBlockSize));
...
static HB_ULONG hb_fptGetMemoLen(FPTAREAP pArea, uint16_t uiIndex)
{
#if 0
  ...
  if (hb_dbfGetMemoData(static_cast<DBFAREAP>(pArea), uiIndex - 1, &ulBlock, &ulSize, &ulType) == Harbour::SUCCESS) {
    if (ulBlock != 0) {
      if (ulSize == 0 && (pArea->bMemoType == DB_MEMO_DBT || pArea->bMemoType == DB_MEMO_FPT)) {
        ...
        if (pArea->bMemoType == DB_MEMO_DBT) {
          uint8_t pBlock[DBT_DEFBLOCKSIZE];
          HB_SIZE n;
...
static const char *hb_fptGetMemoType(FPTAREAP pArea, uint16_t uiIndex)
{
#if 0
...
static HB_ULONG hb_fptCountSMTItemLength(FPTAREAP pArea, HB_ITEM *pItem, HB_ULONG *pulArrayCount, int32_t iTrans)
{
...
static void hb_fptStoreSMTItem(FPTAREAP pArea, HB_ITEM *pItem, uint8_t **bBufPtr, int32_t iTrans)
{
  HB_ULONG ulLen, u;
  ...
  case Harbour::Item::DOUBLE: {
    ...
    *(*bBufPtr)++ = static_cast<uint8_t>(iWidth);
    *(*bBufPtr)++ = static_cast<uint8_t>(iDec);
...
static HB_ERRCODE hb_fptReadRawSMTItem(FPTAREAP pArea, HB_ITEM *pItem, HB_FOFFSET *pfOffset, int32_t iTrans)
{
  HB_ULONG ulLen, u;
  uint8_t buffer[10];
  char *pBuffer;
...
static HB_ERRCODE hb_fptReadSMTItem(FPTAREAP pArea, uint8_t **pbMemoBuf, uint8_t *bBufEnd, HB_ITEM *pItem, int32_t iTrans)
{
  HB_ERRCODE errCode = Harbour::SUCCESS;
...
static HB_ULONG hb_fptCountSixItemLength(FPTAREAP pArea, HB_ITEM *pItem, HB_ULONG *pulArrayCount, int32_t iTrans)
{
...
static HB_ULONG hb_fptStoreSixItem(FPTAREAP pArea, HB_ITEM *pItem, uint8_t **bBufPtr, int32_t iTrans)
{
  HB_ULONG ulLen, u, ulSize;
  int32_t iWidth, iDec;
...
static HB_ERRCODE hb_fptReadSixItem(FPTAREAP pArea, uint8_t **pbMemoBuf, uint8_t *bBufEnd, HB_ITEM *pItem, int32_t iTrans)
{
  HB_ULONG ulLen, u;
  HB_ERRCODE errCode = Harbour::SUCCESS;
  ...
  if (bBufEnd - (*pbMemoBuf) >= static_cast<HB_LONG>(ulLen)) {
    uint16_t usType = HB_GET_LE_UINT16(&(*pbMemoBuf)[0]);
    switch (usType) {
    case FPTIT_SIX_LNUM:
...
static HB_ULONG hb_fptCountFlexItemLength(FPTAREAP pArea, HB_ITEM *pItem, HB_ULONG *pulArrayCount, int32_t iTrans)
{
...
static void hb_fptStoreFlexItem(FPTAREAP pArea, HB_ITEM *pItem, uint8_t **bBufPtr, int32_t iTrans)
{
  HB_ULONG ulLen, u;
  int32_t iWidth, iDec;
  ...
  case Harbour::Item::ARRAY:
    ...
    *(*bBufPtr)++ = FPTIT_FLEXAR_ARAY;
    HB_PUT_LE_UINT16(*bBufPtr, static_cast<uint16_t>(ulLen));
    *bBufPtr += 2;
    for (u = 1; u <= ulLen; u++) {
  ...
  case Harbour::Item::LONG: {
    ...
    if (HB_LIM_INT8(iVal)) {
      ...
      *(*bBufPtr)++ = static_cast<uint8_t>(iVal);
      *(*bBufPtr)++ = static_cast<uint8_t>(iWidth);
...
static HB_ERRCODE hb_fptReadFlexItem(FPTAREAP pArea, uint8_t **pbMemoBuf, uint8_t *bBufEnd, HB_ITEM *pItem, bool bRoot,
                                     int32_t iTrans)
{
  uint8_t usType;
  HB_ULONG ulLen, i;
  ...
  case FPTIT_FLEXAR_CHAR2:
    if (bBufEnd - (*pbMemoBuf) >= 3) {
      int32_t iLen = (*pbMemoBuf)[1], iDec = (*pbMemoBuf)[2];
      if (iDec) {
        iLen -= iDec + 1;
  ...
  case FPTIT_FLEXAR_SHORT2:
    if (bBufEnd - (*pbMemoBuf) >= 4) {
      int32_t iLen = (*pbMemoBuf)[2], iDec = (*pbMemoBuf)[3];
      if (iDec) {
        iLen -= iDec + 1;
  ...
  case FPTIT_FLEXAR_USHORT:
    if (bBufEnd - (*pbMemoBuf) >= 2) {
      hb_itemPutNInt(pItem, static_cast<uint16_t>(HB_GET_LE_UINT16(*pbMemoBuf)));
      *pbMemoBuf += 2;
    } else {
      errCode = EDBF_CORRUPT;
    }
  ...
  case FPTIT_FLEXAR_USHORT1:
    if (bBufEnd - (*pbMemoBuf) >= 3) {
      hb_itemPutNIntLen(pItem, static_cast<uint16_t>(HB_GET_LE_UINT16(*pbMemoBuf)), (*pbMemoBuf)[2]);
      *pbMemoBuf += 3;
    } else {
      errCode = EDBF_CORRUPT;
    }
  ...
  case FPTIT_FLEXAR_USHORT2:
    if (bBufEnd - (*pbMemoBuf) >= 4) {
      ...
      if (iDec) {
        iLen -= iDec + 1;
        hb_itemPutNDLen(pItem, static_cast<uint16_t>(HB_GET_LE_UINT16(*pbMemoBuf)), iLen, iDec);
      } else {
        hb_itemPutNIntLen(pItem, static_cast<uint16_t>(HB_GET_LE_UINT16(*pbMemoBuf)), iLen);
      }
      *pbMemoBuf += 4;
    } else {
      errCode = EDBF_CORRUPT;
  ...
  case FPTIT_FLEXAR_LONG2:
    if (bBufEnd - (*pbMemoBuf) >= 6) {
      int32_t iLen = (*pbMemoBuf)[4], iDec = (*pbMemoBuf)[5];
      if (iDec) {
        iLen -= iDec + 1;
  ...
  case FPTIT_FLEXAR_DOUBLE2:
    if (bBufEnd - (*pbMemoBuf) >= 10) {
      int32_t iLen = (*pbMemoBuf)[0], iDec = (*pbMemoBuf)[1];
      if (iDec) {
        iLen -= iDec + 1;
      }
...
static HB_ERRCODE hb_fptCopyToRawFile(PHB_FILE pSrc, HB_FOFFSET from, PHB_FILE pDst, HB_FOFFSET size)
{
  ...
  if (size) {
    ...
    auto pBuffer = static_cast<uint8_t *>(hb_xgrab(nBufSize));
...
static HB_ERRCODE hb_fptReadRawBlock(FPTAREAP pArea, uint8_t *bBuffer, PHB_FILE pFile, HB_ULONG ulBlock,
                                     HB_ULONG ulSize)
{
  HB_ERRCODE errCode = Harbour::SUCCESS;
...
static HB_ERRCODE hb_fptReadBlobBlock(FPTAREAP pArea, HB_ITEM *pItem, PHB_FILE pFile, HB_ULONG ulBlock,
                                      uint16_t uiMode)
{
  HB_ULONG ulSize;
  uint8_t buffer[4];
  ...
  if (ulSize == 0) {
    ...
    uint8_t *bBuffer = static_cast<uint8_t *>(hb_xalloc(ulSize + 1));
...
static HB_ERRCODE hb_fptReadSMTBlock(FPTAREAP pArea, HB_ITEM *pItem, HB_ULONG ulBlock, HB_ULONG ulSize, int32_t iTrans)
{
  if (ulBlock == 0) {
    return EDBF_CORRUPT;
  }
  ...
  if (ulSize == 0) {
    ...
    HB_ERRCODE errCode;
    uint8_t *bBuffer = static_cast<uint8_t *>(hb_xalloc(ulSize)), *bMemoBuf;
...
static HB_ERRCODE hb_fptGetMemo(FPTAREAP pArea, uint16_t uiIndex, HB_ITEM *pItem, PHB_FILE pFile, HB_ULONG ulBlock,
                                HB_ULONG ulStart, HB_ULONG ulCount, int32_t iTrans)
{
#if 0
  ...
  if (ulBlock > 0) {
    ...
    if (ulSize != 0 && hb_fileReadAt(pArea->pMemoFile, pBuffer, ulSize, fOffset) != ulSize) {
      ...
      if (ulType == SMT_IT_CHAR) {
        ...
        bMemoBuf = reinterpret_cast<uint8_t *>(pBuffer);
        errCode = hb_fptReadSMTItem(pArea, &bMemoBuf, bMemoBuf + ulSize, pItem, iTrans);
      }
    } else {
      switch (ulType) {
...
// Write memo data.
static HB_ERRCODE hb_fptWriteMemo(FPTAREAP pArea, HB_ULONG ulBlock, HB_ULONG ulSize, const uint8_t *bBufPtr,
  ...
  if (bWrite) {
    ...
    if (errCode == Harbour::SUCCESS && ulLen > 0) {
      ...
      if (pFile != nullptr) {
        ...
        auto bBuffer = static_cast<uint8_t *>(hb_xgrab(nBufSize));
...
static HB_ERRCODE hb_fptPutMemo(FPTAREAP pArea, uint16_t uiIndex, HB_ITEM *pItem, HB_ULONG *pulBlock, int32_t iTrans)
{
#if 0
  ...
  uint8_t itmBuffer[FLEX_ITEM_BUFSIZE];
  const uint8_t *bBufPtr = nullptr;
  uint8_t *bBufAlloc = nullptr, *pbTmp;
  HB_ERRCODE errCode;
  HB_MAXINT iVal;
  HB_LONG lVal;
  ...
  if (pItem->isString()) {
    ...
    if (iTrans == FPT_TRANS_UNICODE) {
      ...
      if (ulSize > 0) {
        bBufPtr = bBufAlloc = static_cast<uint8_t *>(hb_xgrab(ulSize));
      ...
      bBufPtr = reinterpret_cast<const uint8_t *>(pItem->getCPtr());
      if (iTrans == FPT_TRANS_CP && ulSize > 0) {
        HB_SIZE nSize = ulSize;
        bBufAlloc = reinterpret_cast<uint8_t *>(
    ...
    pbTmp = bBufAlloc = static_cast<uint8_t *>(hb_xgrab(ulSize));
    ...
    case Harbour::Item::ARRAY:
      ...
      if (ulSize > 0) {
        pbTmp = bBufAlloc = static_cast<uint8_t *>(hb_xgrab(ulSize + 1));
    ...
    case Harbour::Item::LONG:
      ...
      if (HB_LIM_INT8(iVal)) {
        ...
        ulSize = 1;
        *itmBuffer = static_cast<uint8_t>(iVal);
        bBufPtr = itmBuffer;
      } else if (HB_LIM_INT16(iVal)) {
...
static bool hb_fptHasMemoData(FPTAREAP pArea, uint16_t uiIndex)
{
  if (--uiIndex < pArea->area.uiFieldCount) {
    ...
    if (pField->uiType == Harbour::DB::Field::ANY) {
      if (pField->uiLen >= 6) {
        uint8_t *pFieldBuf = pArea->pRecord + pArea->pFieldOffset[uiIndex];
        uint16_t uiType = HB_GET_LE_UINT16(pFieldBuf + pField->uiLen - 2);
...
static HB_ERRCODE hb_fptLockForRead(FPTAREAP pArea, uint16_t uiIndex, bool *fUnLock)
{
  HB_ERRCODE errCode;
  HB_BOOL fLocked;
...
static HB_ERRCODE hb_fptGetVarField(FPTAREAP pArea, uint16_t uiIndex, HB_ITEM *pItem, PHB_FILE pFile)
{
#if 0
    ...
    if (pField->uiLen == 3 || uiType == HB_VF_DATE) {
      ...
      if (uiType <= HB_VF_CHAR) { // 64000 max string size
        ...
        if (uiType <= pField->uiLen - 2) {
          ...
          if (pField->uiLen > 6) {
            uint16_t uiVLen = pField->uiLen - 6;
            memcpy(pPtr, pFieldBuf, uiVLen);
            ulSize -= uiVLen;
            pPtr += uiVLen;
          }
          errCode = hb_fptReadRawBlock(pArea, reinterpret_cast<uint8_t *>(pPtr), nullptr, ulBlock, ulSize);
...
static HB_ERRCODE hb_fptGetVarFile(FPTAREAP pArea, HB_ULONG ulBlock, const char *szFile, uint16_t uiMode, int32_t iTrans)
{
#if 0
...
static HB_ERRCODE hb_fptPutVarField(FPTAREAP pArea, uint16_t uiIndex, HB_ITEM *pItem)
{
#if 0
  ...
  if (pField->uiType == Harbour::DB::Field::ANY || pField->uiType == Harbour::DB::Field::MEMO ||
    ...
    uint8_t *pFieldBuf = pArea->pRecord + pArea->pFieldOffset[uiIndex - 1];
    HB_BOOL bDeleted;
    ...
    if (pField->uiType != Harbour::DB::Field::ANY) {
      ...
      HB_PUT_LE_UINT32(pFieldBuf, static_cast<uint32_t>(lVal));
    } else if (pField->uiLen < 6) {
      return EDBF_DATATYPE;
    } else {
      uint8_t buffer[11], *pAlloc = nullptr, *pbTmp;
      ...
      if ((uiType <= HB_VF_CHAR && uiType > pField->uiLen - 2) || (uiType == HB_VF_DNUM && pField->uiLen <= 12) ||
          uiType == HB_VF_ARRAY || uiType == HB_VF_BLOB || uiType == HB_VF_BLOBCOMPRESS ||
          uiType == HB_VF_BLOBENCRYPT) {
      ...
      if (pItem->isDateTime()) {
        ...
        if (!pItem->isDouble() && HB_LIM_INT32(lVal)) {
          ...
          buffer[1] = static_cast<uint8_t>(iWidth);
          buffer[2] = static_cast<uint8_t>(iDec);
        ...
        pbTmp = pAlloc = static_cast<uint8_t *>(hb_xgrab(ulNewSize));
        hb_fptStoreSMTItem(pArea, pItem, &pbTmp, iTrans);
        pBlock = pAlloc;
        uiType = HB_VF_ARRAY;
      } else {
...
static HB_ERRCODE hb_fptStructSize(FPTAREAP pArea, uint16_t *uiSize)
{
#if 0
...
static HB_ERRCODE hb_fptGetVarLen(FPTAREAP pArea, uint16_t uiIndex, HB_ULONG *pLength)
{
#if 0
...
static HB_ERRCODE hb_fptGetValue(FPTAREAP pArea, uint16_t uiIndex, HB_ITEM *pItem)
{
#if 0
...
static HB_ERRCODE hb_fptPutValue(FPTAREAP pArea, uint16_t uiIndex, HB_ITEM *pItem)
{
#if 0
...
static HB_ERRCODE hb_fptCreateMemFile(FPTAREAP pArea, LPDBOPENINFO pCreateInfo)
{
  ...
  if (pCreateInfo) {
    ...
    HB_FNAME *pFileName;
    HB_ITEM *pError = nullptr, *pItem = nullptr;
    auto bRetry = false;
  ...
  if (pArea->bMemoType == DB_MEMO_SMT || pArea->bMemoType == DB_MEMO_DBT) {
    ...
    HB_PUT_LE_UINT32(fptHeader.blockSize, static_cast<uint32_t>(pArea->ulMemoBlockSize));
  } else {
    HB_PUT_BE_UINT32(fptHeader.nextBlock, ulNextBlock);
    HB_PUT_BE_UINT32(fptHeader.blockSize, static_cast<uint32_t>(pArea->ulMemoBlockSize));
  }
...
static HB_ERRCODE hb_fptGetValueFile(FPTAREAP pArea, uint16_t uiIndex, const char *szFile, uint16_t uiMode)
{
#if 0
    ...
    if (hb_fptFileLockSh(pArea, true)) {
      ...
      if (nRead >= 512 && nRead != static_cast<HB_SIZE>(FS_ERROR)) {
        ...
        if (pArea->uiMemoVersion != DB_MEMOVER_SIX && memcmp(fptHeader.signature2, "FlexFile3\003", 10) == 0) {
          uint16_t usSize = HB_GET_LE_UINT16(fptHeader.flexSize);
          pArea->uiMemoVersion = DB_MEMOVER_FLEX;
...
static HB_ERRCODE hb_fptPutValueFile(FPTAREAP pArea, uint16_t uiIndex, const char *szFile, uint16_t uiMode)
{
#if 0
  ...
  if (pArea->fHasMemo && pArea->pMemoFile &&
    ...
    if (pFile == nullptr) {
      ...
      auto pAlloc = static_cast<uint8_t *>(hb_xgrab(ulSize + 1));
...
static HB_ERRCODE hb_fptDoPackRec(FPTAREAP pArea)
{
  ...
  for (uint16_t uiField = 0; uiField < pArea->area.uiFieldCount; ++uiField) {
    ...
    if (pField->uiType == Harbour::DB::Field::MEMO || pField->uiType == Harbour::DB::Field::IMAGE ||
      ...
      uint8_t *pFieldBuf = pArea->pRecord + pArea->pFieldOffset[uiField];
      uint8_t buffer[4];
...
static HB_ERRCODE hb_fptDoPack(FPTAREAP pArea, HB_ULONG ulBlockSize, HB_ITEM *pEvalBlock, HB_LONG lEvalStep)
{
#if 0
...
static HB_ERRCODE hb_fptInfo(FPTAREAP pArea, uint16_t uiIndex, HB_ITEM *pItem)
{
#if 0
  ...
  case DBI_MEMOEXT:
    if (pArea->fHasMemo && pArea->pMemoFile) {
      auto pFileName = hb_fsFNameSplit(pArea->szMemoFileName);
  ...
  case DBI_BLOB_DIRECT_EXPORT: // BLOBDirectExport() { <nPointer>, <cTargetFile>, <kMOde> }
  {
    ...
    if (pItem->isArray()) {
      ...
      if (ulBlock && szFile && *szFile) {
        errCode = hb_fptGetVarFile(pArea, ulBlock, szFile, static_cast<uint16_t>(hb_arrayGetNI(pItem, 3)),
...
static HB_ERRCODE hb_fptFieldInfo(FPTAREAP pArea, uint16_t uiIndex, uint16_t uiType, HB_ITEM *pItem)
{
#if 0
...
static HB_ERRCODE hb_fptRddInfo(LPRDDNODE pRDD, uint16_t uiIndex, HB_ULONG ulConnect, HB_ITEM *pItem)
{
#if 0
  ...
  case RDDI_MEMOEXT: {
    ...
    if (pData->szMemoExt[0]) {
      ...
      int32_t iType = hb_memoDefaultType(pRDD, ulConnect);
  ...
  case RDDI_MEMOTYPE: {
    ...
    if (pRDD->rddID != s_uiRddIdBLOB) {
      ...
      case DB_MEMO_FPT:
      case DB_MEMO_SMT:
        pData->bMemoType = static_cast<uint8_t>(iType);
      }
    }
    break;
  }
  ...
  case RDDI_MEMOVERSION: {
    ...
    case DB_MEMOVER_CLIP:
      pData->bMemoExtType = static_cast<uint8_t>(iType);
    }
    break;
  }
...
static void hb_dbffptRegisterRDD(uint16_t *pusRddId)
{
  RDDFUNCS *pTable;
  uint16_t *puiCount, uiRddId, *puiSuperRddId;
