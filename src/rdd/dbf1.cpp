   ...
   if( pArea->ulRecCount > 0 || (pArea->fShared && !pArea->fFLocked) ) {
      ...
      for( uiCount = 0; uiCount < lpdbTransInfo->uiItemCount; ++uiCount ) {
         ...
         if( hb_dbfIsAutoIncField(pField) != HB_AUTOINC_NONE ) {
            ...
            if( SELF_FIELDINFO(lpdbTransInfo->lpaSource, lpdbTransInfo->lpTransItems[uiCount].uiSource, DBS_COUNTER, pItem) != Harbour::SUCCESS ) {
               fCopyCtr = false;
               break;
            }
         }
...
static void hb_dbfTableCrypt(DBFAREAP pArea, PHB_ITEM pPasswd, bool fEncrypt)
{
   ...
   if( !pArea->fReadonly && !pArea->fShared && fEncrypt ? !pArea->fTableEncrypted && !pArea->fHasMemo : pArea->fTableEncrypted ) {
      HB_ULONG ulRecords;
 ...
 * so I left it in DBF.
 */
HB_ERRCODE hb_dbfGetMemoData(DBFAREAP pArea, HB_USHORT uiIndex, HB_ULONG * pulBlock, HB_ULONG * pulSize, HB_ULONG * pulType)
{
#if 0
       ...
       */
         ...
         for( HB_USHORT uiCount = 0; uiCount < 10; uiCount++ ) {
            ...
            if( bByte >= '0' && bByte <= '9' ) {
               ...
               return hb_dbfErrorRT(pArea, EG_CORRUPTION, EDBF_CORRUPT, pArea->szDataFileName, 0, EF_CANDEFAULT, nullptr) == E_DEFAULT ? Harbour::SUCCESS : Harbour::FAILURE;
            }
         }
         *pulBlock = ulValue;
      }
 ...
 */
static HB_ERRCODE hb_dbfLockData(DBFAREAP pArea, HB_FOFFSET * pnPos, HB_FOFFSET * pnFlSize, HB_FOFFSET * pnRlSize, int * iDir)
{
      ...
      *pBuffer = pArea->pRecord;
      ...
      if( pArea->pRecord[0] == 'D' || pArea->pRecord[0] == 'E' ) {
         ...
         if( pArea->pCryptKey && pArea->bCryptType == DB_CRYPT_SIX ) {
            hb_sxDeCrypt(reinterpret_cast<const char*>(pArea->pRecord) + 1, reinterpret_cast<char*>(pArea->pRecord) + 1, pArea->pCryptKey, pArea->uiRecordLen - 1);
         }
      } else {
   ...
   switch( pField->uiType ) {
      case Harbour::DB::Field::STRING:
         ...
         if( (pField->uiFlags & HB_FF_UNICODE) != 0 ) {
            ...
            pszVal = hb_cdpnDup(reinterpret_cast<const char*>(pArea->pRecord) + pArea->pFieldOffset[uiIndex], &nLen, pArea->area.cdPage, hb_vmCDP());
      ...
      case Harbour::DB::Field::VARLENGTH:
         ...
         if( (pField->uiFlags & HB_FF_UNICODE) != 0 ) {
            ...
            if( nLen == 0xFFFF || nLen > static_cast<HB_SIZE>(pField->uiLen) ) { /* protection against corrupted files */
               nLen = 0;
            }
            hb_itemPutStrLenU16(pItem, HB_CDP_ENDIAN_LITTLE, reinterpret_cast<const HB_WCHAR*>(&pArea->pRecord[pArea->pFieldOffset[uiIndex]]), nLen);
         } else {
      ...
      case Harbour::DB::Field::ROWVER:
         if( pField->uiDec ) {
            ...
            hb_itemPutNDLen(pItem, hb_numDecConv(dVal, static_cast<int>(pField->uiDec)), iLen, static_cast<int>(pField->uiDec));
         } else {
            switch( pField->uiLen ) {
               case 1:
      ...
      if( HB_IS_MEMO(pItem) || HB_IS_STRING(pItem) ) {
         ...
         if( pField->uiType == Harbour::DB::Field::STRING ) {
            if( (pField->uiFlags & HB_FF_UNICODE) != 0 ) {
               ...
               if( (pField->uiFlags & HB_FF_BINARY) == 0 ) {
                  hb_cdpnDup2(pszPtr, nSize, reinterpret_cast<char*>(pArea->pRecord) + pArea->pFieldOffset[uiIndex], &nLen, hb_vmCDP(), pArea->area.cdPage);
               } else {
                  if( nLen > nSize ) {
                     nLen = nSize;
                  }
...
#ifndef HB_LONG_LONG_OFF
                  iSize = HB_LIM_INT8(lVal) ? 1 : (HB_LIM_INT16(lVal) ? 2 : (HB_LIM_INT24(lVal) ? 3 : (HB_LIM_INT32(lVal) ? 4 : 8)));
#else
                  iSize = HB_DBL_LIM_INT8(dVal) ? 1 : (HB_DBL_LIM_INT16(dVal) ? 2 : (HB_DBL_LIM_INT24(dVal) ? 3 : (HB_DBL_LIM_INT32(dVal) ? 4 : 8)));
#endif
            } else {
...
static HB_ERRCODE hb_dbfCreate(DBFAREAP pArea, LPDBOPENINFO pCreateInfo)
{
   ...
   fRawBlob = SELF_RDDINFO(SELF_RDDNODE(&pArea->area), RDDI_BLOB_SUPPORT, pCreateInfo->ulConnection, pItem) == Harbour::SUCCESS && hb_itemGetL(pItem);
   ...
   for( uiCount = 0; uiCount < pArea->area.uiFieldCount; uiCount++ ) {
      ...
      hb_strncpy(reinterpret_cast<char*>(pThisField->bName), hb_dynsymName(static_cast<PHB_DYNS>(pField->sym)), sizeof(pThisField->bName) - 1);
   ...
   pItem = hb_itemNew(nullptr);
   if( SELF_RDDINFO(SELF_RDDNODE(&pArea->area), RDDI_PENDINGPASSWORD, pCreateInfo->ulConnection, pItem) == Harbour::SUCCESS ) {
...
static HB_ERRCODE hb_dbfInfo(DBFAREAP pArea, HB_USHORT uiIndex, PHB_ITEM pItem)
{
   ...
   switch( uiIndex ) {
      ...
      case DBI_FILEHANDLE:
         hb_itemPutNInt(pItem, !pArea->pDataFile ? FS_ERROR : static_cast<HB_MAXINT>(static_cast<HB_NHANDLE>(hb_fileHandle(pArea->pDataFile))));
         break;
...
static HB_ERRCODE hb_dbfFieldInfo(DBFAREAP pArea, HB_USHORT uiIndex, HB_USHORT uiType, PHB_ITEM pItem)
{
   ...
   switch( uiType ) {
      case DBS_ISNULL:
         ...
         hb_itemPutL(pItem, (pField->uiFlags & HB_FF_NULLABLE) != 0 && hb_dbfGetNullFlag(pArea, pArea->pFieldBits[uiIndex - 1].uiNullBit));
         return Harbour::SUCCESS;
      case DBS_COUNTER:
...
static HB_ERRCODE hb_dbfOpen(DBFAREAP pArea, LPDBOPENINFO pOpenInfo)
{
   ...
   hb_itemClear(pItem);
   fRawBlob = SELF_RDDINFO(SELF_RDDNODE(&pArea->area), RDDI_BLOB_SUPPORT, pOpenInfo->ulConnection, pItem) == Harbour::SUCCESS && hb_itemGetL(pItem);
   hb_itemClear(pItem);
   uiDecimals = static_cast<HB_USHORT>(SELF_RDDINFO(SELF_RDDNODE(&pArea->area), RDDI_DECIMALS, pOpenInfo->ulConnection, pItem) == Harbour::SUCCESS ? hb_itemGetNI(pItem) : 0);
   hb_itemRelease(pItem);
   uiFlagsMask = 0;
   ...
   if( fRawBlob ) {
      ...
      /* Try open */
      do {
         pArea->pDataFile = hb_fileExtOpen(szFileName, nullptr, uiFlags |
      ...
      } while( hb_dbfErrorRT(pArea, EG_OPEN, EDBF_OPEN_DBF, szFileName, hb_fsError(), EF_CANRETRY | EF_CANDEFAULT, &pError) == E_RETRY );
   ...
   pItem = hb_itemNew(nullptr);
   if( SELF_RDDINFO(SELF_RDDNODE(&pArea->area), RDDI_PENDINGPASSWORD, pOpenInfo->ulConnection, pItem) == Harbour::SUCCESS ) {
      hb_dbfPasswordSet(pArea, pItem, false);
   } else {
      hb_itemClear(pItem);
...
static void hb_dbfSortInsPage(LPDBSORTREC pSortRec, HB_SORTIDX * pIndex, HB_SORTIDX nFirst, HB_SORTIDX nLast, HB_SORTIDX nAt)
{
   while( nFirst < nLast ) {
...
static HB_ERRCODE hb_dbfSortWritePage(LPDBSORTREC pSortRec)
{
   ...
   if( hb_fileWriteAt(pSortRec->pTempFile, pData, nSize, pSortRec->pSwapPages[pSortRec->nPages].nOffset) != nSize ) {
      hb_dbfErrorRT(reinterpret_cast<DBFAREAP>(pArea), EG_WRITE, EDBF_WRITE_TEMP, pSortRec->szTempFileName, hb_fsError(), 0, nullptr);
      return Harbour::FAILURE;
   }
   pSortRec->nPages++;
   pSortRec->nCount = 0;
...
static HB_ERRCODE hb_dbfSortReadPage(LPDBSORTREC pSortRec, PHB_DBSORTPAGE pPage)
{
   ...
   if( hb_fileReadAt(pSortRec->pTempFile, pPage->pnRecords, nSize, pPage->nOffset) != nSize ) {
      hb_dbfErrorRT(reinterpret_cast<DBFAREAP>(pArea), EG_READ, EDBF_READ_TEMP, pSortRec->szTempFileName, hb_fsError(), 0, nullptr);
      return Harbour::FAILURE;
   }
   pPage->nOffset += nSize;
   pPage->nInBuf = nCount;
...
static HB_ERRCODE hb_dbfSortFinish( LPDBSORTREC pSortRec )
{
   ...
   while( pSortRec->nPages > 0 ) {
      ...
      if( hb_dbfSortGetRec(pSortRec, &nRecNo) != Harbour::SUCCESS ||
          SELF_GOTO(pArea, nRecNo) != Harbour::SUCCESS ||
...
static HB_ERRCODE hb_dbfSortAdd(LPDBSORTREC pSortRec)
{
   ...
   if( SELF_RECNO(pArea, &ulRecNo) != Harbour::SUCCESS || hb_dbfSortReadRec(pSortRec, hb_arrayGetItemPtr(pSortRec->pSortArray, pSortRec->nCount + 1)) != Harbour::SUCCESS ) {
      return Harbour::FAILURE;
   }
...
static HB_ERRCODE hb_dbfGetValueFile(DBFAREAP pArea, HB_USHORT uiIndex, const char * szFile, HB_USHORT uiMode)
{
   ...
   if( pField->uiType == Harbour::DB::Field::STRING ) {
      ...
      if( !pFile ) {
         ...
         if( hb_fileWriteAt(pFile, pArea->pRecord + pArea->pFieldOffset[uiIndex], pField->uiLen, hb_fileSize(pFile)) != static_cast<HB_SIZE>(pField->uiLen) ) {
            errCode = EDBF_WRITE;
         }
         hb_fileClose(pFile);
      }
   ...
   if( errCode != Harbour::SUCCESS ) {
      hb_dbfErrorRT(pArea, hb_dbfGetEGcode(errCode), errCode,
                    errCode != EDBF_DATATYPE ? szFile : nullptr,
...
static HB_ERRCODE hb_dbfWriteDBHeader(DBFAREAP pArea)
{
   ...
   pArea->dbfHeader.bYear = static_cast<HB_BYTE>(pArea->bTableType == DB_DBF_STD && (pArea->uiSetHeader & DB_SETHEADER_YYEAR) == 0 ? iYear - 1900 : iYear % 100);
   ...
   if( hb_fileWriteAt(pArea->pDataFile, &pArea->dbfHeader, sizeof(DBFHEADER), 0) == sizeof(DBFHEADER) ) {
      ...
      if( !pArea->fShared || (pArea->uiSetHeader & DB_SETHEADER_EOL) != 0 ) {
         /* write eof mark */
         HB_FOFFSET nOffset = static_cast<HB_FOFFSET>(pArea->uiHeaderLen) + static_cast<HB_FOFFSET>(pArea->uiRecordLen) * static_cast<HB_FOFFSET>(pArea->ulRecCount);
...
{"_DBF",             {HB_FS_PUBLIC|HB_FS_LOCAL}, {HB_FUNCNAME( _DBF )}, nullptr},
{"DBF_GETFUNCTABLE", {HB_FS_PUBLIC|HB_FS_LOCAL}, {HB_FUNCNAME( DBF_GETFUNCTABLE )}, nullptr}
HB_INIT_SYMBOLS_END(dbf1__InitSymbols)

HB_CALL_ON_STARTUP_BEGIN(_hb_dbf_rdd_init_)
   hb_vmAtInit(hb_dbfRddInit, nullptr);
