...
#define HB_COM_LOCK()                                                                                                  \
  do {                                                                                                                 \
  hb_threadEnterCriticalSection(&s_comMtx)
#define HB_COM_UNLOCK()                                                                                                \
  hb_threadLeaveCriticalSection(&s_comMtx);                                                                            \
  }                                                                                                                    \
  while (false)
...
{
  for (auto iPort = 0; iPort < HB_COM_PORT_MAX; ++iPort) {
...
static int hb_comGetPortNum(const char *pszName)
{
  ...
  if (HB_TOUPPER(pszName[0]) == 'C' && HB_TOUPPER(pszName[1]) == 'O' && HB_TOUPPER(pszName[2]) == 'M' &&
...
HB_FHANDLE hb_comGetDeviceHandle(int iPort)
{
  ...
  if (pCom != nullptr) {
    ...
    hFile = pCom->fd;
#elif defined(HB_OS_WIN)
    hFile = reinterpret_cast<HB_FHANDLE>(pCom->hComm);
#endif
  }
...
static int hb_comCanRead(PHB_COM pCom, HB_MAXINT timeout)
{
  ...
  for (;;) {
    ...
    FD_ZERO(&rfds);
    FD_SET(pCom->fd, &rfds);
    iResult = select(static_cast<int32_t>(pCom->fd + 1), &rfds, nullptr, nullptr, &tv);
    hb_comSetOsError(pCom, iResult == -1);
...
static int hb_comCanWrite(PHB_COM pCom, HB_MAXINT timeout)
{
  ...
  for (;;) {
    ...
    FD_ZERO(&wfds);
    FD_SET(pCom->fd, &wfds);
    iResult = select(static_cast<int32_t>(pCom->fd + 1), nullptr, &wfds, nullptr, &tv);
    hb_comSetOsError(pCom, iResult == -1);
...
int hb_comClose(int iPort)
{
  ...
  if (pCom != nullptr) {
    ...
    if (iResult != -1 || HB_COM_IS_EBADF(pCom)) {
      pCom->fd = static_cast<HB_FHANDLE>(FS_ERROR);
...
int hb_comOpen(int iPort)
{
  ...
  if (pCom != nullptr) {
    if (pCom->status & HB_COM_OPEN) {
      ...
      pCom->hComm =
          CreateFile(lpName, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_FLAG_NO_BUFFERING, nullptr);
