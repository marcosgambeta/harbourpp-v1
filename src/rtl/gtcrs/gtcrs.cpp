   ...
   else if( ioBase->terminal_type == TERM_LINUX )
   {
      ...
      if( Gpm_Open( &ioBase->Conn, 0 ) >= 0 && gpm_fd >= 0 )
      {
         ...
         add_efds( ioBase, gpm_fd, O_RDONLY, set_gpmevt,
                   static_cast< void * >( &ioBase->mLastEvt ) );
      ...
      for( i = n = 0; i < ioBase->efds_no; i++ )
      {
         ...
         else
         {
            ...
            if( ioBase->event_fds[ i ]->status == EVTFDSTAT_STOP &&
                ioBase->event_fds[ i ]->eventFunc == nullptr )
...
static InOutBase * create_newXterm( void )
{
   ...
   if( termpid == 0 )
   {
      ...
      execlp( "xterm", "xterm", buf, "+sb",
              "-fg", "white",
              "-bg", "black", "-fn", "fixed", "-T", "HB-XTERM Window", nullptr );
      _exit( EXIT_FAILURE );
   }
   close( masterfd );
...
{
   HB_TRACE( HB_TR_DEBUG, ( "hb_gt_crs_Init(%p,%p,%p,%p)", static_cast< void * >( pGT ), static_cast< void * >( static_cast< HB_PTRUINT >( hFilenoStdin ) ), static_cast< void * >( static_cast< HB_PTRUINT >( hFilenoStdout ) ), static_cast< void * >( static_cast< HB_PTRUINT >( hFilenoStderr ) ) ) );
