...
HB_FUNC(SOUNDEX)
{
  ...
  if (pString) {
    ...
    while (nPos < nLen && nResultPos < SOUNDEX_LEN_MAX) {
      ...
      // Check if IsAlpha()
      if (cChar >= 'A' && cChar <= 'Z') {
        static const char s_szTable[] = "01230120022455012623010202"; // NOTE: SoundEx result codes for letters from "A"
                                                                      // to "Z" "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        char cCharConverted =
            ((cChar - 'A') > (static_cast<int32_t>(sizeof(s_szTable)) - 1)) ? '9' : s_szTable[cChar - 'A'];
