...
static void hb_itemHexStr(HB_ITEM *pItem, char *pStr, HB_BOOL fUpper)
{
  ...
  pStr[iLen] = '\0';
  do {
    auto iDigit = static_cast<int32_t>(nValue & 0x0F);
...
HB_ITEM *hb_strFormat(HB_ITEM *pItemReturn, HB_ITEM *pItemFormat, int32_t iCount, HB_ITEM **pItemArray)
{
  ...
  while (pFmt < pFmtEnd) {
    ...
    /* Parse specifier */
    if (*pFmt == 'c' || *pFmt == 'd' || *pFmt == 'x' || *pFmt == 'X' || *pFmt == 'f' || *pFmt == 's') {
      if (iParamNo == -1) {
        iParamNo = ++iParam;
      }
    ...
    case 'X': {
      ...
      if (pItem && pItem->isNumeric()) {
        ...
        iSize = static_cast<int32_t>(strlen(pStr2));
        if (*pStr2 == '-') {
          fSign = 1;
          iSize--;
          pStr2++;
    ...
    case 's': {
      ...
      if (iWidth > 1) {
        for (i = static_cast<int32_t>(nSize); i < iWidth; i++) {
          bufadd(&buffer, " ", 1);
        }
      }
