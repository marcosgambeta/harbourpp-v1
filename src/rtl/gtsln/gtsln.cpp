...
static void hb_sln_colorTrans( void )
{
   for( int i = 0; i < 256; i++ )
   {
       ...
       */
      clr = ( bg << 4 ) | ( fg ^ 0x07 );
      SLtt_set_color( clr, nullptr, const_cast< char * >( s_colorNames[ fg ] ),
                                 const_cast< char * >( s_colorNames[ bg ] ) );
#ifdef HB_SLN_UTF8
      s_colorTab[ i ] = clr;
#else
...
static void hb_gt_sln_Init( PHB_GT pGT, HB_FHANDLE hFilenoStdin, HB_FHANDLE hFilenoStdout, HB_FHANDLE hFilenoStderr )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "hb_gt_sln_Init(%p,%p,%p,%p)", static_cast< void * >( pGT ), static_cast< void * >( static_cast< HB_PTRUINT >( hFilenoStdin ) ), static_cast< void * >( static_cast< HB_PTRUINT >( hFilenoStdout ) ), static_cast< void * >( static_cast< HB_PTRUINT >( hFilenoStderr ) ) ) );
