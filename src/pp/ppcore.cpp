...
static void hb_pp_error(PHB_PP_STATE pState, char type, int iError, const char * szParam)
{
   ...
   else
   {
      ...
      hb_snprintf(buffer, sizeof(buffer), "%s%s: %s\n", line, type == 'F' ? "Fatal" : type == 'W' ? "Warning" : "Error", msg);
...
static HB_BOOL hb_pp_canQuote(HB_BOOL fQuote, char * pBuffer, HB_SIZE nLen, HB_SIZE n, HB_SIZE * pnAt)
{
   char cQuote = 0;
...
static HB_BOOL hb_pp_hasCommand(char * pBuffer, HB_SIZE nLen, HB_SIZE * pnAt, int iCmds, ...)
{
   ...
   for( i = 0; i < iCmds && n < nLen; ++i )
   {
      ...
      n += nl;
      if( n < nLen && (HB_PP_ISNEXTIDCHAR(cmd[nl - 1]) || HB_PP_ISTEXTCHAR(cmd[nl - 1]) ) && (HB_PP_ISNEXTIDCHAR(pBuffer[n]) || HB_PP_ISTEXTCHAR(pBuffer[n])) )
      {
         break;
      }
   }
...
static void hb_pp_dumpEnd(PHB_PP_STATE pState)
{
   ...
   {
      (pState->pDumpFunc)(pState->cargo, hb_membufPtr(pState->pDumpBuffer), hb_membufLen(pState->pDumpBuffer), pState->iDumpLine + 1);
   ...
   do
   {
      ...
      while( n < nLen || fDump )
      {
         ...
         if( pState->iStreamDump )
         {
               ...
               if( nLen == n || pState->iStreamDump == HB_PP_STREAM_OFF )
               {
                  ...
                  if( pState->iStreamDump == HB_PP_STREAM_OFF )
                  {
                     ...
                     else if( pState->pInLineFunc )
                     {
                        ...
                        pState->pInLineFunc(pState->cargo, szFunc, hb_membufPtr(pState->pStreamBuffer), hb_membufLen(pState->pStreamBuffer), pState->iDumpLine);
                     }
                     else
                     {
                        hb_pp_tokenAddNext(pState, hb_membufPtr(pState->pStreamBuffer), hb_membufLen(pState->pStreamBuffer), HB_PP_TOKEN_TEXT);
                     }
            ...
            else if( hb_pp_hasCommand(pBuffer, nLen, &n, 1, "ENDTEXT") || hb_pp_hasCommand(pBuffer, nLen, &n, 3, "#", "pragma", "__endtext") )
            {
               ...
               else
               {
                  ...
                  hb_pp_tokenAddStreamFunc(pState, pState->pFuncOut, hb_membufPtr(pState->pStreamBuffer), hb_membufLen(pState->pStreamBuffer));
                  if( pState->pFuncEnd )
                  {
                     if( pState->pFuncOut )
                     {
                     ...
                     hb_pp_tokenAddStreamFunc(pState, pState->pFuncEnd, hb_membufPtr(pState->pStreamBuffer), hb_membufLen(pState->pStreamBuffer));
                  }
                  hb_membufFlush(pState->pStreamBuffer);
               }
         ...
         else if( ch == '[' && !pState->fDirective && hb_pp_canQuote(pState->fCanNextLine || HB_PP_TOKEN_CANQUOTE(pState->usLastType), pBuffer, nLen, 1, &n) )
         {
            hb_pp_tokenAddNext(pState, pBuffer + 1, n - 1, HB_PP_TOKEN_STRING);
            ++n;
         }
         ...
         else if( HB_PP_ISFIRSTIDCHAR(ch) )
         {
            ...
            else
            {
               ...
               else if( pState->pInLineFunc && pState->iInLineState == HB_PP_INLINE_OFF && n == 9 && hb_strnicmp("hb_inline", pBuffer, 9) == 0 )
               {
                  if( pState->fCanNextLine )
                  {
                     hb_pp_tokenAddCmdSep(pState);
         ...
         else if( HB_PP_ISDIGIT(ch) )
         {
            ...
            else if( nLen >= 3 && pBuffer[0] == '0' && (pBuffer[1] == 'd' || pBuffer[1] == 'D') && HB_PP_ISDIGIT(pBuffer[2]) )
            {
               n = 2;
         ...
         else if( ch == '{' && !pState->fCanNextLine && (pState->iInLineState == HB_PP_INLINE_BODY || pState->iInLineState == HB_PP_INLINE_START) )
         {
      ...
      if( pEolTokenPtr && (pEolTokenPtr != pState->pNextTokenPtr || (pState->iNestedBlock && pState->pFile->iTokens &&
...
static int hb_pp_tokenStr(PHB_PP_TOKEN pToken, PHB_MEM_BUFFER pBuffer, HB_BOOL fSpaces, HB_BOOL fQuote, HB_USHORT ltype)
{
   int iLines = 0;
...
static HB_BOOL hb_pp_tokenEqual(PHB_PP_TOKEN pToken, PHB_PP_TOKEN pMatch, HB_USHORT mode)
{
...
static HB_BOOL hb_pp_patternAddResult(PHB_PP_RULE pRule, HB_USHORT marker, PHB_PP_TOKEN pFirst, PHB_PP_TOKEN pNext)
{
...
static void hb_pp_defineAdd(PHB_PP_STATE pState, HB_USHORT mode, HB_USHORT markers, PHB_PP_MARKER pMarkers, PHB_PP_TOKEN pMatch, PHB_PP_TOKEN pResult)
{
...
static PHB_PP_FILE hb_pp_FileNew(PHB_PP_STATE pState, const char * szFileName, HB_BOOL fSysFile, HB_BOOL * pfNested, FILE * file_in, HB_BOOL fSearchPath, PHB_PP_OPEN_FUNC pOpenFunc, HB_BOOL fBinary )
{
   ...
   if( !file_in )
   {
      ...
      if( pOpenFunc )
      {
         ...
         iAction = (pOpenFunc)(pState->cargo, szFileNameBuf, HB_TRUE, fSysFile, fBinary, fSearchPath ? pState->pIncludePath : nullptr,
      ...
      if( iAction == HB_PP_OPEN_FILE )
      {
         ...
         if( !fSysFile )
         {
            ...
            if( !file_in && (!pFileName->szPath || (!pFileName->szDrive && !strchr(HB_OS_PATH_DELIM_CHR_LIST, static_cast<HB_UCHAR>(pFileName->szPath[0])))) )
            {
         ...
         if( iAction != HB_PP_OPEN_OK )
         {
            ...
            if( iAction != HB_PP_OPEN_OK && pOpenFunc && !fNested )
            {
               ...
               iAction = (pOpenFunc)(pState->cargo, szFileNameBuf, HB_FALSE, fSysFile, fBinary, fSearchPath ? pState->pIncludePath : nullptr,
                                     pfNested, &file_in, &pLineBuf, &nLineBufLen, &fFree );
               if( iAction == HB_PP_OPEN_OK )
               {
                  szFileName = szFileNameBuf;
...
{
   PHB_PP_FILE pFile = hb_pp_FileNew(pState, szFileName, HB_FALSE, nullptr, nullptr,
...
static HB_BOOL hb_pp_pragmaOperatorNew(PHB_PP_STATE pState, PHB_PP_TOKEN pToken)
{
   ...
   if( !HB_PP_TOKEN_ISEOC(pToken) && HB_PP_TOKEN_CANJOIN(pToken->type) )
   {
      ...
      if( HB_PP_TOKEN_ISEOC(pToken) && nLen > 0 )
      {
         ...
         {
            pState->pOperators = static_cast<PHB_PP_OPERATOR>(hb_xrealloc(pState->pOperators, sizeof(HB_PP_OPERATOR) * (pState->iOperators + 1)));
         }
         else
         {
...
static HB_BOOL hb_pp_setCompilerSwitch(PHB_PP_STATE pState, const char * szSwitch, int iValue)
{
   HB_BOOL fError = HB_TRUE;
...
static HB_BOOL hb_pp_getCompilerSwitch(PHB_PP_STATE pState, const char * szSwitch, int * piValue)
{
   HB_BOOL fError = HB_TRUE;
   ...
   if( pToken && pToken->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD )
   {
      if( (HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_EQ && HB_PP_TOKEN_ISEOC(pToken->pNext->pNext)) ||
          (pToken->pNext->pNext && HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_LEFT_PB &&
           HB_PP_TOKEN_TYPE(pToken->pNext->pNext->type) == HB_PP_TOKEN_RIGHT_PB && HB_PP_TOKEN_ISEOC(pToken->pNext->pNext->pNext)) )
      {
         pValue = pToken->pNext;
...
static void hb_pp_pragmaNew(PHB_PP_STATE pState, PHB_PP_TOKEN pToken)
{
   ...
   else if( pState->fTracePragmas || pState->fWriteTrace )
   {
      ...
      {
         if( fwrite(hb_membufPtr(pState->pBuffer), sizeof(char), hb_membufLen(pState->pBuffer), pState->file_trace) != hb_membufLen(pState->pBuffer) )
         {
...
static void hb_pp_defineNew(PHB_PP_STATE pState, PHB_PP_TOKEN pToken, HB_BOOL fDirect)
{
   ...
   else
   {
      ...
      if( pLast && HB_PP_TOKEN_TYPE(pLast->type) == HB_PP_TOKEN_LEFT_PB && pLast->spaces == 0 )
      {
         ...
         {
            pLast = pLast->pNext;
            if( pLast && (usPCount == 0 || type == HB_PP_TOKEN_COMMA) && HB_PP_TOKEN_TYPE(pLast->type) == HB_PP_TOKEN_RIGHT_PB )
            {
               break;
            }
...
static HB_BOOL hb_pp_matchMarkerNew(PHB_PP_TOKEN * pTokenPtr, PHB_PP_MARKERLST * pMarkerListPtr)
{
   ...
   if( hb_pp_tokenUnQuotedGet(&pTokenPtr, &fQuoted, HB_TRUE) && !fQuoted )
   {
      ...
      {
         if( hb_pp_tokenUnQuotedGet(&pTokenPtr, &fQuoted, HB_TRUE) && !fQuoted && HB_PP_TOKEN_TYPE((*pTokenPtr)->type) == HB_PP_TOKEN_KEYWORD )
         {
            pMarkerId = *pTokenPtr;
            if( hb_pp_tokenUnQuotedGet(&pTokenPtr, &fQuoted, HB_FALSE) && !fQuoted &&
...
static HB_BOOL hb_pp_matchPatternNew(PHB_PP_STATE pState, PHB_PP_TOKEN * pTokenPtr, PHB_PP_MARKERLST * pMarkerListPtr, PHB_PP_TOKEN ** pOptional)
{
...
static HB_BOOL hb_pp_resultMarkerNew(PHB_PP_STATE pState, PHB_PP_TOKEN * pTokenPtr, PHB_PP_MARKERLST * pMarkerListPtr, HB_BOOL fDump, HB_BOOL fOptional, HB_USHORT * pusPCount, HB_SIZE spaces)
{
   ...
   if( hb_pp_tokenUnQuotedGet(&pTokenPtr, &fQuoted, HB_TRUE) && !fQuoted )
   {
      ...
      {
         pMarkerId = *pTokenPtr;
         if( hb_pp_tokenUnQuotedGet(&pTokenPtr, &fQuoted, HB_FALSE) && !fQuoted && HB_PP_TOKEN_TYPE((*pTokenPtr)->type) == HB_PP_TOKEN_GT )
         {
      ...
      {
         /* <@> */
         if( hb_pp_tokenUnQuotedGet(&pTokenPtr, &fQuoted, HB_TRUE) && !fQuoted && HB_PP_TOKEN_TYPE((*pTokenPtr)->type) == HB_PP_TOKEN_GT )
         {
...
static void hb_pp_directiveDel(PHB_PP_STATE pState, PHB_PP_TOKEN pMatch, HB_USHORT markers, PHB_PP_MARKER pMarkers, HB_USHORT mode, HB_BOOL fCommand)
{
...
static void hb_pp_directiveNew(PHB_PP_STATE pState, PHB_PP_TOKEN pToken, HB_USHORT mode, HB_BOOL fCommand, HB_BOOL fDirect, HB_BOOL fDelete)
{
   ...
   if( pToken->pNext )
   {
      ...
      while( !HB_PP_TOKEN_ISEOP(pStart, fDirect) )
      {
         ...
         else if( pStart->pNext && HB_PP_TOKEN_TYPE(pStart->type) == HB_PP_TOKEN_EQ && HB_PP_TOKEN_TYPE(pStart->pNext->type) == HB_PP_TOKEN_GT )
         {
            fValid = HB_TRUE;
            if( !pLast )
            {
            ...
            do
            {
               if( !fQuoted )
               {
                  ...
                  else if( HB_PP_TOKEN_TYPE((*pTokenPtr)->type) == HB_PP_TOKEN_LT )
                  {
                     ...
                     if( !hb_pp_resultMarkerNew(pState, pTokenPtr, &pMarkerList, pDumpPtr != nullptr, pOptStart != nullptr, &usPCount, spaces) )
                     {
                        fValid = HB_FALSE;
                        break;
                     }
...
static HB_BOOL hb_pp_tokenStartExtBlock(PHB_PP_TOKEN * pTokenPtr)
{
       ...
       pToken->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_PIPE )
   {
      ...
      if( pToken && pToken->pNext && HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_PIPE && HB_PP_TOKEN_ISEOC(pToken->pNext) )
      {
         *pTokenPtr = pToken->pNext;
         return HB_TRUE;
      }
...
static HB_BOOL hb_pp_tokenStopExtBlock(PHB_PP_TOKEN * pTokenPtr)
{
   ...
   if( HB_PP_TOKEN_ISEOC(pToken) && pToken->pNext )
   {
          ...
          HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD )
      {
         ...
         if( hb_pp_tokenValueCmp(pToken, "FUNCTION", HB_PP_CMP_DBASE) || hb_pp_tokenValueCmp(pToken, "PROCEDURE", HB_PP_CMP_DBASE) )
         {
...
static HB_BOOL hb_pp_tokenSkipExp(PHB_PP_TOKEN * pTokenPtr, PHB_PP_TOKEN pStop, HB_USHORT mode, HB_BOOL * pfStop)
{
   ...
   for( ;; )
   {
      ...
      {
         lbrtype = curtype;
         rbrtype = (curtype == HB_PP_TOKEN_LEFT_PB ? HB_PP_TOKEN_RIGHT_PB : (curtype == HB_PP_TOKEN_LEFT_SB ? HB_PP_TOKEN_RIGHT_SB : HB_PP_TOKEN_RIGHT_CB));
         ++iBraces;
      }
...
static HB_BOOL hb_pp_tokenMatch(PHB_PP_TOKEN pMatch, PHB_PP_TOKEN * pTokenPtr, PHB_PP_TOKEN pStop, HB_USHORT mode)
{
   ...
   else if( type == HB_PP_MMARKER_RESTRICT )
   {
      ...
      while( pRestrict )
      {
         ...
         {
            if( HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_MACROVAR || HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_MACROTEXT )
            {
               *pTokenPtr = pToken->pNext;
            }
            else
...
static HB_BOOL hb_pp_patternMatch(PHB_PP_TOKEN pMatch, PHB_PP_TOKEN * pTokenPtr, PHB_PP_TOKEN pStop, HB_USHORT mode, PHB_PP_RULE pRule)
{
   PHB_PP_TOKEN pToken = *pTokenPtr;
   PHB_PP_TOKEN pFirst;
...
static HB_BOOL hb_pp_patternCmp(PHB_PP_RULE pRule, PHB_PP_TOKEN pToken, HB_BOOL fCommand)
{
   PHB_PP_TOKEN pFirst = pToken;
...
static PHB_PP_TOKEN * hb_pp_matchResultLstAdd(PHB_PP_STATE pState, HB_SIZE spaces, HB_USHORT type, PHB_PP_TOKEN * pResultPtr, PHB_PP_TOKEN pToken, PHB_PP_TOKEN pStop)
{
   PHB_PP_TOKEN pNext;
      ...
      if( hb_pp_tokenSkipExp(&pNext, pStop, HB_PP_CMP_ADDR, &fStop) && (fStop ? pToken : pToken->pNext) != pNext )
      {
                  ...
                  (fStop ? pToken->pNext : pToken->pNext->pNext) == pNext )
         {
            ...
            else
            {
               ...
               hb_pp_tokenAdd(&pResultPtr, hb_membufPtr(pState->pBuffer), hb_membufLen(pState->pBuffer), fFirst ? spaces : pToken->spaces, HB_PP_TOKEN_STRING);
            }
            pToken = pToken->pNext;
            fFirst = HB_FALSE;
         }
         ...
         else
         {
            ...
            while( (fStop ? pToken : pToken->pNext) != pNext );
            hb_pp_tokenAdd(&pResultPtr, hb_membufPtr(pState->pBuffer), hb_membufLen(pState->pBuffer), spaces, HB_PP_TOKEN_STRING);
            fFirst = HB_FALSE;
         }
      }
      if( fStop )
...
static PHB_PP_TOKEN * hb_pp_matchResultAdd(PHB_PP_STATE pState, PHB_PP_RULE pRule, PHB_PP_TOKEN * pResultPtr, PHB_PP_TOKEN pMatch, HB_USHORT usMatch)
{
   ...
   else if( HB_PP_TOKEN_TYPE(pMatch->type) == HB_PP_RMARKER_STRDUMP )
   {
      ...
      hb_pp_tokenAdd(&pResultPtr, hb_membufPtr(pState->pBuffer), hb_membufLen(pState->pBuffer), pMatch->spaces, HB_PP_TOKEN_STRING);
   }
            ...
            HB_PP_TOKEN_TYPE(pMatch->type) == HB_PP_RMARKER_BLOCK )
   {
      if( pMarkerResult )
      {
         ...
         if( pToken != pStop )
         {
            pResultPtr = hb_pp_matchResultLstAdd(pState, pMatch->spaces, HB_PP_TOKEN_TYPE(pMatch->type), pResultPtr, pToken, pStop);
         }
      }
   }
   else if( HB_PP_TOKEN_TYPE(pMatch->type) == HB_PP_RMARKER_LOGICAL )
   {
      ...
      hb_pp_tokenAdd(&pResultPtr, pMarkerResult ? ".T." : ".F.", 3, pMatch->spaces, HB_PP_TOKEN_LOGICAL | HB_PP_TOKEN_STATIC);
   }
...
static PHB_PP_TOKEN *  hb_pp_patternStuff(PHB_PP_STATE pState, PHB_PP_RULE pRule, HB_USHORT usMatch, PHB_PP_TOKEN pResultPattern, PHB_PP_TOKEN * pResultPtr)
{
...
static char * hb_pp_tokenListStr(PHB_PP_TOKEN pToken, PHB_PP_TOKEN pStop, HB_BOOL fStop, PHB_MEM_BUFFER pBuffer, HB_BOOL fQuote, HB_BOOL fEol)
{
...
static HB_BOOL hb_pp_concatenateKeywords(PHB_PP_STATE pState, PHB_PP_TOKEN * pFirstPtr)
{
   ...
   {
      pNext = pToken->pNext;
      if( HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_KEYWORD && pNext->spaces == 0 && HB_PP_TOKEN_TYPE(pNext->type) == HB_PP_TOKEN_KEYWORD )
      {
         hb_membufFlush(pState->pBuffer);
         hb_membufAddData(pState->pBuffer, pToken->value, pToken->len);
...
static PHB_PP_TOKEN hb_pp_calcValue(PHB_PP_TOKEN pToken, int iPrecedense, HB_MAXINT * plValue, HB_BOOL * pfError, HB_BOOL * pfUndef)
{
   if( HB_PP_TOKEN_ISEOC(pToken) )
   {
      *pfError = HB_TRUE;
...
static void hb_pp_conditionPush(PHB_PP_STATE pState, HB_BOOL fCond)
{
   if( pState->iCondCount == pState->iCondStackSize )
   {
      ...
      if( pState->pCondStack )
      {
         pState->pCondStack = static_cast<int*>(hb_xrealloc(pState->pCondStack, pState->iCondStackSize * sizeof(HB_BOOL)));
      }
      else
      {
...
static void hb_pp_includeFile(PHB_PP_STATE pState, const char * szFileName, HB_BOOL fSysFile)
{
   ...
   {
      HB_BOOL fNested = HB_FALSE;
      PHB_PP_FILE pFile = hb_pp_FileNew(pState, szFileName, fSysFile, &fNested, nullptr, HB_TRUE, pState->pOpenFunc, HB_FALSE);
      if( pFile )
      {
      ...
      if( HB_PP_TOKEN_ISDIRECTIVE(pState->pFile->pTokenList) )
      {
         ...
         else if( hb_pp_tokenValueCmp(pToken, "INCLUDE", HB_PP_CMP_DBASE) )
         {
            ...
            else if( pToken && HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_LT )
            {
               ...
               if( hb_membufLen(pState->pBuffer) > 0 && !HB_PP_TOKEN_ISEOC(pToken) && HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_GT )
               {
                  hb_membufAddCh(pState->pBuffer, '\0');
         ...
         {
            hb_pp_disp(pState, hb_pp_tokenListStr(pToken->pNext, nullptr, HB_FALSE, pState->pBuffer, HB_FALSE, HB_TRUE));
         }
         else if( hb_pp_tokenValueCmp(pToken, "ERROR", HB_PP_CMP_DBASE) )
         {
            hb_pp_error(pState, 'E', HB_PP_ERR_EXPLICIT, hb_pp_tokenListStr(pToken->pNext, nullptr, HB_FALSE, pState->pBuffer, HB_FALSE, HB_FALSE));
         }
         else if( hb_pp_tokenValueCmp(pToken, "WARNING", HB_PP_CMP_DBASE) )
         {
            hb_pp_error(pState, 'W', HB_PP_WARN_EXPLICIT, hb_pp_tokenListStr(pToken->pNext, nullptr, HB_FALSE, pState->pBuffer, HB_FALSE, HB_FALSE));
         }
...
PHB_PP_TOKEN hb_pp_tokenGet(PHB_PP_STATE pState)
{
   ...
   {
      hb_membufFlush(pState->pBuffer);
      hb_pp_tokenStr(pState->pTokenOut, pState->pBuffer, HB_TRUE, HB_TRUE, pState->usLastType);
      pState->usLastType = HB_PP_TOKEN_TYPE(pState->pTokenOut->type);
      if( fwrite(hb_membufPtr(pState->pBuffer), sizeof(char), hb_membufLen(pState->pBuffer), pState->file_out) != hb_membufLen(pState->pBuffer) )
      {
         hb_pp_error(pState, 'F', HB_PP_ERR_WRITE_FILE, pState->szOutFileName);
      }
   }
...
void hb_pp_readRules(PHB_PP_STATE pState, const char * szRulesFile)
{
   ...
   pState->pFile = hb_pp_FileNew(pState, szFileName, HB_FALSE, nullptr, nullptr, HB_TRUE, pState->pOpenFunc, HB_FALSE);
   if( !pState->pFile )
   {
      pState->pFile = pFile;
...
HB_BOOL hb_pp_inFile(PHB_PP_STATE pState, const char * szFileName, HB_BOOL fSearchPath, FILE * file_in, HB_BOOL fError)
{
   ...
   pState->pFile = hb_pp_FileNew(pState, szFileName, HB_FALSE, nullptr, file_in, fSearchPath, nullptr, HB_FALSE);
   if( pState->pFile )
   {
      pState->iFiles++;
      return HB_TRUE;
...
char * hb_pp_tokenBlockString(PHB_PP_STATE pState, PHB_PP_TOKEN pToken, int * piType, HB_SIZE * pnLen)
{
   *piType = 0;
   ...
   if( HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_LEFT_CB )
   {
      ...
      do
      {
         hb_pp_tokenStr(pToken, pState->pBuffer, ltype != HB_PP_TOKEN_NUL, HB_TRUE, ltype);
         ltype = HB_PP_TOKEN_TYPE(pToken->type);
         switch( ltype )
         {
