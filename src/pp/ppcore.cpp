...
static void hb_pp_error(PHB_PP_STATE pState, char type, int32_t iError, const char *szParam)
{
...
static void hb_pp_operatorsFree(PHB_PP_OPERATOR pOperators, int32_t iOperators)
{
  PHB_PP_OPERATOR pOperator = pOperators;

  while (--iOperators >= 0) {
    hb_xfree(const_cast<char *>(pOperator->name));
    hb_xfree(const_cast<char *>(pOperator->value));
    ++pOperator;
  }
  hb_xfree(pOperators);
}
...
static void hb_pp_tokenFree(HB_PP_TOKEN *pToken)
{
  ...
  if (HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_MMARKER_RESTRICT ||
    ...
    while (pToken->pMTokens) {
      HB_PP_TOKEN *pMTokens = pToken->pMTokens;
...
static void hb_pp_tokenListFree(HB_PP_TOKEN **pTokenPtr)
{
  if (*pTokenPtr && !HB_PP_TOKEN_ISPREDEF(*pTokenPtr)) {
    do {
      HB_PP_TOKEN *pToken = *pTokenPtr;
...
static int32_t hb_pp_tokenListFreeCmd(HB_PP_TOKEN **pTokenPtr)
{
  auto fStop = false;
  int32_t iLines = 0;
...
static void hb_pp_tokenMoveCommand(PHB_PP_STATE pState, HB_PP_TOKEN **pDestPtr, HB_PP_TOKEN **pSrcPtr)
{
  HB_PP_TOKEN *pToken;
  int32_t iLines = 0;
...
static HB_PP_TOKEN *hb_pp_tokenResultEnd(HB_PP_TOKEN **pTokenPtr, bool fDirect)
{
  HB_PP_TOKEN *pNext = nullptr;
...
static HB_PP_TOKEN *hb_pp_tokenNew(const char *value, HB_SIZE nLen, HB_SIZE nSpaces, uint16_t type)
{
  auto pToken = static_cast<HB_PP_TOKEN *>(hb_xgrab(sizeof(HB_PP_TOKEN)));
...
static void hb_pp_tokenSetValue(HB_PP_TOKEN *pToken, const char *value, HB_SIZE nLen)
{
  if (HB_PP_TOKEN_ALLOC(pToken->type)) {
    hb_xfree(const_cast<char *>(pToken->value));
  }
...
static HB_PP_TOKEN *hb_pp_tokenClone(HB_PP_TOKEN *pSource)
{
  auto pDest = static_cast<HB_PP_TOKEN *>(hb_xgrab(sizeof(HB_PP_TOKEN)));
...
static void hb_pp_tokenAdd(HB_PP_TOKEN ***pTokenPtr, const char *value, HB_SIZE nLen, HB_SIZE nSpaces, uint16_t type)
{
  HB_PP_TOKEN *pToken = hb_pp_tokenNew(value, nLen, nSpaces, type);
...
static void hb_pp_tokenAddNext(PHB_PP_STATE pState, const char *value, HB_SIZE nLen, uint16_t type)
{
...
static void hb_pp_tokenAddStreamFunc(PHB_PP_STATE pState, HB_PP_TOKEN *pToken, const char *value, HB_SIZE nLen)
{
  while (pToken) {
...
{
  int32_t ch, iLine = 0, iBOM = pState->pFile->iCurrentLine == 0 ? 1 : 0;
...
static bool hb_pp_hasCommand(char *pBuffer, HB_SIZE nLen, HB_SIZE *pnAt, int32_t iCmds, ...)
{
  HB_SIZE n = 0;
  va_list va;
  int32_t i;
...
{
  HB_PP_TOKEN **pInLinePtr, **pEolTokenPtr;
  char *pBuffer;
  auto fDump = false;
  int32_t iLines = 0, iStartLine;
  ...
  do {
      ...
      if (pState->iStreamDump) {
        ...
        if (n == nLen) {
          ...
          ++n;
                 ...
                 hb_pp_canQuote(pState->fCanNextLine || HB_PP_TOKEN_CANQUOTE(pState->usLastType), pBuffer, nLen, 1,
                                &n)) {
        hb_pp_tokenAddNext(pState, pBuffer + 1, n - 1, HB_PP_TOKEN_STRING);
        ++n;
    ...
    if (pEolTokenPtr && (pEolTokenPtr != pState->pNextTokenPtr ||
      ...
      HB_PP_TOKEN *pToken = *pEolTokenPtr;
...
static int32_t hb_pp_tokenStr(HB_PP_TOKEN *pToken, PHB_MEM_BUFFER pBuffer, bool fSpaces, bool fQuote, uint16_t ltype)
{
  int32_t iLines = 0;
...
static bool hb_pp_tokenValueCmp(HB_PP_TOKEN *pToken, const char *szValue, uint16_t mode)
{
  if (pToken->len) {
...
static bool hb_pp_tokenEqual(HB_PP_TOKEN *pToken, HB_PP_TOKEN *pMatch, uint16_t mode)
{
...
static bool hb_pp_patternAddResult(PHB_PP_RULE pRule, uint16_t marker, HB_PP_TOKEN *pFirst, HB_PP_TOKEN *pNext)
{
...
static PHB_PP_RULE hb_pp_ruleNew(HB_PP_TOKEN *pMatch, HB_PP_TOKEN *pResult, uint16_t mode, uint16_t markers,
...
static void hb_pp_ruleSetId(PHB_PP_STATE pState, HB_PP_TOKEN *pMatch, uint8_t id)
{
...
static void hb_pp_ruleListSetId(PHB_PP_STATE pState, PHB_PP_RULE pRule, uint8_t id)
{
  while (pRule) {
...
static PHB_PP_RULE hb_pp_defineFind(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
...
static void hb_pp_defineAdd(PHB_PP_STATE pState, uint16_t mode, uint16_t markers, PHB_PP_MARKER pMarkers,
                            HB_PP_TOKEN *pMatch, HB_PP_TOKEN *pResult)
{
...
static void hb_pp_defineDel(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
                                 ...
                                 FILE *file_in, HB_BOOL fSearchPath, PHB_PP_OPEN_FUNC pOpenFunc, HB_BOOL fBinary)
{
  ...
  if (!file_in) {
    int32_t iAction = HB_PP_OPEN_FILE;
      ...
      if (!fSysFile) {
        ...
        if (!file_in &&
          ...
          if (szFirstFName != nullptr) {
            auto pFirstFName = hb_fsFNameSplit(szFirstFName);
...
static void hb_pp_FileFree(PHB_PP_STATE pState, PHB_PP_FILE pFile, PHB_PP_CLOSE_FUNC pCloseFunc)
{
  ...
  if (pFile->fFree && pFile->pLineBuf) {
    hb_xfree(const_cast<char *>(pFile->pLineBuf));
  }
...
static HB_PP_TOKEN *hb_pp_streamFuncGet(HB_PP_TOKEN *pToken, HB_PP_TOKEN **pFuncPtr)
{
  hb_pp_tokenListFree(pFuncPtr);

  if (pToken && HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_PIPE && !HB_PP_TOKEN_ISEOC(pToken->pNext)) {
    HB_PP_TOKEN **pStartPtr, **pEndPtr, *pStart, *pNext;
...
static bool hb_pp_pragmaStream(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
  auto fError = false;
...
static bool hb_pp_pragmaOperatorNew(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
  auto fError = true;
...
static bool hb_pp_setCompilerSwitch(PHB_PP_STATE pState, const char *szSwitch, int32_t iValue)
{
  auto fError = true;
...
static bool hb_pp_getCompilerSwitch(PHB_PP_STATE pState, const char *szSwitch, int32_t *piValue)
{
  auto fError = true;
...
static HB_PP_TOKEN *hb_pp_pragmaGetLogical(HB_PP_TOKEN *pToken, HB_BOOL *pfValue)
{
  HB_PP_TOKEN *pValue = nullptr;
...
static HB_PP_TOKEN *hb_pp_pragmaGetInt(HB_PP_TOKEN *pToken, int32_t *piValue)
{
  HB_PP_TOKEN *pValue = nullptr;
...
static HB_PP_TOKEN *hb_pp_pragmaGetSwitch(HB_PP_TOKEN *pToken, int32_t *piValue)
{
  HB_PP_TOKEN *pValue = nullptr;
...
static void hb_pp_pragmaNew(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
  HB_PP_TOKEN *pValue = nullptr;
  auto fError = false;
...
static void hb_pp_defineNew(PHB_PP_STATE pState, HB_PP_TOKEN *pToken, HB_BOOL fDirect)
{
  HB_PP_TOKEN *pMatch = pToken ? pToken->pNext : nullptr;

  if (!pMatch || HB_PP_TOKEN_TYPE(pMatch->type) != HB_PP_TOKEN_KEYWORD) {
    hb_pp_error(pState, 'E', HB_PP_ERR_DEFINE_SYNTAX, nullptr);
  } else {
    HB_PP_TOKEN *pResult, *pLast = pMatch->pNext, *pParam;
    PHB_PP_MARKER pMarkers = nullptr;
    uint16_t usPCount = 0, usParam;

    // pseudo function?
    if (pLast && HB_PP_TOKEN_TYPE(pLast->type) == HB_PP_TOKEN_LEFT_PB && pLast->spaces == 0) {
      uint16_t type = HB_PP_TOKEN_KEYWORD;
      for (;;) {
        pLast = pLast->pNext;
...
static bool hb_pp_tokenUnQuotedGet(HB_PP_TOKEN ***pTokenPtr, bool *pfQuoted, bool fFree)
{
  HB_PP_TOKEN *pToken = **pTokenPtr;
...
static bool hb_pp_matchMarkerNew(HB_PP_TOKEN **pTokenPtr, PHB_PP_MARKERLST *pMarkerListPtr)
{
  uint16_t type = HB_PP_TOKEN_NUL;
  HB_PP_TOKEN *pMarkerId = nullptr, *pMTokens = nullptr;
  bool fQuoted;
...
static bool hb_pp_matchHasKeywords(HB_PP_TOKEN *pToken)
{
...
static bool hb_pp_matchPatternNew(PHB_PP_STATE pState, HB_PP_TOKEN **pTokenPtr, PHB_PP_MARKERLST *pMarkerListPtr,
                                  HB_PP_TOKEN ***pOptional)
{
  HB_PP_TOKEN **pLastPtr = nullptr;
  bool fQuoted = false;
  ...
  do {
    if (!fQuoted) {
      if (HB_PP_TOKEN_TYPE((*pTokenPtr)->type) == HB_PP_TOKEN_LT) {
        ...
        HB_PP_TOKEN **pStopOptPtr = nullptr;
        if (!(*pTokenPtr)->pNext) {
          ...
          HB_PP_TOKEN *pToken, *pOptTok = (*pTokenPtr)->pNext;
          pToken = *pStopOptPtr;
          *pStopOptPtr = nullptr;
...
static bool hb_pp_resultMarkerNew(PHB_PP_STATE pState, HB_PP_TOKEN **pTokenPtr, PHB_PP_MARKERLST *pMarkerListPtr,
                                  HB_BOOL fDump, HB_BOOL fOptional, uint16_t *pusPCount, HB_SIZE spaces)
{
  uint16_t type = HB_PP_TOKEN_NUL, rtype;
  HB_PP_TOKEN *pMarkerId = nullptr, *pToken;
  bool fQuoted;
...
static bool hb_pp_patternCompare(HB_PP_TOKEN *pToken1, HB_PP_TOKEN *pToken2)
{
...
static void hb_pp_directiveDel(PHB_PP_STATE pState, HB_PP_TOKEN *pMatch, uint16_t markers, PHB_PP_MARKER pMarkers,
                               uint16_t mode, HB_BOOL fCommand)
{
...
static void hb_pp_directiveNew(PHB_PP_STATE pState, HB_PP_TOKEN *pToken, uint16_t mode, HB_BOOL fCommand,
                               HB_BOOL fDirect, HB_BOOL fDelete)
{
  HB_PP_TOKEN *pResult, *pMatch, *pStart, *pLast;
  auto fValid = false;
  ...
  if (!fValid) {
    ...
    if (fValid) {
      if (pResult) {
        HB_PP_TOKEN **pTokenPtr, **pDumpPtr = nullptr, **pOptStart = nullptr;
        bool fQuoted = false;
...
static bool hb_pp_tokenStartExtBlock(HB_PP_TOKEN **pTokenPtr)
{
  HB_PP_TOKEN *pToken = *pTokenPtr;
  ...
  if (pToken && HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_LEFT_CB && pToken->pNext &&
    ...
    uint16_t prevtype = HB_PP_TOKEN_COMMA;
    pToken = pToken->pNext->pNext;
    while (pToken) {
      uint16_t type = HB_PP_TOKEN_TYPE(pToken->type);
...
static bool hb_pp_tokenStopExtBlock(HB_PP_TOKEN **pTokenPtr)
{
  HB_PP_TOKEN *pToken = *pTokenPtr;
...
static bool hb_pp_tokenSkipExp(HB_PP_TOKEN **pTokenPtr, HB_PP_TOKEN *pStop, uint16_t mode, HB_BOOL *pfStop)
{
  uint16_t curtype, prevtype = 0, lbrtype = 0, rbrtype = 0;
  HB_PP_TOKEN *pToken = *pTokenPtr, *pPrev;
  int32_t iBraces = 0;
  auto fMatch = false;
...
static bool hb_pp_tokenMatch(HB_PP_TOKEN *pMatch, HB_PP_TOKEN **pTokenPtr, HB_PP_TOKEN *pStop, uint16_t mode)
{
  auto fMatch = false;
  uint16_t type;
  ...
  if (type == HB_PP_MMARKER_REGULAR) {
    ...
    HB_PP_TOKEN *pRestrict = pMatch->pMTokens, *pToken = *pTokenPtr;
...
static bool hb_pp_patternMatch(HB_PP_TOKEN *pMatch, HB_PP_TOKEN **pTokenPtr, HB_PP_TOKEN *pStop, uint16_t mode,
                               PHB_PP_RULE pRule)
{
  HB_PP_TOKEN *pToken = *pTokenPtr;
  HB_PP_TOKEN *pFirst;
  ...
  while (pMatch && !HB_PP_TOKEN_ISEOS(pToken)) {
    if (HB_PP_TOKEN_TYPE(pMatch->type) == HB_PP_MMARKER_OPTIONAL) {
      HB_PP_TOKEN *pOptional = pMatch, *pLast, *pNewStop = pMatch->pNext;
...
static bool hb_pp_patternCmp(PHB_PP_RULE pRule, HB_PP_TOKEN *pToken, HB_BOOL fCommand)
{
  HB_PP_TOKEN *pFirst = pToken;
...
static PHB_PP_RESULT hb_pp_matchResultGet(PHB_PP_RULE pRule, uint16_t usMatch, uint16_t usIndex)
{
...
static HB_PP_TOKEN **hb_pp_matchResultLstAdd(PHB_PP_STATE pState, HB_SIZE spaces, uint16_t type,
                                             HB_PP_TOKEN **pResultPtr, HB_PP_TOKEN *pToken, HB_PP_TOKEN *pStop)
{
  HB_PP_TOKEN *pNext;
  auto fFirst = true;
  HB_BOOL fStop = false;
...
static HB_PP_TOKEN **hb_pp_matchResultAdd(PHB_PP_STATE pState, PHB_PP_RULE pRule, HB_PP_TOKEN **pResultPtr,
                                          HB_PP_TOKEN *pMatch, uint16_t usMatch)
{
  PHB_PP_RESULT pMarkerResult = hb_pp_matchResultGet(pRule, usMatch, pMatch->index);
  HB_PP_TOKEN *pToken, *pStop;
...
static HB_PP_TOKEN **hb_pp_patternStuff(PHB_PP_STATE pState, PHB_PP_RULE pRule, uint16_t usMatch,
                                        HB_PP_TOKEN *pResultPattern, HB_PP_TOKEN **pResultPtr)
{
  while (pResultPattern) {
    if (pResultPattern->index) {
      ...
      uint16_t usMaxMatch = 0, matches;
      HB_PP_TOKEN *pToken = pResultPattern->pMTokens;
      while (pToken) {
        if (pToken->index) {
      ...
      HB_PP_TOKEN **pTokenPtr = pResultPtr;
...
static char *hb_pp_tokenListStr(HB_PP_TOKEN *pToken, HB_PP_TOKEN *pStop, HB_BOOL fStop, PHB_MEM_BUFFER pBuffer,
                                HB_BOOL fQuote, HB_BOOL fEol)
{
  uint16_t ltype = HB_PP_TOKEN_NUL;
  auto fSpaces = false;
...
static void hb_pp_patternReplace(PHB_PP_STATE pState, PHB_PP_RULE pRule, HB_PP_TOKEN **pTokenPtr, const char *szType)
{
  HB_PP_TOKEN *pFinalResult = nullptr, **pResultPtr, *pSource;
...
static void hb_pp_processCondDefined(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
  HB_PP_TOKEN *pNext;
...
static bool hb_pp_processDefine(PHB_PP_STATE pState, HB_PP_TOKEN **pFirstPtr)
{
  HB_PP_TOKEN **pPrevPtr;
  auto fSubst = false;
  auto fRepeat = false;
  int32_t iCycle = 0;
...
static bool hb_pp_processTranslate(PHB_PP_STATE pState, HB_PP_TOKEN **pFirstPtr)
{
  auto fSubst = false;
  auto fRepeat = false;
  int32_t iCycle = 0;
  ...
  do {
    HB_PP_TOKEN **pTokenPtr = pFirstPtr;
    fRepeat = false;
    while (!HB_PP_TOKEN_ISEOS(*pTokenPtr)) {
...
static bool hb_pp_processCommand(PHB_PP_STATE pState, HB_PP_TOKEN **pFirstPtr)
{
  PHB_PP_RULE pRule;
  auto fSubst = false;
  auto fRepeat = true;
  ...
  if (!HB_PP_TOKEN_ISEOC(*pFirstPtr) && hb_pp_tokenValueCmp(*pFirstPtr, "TEXT", HB_PP_CMP_DBASE)) {
    HB_PP_TOKEN *pToken = (*pFirstPtr)->pNext, **pFuncPtr;
...
static bool hb_pp_concatenateKeywords(PHB_PP_STATE pState, HB_PP_TOKEN **pFirstPtr)
{
  HB_PP_TOKEN *pToken = *pFirstPtr, *pNext;
  auto fChanged = false;
...
static HB_PP_TOKEN *hb_pp_calcPrecedence(HB_PP_TOKEN *pToken, int32_t *piNextOper, int32_t *piNextPrec)
{
...
static bool hb_pp_calcReduce(HB_MAXINT *plValue, int32_t iOperation)
{
  switch (iOperation) {
  case HB_PP_TOKEN_AND:
    if (*plValue == 0) {
...
static HB_MAXINT hb_pp_calcOperation(HB_MAXINT lValueLeft, HB_MAXINT lValueRight, int32_t iOperation, HB_BOOL *pfError)
{
  switch (iOperation) {
  case HB_PP_TOKEN_EQUAL:
...
static HB_PP_TOKEN *hb_pp_calcValue(HB_PP_TOKEN *pToken, int32_t iPrecedense, HB_MAXINT *plValue, HB_BOOL *pfError,
                                    HB_BOOL *pfUndef)
{
  if (HB_PP_TOKEN_ISEOC(pToken)) {
    *pfError = true;
...
static HB_MAXINT hb_pp_calculateValue(PHB_PP_STATE pState, HB_PP_TOKEN *pToken, HB_BOOL fNoError)
{
...
static void hb_pp_condCompile(PHB_PP_STATE pState, HB_PP_TOKEN *pToken, HB_BOOL fNot)
{
...
static void hb_pp_condCompileIf(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
...
static void hb_pp_condCompileElif(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
...
static void hb_pp_lineTokens(HB_PP_TOKEN ***pTokenPtr, const char *szFileName, int32_t iLine)
{
  char szLine[12];
...
static void hb_pp_preprocessToken(PHB_PP_STATE pState)
{
  while (!pState->pTokenOut && pState->pFile) {
    ...
    if (HB_PP_TOKEN_ISDIRECTIVE(pState->pFile->pTokenList)) {
      ...
      HB_PP_TOKEN **pFreePtr = &pState->pFile->pTokenList;
      HB_PP_TOKEN *pToken = *pFreePtr;
...
void hb_pp_initRules(PHB_PP_RULE *pRulesPtr, int32_t *piRules, const HB_PP_DEFRULE pDefRules[], int32_t iDefRules)
{
  PHB_PP_MARKER pMarkers;
  PHB_PP_RULE pRule;
...
HB_PP_TOKEN *hb_pp_tokenGet(PHB_PP_STATE pState)
{
  pState->fError = false;

  if (pState->pTokenOut) {
    HB_PP_TOKEN *pToken = pState->pTokenOut;
...
void hb_pp_init(PHB_PP_STATE pState, HB_BOOL fQuiet, HB_BOOL fGauge, int32_t iCycles, void *cargo,
...
void hb_pp_readRules(PHB_PP_STATE pState, const char *szRulesFile)
{
  ...
  auto pFileName = hb_fsFNameSplit(szRulesFile);
...
HB_BOOL hb_pp_inBuffer(PHB_PP_STATE pState, const char *szFileName, const char *pBuffer, HB_SIZE nLen, int32_t iStartLine)
{
  hb_pp_InFileFree(pState);
...
int32_t hb_pp_errorCount(PHB_PP_STATE pState)
{
...
int32_t hb_pp_line(PHB_PP_STATE pState)
{
  if (pState->pFile) {
...
int32_t hb_pp_lineTot(PHB_PP_STATE pState)
{
...
{
  HB_PP_TOKEN *pMatch, *pResult, *pToken;
  PHB_PP_FILE pFile;
...
// set stream mode
void hb_pp_setStream(PHB_PP_STATE pState, int32_t iMode)
{
  pState->fError = false;
  switch (iMode) {
...
HB_PP_TOKEN *hb_pp_lexGet(PHB_PP_STATE pState)
{
  HB_PP_TOKEN *pToken = *pState->pNextTokenPtr;
...
HB_BOOL hb_pp_tokenNextExp(HB_PP_TOKEN **pTokenPtr)
{
  if (hb_pp_tokenCanStartExp(*pTokenPtr)) {
    HB_BOOL fStop = false;
...
void hb_pp_tokenUpper(HB_PP_TOKEN *pToken)
{
  if (HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_MACROVAR) {
    ...
    if (pToken->len <= 1) {
      ...
      if (!HB_PP_TOKEN_ALLOC(pToken->type)) {
        ...
        memmove(const_cast<char *>(pToken->value), pToken->value + 1, pToken->len);
      }
      (static_cast<char *>(const_cast<char *>(pToken->value)))[pToken->len] = '\0';
    }
    ...
    if (HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_KEYWORD && pToken->len > HB_SYMBOL_NAME_LEN) {
      ...
      (static_cast<char *>(const_cast<char *>(pToken->value)))[HB_SYMBOL_NAME_LEN] = '\0';
    }
  }
  ...
  if (pToken->len <= 1) {
    ...
    hb_strupr(static_cast<char *>(const_cast<char *>(pToken->value)));
  }
}

// convert tokens between '[' and ']' tokens into single string token
// and replace the converted tokens with the new string
void hb_pp_tokenToString(PHB_PP_STATE pState, HB_PP_TOKEN *pToken)
{
  auto fError = true;
  ...
  hb_membufFlush(pState->pBuffer);
  if (HB_PP_TOKEN_TYPE(pToken->type) == HB_PP_TOKEN_LEFT_SB) {
    HB_PP_TOKEN *pTok, *pFirst, *pLast = nullptr;
...
char *hb_pp_tokenBlockString(PHB_PP_STATE pState, HB_PP_TOKEN *pToken, int32_t *piType, HB_SIZE *pnLen)
{
  *piType = 0;
  hb_membufFlush(pState->pBuffer);
