...
using PHB_CODETRACE_INFO = HB_CODETRACE_INFO *;

#define HB_CODETRACE_FUNC(func) HB_PCODE_FUNC(func, PHB_CODETRACE_INFO)
typedef HB_CODETRACE_FUNC(HB_CODETRACE_FUNC_);
using PHB_CODETRACE_FUNC = HB_CODETRACE_FUNC_ *;

constexpr HB_SIZE HB_JUMPADDR_ALLOC = 64; // #define HB_JUMPADDR_ALLOC 64
...
void hb_compCodeTraceMarkDead(HB_COMP_DECL, HB_HFUNC *pFunc)
{
  ...
  code_info.pCodeMark = static_cast<uint8_t *>(hb_xgrabz(code_info.nPCodeSize));
  ...
  if (code_info.fFinished) {
    ...
    uint8_t bLastCode = HB_P_LAST_PCODE;
