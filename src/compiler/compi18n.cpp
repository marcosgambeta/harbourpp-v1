...
void hb_compI18nFree(HB_COMP_DECL)
{
  ...
  if (pI18n) {
    if (pI18n->pString) {
      for (uint32_t ui = 0; ui < pI18n->uiCount; ui++) {
...
static HB_I18NSTRING *hb_compI18nAddSingle(HB_COMP_DECL, const char *szText, const char *szContext,
                                           const char *szModule, uint32_t uiLine)
{
  ...
  uint32_t uiLeft = 0;
  uint32_t uiRight = pI18n->uiCount;

  HB_I18NSTRING *pString;

  while (uiLeft < uiRight) {
    uint32_t uiMiddle = (uiLeft + uiRight) >> 1;
...
void hb_compI18nAdd(HB_COMP_DECL, const char *szText, const char *szContext, const char *szModule, uint32_t uiLine)
{
                          ...
                          const char *szModule, uint32_t uiLine)
{
  ...
  if (ulCount == 1) {
    ...
    for (HB_ULONG ul = 1; ul < ulCount && pString->uiPlurals < HB_I18N_PLURAL_MAX; ++ul) {
      ...
      uint32_t uiPlural = pString->uiPlurals;
...
HB_BOOL hb_compI18nSave(HB_COMP_DECL, HB_BOOL fFinal)
{
  ...
  for (uint32_t uiIndex = 0; uiIndex < pI18n->uiCount; uiIndex++) {
    ...
    for (uint32_t uiLine = 0; uiLine < pString->uiPosCount; ++uiLine) {
    ...
    for (uint32_t uiLine = 0; uiLine < pString->uiPlurals; ++uiLine) {
      if (uiLine == 0) {
        fprintf(file, "\"\nmsgid_plural \"");
      } else {
