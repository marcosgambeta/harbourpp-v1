...
static HB_GENC_FUNC(hb_p_pushdouble)
{
   ...
   fprintf(cargo->yyc, ", %u, %u);\n", pFunc->pCode[nPCodePos + 1 + sizeof(double)], pFunc->pCode[nPCodePos + 1 + sizeof(double) + sizeof(HB_BYTE)]);
#endif
...
static HB_GENC_FUNC(hb_p_pushlocal)
{
   ...
   if( HB_GENC_GETLABEL(nPCodePos + 3) == 0 ) {
      switch( pFunc->pCode[nPCodePos + 3] ) {
         case HB_P_POPLOCALNEAR:
            hb_gencc_copyLocals(cargo->yyc, HB_PCODE_MKSHORT(&pFunc->pCode[nPCodePos + 1]), static_cast<signed char>(pFunc->pCode[nPCodePos + 4]));
            return 5;

         case HB_P_POPLOCAL:
            hb_gencc_copyLocals(cargo->yyc, HB_PCODE_MKSHORT(&pFunc->pCode[nPCodePos + 1]), HB_PCODE_MKSHORT(&pFunc->pCode[nPCodePos + 4]));
            return 6;
      }
   }
...
static HB_GENC_FUNC(hb_p_pushlocalnear)
{
   ...
   if( HB_GENC_GETLABEL(nPCodePos + 2) == 0 ) {
      switch( pFunc->pCode[nPCodePos + 2] ) {
         case HB_P_POPLOCALNEAR:
            hb_gencc_copyLocals(cargo->yyc, static_cast<signed char>(pFunc->pCode[nPCodePos + 1]), static_cast<signed char>(pFunc->pCode[nPCodePos + 3]));
            return 4;

         case HB_P_POPLOCAL:
            hb_gencc_copyLocals(cargo->yyc, static_cast<signed char>(pFunc->pCode[nPCodePos + 1]), HB_PCODE_MKSHORT(&pFunc->pCode[nPCodePos + 3]));
            return 5;
      }
   }
