...
#define HB_PP_LEX_SELF(t)                                                                                              \
  (HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_SEND && (t)->pNext && (t)->pNext->spaces == 0 &&                         \
   HB_PP_TOKEN_TYPE((t)->pNext->type) == HB_PP_TOKEN_SEND)

#define HB_PP_LEX_NEEDLEFT(t)                                                                                          \
  (HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_ASSIGN || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_PLUSEQ ||           \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_MINUSEQ || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_MULTEQ ||          \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_DIVEQ || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_MODEQ ||             \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_EXPEQ || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_EQUAL ||             \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_EQ || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_ALIAS ||                \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_MULT || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_DIV ||                \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_MOD || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_POWER ||               \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_IN || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_AND ||                  \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_OR || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_PIPE ||                 \
   HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_RIGHT_PB || HB_PP_TOKEN_TYPE((t)->type) == HB_PP_TOKEN_RIGHT_SB ||       \
...
static const char *hb_comp_tokenString(YYSTYPE *yylval_ptr, HB_COMP_DECL, PHB_PP_TOKEN pToken)
{
  ...
  if (HB_PP_TOKEN_ALLOC(pToken->type))
  {
    ...
    pToken->value =
        hb_compIdentifierNew(HB_COMP_PARAM, pToken->value, yylval_ptr->valChar.dealloc ? HB_IDENT_COPY : HB_IDENT_FREE);
...
static bool hb_comp_timeDecode(PHB_PP_TOKEN pTime, long *plTime)
{
  ...
  if (HB_PP_TOKEN_TYPE(pTime->type) == HB_PP_TOKEN_SEND)
  {
    ...
    if (hb_compStrToNum(pTime->value, pTime->len, &lMilliSec, &dNumber, &iDec, &iWidth))
    {
  ...
  {
    if ((pTime->len == 1 && (pTime->value[0] == 'A' || pTime->value[0] == 'a')) ||
    ...
    else if ((pTime->len == 1 && (pTime->value[0] == 'P' || pTime->value[0] == 'p')) ||
...
static int hb_comp_dayTimeDecode(PHB_COMP_LEX pLex, PHB_PP_TOKEN pToken, YYSTYPE *yylval_ptr)
{
  ...
  if (pYear && HB_PP_TOKEN_TYPE(pYear->type) == HB_PP_TOKEN_NUMBER && pYear->pNext)
  {
    ...
    else if (HB_PP_TOKEN_TYPE(pYear->pNext->type) == HB_PP_TOKEN_NUMBER && pYear->pNext->pNext)
    {
      if (hb_compStrToNum(pYear->value, pYear->len, &lYear, &dNumber, &iDec, &iWidth))
      {
        if (iDec == 2)
        {
          ...
          pDay = pYear->pNext;
          if (hb_compStrToNum(pDay->value, pDay->len, &lDay, &dNumber, &iDec, &iWidth))
          {
            if (iDec == 2)
            {
...
static int hb_comp_funcStart(HB_COMP_DECL, YYSTYPE *yylval_ptr)
{
  HB_COMP_PARAM->pLex->iClose =
      ...
      HB_COMP_PARAM->functions.pLast->wWhileCounter + HB_COMP_PARAM->functions.pLast->wCaseCounter +
      HB_COMP_PARAM->functions.pLast->wSwitchCounter + HB_COMP_PARAM->functions.pLast->wWithObjectCnt +
...
int hb_comp_yylex(YYSTYPE *yylval_ptr, HB_COMP_DECL)
{
  ...
  case HB_PP_TOKEN_DATE:
  {
    ...
    if (yylval_ptr->valLong.lNumber == 0 && (iYear != 0 || iMonth != 0 || iDay != 0))
    {
    ...
    if (pToken->pNext)
    {
      if (HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_PIPE)
      {
        yylval_ptr->asCodeblock.string = hb_strdup(
            hb_pp_tokenBlockString(pLex->pPP, pToken, &yylval_ptr->asCodeblock.flags, &yylval_ptr->asCodeblock.length));
  ...
  case HB_PP_TOKEN_DIRECTIVE:
    if (pLex->iState == LOOKUP && pToken->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD &&
  ...
  case HB_PP_TOKEN_KEYWORD:
  {
    ...
    case PROCEDURE:
      if (!HB_SUPPORT_HARBOUR ||
          (pLex->iState == LOOKUP && pToken->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD))
      {
    ...
    case STATIC:
      if (pLex->iState == LOOKUP && pToken->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD &&
          pToken->pNext->len >= 4 &&
          (hb_strnicmp("FUNCTION", pToken->pNext->value, pToken->pNext->len) == 0 ||
           hb_strnicmp("PROCEDURE", pToken->pNext->value, pToken->pNext->len) == 0))
      {
        ...
        return hb_comp_funcStart(HB_COMP_PARAM, yylval_ptr);
               ...
               ((iType == EXIT && HB_PP_TOKEN_ISEOC(pToken->pNext)) ||
                (iType == STATIC && pToken->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD)))
      {
        pLex->iState = iType;
        return iType;
      }
    ...
    case THREAD_STATIC:
      if (pLex->iState == LOOKUP && pToken->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD &&
          pToken->pNext->len >= 4 && hb_strnicmp("STATIC", pToken->pNext->value, pToken->pNext->len) == 0)
      {
        hb_pp_tokenGet(pLex->pPP);
    ...
    case RECOVER:
      if (pLex->iState == LOOKUP)
      {
        ...
        else if (pToken->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD &&
    ...
    case END:
      if (pLex->iState == LOOKUP)
      {
        ...
        else if (HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD && pToken->pNext->len >= 4 &&
                 pToken->pNext->len <= 8 && hb_strnicmp("SEQUENCE", pToken->pNext->value, pToken->pNext->len) == 0)
        {
          hb_pp_tokenGet(pLex->pPP);
          pLex->iState = ENDSEQ;
          return ENDSEQ;
        ...
        if (!HB_SUPPORT_HARBOUR && HB_COMP_PARAM->iSyntaxCheckOnly < 2)
        {
          ...
          {
            hb_compGenError(HB_COMP_PARAM, hb_comp_szErrors, 'E', HB_COMP_ERR_ENDIF, nullptr, nullptr);
          }
        }
      }
      iType = IDENTIFIER;
    ...
    case CASE:
      if (HB_SUPPORT_HARBOUR &&
          (pLex->iState != LOOKUP || HB_PP_TOKEN_ISEOC(pToken->pNext) || HB_PP_LEX_NEEDLEFT(pToken->pNext)))
      {
        iType = IDENTIFIER;
        break;
      }
    ...
    case FOR:
      if (pLex->iState == LOOKUP && !HB_PP_TOKEN_ISEOC(pToken->pNext))
      {
        ...
        if (!hb_pp_tokenNextExp(&pNext) && pNext && HB_PP_TOKEN_TYPE(pNext->type) == HB_PP_TOKEN_KEYWORD &&
    ...
    case NEXT:
      ...
      {
        if (HB_PP_TOKEN_ISEOC(pToken->pNext) ||
            (HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD && HB_PP_TOKEN_ISEOC(pToken->pNext->pNext)))
        {
          pLex->iState = iType;
          return iType;
        }
        if (!HB_SUPPORT_HARBOUR && HB_COMP_PARAM->iSyntaxCheckOnly < 2)
        {
          ...
          if (HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_LEFT_PB ||
              ...
              HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_ALIAS)
          {
            hb_compGenError(HB_COMP_PARAM, hb_comp_szErrors, 'E', HB_COMP_ERR_NEXTFOR, nullptr, nullptr);
          }
        }
      }
      iType = IDENTIFIER;
    ...
    case DO:
      if (pLex->iState == LOOKUP && !HB_PP_TOKEN_ISEOC(pToken->pNext))
      {
        ...
        {
          if (pToken->pNext->len == 4 && hb_stricmp("CASE", pToken->pNext->value) == 0)
          {
            if (HB_PP_TOKEN_ISEOC(pToken->pNext->pNext))
            {
              hb_pp_tokenGet(pLex->pPP);
...
/* do not upper next token for case sensitive file systems */
    ...
    case WITH:
      if (!HB_PP_TOKEN_ISEOC(pToken->pNext))
      {
        if (pLex->iState == LOOKUP && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD &&
            pToken->pNext->len >= 4 && hb_strnicmp("OBJECT", pToken->pNext->value, pToken->pNext->len) == 0)
        {
          hb_pp_tokenGet(pLex->pPP);
          pLex->iState = WITHOBJECT;
          return WITHOBJECT;
        }
        else if (pLex->iState == MACROVAR || pLex->iState == MACROTEXT || pLex->iState == IDENTIFIER ||
                 pLex->iState == BEGINSEQ)
        {
          pLex->iState = WITH;
          return WITH;
    ...
    case FIELD:
      if (pToken->pNext && ((pLex->iState == LOOKUP && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_KEYWORD) ||
                            HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_ALIAS))
      {
        pLex->iState = FIELD;
        return FIELD;
    ...
    case SELF:
      if (pToken->pNext && pToken->pNext->pNext && HB_PP_TOKEN_TYPE(pToken->pNext->type) == HB_PP_TOKEN_LEFT_PB &&
