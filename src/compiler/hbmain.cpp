...
static int32_t hb_compCompile(HB_COMP_DECL, const char *szPrg, const char *szBuffer, int32_t iStartLine);
static bool hb_compRegisterFunc(HB_COMP_DECL, HB_HFUNC *pFunc, bool fError);

// *************************************************************************

int32_t hb_compMainExt(int32_t argc, const char *const argv[], uint8_t **pBufPtr, HB_SIZE *pnSize, const char *szSource,
                   int32_t iStartLine, void *cargo, PHB_PP_OPEN_FUNC pOpenFunc, PHB_PP_MSG_FUNC pMsgFunc)
{
#if 0
  ...
  HB_COMP_DECL;
  int32_t iStatus = EXIT_SUCCESS, iFileCount = 0;

  int32_t iFileCase = hb_setGetFileCase();
  int32_t iDirCase = hb_setGetDirCase();
  int32_t iDirSep = hb_setGetDirSeparator();
  bool fTrimFN = hb_setGetTrimFileName();
  ...
  if (!HB_COMP_PARAM->fExit) {
    if (HB_COMP_PARAM->iTraceInclude == 0 && HB_COMP_PARAM->iSyntaxCheckOnly == 2) {
      HB_COMP_PARAM->iTraceInclude = 1;
    }

    if (pBufPtr && pnSize) {
      if (HB_COMP_PARAM->iTraceInclude > 0 && HB_COMP_PARAM->iSyntaxCheckOnly > 0) {
  ...
  if (iFileCount == 0 && !HB_COMP_PARAM->fQuiet && !HB_COMP_PARAM->fExit && !HB_COMP_PARAM->fBuildInfo &&
      !HB_COMP_PARAM->fCredits) {
...
int32_t hb_compMain(int32_t argc, const char *const argv[])
{
  return hb_compMainExt(argc, argv, nullptr, nullptr, nullptr, 0, nullptr, nullptr, nullptr);
}

static int32_t hb_compReadClpFile(HB_COMP_DECL, const char *szClpFile)
{
  int32_t iStatus = EXIT_SUCCESS;
  ...
  if (!inFile) {
    ...
    hb_snprintf(buffer, sizeof(buffer), "Cannot open input file: %s\n", szClpFile);
...
{
  uint16_t wCnt = 0;
  int32_t iFunc = bFunction ? HB_COMP_PARAM->iModulesCount : 0;
...
static void hb_compCheckDuplVars(HB_COMP_DECL, HB_HVAR *pVar, const char *szVarName)
{
  while (pVar) {
...
void hb_compVariableAdd(HB_COMP_DECL, const char *szVarName, HB_VARTYPE *pVarType)
{
  ...
  if (pFunc->szName) {
    ...
    if (!(HB_COMP_PARAM->iVarScope == HB_VSCOMP_PRIVATE || HB_COMP_PARAM->iVarScope == HB_VSCOMP_PUBLIC)) {
    ...
    char buffer[80];
    hb_snprintf(buffer, sizeof(buffer), "Wrong type of codeblock parameter, is: %d, should be: %d\n",
  ...
  auto pVar = static_cast<HB_HVAR *>(hb_xgrab(sizeof(HB_HVAR)));
  ...
  if (HB_TOUPPER(pVarType->cVarType) == 'S') {
    ...
    if (!pVar->pClass) {
      hb_compGenWarning(HB_COMP_PARAM, hb_comp_szWarnings, 'W', HB_COMP_WARN_CLASS_NOT_FOUND, pVarType->szFromClass,
                        szVarName);
      pVar->cType = 'O';
    }
  }
  ...
  if (HB_COMP_PARAM->iVarScope & HB_VSCOMP_MEMVAR) {
    ...
    case (HB_VSCOMP_PARAMETER | HB_VSCOMP_PRIVATE): {
      ...
      if (HB_COMP_PARAM->iWarnings >= 3 && bFreeVar) {
        HB_HVAR *pMemVar = pFunc->pMemvars;
    ...
    case HB_VSCOMP_PARAMETER: {
      ...
      if (HB_COMP_PARAM->fDebugInfo) {
        ...
        hb_compGenPCodeN(reinterpret_cast<const uint8_t *>(szVarName), strlen(szVarName) + 1, HB_COMP_PARAM);
      }
      break;
    }
    case HB_VSCOMP_TH_STATIC: {
...
void hb_compFieldSetAlias(HB_COMP_DECL, const char *szAlias, int32_t iField)
{
  HB_HVAR *pVar = HB_COMP_PARAM->functions.pLast->pFields;
  while (iField-- && pVar) {
    pVar = pVar->pNext;
  }
...
int32_t hb_compFieldsCount(HB_COMP_DECL)
{
  int32_t iFields = 0;
  HB_HVAR *pVar = HB_COMP_PARAM->functions.pLast->pFields;
...
static HB_HVAR *hb_compVariableGet(HB_HVAR *pVars, const char *szVarName, int32_t *piPos)
{
  int32_t iVar = 1;
...
HB_HVAR *hb_compVariableFind(HB_COMP_DECL, const char *szVarName, int32_t *piPos, int32_t *piScope)
{
  int32_t iPos = 0, iScope = 0;
  ...
  HB_HFUNC *pFunc = HB_COMP_PARAM->functions.pLast;
  HB_HFUNC *pGlobal = (HB_COMP_PARAM->pDeclFunc && HB_COMP_PARAM->pDeclFunc != pFunc &&
                       (HB_COMP_PARAM->pDeclFunc->funFlags & HB_FUNF_FILE_DECL))
                          ? HB_COMP_PARAM->pDeclFunc
                          : nullptr;

  HB_HFUNC *pOutBlock = nullptr;
  auto fStatic = false;
  auto fBlock = false;
  auto fGlobal = false;
  HB_HVAR *pVar = nullptr;
  int32_t iLevel = 0;
        ...
        if (fStatic) {
          ...
          *piPos = -hb_compVariableGetPos(pOutBlock->pDetached, szVarName);
          if (*piPos == 0) {
            ...
            pVar = static_cast<HB_HVAR *>(hb_xgrab(sizeof(HB_HVAR)));
...
const char *hb_compLocalVariableName(HB_HFUNC *pFunc, uint16_t wVar)
{
  ...
  HB_HVAR *pVar = hb_compVariableGetVar(pFunc->pLocals, wVar);
...
static int32_t hb_compVariableScope(HB_COMP_DECL, const char *szVarName)
{
  int32_t iScope;
...
void hb_compPushMacroVar(HB_COMP_DECL, const char *szVarName)
{
  ...
  int32_t iEarlyEvalPass = HB_COMP_PARAM->functions.pLast->iEarlyEvalPass;
  ...
  while (n < nLen) {
    if (szText[n++] == '&') {
      ...
      while (n < nLen && iSize < HB_SYMBOL_NAME_LEN) {
        ...
        } else if (ch == '_' || (ch >= 'A' && ch <= 'Z') || (iSize > 0 && ch >= '0' && ch <= '9')) {
          szSymName[iSize++] = ch;
        } else {
          break;
        }
...
void hb_compDeclaredParameterAdd(HB_COMP_DECL, const char *szVarName, HB_VARTYPE *pVarType)
{
  ...
  if (HB_COMP_PARAM->szDeclaredFun) {
    ...
    if (pDeclared) {
      ...
      if (pDeclared->cParamTypes) {
        pDeclared->cParamTypes = static_cast<uint8_t *>(hb_xrealloc(pDeclared->cParamTypes, pDeclared->iParamCount));
        pDeclared->pParamClasses = static_cast<HB_HCLASS **>(
            hb_xrealloc(pDeclared->pParamClasses, pDeclared->iParamCount * sizeof(HB_HCLASS *)));
      } else {
        pDeclared->cParamTypes = static_cast<uint8_t *>(hb_xgrab(1));
    ...
    if (HB_COMP_PARAM->pLastMethod->cParamTypes) {
      HB_COMP_PARAM->pLastMethod->cParamTypes = static_cast<uint8_t *>(
    ...
    if (HB_TOUPPER(pVarType->cVarType) == 'S') {
      HB_COMP_PARAM->pLastMethod->pParamClasses[HB_COMP_PARAM->pLastMethod->iParamCount - 1] =
          hb_compClassFind(HB_COMP_PARAM, pVarType->szFromClass);
...
HB_VARTYPE *hb_compVarTypeNew(HB_COMP_DECL, uint8_t cVarType, const char *szFromClass)
{
...
static int32_t hb_compSort_HB_SIZE(const void *pLeft, const void *pRight)
{
  HB_SIZE nLeft = *(static_cast<const HB_SIZE *>(pLeft));
  HB_SIZE nRight = *(static_cast<const HB_SIZE *>(pRight));
...
{
  uint8_t *pCode = HB_COMP_PARAM->functions.pLast->pCode;
  ...
  for (auto iPass = 0; iPass < 4 && !HB_COMP_PARAM->fExit; ++iPass) {
    ...
    if (HB_COMP_PARAM->functions.pLast->nJumps > 0) {
      ...
      do {
        ...
        if (pCode[nJumpAddr] == HB_P_NOOP || pCode[nJumpAddr] == HB_P_POP) {
          if (HB_COMP_PARAM->functions.pLast->nJumps > nJump + 1) {
            memmove(&pJumps[nJump], &pJumps[nJump + 1],
                    (HB_COMP_PARAM->functions.pLast->nJumps - nJump - 1) * sizeof(HB_SIZE));
          }
      ...
      for (nNOOP = 0; nNOOP < HB_COMP_PARAM->functions.pLast->nNOOPs; nNOOP++) {
        ...
        for (nJump = 0; nJump < HB_COMP_PARAM->functions.pLast->nJumps; nJump++) {
          ...
          if (nOffset > 0) { // forward (positive) jump
            ...
            if (pNOOPs[nNOOP] > nJumpAddr && pNOOPs[nNOOP] < static_cast<HB_SIZE>(nJumpAddr + nOffset)) {
              plSizes[nJump]--;
            }
...
static void hb_compOptimizeFrames(HB_COMP_DECL, HB_HFUNC *pFunc)
{
  if (pFunc == nullptr) {
    return;
  }
...
// function, so I'm using an ugly hack instead. [vszakats]
      ...
      if (pFunc->pCode[8] == HB_P_ENDPROC) {
...
#if 0
            HB_HVAR *pVar = pFunc->pStatics;
#endif
        HB_HVAR *pVar = HB_COMP_PARAM->functions.pFirst->pStatics;
        ...
        while (pVar) {
          ...
          // hb_compGenWarning(HB_COMP_PARAM, hb_comp_szWarnings, 'W', HB_COMP_WARN_NOT_INITIALIZED, pVar->szName,
          // nullptr);
    ...
    if (iLocals || pFunc->wParamCount) {
      ...
      if (iLocals > 255) {
        ...
        pFunc->pCode[3] = static_cast<uint8_t>(pFunc->wParamCount);
        iOffset = 1;
      } else {
        pFunc->pCode[1] = static_cast<uint8_t>(iLocals);
        pFunc->pCode[2] = static_cast<uint8_t>(pFunc->wParamCount);
      }
      bSkipFRAME = false;
    } else {
    ...
    if (bSkipFRAME) {
      ...
      if (pFunc->fVParams) {
        pFunc->pCode[0] = static_cast<uint8_t>(iOffset ? HB_P_LARGEVFRAME : HB_P_VFRAME);
      }

      if (bSkipSFRAME) {
        pFunc->nPCodePos -= 3;
        memmove(pFunc->pCode + 3 + iOffset, pFunc->pCode + 6 + iOffset, pFunc->nPCodePos - 3 - iOffset);
      }
    }
  }
}

static void hb_compWarnUnusedVar(HB_COMP_DECL, const char *szFuncName, const char *szVarName, int32_t iDeclLine)
{
  ...
  hb_compGenWarning(HB_COMP_PARAM, hb_comp_szWarnings, 'W', HB_COMP_WARN_VAR_NOT_USED, szVarName, szFun);
}
...
static void hb_compFinalizeFunction(HB_COMP_DECL) // fixes all last defined function returns jumps offsets
{
  ...
  if (pFunc) {
    ...
    if (HB_COMP_PARAM->iWarnings && (pFunc->funFlags & HB_FUNF_FILE_DECL) == 0) {
      HB_HVAR *pVar;
      ...
      if ((pFunc->funFlags & HB_FUNF_WITH_RETURN) == 0 && (pFunc->funFlags & HB_FUNF_PROCEDURE) == 0) {
        hb_compGenWarning(HB_COMP_PARAM, hb_comp_szWarnings, 'W', HB_COMP_WARN_FUN_WITH_NO_RETURN, pFunc->szName,
                          nullptr);
      }
    }
...
static HB_HFUNC *hb_compFunctionNew(HB_COMP_DECL, const char *szName, HB_SYMBOLSCOPE cScope)
{
  auto pFunc = static_cast<HB_HFUNC *>(hb_xgrabz(sizeof(HB_HFUNC)));
...
static HB_HINLINE *hb_compInlineNew(HB_COMP_DECL, const char *szName, int32_t iLine)
{
...
{
  while (HB_COMP_PARAM->functions.pLast && !HB_COMP_PARAM->functions.pLast->szName) {
    HB_HFUNC *pBlock = HB_COMP_PARAM->functions.pLast;
...
static HB_HFUNC *hb_compFunctionFind(HB_COMP_DECL, const char *szName, HB_BOOL fLocal)
{
  HB_HFUNC *pFunc;
  ...
  if (HB_COMP_PARAM->iModulesCount <= 1) {
    ...
    pFunc = fLocal ? HB_COMP_PARAM->pDeclFunc : HB_COMP_PARAM->functions.pFirst;
  }
  ...
  while (pFunc) {
    ...
    if ((pFunc->funFlags & HB_FUNF_FILE_DECL) == 0 &&
        (fLocal || (pFunc->cScope & (HB_FS_STATIC | HB_FS_INITEXIT)) == 0) && strcmp(pFunc->szName, szName) == 0) {
      break;
    }
...
static bool hb_compIsModuleFunc(HB_COMP_DECL, const char *szFunctionName)
{
  ...
  while (pFunc) {
    if ((pFunc->cScope & HB_FS_STATIC) == 0 && hb_stricmp(pFunc->szName, szFunctionName) == 0) {
      break;
    }
    pFunc = pFunc->pNext;
  }
      ...
      if ((pFunc->cScope & (HB_FS_STATIC | HB_FS_INITEXIT)) != 0) {
        ...
        for (auto pSym : HB_COMP_PARAM->symbols) {
          if (pSym->iFunc) {
            if (pSym->pFunc == pFunc) {
              pFuncSym = pSym;
            } else if (((pSym->cScope & HB_FS_LOCAL) != 0 || (pSym->cScope & HB_FS_DEFERRED) == 0) &&
...
static bool hb_compCheckReservedNames(HB_COMP_DECL, const char *szFunName, bool fError)
{
  ...
  const char *szFunction = hb_compGetFuncID(szFunName, &funcID, &iFlags);
...
void hb_compFunctionAdd(HB_COMP_DECL, const char *szFunName, HB_SYMBOLSCOPE cScope, int32_t iType)
{
  ...
  if (cScope & (HB_FS_INIT | HB_FS_EXIT)) {
    ...
    auto iLen = static_cast<int32_t>(strlen(szFunName));
  ...
  HB_HFUNC *pFunc = hb_compFunctionNew(HB_COMP_PARAM, szFunName, cScope);
...
static void hb_compAnnounce(HB_COMP_DECL, const char *szFunName)
{
  ...
  HB_HFUNC *pFunc = hb_compFunctionFind(HB_COMP_PARAM, szFunName, false);
  if (pFunc) {
...
HB_HINLINE *hb_compInlineAdd(HB_COMP_DECL, const char *szFunName, int32_t iLine)
{
...
static void hb_compNOOPadd(HB_HFUNC *pFunc, HB_SIZE nPos)
{
  ...
  if (pFunc->nNOOPs) {
    pFunc->pNOOPs = static_cast<HB_SIZE *>(hb_xrealloc(pFunc->pNOOPs, sizeof(HB_SIZE) * (pFunc->nNOOPs + 1)));
  } else {
    pFunc->pNOOPs = static_cast<HB_SIZE *>(hb_xgrab(sizeof(HB_SIZE)));
  }
...
static void hb_compPrepareJumps(HB_COMP_DECL)
{
  ...
  if (pFunc->nJumps) {
    pFunc->pJumps = static_cast<HB_SIZE *>(hb_xrealloc(pFunc->pJumps, sizeof(HB_SIZE) * (pFunc->nJumps + 1)));
  } else {
    pFunc->pJumps = static_cast<HB_SIZE *>(hb_xgrab(sizeof(HB_SIZE)));
  }
  pFunc->pJumps[pFunc->nJumps++] = static_cast<HB_SIZE>(pFunc->nPCodePos - 4);
}
...
HB_SIZE hb_compGenJump(HB_ISIZ nOffset, HB_COMP_DECL)
{
  ...
  hb_compGenPCode4(HB_P_JUMPFAR, HB_LOBYTE(nOffset), HB_HIBYTE(nOffset), static_cast<uint8_t>((nOffset >> 16) & 0xFF),
                   HB_COMP_PARAM);
...
HB_SIZE hb_compGenJumpFalse(HB_ISIZ nOffset, HB_COMP_DECL)
{
  ...
  hb_compGenPCode4(HB_P_JUMPFALSEFAR, HB_LOBYTE(nOffset), HB_HIBYTE(nOffset),
                   static_cast<uint8_t>((nOffset >> 16) & 0xFF), HB_COMP_PARAM);
  hb_compPrepareJumps(HB_COMP_PARAM);
...
HB_SIZE hb_compGenJumpTrue(HB_ISIZ nOffset, HB_COMP_DECL)
{
  ...
  hb_compGenPCode4(HB_P_JUMPTRUEFAR, HB_LOBYTE(nOffset), HB_HIBYTE(nOffset),
                   static_cast<uint8_t>((nOffset >> 16) & 0xFF), HB_COMP_PARAM);
  hb_compPrepareJumps(HB_COMP_PARAM);
...
void hb_compLinePush(HB_COMP_DECL) // generates the pcode with the currently compiled source code line
{
  if (HB_COMP_PARAM->fLineNumbers) {
    ...
    if (HB_COMP_PARAM->currLine != HB_COMP_PARAM->lastLine) {
      hb_compGenPCode3(HB_P_LINE, HB_LOBYTE(HB_COMP_PARAM->currLine), HB_HIBYTE(HB_COMP_PARAM->currLine),
                       HB_COMP_PARAM);
...
void hb_compGenModuleName(HB_COMP_DECL, const char *szFunName)
{
  ...
  hb_compGenPCodeN(reinterpret_cast<const uint8_t *>(HB_COMP_PARAM->currModule), strlen(HB_COMP_PARAM->currModule),
                   HB_COMP_PARAM);
  hb_compGenPCode1(':', HB_COMP_PARAM);
  if (szFunName != nullptr && *szFunName) {
    hb_compGenPCodeN(reinterpret_cast<const uint8_t *>(szFunName), strlen(szFunName) + 1, HB_COMP_PARAM);
...
void hb_compGenStaticName(const char * szVarName, HB_COMP_DECL)
{
   if( HB_COMP_PARAM->fDebugInfo ) {
      ...
      HB_HFUNC *pFunc = HB_COMP_PARAM->functions.pLast;
      int32_t iVar = hb_compStaticGetPos(szVarName, pFunc);

      hb_compGenPCode4(HB_P_STATICNAME, bGlobal, HB_LOBYTE(iVar), HB_HIBYTE(iVar), HB_COMP_PARAM);
      hb_compGenPCodeN(static_cast<uint8_t*>(szVarName), strlen(szVarName) + 1, HB_COMP_PARAM);
...
static void hb_compGenVarPCode(uint8_t bPCode, const char *szVarName, HB_COMP_DECL)
{
  uint16_t wVar;
  ...
  if (bPCode == HB_P_PUSHALIASEDFIELD && wVar <= 255) {
    hb_compGenPCode2(HB_P_PUSHALIASEDFIELDNEAR, static_cast<uint8_t>(wVar), HB_COMP_PARAM);
  } else if (bPCode == HB_P_POPALIASEDFIELD && wVar <= 255) {
    hb_compGenPCode2(HB_P_POPALIASEDFIELDNEAR, static_cast<uint8_t>(wVar), HB_COMP_PARAM);
  } else {
...
static void hb_compGenVariablePCode(HB_COMP_DECL, uint8_t bPCode, const char *szVarName)
{
  auto bGenCode = false;
...
static void hb_compGenFieldPCode(HB_COMP_DECL, uint8_t bPCode, HB_HVAR *pField)
{
  if (pField->szAlias) {
...
void hb_compGenMessageData(const char *szMsg, HB_BOOL bIsObject,
                           HB_COMP_DECL) // generates an underscore-symbol name for a data assignment
{
...
static void hb_compCheckEarlyMacroEval(HB_COMP_DECL, const char *szVarName, int32_t iScope)
{
  if (iScope == HB_VS_CBLOCAL_VAR ||
      /* iScope == HB_VS_LOCAL_VAR || */ /* codeblock parameters */
          iScope == HB_VS_STATIC_VAR ||
      iScope == HB_VS_GLOBAL_STATIC || iScope == HB_VS_LOCAL_FIELD || iScope == HB_VS_GLOBAL_FIELD) {
    ...
    if (HB_COMP_PARAM->functions.pLast->iEarlyEvalPass == 1 && HB_SUPPORT_MACRODECL) {
      HB_COMP_PARAM->functions.pLast->iEarlyEvalPass = 0;
    } else {
...
void hb_compGenPopVar(const char *szVarName,
                      HB_COMP_DECL) // generates the pcode to pop a value from the virtual machine stack onto a variable
{
  int32_t iVar, iScope;

  HB_HVAR *pVar = hb_compVariableFind(HB_COMP_PARAM, szVarName, &iVar, &iScope);
  if (pVar) {
    ...
    case HB_VS_CBLOCAL_VAR:
      ...
      if (HB_LIM_INT8(iVar) && !HB_COMP_PARAM->functions.pLast->szName &&
        ...
        hb_compGenPCode2(HB_P_POPLOCALNEAR, static_cast<uint8_t>(iVar), HB_COMP_PARAM);
      } else {
...
void hb_compGenPushVar(const char *szVarName, HB_COMP_DECL)
{
  ...
  if (pVar) {
    ...
    case HB_VS_CBLOCAL_VAR:
      ...
      if (HB_LIM_INT8(iVar) && !HB_COMP_PARAM->functions.pLast->szName &&
          !(HB_COMP_PARAM->functions.pLast->funFlags & HB_FUNF_EXTBLOCK)) {
        hb_compGenPCode2(HB_P_PUSHLOCALNEAR, static_cast<uint8_t>(iVar), HB_COMP_PARAM);
      } else {
...
void hb_compGenPushVarRef(
    const char *szVarName,
    HB_COMP_DECL) // generates the pcode to push a variable by reference to the virtual machine stack
{
  int32_t iVar, iScope;
...
void hb_compGenPushMemvarRef(
    const char *szVarName,
    HB_COMP_DECL) // generates the pcode to push memvar variable by reference to the virtual machine stack
{
  hb_compGenVarPCode(HB_P_PUSHMEMVARREF, szVarName, HB_COMP_PARAM);
}
...
void hb_compGenPopAliasedVar(const char *szVarName, HB_BOOL bPushAliasValue, const char *szAlias, HB_MAXINT nWorkarea,
                             HB_COMP_DECL)
{
  if (bPushAliasValue) {
    if (szAlias != nullptr) {
      ...
      if (szAlias[0] == 'M' && (iLen == 1 || (iLen >= 4 && iLen <= 6 && memcmp(szAlias, "MEMVAR", iLen) == 0))) {
        ...
        hb_compGenVarPCode(HB_P_POPMEMVAR, szVarName, HB_COMP_PARAM);
      } else if (iLen >= 4 && iLen <= 5 && memcmp(szAlias, "FIELD", iLen) == 0) {
        // FIEL[D]->variable
...
void hb_compGenPushAliasedVar(const char *szVarName, HB_BOOL bPushAliasValue, const char *szAlias, HB_MAXINT nWorkarea,
                              HB_COMP_DECL)
{
  if (bPushAliasValue) {
    if (szAlias != nullptr) {
...
void hb_compGenPushDouble(double dNumber, uint8_t bWidth, uint8_t bDec, HB_COMP_DECL)
{
...
void hb_compGenPushFunCall(const char *szFunName, int32_t iFlags, HB_COMP_DECL)
{
...
void hb_compGenPushFunSym(const char *szFunName, int32_t iFlags, HB_COMP_DECL)
{
  HB_SYMBOL_UNUSED(iFlags);
...
void hb_compGenPushSymbol(const char *szSymbolName, HB_BOOL bFunction, HB_COMP_DECL)
{
  ...
  if (wSym > 255) {
    ...
    hb_compGenPCode2(HB_P_PUSHSYMNEAR, static_cast<uint8_t>(wSym), HB_COMP_PARAM);
  }
}
...
void hb_compGenPushLong(HB_MAXINT nNumber, HB_COMP_DECL)
{
  if (!HB_COMP_PARAM->fSwitchCase) {
    if (nNumber == 0) {
      ...
      hb_compGenPCode2(HB_P_PUSHBYTE, static_cast<uint8_t>(nNumber), HB_COMP_PARAM);
...
void hb_compGenPushString(const char *szText, HB_SIZE nStrLen, HB_COMP_DECL)
{
  if (HB_COMP_PARAM->iHidden && !HB_COMP_PARAM->fSwitchCase) {
    --nStrLen;
    char *szTemp = hb_compEncodeString(HB_COMP_PARAM->iHidden, szText, &nStrLen);
    if (nStrLen > UINT16_MAX) {
      hb_compGenError(HB_COMP_PARAM, hb_comp_szErrors, 'E', HB_COMP_ERR_STRING_TOO_LONG, nullptr, nullptr);
    } else {
      hb_compGenPCode4(HB_P_PUSHSTRHIDDEN, static_cast<uint8_t>(HB_COMP_PARAM->iHidden), HB_LOBYTE(nStrLen),
                       HB_HIBYTE(nStrLen), HB_COMP_PARAM);
      hb_compGenPCodeN(reinterpret_cast<uint8_t *>(szTemp), nStrLen, HB_COMP_PARAM);
    }
    hb_xfree(szTemp);
  } else {
    if (nStrLen > UINT24_MAX) {
      ...
      if (nStrLen > USHRT_MAX) {
        ...
        hb_compGenPCode2(HB_P_PUSHSTRSHORT, static_cast<uint8_t>(nStrLen), HB_COMP_PARAM);
      }
      hb_compGenPCodeN(reinterpret_cast<const uint8_t *>(szText), nStrLen, HB_COMP_PARAM);
    }
  }
}

void hb_compNOOPfill(HB_HFUNC *pFunc, HB_SIZE nFrom, HB_ISIZ nCount, HB_BOOL fPop, HB_BOOL fCheck)
{
  while (nCount--) {
    if (fPop) {
...
static void hb_compRemovePCODE(HB_COMP_DECL, HB_SIZE nPos, HB_SIZE nCount, HB_BOOL fCanMove)
{
  HB_HFUNC *pFunc = HB_COMP_PARAM->functions.pLast;
  ...
  if (HB_COMP_ISSUPPORTED(HB_COMPFLAG_OPTJUMP) || !fCanMove) {
    ...
    memmove(pFunc->pCode + nPos, pFunc->pCode + nPos + nCount, pFunc->nPCodePos - nPos - nCount);
    pFunc->nPCodePos -= nCount;

    for (HB_SIZE n = pFunc->nNOOPs; n; --n) {
      if (pFunc->pNOOPs[n] >= nPos) {
        if (pFunc->pNOOPs[n] < nPos + nCount) {
          memmove(&pFunc->pNOOPs[n], &pFunc->pNOOPs[n + 1], pFunc->nNOOPs - n);
          pFunc->nNOOPs--;
        } else {
...
HB_BOOL hb_compHasJump(HB_HFUNC *pFunc, HB_SIZE nPos)
{
...
void hb_compSequenceFinish(HB_COMP_DECL, HB_SIZE nStartPos, HB_SIZE nEndPos, HB_SIZE nAlways, HB_BOOL fUsualStmts,
                           HB_BOOL fRecover, HB_BOOL fCanMove)
{
  ...
  if (!fUsualStmts && fCanMove && !HB_COMP_PARAM->fDebugInfo) {
    ...
    if (nAlways) {
      ...
      hb_compRemovePCODE(HB_COMP_PARAM, nStartPos, nAlways - nStartPos + 4, fCanMove);
    } else {
      hb_compRemovePCODE(HB_COMP_PARAM, nStartPos, HB_COMP_PARAM->functions.pLast->nPCodePos - nStartPos, fCanMove);
    }
  } else if (!nAlways) {
...
static void hb_compStaticDefThreadSet(HB_COMP_DECL)
{
  if (HB_COMP_PARAM->pInitFunc) {
    ...
    if (uiCount) {
      ...
      auto pBuffer = static_cast<uint8_t *>(hb_xgrab(nSize));
      uint8_t *ptr;
      uint16_t uiVar = 0;
...
void hb_compCodeBlockStart(HB_COMP_DECL, int32_t iEarlyEvalPass)
{
...
{
  HB_HFUNC *pCodeblock = HB_COMP_PARAM->functions.pLast; // pointer to the current codeblock
  ...
  if ((pCodeblock->funFlags & HB_FUNF_EXTBLOCK)) {
    if (!(pCodeblock->funFlags & HB_FUNF_WITH_RETURN)) {
      if (HB_COMP_PARAM->iWarnings >= 1) {
        hb_compGenWarning(HB_COMP_PARAM, hb_comp_szWarnings, 'W', HB_COMP_WARN_FUN_WITH_NO_RETURN, "{||...}", nullptr);
      }
  ...
  HB_HFUNC *pFunc = pCodeblock->pOwner; // pointer to a function that owns a codeblock
  const char *pFuncName = pFunc->szName;
  while (pFunc->pOwner) {
    pFunc = pFunc->pOwner;
  ...
  if (nSize <= 255 && pCodeblock->wParamCount == 0 && wLocals == 0) {
    // NOTE: 2 = HB_P_PUSHBLOCK + uint8_t(size)
    hb_compGenPCode2(HB_P_PUSHBLOCKSHORT, static_cast<uint8_t>(nSize), HB_COMP_PARAM);
  } else {
  ...
  if (HB_COMP_PARAM->fDebugInfo) {
    ...
    while (wLocalsCnt--) {
      ...
      hb_compGenPCodeN(reinterpret_cast<const uint8_t *>(pVar->szName), strlen(pVar->szName) + 1, HB_COMP_PARAM);
      iLocalPos--;
      pVar = pVar->pNext;
    }
  }
  ...
  if (HB_COMP_PARAM->iWarnings) {
    ...
    while (pVar) {
      if (HB_COMP_PARAM->iWarnings && pVar->szName && !(pVar->iUsed & HB_VU_USED)) {
        hb_compGenWarning(HB_COMP_PARAM, hb_comp_szWarnings, 'W', HB_COMP_WARN_BLOCKVAR_NOT_USED, pVar->szName,
                          pFuncName);
      }
      pVar = pVar->pNext;
    }
...
void hb_compCodeBlockStop(HB_COMP_DECL)
{
  ...
  if (HB_COMP_PARAM->iWarnings) {
    HB_HVAR *pVar = pCodeblock->pLocals;
    // find the function that owns the codeblock
    HB_HFUNC *pFunc = pCodeblock->pOwner;
    while (pFunc->pOwner) {
      pFunc = pFunc->pOwner;
    }
    while (pVar) {
...
static void hb_compGenOutput(HB_COMP_DECL, int32_t iLanguage)
{
#if 0
...
static void hb_compAddInitFunc(HB_COMP_DECL, HB_HFUNC *pFunc)
{
...
void hb_compCompileEnd(HB_COMP_DECL)
{
  ...
  if (HB_COMP_PARAM->pInitFunc && (HB_COMP_PARAM->pInitFunc->funFlags & HB_FUNF_ATTACHED) == 0) {
  ...
  if (HB_COMP_PARAM->iTraceInclude > 0 && HB_COMP_PARAM->incfiles) {
    ...
    if ((HB_COMP_PARAM->iTraceInclude & 0x100) != 0) {
      ...
      HB_COMP_PARAM->pOutBuf =
          static_cast<uint8_t *>(hb_xrealloc(HB_COMP_PARAM->pOutBuf, HB_COMP_PARAM->nOutBufSize + nLen));
...
static int32_t hb_compCompile(HB_COMP_DECL, const char *szPrg, const char *szBuffer, int32_t iStartLine)
{
  ...
  while (iStatus == EXIT_SUCCESS && !HB_COMP_PARAM->fExit && (pModule || szBuffer)) {
    ...
    HB_COMP_PARAM->pLex->iState = HB_COMP_PARAM->pLex->iClose = HB_COMP_PARAM->pLex->iScope = 0;
    ...
    if (!fSkip) {
      ...
      if (HB_COMP_PARAM->iLanguage != HB_LANG_PORT_OBJ && HB_COMP_PARAM->iLanguage != HB_LANG_PORT_OBJ_BUF) {
      ...
      HB_COMP_PARAM->currModule = hb_compIdentifierNew(HB_COMP_PARAM, szFileName, HB_IDENT_COPY);
      ...
      if (szBuffer != nullptr) {
        ...
        if (!HB_COMP_PARAM->fQuiet) {
          if (HB_COMP_PARAM->fPPO) {
            hb_snprintf(buffer, sizeof(buffer), "Compiling '%s' and generating preprocessed output to '%s'...\n",
        ...
        hb_compFunctionAdd(
            HB_COMP_PARAM, hb_compIdentifierNew(HB_COMP_PARAM, hb_strupr(hb_strdup(pFileName->szName)), HB_IDENT_FREE),
            HB_FS_PUBLIC,
            HB_FUNF_PROCEDURE | HB_FUNF_FILE_FIRST | (HB_COMP_PARAM->iStartProc == 0 ? 0 : HB_FUNF_FILE_DECL));
      }

      if (!HB_COMP_PARAM->fExit) {
        int32_t iExitLevel = HB_COMP_PARAM->iExitLevel;
    ...
    while (pModule && !pModule->force && hb_compIsModuleFunc(HB_COMP_PARAM, pModule->szName)) {
      pModule = pModule->pNext;
    }
  }
  ...
  if (!HB_COMP_PARAM->fExit && iStatus == EXIT_SUCCESS) {
    ...
    if (HB_COMP_PARAM->fLineNumbers && HB_COMP_PARAM->fDebugInfo) {
      ...
      if (pInfo) {
        ...
        do {
          ...
          hb_compGenPushString(reinterpret_cast<char *>(pInfo->pLineMap) + nSkip, nLen + 1, HB_COMP_PARAM);
    ...
    if (HB_COMP_PARAM->iErrorCount || HB_COMP_PARAM->fAnyWarning) {
      if (HB_COMP_PARAM->iErrorCount) {
        iStatus = EXIT_FAILURE;
        fGenCode = false;
        hb_snprintf(buffer, sizeof(buffer), "\r%i error%s\n", HB_COMP_PARAM->iErrorCount,
    ...
    if (fGenCode && HB_COMP_PARAM->iErrorCount == 0 && HB_COMP_PARAM->iTraceInclude > 0) {
    ...
    if (HB_COMP_PARAM->iSyntaxCheckOnly == 0 && fGenCode && HB_COMP_PARAM->iErrorCount == 0) {
      ...
      while (pFunc && !HB_COMP_PARAM->fExit) {
        ...
        if ((pFunc->funFlags & HB_FUNF_FILE_DECL) == 0) {
          ...
          if (szFirstFunction == nullptr && !(pFunc->cScope & (HB_FS_INIT | HB_FS_EXIT))) {
            szFirstFunction = pFunc->szName;
          }
          if (pFunc != HB_COMP_PARAM->pInitFunc && pFunc != HB_COMP_PARAM->pLineFunc) {
            ++iFunctionCount;
          }
        }
        pFunc = pFunc->pNext;
      ...
      if (!HB_COMP_PARAM->fQuiet) {
        hb_snprintf(buffer, sizeof(buffer), "\rLines %i, Functions/Procedures %i\n",
                    hb_pp_lineTot(HB_COMP_PARAM->pLex->pPP), iFunctionCount);
        hb_compOutStd(HB_COMP_PARAM, buffer);
      }
  ...
  if (!fGenCode && !HB_COMP_PARAM->fExit && HB_COMP_PARAM->iSyntaxCheckOnly == 0) {
    hb_compOutStd(HB_COMP_PARAM, "\nNo code generated.\n");
  }
