...
int hb_compMainExt(int argc, const char * const argv[], HB_BYTE ** pBufPtr, HB_SIZE * pnSize, const char * szSource, int iStartLine,
                   void * cargo, PHB_PP_OPEN_FUNC pOpenFunc, PHB_PP_MSG_FUNC pMsgFunc)
{
#if 0
...
PHB_HVAR hb_compVariableFind(HB_COMP_DECL, const char * szVarName, int * piPos, int * piScope)
{
   ...
   PHB_HFUNC pFunc = HB_COMP_PARAM->functions.pLast;
   PHB_HFUNC pGlobal = (HB_COMP_PARAM->pDeclFunc &&
   ...
   if( HB_COMP_PARAM->szDeclaredFun ) {
      ...
      if( pDeclared ) {
         ...
         if( pDeclared->cParamTypes ) {
            ...
            pDeclared->pParamClasses = static_cast<PHB_HCLASS*>(hb_xrealloc(pDeclared->pParamClasses, pDeclared->iParamCount * sizeof(PHB_HCLASS)));
         } else {
      ...
      if( HB_COMP_PARAM->pLastMethod->cParamTypes ) {
         HB_COMP_PARAM->pLastMethod->cParamTypes = static_cast<HB_BYTE*>(hb_xrealloc(HB_COMP_PARAM->pLastMethod->cParamTypes, HB_COMP_PARAM->pLastMethod->iParamCount));
         HB_COMP_PARAM->pLastMethod->pParamClasses = static_cast<PHB_HCLASS*>(hb_xrealloc(HB_COMP_PARAM->pLastMethod->pParamClasses, HB_COMP_PARAM->pLastMethod->iParamCount * sizeof(HB_HCLASS)));
      } else {
         ...
         do {
            ...
            if( pCode[nJumpAddr] == HB_P_NOOP || pCode[nJumpAddr] == HB_P_POP ) {
               if( HB_COMP_PARAM->functions.pLast->nJumps > nJump + 1 ) {
                  memmove(&pJumps[nJump], &pJumps[nJump + 1], (HB_COMP_PARAM->functions.pLast->nJumps - nJump - 1) * sizeof(HB_SIZE));
               }
   ...
   if( pFunc ) {
      ...
      if( HB_COMP_PARAM->iWarnings && (pFunc->funFlags & HB_FUNF_FILE_DECL) == 0 ) {
         ...
         if( (pFunc->funFlags & HB_FUNF_WITH_RETURN) == 0 && (pFunc->funFlags & HB_FUNF_PROCEDURE) == 0 ) {
            hb_compGenWarning(HB_COMP_PARAM, hb_comp_szWarnings, 'W', HB_COMP_WARN_FUN_WITH_NO_RETURN, pFunc->szName, nullptr);
         }
      }
         ...
         if( (pFunc->cScope & (HB_FS_STATIC | HB_FS_INITEXIT)) != 0 ) {
            ...
            while( pSym ) {
               if( pSym->iFunc ) {
                  if( pSym->pFunc == pFunc ) {
                     pFuncSym = pSym;
                  } else if( ((pSym->cScope & HB_FS_LOCAL) != 0 || (pSym->cScope & HB_FS_DEFERRED) == 0) && strcmp(pFunc->szName, pSym->szName) == 0 ) {
                     fExists = true;
                  }
...
HB_SIZE hb_compGenJump(HB_ISIZ nOffset, HB_COMP_DECL)
{
   ...
   hb_compGenPCode4(HB_P_JUMPFAR, HB_LOBYTE(nOffset), HB_HIBYTE(nOffset), static_cast<HB_BYTE>((nOffset >> 16) & 0xFF), HB_COMP_PARAM);
...
HB_SIZE hb_compGenJumpFalse(HB_ISIZ nOffset, HB_COMP_DECL)
{
   ...
   hb_compGenPCode4(HB_P_JUMPFALSEFAR, HB_LOBYTE(nOffset), HB_HIBYTE(nOffset), static_cast<HB_BYTE>((nOffset >> 16) & 0xFF), HB_COMP_PARAM);
   hb_compPrepareJumps(HB_COMP_PARAM);
...
HB_SIZE hb_compGenJumpTrue(HB_ISIZ nOffset, HB_COMP_DECL)
{
   ...
   hb_compGenPCode4(HB_P_JUMPTRUEFAR, HB_LOBYTE(nOffset), HB_HIBYTE(nOffset), static_cast<HB_BYTE>((nOffset >> 16) & 0xFF), HB_COMP_PARAM);
   hb_compPrepareJumps(HB_COMP_PARAM);
...
void hb_compLinePush(HB_COMP_DECL) /* generates the pcode with the currently compiled source code line */
{
   if( HB_COMP_PARAM->fLineNumbers ) {
      ...
      if( HB_COMP_PARAM->currLine != HB_COMP_PARAM->lastLine ) {
         hb_compGenPCode3(HB_P_LINE, HB_LOBYTE(HB_COMP_PARAM->currLine), HB_HIBYTE(HB_COMP_PARAM->currLine), HB_COMP_PARAM);
...
void hb_compGenModuleName(HB_COMP_DECL, const char * szFunName)
{
   ...
   hb_compGenPCodeN(reinterpret_cast<const HB_BYTE*>(HB_COMP_PARAM->currModule), strlen(HB_COMP_PARAM->currModule), HB_COMP_PARAM);
 ...
 * an aliased variable
 */
void hb_compGenPopAliasedVar(const char * szVarName, HB_BOOL bPushAliasValue, const char * szAlias, HB_MAXINT nWorkarea, HB_COMP_DECL)
{
   if( bPushAliasValue ) {
      if( szAlias ) {
 ...
 * machine stack
 */
void hb_compGenPushAliasedVar(const char * szVarName, HB_BOOL bPushAliasValue, const char * szAlias, HB_MAXINT nWorkarea, HB_COMP_DECL)
{
   if( bPushAliasValue ) {
      if( szAlias ) {
         auto iLen = static_cast<int>(strlen(szAlias));
...
void hb_compGenPushString(const char * szText, HB_SIZE nStrLen, HB_COMP_DECL)
{
   if( HB_COMP_PARAM->iHidden && !HB_COMP_PARAM->fSwitchCase ) {
      ...
      if( nStrLen > UINT16_MAX ) {
         ...
         hb_compGenPCode4(HB_P_PUSHSTRHIDDEN, static_cast<HB_BYTE>(HB_COMP_PARAM->iHidden), HB_LOBYTE(nStrLen), HB_HIBYTE(nStrLen), HB_COMP_PARAM);
 ...
 */
void hb_compSequenceFinish(HB_COMP_DECL, HB_SIZE nStartPos, HB_SIZE nEndPos, HB_SIZE nAlways, HB_BOOL fUsualStmts, HB_BOOL fRecover, HB_BOOL fCanMove)
{
...
static void hb_compGenIncluded(HB_COMP_DECL)
{
   if( HB_COMP_PARAM->iTraceInclude > 0 && HB_COMP_PARAM->incfiles ) {
      ...
      if( (HB_COMP_PARAM->iTraceInclude & 0x100) != 0 ) {
         ...
         HB_COMP_PARAM->pOutBuf = static_cast<HB_BYTE*>(hb_xrealloc(HB_COMP_PARAM->pOutBuf, HB_COMP_PARAM->nOutBufSize + nLen));
   ...
   while( iStatus == EXIT_SUCCESS && !HB_COMP_PARAM->fExit && (pModule || szBuffer) ) {
      ...
      if( !fSkip ) {
         ...
         if( szBuffer ) {
             ...
             */
            hb_compFunctionAdd(HB_COMP_PARAM, "__hbInit", HB_FS_STATIC, HB_FUNF_PROCEDURE | HB_FUNF_FILE_FIRST | HB_FUNF_FILE_DECL);
         } else {
