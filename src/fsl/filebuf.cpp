...
static PHB_FILE hb_fileNew(HB_FHANDLE hFile, bool fShared, int32_t iMode, HB_ULONG device, HB_ULONG inode, bool fBind)
{
    ...
    if (nStart >= pLock->start && pLock->len >= nLen && nStart - pLock->start <= pLock->len - nLen) {
      if (pfLockFS && pFile->shared) {
        *pfLockFS = true;
        ...
        hb_fileInsertLock(pFile, uiPos + 1, nStart + nLen, pLock->start + pLock->len - nStart - nLen);
                              ...
                              const char *pPaths, HB_ITEM *pError)
{
  ...
  auto iMode = static_cast<int32_t>(nExFlags & (FO_READ | FO_WRITE | FO_READWRITE));
  ...
  if (nExFlags & FXO_SHARELOCK) {
    if (iMode == FO_WRITE && fShared) {
      if (access(pszFile, R_OK) == 0 || access(pszFile, F_OK) != 0) {
...
static const HB_FILE_FUNCS *s_fileMethods(void)
{
  ...
  static const HB_FILE_FUNCS s_fileFuncs = {
      ...
      s_fileExtOpen,   s_fileClose,   s_fileLock,      s_fileLockTest, s_fileRead,      s_fileWrite,
...
static HB_BOOL s_fileposLock(PHB_FILE pFilePos, HB_FOFFSET nStart, HB_FOFFSET nLen, int32_t iType)
{
  return _PHB_FILE->pFuncs->Lock(_PHB_FILE, nStart, nLen, iType);
}

static int32_t s_fileposLockTest(PHB_FILE pFilePos, HB_FOFFSET nStart, HB_FOFFSET nLen, int32_t iType)
{
  return _PHB_FILE->pFuncs->LockTest(_PHB_FILE, nStart, nLen, iType);
}

static HB_SIZE s_fileposRead(PHB_FILE pFilePos, void *buffer, HB_SIZE nSize, HB_MAXINT nTimeout)
{
  HB_SYMBOL_UNUSED(nTimeout);
  HB_SIZE nDone = _PHB_FILE->pFuncs->ReadAt(_PHB_FILE, buffer, nSize, _PHB_FILEPOS->seek_pos);
  _PHB_FILEPOS->seek_pos += nDone;
  return nDone;
}

static HB_SIZE s_fileposWrite(PHB_FILE pFilePos, const void *buffer, HB_SIZE nSize, HB_MAXINT nTimeout)
{
  HB_SYMBOL_UNUSED(nTimeout);
  HB_SIZE nDone = _PHB_FILE->pFuncs->WriteAt(_PHB_FILE, buffer, nSize, _PHB_FILEPOS->seek_pos);
  _PHB_FILEPOS->seek_pos += nDone;
  return nDone;
}

static HB_SIZE s_fileposReadAt(PHB_FILE pFilePos, void *buffer, HB_SIZE nSize, HB_FOFFSET nOffset)
{
  return _PHB_FILE->pFuncs->ReadAt(_PHB_FILE, buffer, nSize, nOffset);
}

static HB_SIZE s_fileposWriteAt(PHB_FILE pFilePos, const void *buffer, HB_SIZE nSize, HB_FOFFSET nOffset)
{
...
static const HB_FILE_FUNCS *s_fileposMethods(void)
{
  ...
  static const HB_FILE_FUNCS s_fileFuncs = {
      ...
      s_fileExtOpen,   s_fileposClose,   s_fileposLock,      s_fileposLockTest, s_fileposRead,   s_fileposWrite,
      s_fileposReadAt, s_fileposWriteAt, s_fileposTruncAt,   s_fileposSeek,     s_fileposSize,   s_fileposEof,
      s_fileposFlush,  s_fileposCommit,  s_fileposConfigure, s_fileposHandle};
...
static int32_t s_iFileTypes = 0;

static int32_t s_fileFindDrv(const char *pszFileName)
{
  int32_t i = -1;
...
{
  int32_t i = s_fileFindDrv(pszSrcFile);
...
{
  int32_t iS = s_fileFindDrv(pszSrcFile), iD = s_fileFindDrv(pszDstFile);

  if (iS == iD) {
    if (iS >= 0 ? s_pFileTypes[iS]->Rename(s_pFileTypes[iS], pszSrcFile, pszDstFile)
                : hb_fsRename(pszSrcFile, pszDstFile)) {
      return true;
    }
  }

  return hb_fsCopy(pszSrcFile, pszDstFile) && hb_fileDelete(pszSrcFile);
}

HB_BOOL hb_fileDirExists(const char *pszDirName)
{
  int32_t i = s_fileFindDrv(pszDirName);
...
{
  int32_t i = s_fileFindDrv(pszExisting);
...
{
  int32_t i = s_fileFindDrv(pszTarget);
...
HB_BOOL hb_fileLock(PHB_FILE pFile, HB_FOFFSET nStart, HB_FOFFSET nLen, int32_t iType)
{
  return pFile->pFuncs->Lock(pFile, nStart, nLen, iType);
}

int32_t hb_fileLockTest(PHB_FILE pFile, HB_FOFFSET nStart, HB_FOFFSET nLen, int32_t iType)
{
  return pFile->pFuncs->LockTest(pFile, nStart, nLen, iType);
}

HB_SIZE hb_fileRead(PHB_FILE pFile, void *buffer, HB_SIZE nSize, HB_MAXINT nTimeout)
{
  return pFile->pFuncs->Read(pFile, buffer, nSize, nTimeout);
}

HB_SIZE hb_fileWrite(PHB_FILE pFile, const void *buffer, HB_SIZE nSize, HB_MAXINT nTimeout)
{
  return pFile->pFuncs->Write(pFile, buffer, nSize, nTimeout);
}

HB_SIZE hb_fileReadAt(PHB_FILE pFile, void *buffer, HB_SIZE nSize, HB_FOFFSET nOffset)
{
  return pFile->pFuncs->ReadAt(pFile, buffer, nSize, nOffset);
}

HB_SIZE hb_fileWriteAt(PHB_FILE pFile, const void *buffer, HB_SIZE nSize, HB_FOFFSET nOffset)
{
...
PHB_FILE hb_fileCreateTemp(const char *pszDir, const char *pszPrefix, HB_FATTR ulAttr, char *pszName)
{
  ...
  HB_FHANDLE hFile = hb_fsCreateTemp(pszDir, pszPrefix, ulAttr, pszName);
  if (hFile != FS_ERROR) {
...
PHB_FILE hb_fileCreateTempEx(char *pszName, const char *pszDir, const char *pszPrefix, const char *pszExt,
                             HB_FATTR ulAttr)
{
  PHB_FILE pFile = nullptr;

  HB_FHANDLE hFile = hb_fsCreateTempEx(pszName, pszDir, pszPrefix, pszExt, ulAttr);
  if (hFile != FS_ERROR) {
    pFile = hb_fileNew(hFile, false, false, 0, 0, false);
  }
...
{
  return hb_fileNew(hFile, false, false, 0, 0, false);
}
...
PHB_FILE hb_filePOpen(const char *pszFileName, const char *pszMode)
{
  ...
  HB_FHANDLE hFile = hb_fsPOpen(pszFileName, pszMode);
  if (hFile != FS_ERROR) {
    pFile = hb_fileNew(hFile, false, false, 0, 0, false);
  }
...
uint8_t *hb_fileLoadData(PHB_FILE pFile, HB_SIZE nMaxSize, HB_SIZE *pnSize)
{
  ...
  if (nFileSize == FS_ERROR || (nFileSize == 0 && hb_fsError() != 0)) {
    for (nBufSize = 0;;) {
      if (nBufSize == nSize) {
