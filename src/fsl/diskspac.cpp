...
#define HB_GET_LARGE_UINT(v)                                                                                           \
  (static_cast<double>((v).u.LowPart) + static_cast<double>((v).u.HighPart) * ((static_cast<double>(0xFFFFFFFF)) + 1))
    ...
    if (iDrive >= 0) {
      ...
      bError =
          GetDiskFreeSpaceEx(lpPath, static_cast<PULARGE_INTEGER>(&i64FreeBytesToCaller),
                             static_cast<PULARGE_INTEGER>(&i64TotalBytes), static_cast<PULARGE_INTEGER>(&i64FreeBytes))
              ? false
              : true;
      if (!bError) {
