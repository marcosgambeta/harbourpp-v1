...
#define HB_MEMINFO_SIZE                                                                                                \
  ((sizeof(HB_MEMINFO) + HB_ALLOC_ALIGNMENT - 1) - (sizeof(HB_MEMINFO) + HB_ALLOC_ALIGNMENT - 1) % HB_ALLOC_ALIGNMENT)
#else
...
void hb_xexit(void)
{
  ...
  if (s_nMemoryBlocks /* || hb_cmdargCheck("INFO") */)
  {
    ...
    hb_conOutErr("----------------------------------------", 0);
    hb_conOutErr(hb_conNewLine(), 0);
    hb_snprintf(szBuffer, sizeof(szBuffer), "Total memory allocated: %" HB_PFS "u bytes (%" HB_PFS "u blocks)",
    ...
    {
      hb_conOutErr(hb_conNewLine(), 0);
      hb_snprintf(szBuffer, sizeof(szBuffer),
                  "WARNING! Memory allocated but not released: %" HB_PFS "u bytes (%" HB_PFS "u blocks)",
    ...
    for (auto i = 1, pMemBlock = s_pMemBlocks; pMemBlock; ++i, pMemBlock = pMemBlock->pNextBlock)
    {
      HB_TRACE(HB_TR_ERROR,
               ("Block %i %p (size %" HB_PFS "u) \"%s\"", i, static_cast<char *>(pMemBlock) + HB_MEMINFO_SIZE,
                pMemBlock->nSize,
