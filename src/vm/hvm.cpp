...
static void hb_vmInc(HB_ITEM *pItem);
// decrements the latest numeric value on the stack
static void hb_vmDec(HB_ITEM *pItem);
// pushes a function address pointer. Removes the symbol from the stack
static void hb_vmFuncPtr();
// add integer to given item
static void hb_vmAddInt(HB_ITEM *pResult, HB_LONG lAdd);
// sums given values
static void hb_vmPlus(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2);
// subtracts given values
static void hb_vmMinus(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2);
// multiplies given values
static void hb_vmMult(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2);
// divides the given values
static void hb_vmDivide(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2);
// calculates modulus given values
static void hb_vmModulus(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2);
// power given values
static void hb_vmPower(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2);
...
static void hb_vmArrayDim(uint16_t uiDimensions);
...
static void hb_vmMacroDo(uint16_t uiArgSets);
// execute procedure passing arguments set on HVM stack func(&var)
static void hb_vmMacroFunc(uint16_t uiArgSets);
// execute procedure passing arguments set on HVM stack func(&var)
static void hb_vmMacroSend(uint16_t uiArgSets);
// generate array from arguments set on HVM stack { &var }
static void hb_vmMacroArrayGen(uint16_t uiArgSets);
...
static HB_ERRCODE hb_vmSelectWorkarea(HB_ITEM *, HB_SYMB *);
...
static void hb_vmFrame(uint16_t usLocals, unsigned char ucParams);
...
static void hb_vmPushStatic(uint16_t uiStatic);
...
static void hb_vmMsgIndexReference(HB_ITEM *pRefer, HB_ITEM *pObject, HB_ITEM *pIndex);
...
static void hb_vmDebugEntry(int32_t nMode, int32_t nLine, const char *szName, int32_t nIndex, HB_ITEM *pFrame);
// shuts down the debugger
static void hb_vmDebuggerExit(HB_BOOL fRemove);
// makes the debugger shows a specific source code line
static void hb_vmDebuggerShowLine(uint16_t uiLine);
...
static HB_ITEM *s_pSymbolsMtx = nullptr;
...
static HB_ITEM *s_breakBlock = nullptr;
...
static HB_ITEM *hb_vmStackDel(PHB_THREADSTATE pState, HB_BOOL fCounter)
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_vmStackDel(%p,%d)", static_cast<void*>(pState), static_cast<int32_t>(fCounter)));
#endif
...
static void hb_vmStackRelease()
{
  ...
  HB_ITEM *pThItm = hb_vmStackDel(static_cast<PHB_THREADSTATE>(hb_stackList()), false);
  HB_VM_UNLOCK();
...
void hb_vmThreadRelease(void *Cargo)
{
  ...
  HB_VM_LOCK();
  HB_ITEM *pThItm = hb_vmStackDel(static_cast<PHB_THREADSTATE>(Cargo), true);
  hb_threadCondBroadcast(&s_vmCond);
  HB_VM_UNLOCK();
...
HB_ITEM *hb_vmThreadStart(HB_ULONG ulAttr, PHB_CARGO_FUNC pFunc, void *cargo)
{
#if 0
...
void hb_vmInit(HB_BOOL bStartMainProc)
{
  ...
  if (bStartMainProc && s_pSymStart) {
    ...
    hb_vmProc(static_cast<uint16_t>(hb_cmdargPushArgs())); // invoke it with number of supplied parameters
  }
}
...
void hb_vmExecute(const uint8_t *pCode, HB_SYMB *pSymbols)
{
  ...
  for (;;) {
    ...
    case HB_P_SWITCH: {
      pCode = hb_vmSwitch(pCode + 3, HB_PCODE_MKUINT16(&pCode[1]));
      break;
    }
    ...
    case HB_P_ARRAYDIM: {
      hb_vmArrayDim(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_ARRAYGEN: {
      hb_vmArrayGen(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_HASHGEN: {
      hb_vmHashGen(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_MESSAGE: {
      hb_vmPushSymbol(pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_DO: {
      hb_vmProc(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_SEND: {
      hb_itemSetNil(hb_stackReturnItem());
      hb_vmSend(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      // Small opt
      if (pCode[0] == HB_P_POP) {
        pCode++;
    ...
    case HB_P_LINE: {
      ...
      hb_stackBaseItem()->symbolStackState()->uiLineNo = HB_PCODE_MKUINT16(&pCode[1]);
#ifndef HB_NO_DEBUG
    ...
    case HB_P_PARAMETER: {
      hb_memvarNewParameter(pSymbols + HB_PCODE_MKUINT16(&pCode[1]), hb_stackItemFromBase(pCode[3]));
#if 0
    ...
    case HB_P_LARGEFRAME: {
      hb_vmFrame(HB_PCODE_MKUINT16(&pCode[1]), static_cast<unsigned char>(pCode[3]));
      pCode += 4;
      break;
    }

    case HB_P_LARGEVFRAME: {
      hb_vmVFrame(HB_PCODE_MKUINT16(&pCode[1]), static_cast<unsigned char>(pCode[3]));
      pCode += 4;
      break;
    }

    case HB_P_SFRAME: {
      hb_vmSFrame(pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_STATICS: {
      hb_vmStatics(pSymbols + HB_PCODE_MKUINT16(&pCode[1]), HB_PCODE_MKUINT16(&pCode[3]));
      pCode += 5;
      break;
    }

    case HB_P_THREADSTATICS: {
      uint16_t uiCount = HB_PCODE_MKUINT16(&pCode[1]);
    ...
    case HB_P_LOCALNAME: {
#ifndef HB_NO_DEBUG
      hb_vmLocalName(HB_PCODE_MKUINT16(&pCode[1]), reinterpret_cast<const char *>(pCode) + 3);
#endif
      pCode += 3;
      while (*pCode++) {
        ;
...
#ifndef HB_NO_DEBUG
      hb_vmStaticName(pCode[1], HB_PCODE_MKUINT16(&pCode[2]), reinterpret_cast<const char *>(pCode) + 4);
#endif
      pCode += 4;
      while (*pCode++) {
        ;
...
#endif
      uint16_t uiPrevAction = hb_stackItemFromTop(HB_RECOVER_STATE)->item.asRecover.flags;
      uint16_t uiCurrAction = hb_stackItemFromTop(HB_RECOVER_STATE)->item.asRecover.request;
    ...
    case HB_P_PUSHDOUBLE: {
      hb_vmPushDoubleConst(HB_PCODE_MKDOUBLE(&pCode[1]),
                           static_cast<int32_t>(*static_cast<const unsigned char *>(&pCode[1 + sizeof(double)])),
                           static_cast<int32_t>(*static_cast<const unsigned char *>(&pCode[2 + sizeof(double)])));
    ...
    case HB_P_PUSHSTR: {
      uint16_t uiSize = HB_PCODE_MKUINT16(&pCode[1]);
      if (bDynCode) {
    ...
    case HB_P_PUSHSTRHIDDEN: {
      auto nSize = static_cast<HB_SIZE>(HB_PCODE_MKUINT16(&pCode[2]));
    ...
    case HB_P_PUSHBLOCK: {
      ...
      HB_SIZE nSize = HB_PCODE_MKUINT16(&pCode[1]);
    ...
    case HB_P_PUSHALIASEDFIELD: {
      hb_vmPushAliasedField(pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_PUSHALIASEDVAR: {
      hb_vmPushAliasedVar(pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_PUSHFIELD: {
      ...
      hb_rddGetFieldValue(hb_stackAllocItem(), pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
#if 0
    ...
    case HB_P_PUSHSTATIC: {
      hb_vmPushStatic(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_PUSHSTATICREF: {
      hb_vmPushStaticByRef(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_PUSHMEMVAR: {
      hb_memvarGetValue(hb_stackAllocItem(), pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
#if 0
    ...
    case HB_P_PUSHMEMVARREF: {
      hb_memvarGetRefer(hb_stackAllocItem(), pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
#if 0
    ...
    case HB_P_PUSHVARIABLE: {
      ...
      hb_vmPushVariable(pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_POPALIASEDFIELD: {
      hb_vmPopAliasedField(pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_POPALIASEDVAR: {
      hb_vmPopAliasedVar(pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_POPFIELD: {
      ...
      hb_rddPutFieldValue(hb_stackItemFromTop(-1), pSymbols + HB_PCODE_MKUINT16(&pCode[1]));
      hb_stackPop();
#if 0
    ...
    case HB_P_POPSTATIC: {
      hb_vmPopStatic(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_POPMEMVAR: {
      hb_memvarSetValue(pSymbols + HB_PCODE_MKUINT16(&pCode[1]), hb_stackItemFromTop(-1));
      hb_stackPop();
#if 0
    ...
    case HB_P_MACROARRAYGEN: {
      hb_vmMacroArrayGen(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_MACRODO: {
      hb_vmMacroDo(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_MACROFUNC: {
      hb_vmMacroFunc(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }

    case HB_P_MACROSEND: {
      hb_vmMacroSend(HB_PCODE_MKUINT16(&pCode[1]));
      pCode += 3;
      break;
    }
    ...
    case HB_P_MPUSHBLOCK: {
      ...
      // +5    -> pcode bytes
      HB_SIZE nSize = HB_PCODE_MKUINT16(&pCode[1]);
      hb_vmPushMacroBlock(pCode + 5, nSize - 5, HB_PCODE_MKUINT16(&pCode[3]));
      pCode += nSize;
      break;
    }
    ...
    case HB_P_MPUSHBLOCKLARGE: {
      ...
      // +6       -> pcode bytes
      HB_SIZE nSize = HB_PCODE_MKUINT24(&pCode[1]);
      hb_vmPushMacroBlock(pCode + 6, nSize - 6, HB_PCODE_MKUINT16(&pCode[4]));
      pCode += nSize;
      break;
    }
    ...
    case HB_P_LOCALADDINT: {
      int32_t iLocal = HB_PCODE_MKUINT16(&pCode[1]);
#if 0
    ...
    case HB_P_LOCALINC: {
      int32_t iLocal = HB_PCODE_MKUINT16(&pCode[1]);
      HB_ITEM *pLocal = hb_stackLocalVariable(iLocal);
    ...
    case HB_P_WITHOBJECTMESSAGE: {
      uint16_t wSymPos = HB_PCODE_MKUINT16(&pCode[1]);
      if (wSymPos != 0xFFFF) {
      ...
      HB_ITEM *pWith = hb_stackWithObjectItem();
      if (pWith) {
        hb_vmPush(pWith);
      } else {
        hb_stackAllocItem()->setType(Harbour::Item::NIL);
...
static void hb_vmAddInt(HB_ITEM *pResult, HB_LONG lAdd)
{
  ...
  if (pResult->isNumInt()) {
    ...
    if (lAdd >= 0 ? nResult >= nVal : nResult < nVal) {
      ...
      pResult->setType(Harbour::Item::DOUBLE);
      pResult->setDoubleValue(static_cast<double>(nVal) + lAdd);
      pResult->setDoubleLength(HB_DBL_LENGTH(pResult->doubleValue()));
      pResult->setDoubleDecimal(0);
    }
  } else if (pResult->isDouble()) {
    pResult->item.asDouble.value += lAdd; // TODO:
    pResult->setDoubleLength(HB_DBL_LENGTH(pResult->doubleValue()));
...
static void hb_vmNegate()
{
  ...
  if (pItem->isInteger()) {
    ...
    if (pItem->integerValue() < -HB_VMINT_MAX) {
      ...
      pItem->setLongValue(-nValue);
      pItem->setLongLength(HB_LONG_EXPLENGTH(-nValue));
#else
      auto dValue = static_cast<double>(pItem->integerValue());
      pItem->setType(Harbour::Item::DOUBLE);
      pItem->setDoubleValue(-dValue);
      pItem->setDoubleLength(HB_DBL_LENGTH(-dValue));
    ...
    if (pItem->longValue() < -HB_VMLONG_MAX) {
      auto dValue = static_cast<double>(pItem->longValue());
      pItem->setType(Harbour::Item::DOUBLE);
      pItem->setDoubleValue(-dValue);
      pItem->setDoubleLength(HB_DBL_LENGTH(-dValue));
      pItem->setDoubleDecimal(0);
    ...
    {
      pItem->setType(Harbour::Item::LONG);
      pItem->setLongValue(-pItem->longValue());
      pItem->setLongLength(HB_LONG_EXPLENGTH(pItem->longValue()));
    }
  } else if (pItem->isDouble()) {
    pItem->setType(Harbour::Item::DOUBLE);
    pItem->setDoubleValue(-pItem->doubleValue());
    pItem->setDoubleLength(HB_DBL_LENGTH(pItem->doubleValue()));
  } else {
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1080, nullptr, "-", 1, pItem);
...
static void hb_vmTimeStampPut(HB_ITEM *pItem, long lJulian, long lMilliSec)
{
#if 0
...
static void hb_vmTimeStampAdd(HB_ITEM *pResult, HB_ITEM *pItem, double dValue)
{
#if 0
  ...
  lJulian += pItem->dateTimeJulian();
  lMilliSec += pItem->dateTimeTime();
...
static void hb_vmPlus(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2)
{
  ...
  if (pItem1->isNumInt() && pItem2->isNumInt()) {
    ...
    if (nNumber2 >= 0 ? nResult >= nNumber1 : nResult < nNumber1) {
      ...
      pResult->setType(Harbour::Item::DOUBLE);
      pResult->setDoubleValue(dResult);
      pResult->setDoubleLength(HB_DBL_LENGTH(dResult));
      pResult->setDoubleDecimal(0);
    }
    ...
    if (pItem1->isTimeStamp() || pItem2->isTimeStamp()) {
      hb_vmTimeStampPut(pResult, pItem1->dateTimeJulian() + pItem2->dateTimeJulian(),
    ...
    if (pItem1->isTimeStamp()) {
      if (pItem2->isNumInt()) {
        hb_vmTimeStampPut(pResult, pItem1->dateTimeJulian() + static_cast<long>(HB_ITEM_GET_NUMINTRAW(pItem2)),
                          pItem1->dateTimeTime());
      } else {
        hb_vmTimeStampAdd(pResult, pItem1, pItem2->doubleValue());
      }
    } else {
    ...
    if (pItem2->isTimeStamp()) {
      if (pItem1->isNumInt()) {
        hb_vmTimeStampPut(pResult, static_cast<long>(HB_ITEM_GET_NUMINTRAW(pItem1)) + pItem2->dateTimeJulian(),
                          pItem2->dateTimeTime());
      } else {
        hb_vmTimeStampAdd(pResult, pItem2, pItem1->doubleValue());
      }
    } else {
      hb_itemPutDL(pResult, pItem1->getNL() + pItem2->getDL());
    }
  } else if (!hb_objOperatorCall(HB_OO_OP_PLUS, pResult, pItem1, pItem2, nullptr)) {
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1081, nullptr, "+", 2, pItem1, pItem2);
...
static void hb_vmMinus(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2)
{
  ...
  if (pItem1->isNumInt() && pItem2->isNumInt()) {
    ...
    long lTime = pItem1->dateTimeTime() - pItem2->dateTimeTime(),
         lJulian = pItem1->dateTimeJulian() - pItem2->dateTimeJulian();
    if (lTime != 0) {
    ...
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1082, nullptr, "-", 2, pItem1, pItem2);
...
static void hb_vmMult(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2)
{
  ...
  if (pItem1->isInteger() && pItem2->isInteger()) {
    ...
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1083, nullptr, "*", 2, pItem1, pItem2);
...
static void hb_vmDivide(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2)
{
  ...
  if (pItem1->isNumInt() && pItem2->isNumInt()) {
    ...
    if (nDivisor == 0) {
      HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ZERODIV, 1340, nullptr, "/", 2, pItem1, pItem2);
    ...
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1084, nullptr, "/", 2, pItem1, pItem2);
...
static void hb_vmModulus(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2)
{
  ...
  if (pItem1->isNumInt() && pItem2->isNumInt()) {
    ...
    if (nDivisor == 0) {
      HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ZERODIV, 1341, nullptr, "%", 2, pItem1, pItem2);
    ...
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1085, nullptr, "%", 2, pItem1, pItem2);
...
static void hb_vmPower(HB_ITEM *pResult, HB_ITEM *pItem1, HB_ITEM *pItem2)
{
  ...
  if (pItem1->isNumeric() && pItem2->isNumeric()) {
    ...
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1088, nullptr, "^", 2, pItem1, pItem2);
  ...
  if (pItem->isNumInt()) {
    if (pItem->isInteger()) {
      if (pItem->integerValue() < HB_VMINT_MAX) {
        ...
        pItem->setType(Harbour::Item::LONG);
        pItem->setLongValue(static_cast<HB_MAXINT>(pItem->integerValue()) + 1);
        pItem->setLongLength(HB_LONG_EXPLENGTH(pItem->longValue()));
#else
        pItem->setType(Harbour::Item::DOUBLE);
        pItem->setDoubleValue(static_cast<double>(pItem->integerValue()) + 1);
        pItem->setDoubleLength(HB_DBL_LENGTH(pItem->doubleValue()));
        pItem->setDoubleDecimal(0);
#endif
      }
    } else if (pItem->longValue() < HB_VMLONG_MAX) {
      pItem->setType(Harbour::Item::LONG);
      pItem->item.asLong.value++; // TODO: setLongValue
      pItem->setLongLength(HB_LONG_EXPLENGTH(pItem->longValue()));
    } else {
      pItem->setType(Harbour::Item::DOUBLE);
      pItem->setDoubleValue(static_cast<double>(pItem->longValue()) + 1);
      pItem->setDoubleLength(HB_DBL_LENGTH(pItem->doubleValue()));
      pItem->setDoubleDecimal(0);
    }
  } else if (pItem->isDouble()) {
    pItem->setType(Harbour::Item::DOUBLE);
    pItem->item.asDouble.value++; // TODO:
    pItem->setDoubleLength(HB_DBL_LENGTH(pItem->doubleValue()));
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1086, nullptr, "++", 1, pItem);
...
static void hb_vmDec(HB_ITEM *pItem)
{
  ...
  if (pItem->isNumInt()) {
    ...
    } else if (pItem->longValue() > HB_VMLONG_MIN) {
      pItem->setType(Harbour::Item::LONG);
      pItem->item.asLong.value--; // TODO: setLongValue
      pItem->setLongLength(HB_LONG_EXPLENGTH(pItem->longValue()));
    } else {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1087, nullptr, "--", 1, pItem);
...
static void hb_vmExactlyEqual()
{
  ...
  if (pItem1->isNil()) {
    ...
    pItem1->setLogicalValue(
        (pItem1->dateTimeJulian() == pItem2->dateTimeJulian() && pItem1->dateTimeTime() == pItem2->dateTimeTime()));
    pItem1->setType(Harbour::Item::LOGICAL);
    hb_stackDec();
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1070, nullptr, "==", 2, pItem1, pItem2);
    if (pResult) {
      hb_stackPop();
...
static void hb_vmEqual()
{
  ...
  if (pItem1->isNil()) {
    ...
    if (pItem1->isTimeStamp() && pItem2->isTimeStamp()) {
      pItem1->setLogicalValue((pItem1->dateTimeJulian() == pItem2->dateTimeJulian()) &&
                              (pItem1->dateTimeTime() == pItem2->dateTimeTime()));
    } else {
      pItem1->setLogicalValue((pItem1->dateTimeJulian() == pItem2->dateTimeJulian()));
    }
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1071, nullptr, "=", 2, pItem1, pItem2);
    if (pResult) {
      hb_stackPop();
      hb_itemMove(pItem1, pResult);
      hb_itemRelease(pResult);
...
static void hb_vmNotEqual()
{
  ...
  if (pItem1->isNil()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1072, nullptr, "<>", 2, pItem1, pItem2);
...
static void hb_vmLess()
{
  ...
  if (pItem1->isString() && pItem2->isString()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1073, nullptr, "<", 2, pItem1, pItem2);
...
static void hb_vmLessEqual()
{
  ...
  if (pItem1->isString() && pItem2->isString()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1074, nullptr, "<=", 2, pItem1, pItem2);
...
static void hb_vmGreater()
{
  ...
  if (pItem1->isString() && pItem2->isString()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1075, nullptr, ">", 2, pItem1, pItem2);
...
static void hb_vmGreaterEqual()
{
  ...
  if (pItem1->isString() && pItem2->isString()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1076, nullptr, ">=", 2, pItem1, pItem2);
...
static void hb_vmInstring()
{
  ...
  if (pItem1->isString() && pItem2->isString()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1109, nullptr, "$", 2, pItem1, pItem2);
...
static void hb_vmForTest() // Test to check the end point of the FOR
{
  ...
  if (pStep->isNumeric()) {
    ...
    hb_vmPushInteger(0);
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1073, nullptr, "<", 2, pStep, hb_stackItemFromTop(-1));
...
{
  hb_itemMove(hb_errorBlock(), static_cast<HB_ITEM *>(Cargo));
}
...
static void hb_vmSeqBlock()
{
  ...
  auto pItem = hb_stackItemFromTop(-1);
  if (pItem->isBlock()) {
    HB_ITEM *pBlock = hb_errorBlock();
    auto pBlockCopy = static_cast<HB_ITEM *>(hb_gcAllocRaw(sizeof(HB_ITEM), &s_gcSeqBlockFuncs));
...
void hb_vmEnumRelease(HB_ITEM *pBase, HB_ITEM *pValue)
{
#if 0
...
static HB_ITEM *hb_vmEnumRefRead(HB_ITEM *pRefer)
{
  return &(static_cast<PHB_ENUMREF>(pRefer->item.asExtRef.value))->oldvalue;
}

static HB_ITEM *hb_vmEnumRefWrite(HB_ITEM *pRefer, HB_ITEM *pSource)
{
...
static void hb_vmEnumRefCopy(HB_ITEM *pDest)
{
...
static void hb_vmEnumReference(HB_ITEM *pBase)
{
#if 0
  ...
  pBase->setType(Harbour::Item::BYREF | Harbour::Item::EXTREF);
...
static void hb_vmEnumStart(int32_t nVars, int32_t nDescend)
{
...
#if 0
   pItem = hb_itemUnRef(hb_stackItemFromTop(-(static_cast<int32_t>(nVars) << 1)));
  ...
  for (int32_t i = static_cast<int32_t>(nVars) << 1; i > 0 && fStart; i -= 2) {
    HB_ITEM *pBase;
...
static void hb_vmEnumNext()
{
  ...
  for (i = static_cast<int32_t>(hb_stackItemFromTop(-1)->integerValue()); i > 0; --i) {
    auto pEnumRef = hb_stackItemFromTop(-(i << 1));
    auto pEnum = hb_itemUnRefOnce(pEnumRef);
    HB_ITEM *pBase = pEnum->item.asEnum.basePtr;
    if (pBase->isByRef()) {
      pBase = hb_itemUnRef(pBase);
    }
    if (pBase->isArray()) {
...
static HB_ITEM *hb_vmSwitchGet()
{
  HB_STACK_TLS_PRELOAD
  auto pSwitch = hb_stackItemFromTop(-1);

  if (!(pSwitch->isNumInt() || pSwitch->isString())) {
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 3104, nullptr, "SWITCH", 1, pSwitch);
...
{
  HB_STACK_TLS_PRELOAD
  HB_ITEM *pSwitch = hb_vmSwitchGet();
...
static void hb_vmNot()
{
  ...
  if (pItem->isLogical()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1077, nullptr, ".NOT.", 1, pItem);
...
static void hb_vmAnd()
{
  ...
  if (pItem1->isLogical() && pItem2->isLogical()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1078, nullptr, ".AND.", 2, pItem1, pItem2);
...
static void hb_vmOr()
{
  ...
  if (pItem1->isLogical() && pItem2->isLogical()) {
    ...
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1079, nullptr, ".OR.", 2, pItem1, pItem2);
...
static void hb_vmArrayPush()
{
  ...
  if (pArray->isHash() && pIndex->isHashKey()) {
    ...
    nIndex = static_cast<HB_SIZE>(pIndex->longValue());
  } else if (pIndex->isDouble()) {
    nIndex = static_cast<HB_SIZE>(pIndex->doubleValue());
  } else {
...
static void hb_vmArrayPushRef()
{
  ...
  HB_ITEM *pArray = pRefer->isByRef() ? hb_itemUnRef(pRefer) : pRefer;
...
static void hb_vmArrayGen(HB_SIZE nElements) // generates an nElements Array and fills it from the stack values
{
  ...
  if (nElements) {
    ...
    for (HB_SIZE nPos = 0; nPos < nElements; nPos++) {
      auto pValue = hb_stackItemFromTop(static_cast<int32_t>(nPos - nElements - 1));
      pValue->type &= ~(Harbour::Item::MEMOFLAG | Harbour::Item::DEFAULT);
      hb_itemMove(pArray->arrayItems() + nPos, pValue);
    }
    // move the new array to position of first parameter
    hb_itemMove(hb_stackItemFromTop(-1 - static_cast<int32_t>(nElements)), pArray);
...
static HB_BOOL hb_vmArrayNew(HB_ITEM *pArray, uint16_t uiDimension)
{
#if 0
  ...
  auto pDim = hb_stackItemFromTop(static_cast<int32_t>(-1 - uiDimension));

  // use the proper type of number of elements
  if (pDim->isInteger()) {
    nElements = static_cast<HB_ISIZ>(pDim->integerValue());
  } else if (pDim->isLong()) {
    nElements = static_cast<HB_ISIZ>(pDim->longValue());
  } else if (pDim->isDouble()) {
    nElements = static_cast<HB_ISIZ>(pDim->doubleValue());
  } else {
...
static void hb_vmArrayDim(
    uint16_t uiDimensions) // generates an uiDimensions Array and initialize those dimensions from the stack values
{
#if 0
  ...
  if (uiDimensions) {
    ...
    hb_itemMove(hb_stackItemFromTop(static_cast<int32_t>(-1 - uiDimensions)), hb_stackItemFromTop(-1));
    do {
      hb_stackPop();
    } while (--uiDimensions);
  } else {
...
static void hb_vmHashGen(HB_SIZE nElements) // generates an nElements Hash and fills it from the stack values
{
  ...
  HB_ITEM *pHash = hb_hashNew(nullptr);
  hb_hashPreallocate(pHash, nElements);
  nElements <<= 1;
  int32_t iPos = -static_cast<int32_t>(nElements);
  while (iPos) {
...
static HB_LONG hb_vmArgsJoin(HB_LONG lLevel, uint16_t uiArgSets)
{
  HB_STACK_TLS_PRELOAD
...
static void hb_vmMacroDo(uint16_t uiArgSets)
{
  ...
  hb_vmProc(static_cast<uint16_t>(lArgs));
}
...
static void hb_vmMacroSend(uint16_t uiArgSets)
{
  ...
  hb_stackDecrease(uiArgSets);
  hb_itemSetNil(hb_stackReturnItem());
  hb_vmSend(static_cast<uint16_t>(lArgs));
  hb_stackPushReturn();
}
  ...
  if (pArray->isArray()) {
    ...
    if (nLen) {
      for (HB_SIZE nPos = 1; nPos < nLen; ++nPos) {
        hb_vmPush(pArray->arrayItems() + nPos);
      }
      auto pCount = hb_stackAllocItem();
      hb_itemCopy(pCount, pArray->arrayItems());
...
static HB_ERRCODE hb_vmSelectWorkarea(HB_ITEM *pAlias, HB_SYMB *pField)
{
#if 0
  ...
  do {
    ...
    case Harbour::Item::LONG:
      ...
      hb_rddSelectWorkAreaNumber(static_cast<int32_t>(pAlias->longValue()));
      pAlias->setType(Harbour::Item::NIL);
      break;

    case Harbour::Item::DOUBLE:
      // Alias was evaluated from an expression, (nWorkArea)->field
      hb_rddSelectWorkAreaNumber(static_cast<int32_t>(pAlias->doubleValue()));
      pAlias->setType(Harbour::Item::NIL);
      break;
    ...
    default:
      if (pField) {
        ...
        HB_ITEM *pSubstVal = hb_errRT_BASE_Subst(EG_ARG, 1065, nullptr, "&", 2, pAlias, hb_stackItemFromTop(-1));
        hb_stackPop();
        if (pSubstVal) {
...
static void hb_vmSwapAlias()
{
  ...
  HB_STACK_TLS_PRELOAD
  auto pItem = hb_stackItemFromTop(-1);
  HB_ITEM *pWorkArea = hb_stackItemFromTop(-2);
...
void hb_vmProc(uint16_t uiParams)
{
#if 0
...
void hb_vmDo(uint16_t uiParams)
{
#if 0
...
void hb_vmSend(uint16_t uiParams)
{
#if 0
...
static void hb_vmPushObjectVarRef()
{
  ...
  HB_STACK_TLS_PRELOAD
  HB_STACK_STATE sStackState;
  HB_ITEM *pItem = hb_stackNewFrame(&sStackState, 0); // procedure name
...
void hb_vmEval(uint16_t uiParams)
{
#if 0
...
HB_ITEM *hb_vmEvalBlock(HB_ITEM *pBlock)
{
#if 0
...
HB_ITEM *hb_vmEvalBlockV(HB_ITEM *pBlock, HB_ULONG ulArgCount, ...)
{
#if 0
  ...
  hb_vmSend(static_cast<uint16_t>(ulArgCount));

  return hb_stackReturnItem();
}

// Evaluates a passed codeblock item or macro pointer item
HB_ITEM *hb_vmEvalBlockOrMacro(HB_ITEM *pItem)
{
#if 0
...
void hb_vmDestroyBlockOrMacro(HB_ITEM *pItem)
{
#if 0
...
HB_ITEM *hb_vmCompileMacro(const char *szExpr, HB_ITEM *pDest)
{
#if 0
...
void hb_vmFunction(uint16_t uiParams)
{
#if 0
...
static void hb_vmDummyDebugEntry(int32_t nMode, int32_t nLine, const char *szName, int32_t nIndex, HB_ITEM *pFrame)
{
#if 0
...
static void hb_vmDebuggerShowLine(uint16_t uiLine) // makes the debugger shows a specific source code line
{
#if 0
...
static void hb_vmLocalName(uint16_t uiLocal,
                           const char *szLocalName) // locals and parameters index and name information for the debugger
{
#if 0
                            ...
                            const char *szStaticName) // statics vars information for the debugger
{
  ...
  if (hb_stackBaseItem()->symbolStackState()->fDebugging) {
    s_pFunDbgEntry(HB_DBG_STATICNAME, 0, szStaticName, uiStatic, static_cast<HB_ITEM *>(hb_stackGetStaticsBase()));
  }
}
...
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_vmFrame(%d, %d)", static_cast<int32_t>(usLocals), static_cast<int32_t>(ucParams)));
#endif
...
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_vmVFrame(%d, %d)", static_cast<int32_t>(usLocals), static_cast<int32_t>(ucParams)));
#endif
...
static HB_ITEM *hb_vmTSVRefRead(HB_ITEM *pRefer)
{
  auto pTSVRef = static_cast<PHB_TSVREF>(pRefer->item.asExtRef.value);
  auto pItem = static_cast<HB_ITEM *>(hb_stackTestTSD(&pTSVRef->threadData));

  if (!pItem) {
    pItem = static_cast<HB_ITEM *>(hb_stackGetTSD(&pTSVRef->threadData));
    hb_itemCloneTo(pItem, &pTSVRef->source);
  }
  return pItem;
}

static HB_ITEM *hb_vmTSVRefWrite(HB_ITEM *pRefer, HB_ITEM *pSource)
{
  auto pTSVRef = static_cast<PHB_TSVREF>(pRefer->item.asExtRef.value);
  HB_SYMBOL_UNUSED(pSource);
  return static_cast<HB_ITEM *>(hb_stackGetTSD(&pTSVRef->threadData));
}

static void hb_vmTSVRefCopy(HB_ITEM *pDest)
{
...
static void hb_vmTSVRefMark(void *value)
{
  ...
  auto pItem = static_cast<HB_ITEM *>(hb_stackTestTSD(&(static_cast<PHB_TSVREF>(value))->threadData));
...
{
  if (static_cast<HB_ITEM *>(pThreadItem)->isComplex()) {
    static_cast<HB_ITEM *>(pThreadItem)->clear();
  }
}

// create extended thread static variable reference
static void hb_vmTSVReference(HB_ITEM *pStatic)
{
#if 0
...
static void hb_vmInitThreadStatics(uint16_t uiCount, const uint8_t *pCode)
{
  ...
  while (uiCount--) {
    uint16_t uiStatic = HB_PCODE_MKUINT16(pCode);
    HB_ITEM *pStatic = (static_cast<HB_ITEM *>(hb_stackGetStaticsBase()))->arrayItems() + uiStatic - 1;
...
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_vmPushLogical(%d)", static_cast<int32_t>(bValue)));
#endif
...
void hb_vmPushNumber(double dNumber, int32_t iDec)
{
  ...
  if (iDec) {
    ...
    hb_vmPushInteger(static_cast<int32_t>(dNumber));
...
static uint16_t hb_vmCalcIntWidth(HB_MAXINT nNumber)
{
  uint16_t iWidth;
...
void hb_vmPushSize(HB_ISIZ nNumber)
{
  ...
  hb_vmPushInteger(static_cast<int32_t>(nNumber));
#else
...
static void hb_vmPushHBLong(HB_MAXINT nNumber)
{
  ...
  auto pItem = hb_stackAllocItem();
  pItem->setType(Harbour::Item::LONG);
  pItem->setLongValue(nNumber);
  pItem->setLongLength(HB_LONG_LENGTH(nNumber));
}
...
static void hb_vmPushBlock(const uint8_t *pCode, HB_SYMB *pSymbols, HB_SIZE nLen)
{
  ...
  HB_STACK_TLS_PRELOAD
  uint16_t uiLocals = HB_PCODE_MKUINT16(&pCode[2]);
  ...
  pItem->setBlockParamCnt(HB_PCODE_MKUINT16(pCode));
...
static void hb_vmPushLocalByRef(int32_t iLocal)
{
  ...
  if (iLocal >= 0) {
    HB_ITEM *pLocal = hb_stackLocalVariableAt(&iLocal);
...
static void hb_vmPushStaticByRef(uint16_t uiStatic)
{
  ...
  auto pTop = hb_stackAllocItem();
  auto pBase = static_cast<HB_ITEM *>(hb_stackGetStaticsBase());
...
#define HB_SYM_STATICSBASE(p)                                                                                          \
  (static_cast<HB_ITEM *>(((p)->scope.value & HB_FS_FRAME) ? (p)->value.pStaticsBase : nullptr))
...
static void hb_vmStaticsClear()
{
  ...
  while (pLastSymbols) {
    if (pLastSymbols->uiStaticsOffset) {
      ...
      HB_ITEM *pStatics = HB_SYM_STATICSBASE(pSym);
      if (pStatics) {
...
static HB_ITEM *hb_vmStaticsArray()
{
...
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_vmBeginSymbolGroup(%p,%d)", hDynLib, static_cast<int32_t>(fClone)));
#endif
            ...
            if (pLastSymbols->fActive && (pLastSymbols->hScope & HB_FS_INIT) != 0) {
              ...
              while (ui--) {
                ...
                if (scope == HB_FS_INIT &&
                  ...
                  hb_vmProc(static_cast<uint16_t>(argc));
    ...
    while (pLastSymbols) {
      if (pLastSymbols->hDynLib == hDynLib) {
        ...
        if (pLastSymbols->fActive && (pLastSymbols->hScope & HB_FS_EXIT) != 0) {
          for (uint16_t ui = 0; ui < pLastSymbols->uiModuleSymbols; ui++) {
            HB_SYMBOLSCOPE scope = (pLastSymbols->pModuleSymbols + ui)->scope.value & HB_FS_INITEXIT;
...
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_vmRegisterSymbols(%p,%hu,%s,%lu,%d,%d,%d)", static_cast<void*>(pModuleSymbols), uiSymbols, szModuleName, ulID, static_cast<int32_t>(fDynLib), static_cast<int32_t>(fClone), static_cast<int32_t>(fOverLoad)));
#endif
...
static void hb_vmVerifySymbols(HB_ITEM *pArray)
{
#if 0
...
static void hb_vmVerifyPCodeVersion(const char *szModuleName, uint16_t uiPCodeVer)
{
  if (uiPCodeVer != 0) {
...
static void hb_vmDoInitFunctions(HB_BOOL fClipInit)
{
  ...
  while (pLastSymbols && hb_vmRequestQuery() == 0) {
    ...
    if (pLastSymbols->fActive && (pLastSymbols->hScope & HB_FS_INIT) != 0) {
      uint16_t ui = pLastSymbols->uiModuleSymbols;
      ...
      while (ui--) {
        ...
        if (scope == HB_FS_INIT &&
          ...
          hb_vmPushSymbol(pLastSymbols->pModuleSymbols + ui);
          hb_vmPushNil();
          hb_vmProc(static_cast<uint16_t>(hb_cmdargPushArgs()));
          if (hb_vmRequestQuery() != 0) {
            break;
          }
        }
...
static HB_ITEM *hb_vmItemRawRefRead(HB_ITEM *pRefer)
{
  return static_cast<HB_ITEM *>(pRefer->item.asExtRef.value);
}

static HB_ITEM *hb_vmItemRawRefWrite(HB_ITEM *pRefer, HB_ITEM *pSource)
{
  HB_SYMBOL_UNUSED(pSource);
  return static_cast<HB_ITEM *>(pRefer->item.asExtRef.value);
}

static void hb_vmItemRawRefCopy(HB_ITEM *pDest)
{
  pDest->setType(Harbour::Item::NIL);
  hb_itemCopy(pDest, static_cast<HB_ITEM *>(pDest->item.asExtRef.value));
}
...
static HB_ITEM *hb_vmItemRefRead(HB_ITEM *pRefer)
{
  return &(static_cast<PHB_ITMREF>(pRefer->item.asExtRef.value))->memvar;
}

static HB_ITEM *hb_vmItemRefWrite(HB_ITEM *pRefer, HB_ITEM *pSource)
{
  return hb_itemUnRefWrite((static_cast<PHB_ITMREF>(pRefer->item.asExtRef.value))->value, pSource);
}

static void hb_vmItemRefCopy(HB_ITEM *pDest)
{
  pDest->setType(Harbour::Item::NIL);
...
static void hb_vmItemRefClear(void *value)
{
  ...
  if (!(&pItmRef->memvar)->isMemVar() || pItmRef->memvar.item.asMemvar.value != pItmRef->value ||
      !pItmRef->value->isExtRef() || pItmRef->value->item.asExtRef.func != &s_ItmExtRawRef) {
...
void hb_vmPushItemRef(HB_ITEM *pItem)
{
#if 0
  ...
  pItmRef->value = static_cast<HB_ITEM *>(hb_xgrab(sizeof(HB_ITEM)));
...
static HB_ITEM *hb_vmMsgRefRead(HB_ITEM *pRefer)
{
...
static HB_ITEM *hb_vmMsgRefWrite(HB_ITEM *pRefer, HB_ITEM *pSource)
{
  auto pMsgRef = static_cast<PHB_MSGREF>(pRefer->item.asExtRef.value);
...
static void hb_vmMsgRefCopy(HB_ITEM *pDest)
{
...
HB_BOOL hb_vmMsgReference(HB_ITEM *pObject, HB_DYNS *pMessage, HB_DYNS *pAccMsg)
{
#if 0
...
static HB_ITEM *hb_vmMsgIdxRefRead(HB_ITEM *pRefer)
{
  auto pMsgIdxRef = static_cast<PHB_MSGIDXREF>(pRefer->item.asExtRef.value);

  if (hb_vmRequestQuery() == 0) {
    HB_STACK_TLS_PRELOAD
    HB_ITEM *pObject = (&pMsgIdxRef->object)->isByRef() ? hb_itemUnRef(&pMsgIdxRef->object) : &pMsgIdxRef->object;
...
static HB_ITEM *hb_vmMsgIdxRefWrite(HB_ITEM *pRefer, HB_ITEM *pSource)
{
  auto pMsgIdxRef = static_cast<PHB_MSGIDXREF>(pRefer->item.asExtRef.value);
...
static void hb_vmMsgIdxRefCopy(HB_ITEM *pDest)
{
...
static void hb_vmMsgIndexReference(HB_ITEM *pRefer, HB_ITEM *pObject, HB_ITEM *pIndex)
{
  ...
  pIndex->setType(Harbour::Item::BYREF | Harbour::Item::EXTREF);
...
void hb_vmRequestBreak(HB_ITEM *pItem)
{
#if 0
...
void hb_vmRequestCancel(void)
{
  ...
  if (hb_stackSetStruct()->HB_SET_CANCEL) {
    ...
    while (hb_procinfo(iLevel++, buffer, &uiLine, file)) {
      auto l = static_cast<int32_t>(strlen(buffer));
...
uint16_t hb_vmRequestQuery(void)
{
  HB_STACK_TLS_PRELOAD
...
void hb_vmRequestRestore(void)
{
  ...
  uint16_t uiAction = pItem->item.asRecover.request | hb_stackGetActionRequest();

#if defined(HB_MT_VM)
  if (uiAction & HB_VMSTACK_REQUESTED) {
    hb_vmThreadQuit();
  } else {
    auto iCount = static_cast<int32_t>(pItem->item.asRecover.base);
#else
  {
#endif
...
HB_BOOL hb_vmTryEval(HB_ITEM **pResult, HB_ITEM *pItem, HB_ULONG ulPCount, ...)
{
#if 0
  ...
  *pResult = nullptr;
  if (s_fHVMActive) {
    ...
    if (pSymbol && hb_vmRequestReenter()) {
      ...
      if (pItem != nullptr) {
        hb_vmSend(static_cast<uint16_t>(ulPCount));
      } else {
        hb_vmProc(static_cast<uint16_t>(ulPCount));
      }
...
HB_BOOL hb_xvmAlwaysEnd(void)
{
  ...
  hb_stackSetRecoverBase(hb_stackItemFromTop(HB_RECOVER_STATE)->item.asRecover.base);
  uint16_t uiCurrAction = hb_stackGetActionRequest();
  uint16_t uiPrevAction = hb_stackItemFromTop(HB_RECOVER_STATE)->item.asRecover.request;
  // restore requested action
  if ((uiCurrAction | uiPrevAction) & HB_QUIT_REQUESTED) {
    hb_stackSetActionRequest(HB_QUIT_REQUESTED);
  } else if ((uiCurrAction | uiPrevAction) & HB_BREAK_REQUESTED) {
...
HB_BOOL hb_xvmSwitchGet(HB_ITEM **pSwitchPtr)
{
#if 0
...
void hb_xvmSetLine(uint16_t uiLine)
{
#if 0
...
HB_BOOL hb_xvmDo(uint16_t uiParams)
{
#if 0
...
HB_BOOL hb_xvmFunction(uint16_t uiParams)
{
#if 0
...
HB_BOOL hb_xvmSend(uint16_t uiParams)
{
#if 0
...
static HB_ITEM *hb_xvmLocalPtr(int32_t iLocal)
{
#if 0
...
void hb_xvmPushStaticByRef(uint16_t uiStatic)
{
#if 0
...
void hb_xvmPopStatic(uint16_t uiStatic)
{
#if 0
...
HB_BOOL hb_xvmStaticAdd(uint16_t uiStatic)
{
#if 0
...
HB_BOOL hb_xvmMemvarAdd(HB_SYMB *pSymbol)
{
  ...
  if (pVal1->isString() && pVal2->isString()) {
    HB_ITEM *pMemVar = hb_memvarGetItem(pSymbol);
    if (pMemVar) {
...
HB_BOOL hb_xvmEqualInt(HB_LONG lValue)
{
  ...
  if (pItem->isInteger()) {
...
#else
    pItem->setLogicalValue(pItem->longValue() == static_cast<HB_MAXINT>(lValue));
#endif
    pItem->setType(Harbour::Item::LOGICAL);
  } else if (pItem->isDouble()) {
    pItem->setLogicalValue(pItem->doubleValue() == static_cast<double>(lValue));
    pItem->setType(Harbour::Item::LOGICAL);
  } else if (pItem->isNil()) {
    pItem->setLogicalValue(false);
    pItem->setType(Harbour::Item::LOGICAL);
    ...
    hb_vmPushLong(lValue);
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1071, nullptr, "=", 2, pItem, hb_stackItemFromTop(-1));
    if (pResult) {
      hb_stackPop();
      hb_itemMove(pItem, pResult);
      hb_itemRelease(pResult);
...
HB_BOOL hb_xvmEqualIntIs(HB_LONG lValue, HB_BOOL *pfValue)
{
  ...
  if (pItem->isInteger()) {
    ...
    *pfValue = f;
#else
    *pfValue = pItem->longValue() == static_cast<HB_MAXINT>(lValue);
#endif
    hb_stackDec();
  } else if (pItem->isDouble()) {
    *pfValue = pItem->doubleValue() == static_cast<double>(lValue);
    hb_stackDec();
  } else if (pItem->isNil()) {
    *pfValue = false;
    hb_stackDec();
...
HB_BOOL hb_xvmNotEqualInt(HB_LONG lValue)
{
  ...
  if (pItem->isInteger()) {
    ...
    hb_vmPushLong(lValue);
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1072, nullptr, "<>", 2, pItem, hb_stackItemFromTop(-1));
    if (pResult) {
      hb_stackPop();
      hb_itemMove(pItem, pResult);
      hb_itemRelease(pResult);
...
HB_BOOL hb_xvmLessThenInt(HB_LONG lValue)
{
  ...
  if (pItem->isInteger()) {
    ...
    hb_vmPushLong(lValue);
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1073, nullptr, "<", 2, pItem, hb_stackItemFromTop(-1));
    if (pResult) {
      hb_stackPop();
      hb_itemMove(pItem, pResult);
      hb_itemRelease(pResult);
...
HB_BOOL hb_xvmLessEqualThenInt(HB_LONG lValue)
{
  ...
  if (pItem->isInteger()) {
    ...
    hb_vmPushLong(lValue);
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1074, nullptr, "<=", 2, pItem, hb_stackItemFromTop(-1));
    if (pResult) {
      hb_stackPop();
      hb_itemMove(pItem, pResult);
      hb_itemRelease(pResult);
...
HB_BOOL hb_xvmGreaterThenInt(HB_LONG lValue)
{
  ...
  if (pItem->isInteger()) {
    ...
    hb_vmPushLong(lValue);
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1075, nullptr, ">", 2, pItem, hb_stackItemFromTop(-1));
    if (pResult) {
      hb_stackPop();
      hb_itemMove(pItem, pResult);
      hb_itemRelease(pResult);
...
HB_BOOL hb_xvmGreaterEqualThenInt(HB_LONG lValue)
{
  ...
  if (pItem->isInteger()) {
    ...
    hb_vmPushLong(lValue);
    HB_ITEM *pResult = hb_errRT_BASE_Subst(EG_ARG, 1076, nullptr, ">=", 2, pItem, hb_stackItemFromTop(-1));
    if (pResult) {
      hb_stackPop();
      hb_itemMove(pItem, pResult);
      hb_itemRelease(pResult);
...
HB_BOOL hb_xvmMultByInt(HB_LONG lValue)
{
  ...
  if (pValue->isNumeric()) {
    ...
    hb_vmPushLong(lValue);
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1083, nullptr, "*", 2, pValue, hb_stackItemFromTop(-1));
    if (pSubst) {
      hb_stackPop();
...
HB_BOOL hb_xvmDivideByInt(HB_LONG lDivisor)
{
  ...
  if (pValue->isNumeric()) {
    if (lDivisor == 0) {
      ...
      HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ZERODIV, 1340, nullptr, "/", 2, pValue, hb_stackItemFromTop(-1));
      if (pSubst) {
        hb_stackPop();
        hb_itemMove(pValue, pSubst);
        hb_itemRelease(pSubst);
    ...
    hb_vmPushLong(lDivisor);
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1084, nullptr, "/", 2, pValue, hb_stackItemFromTop(-1));
    if (pSubst) {
      hb_stackPop();
      hb_itemMove(pValue, pSubst);
      hb_itemRelease(pSubst);
...
HB_BOOL hb_xvmModulusByInt(HB_LONG lDivisor)
{
  ...
  if (pValue->isNumeric()) {
    if (lDivisor == 0) {
      hb_vmPushLong(lDivisor);
      HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ZERODIV, 1341, nullptr, "%", 2, pValue, hb_stackItemFromTop(-1));
      if (pSubst) {
        hb_stackPop();
        hb_itemMove(pValue, pSubst);
        hb_itemRelease(pSubst);
    ...
    hb_vmPushLong(lDivisor);
    HB_ITEM *pSubst = hb_errRT_BASE_Subst(EG_ARG, 1085, nullptr, "%", 2, pValue, hb_stackItemFromTop(-1));
    if (pSubst) {
      hb_stackPop();
      hb_itemMove(pValue, pSubst);
      hb_itemRelease(pSubst);
...
void hb_xvmArrayDim(uint16_t uiDimensions)
{
#if 0
...
void hb_xvmLocalName(uint16_t uiLocal, const char *szLocalName)
{
#if 0
...
{
#if 0
   HB_TRACE(HB_TR_DEBUG, ("hb_xvmStaticName(%d, %hu, %s)", static_cast<int32_t>(bIsGlobal), uiStatic, szStaticName));
#endif
...
HB_BOOL hb_xvmMacroArrayGen(uint16_t uiArgSets)
{
#if 0
...
HB_BOOL hb_xvmMacroDo(uint16_t uiArgSets)
{
#if 0
...
HB_BOOL hb_xvmMacroFunc(uint16_t uiArgSets)
{
#if 0
...
HB_BOOL hb_xvmMacroSend(uint16_t uiArgSets)
{
#if 0
...
HB_BOOL hb_xvmMacroPushRef(void)
{
  ...
  HB_STACK_TLS_PRELOAD
  HB_ITEM *pMacro = hb_stackItemFromTop(-1);
  hb_macroPushReference(pMacro);
  HB_XVM_RETURN
}
...
HB_ITEM *hb_dbg_vmVarSGet(HB_ITEM *pStaticsBase, int32_t nOffset)
{
  if (pStaticsBase) {
...
HB_ITEM *hb_dbg_vmVarGGet(int32_t nGlobal, int32_t nOffset)
{
#if 0
...
HB_FUNC(__DBGVMVARGLIST)
{
  if (hb_vmInternalsEnabled()) {
#if 0
      HB_ITEM *pGlobals = hb_itemClone(&s_aGlobals);
#else
...
HB_FUNC(__RECOVERERRORBLOCK)
{
  ...
  if (nRecoverBase > 0 && nRecoverBase < hb_stackTopOffset()) {
    ...
    if (pItem->isPointer() && pItem->pointerCollect() && pItem->pointerSingle() &&
      ...
      hb_itemReturn(static_cast<HB_ITEM *>(pItem->pointerValue()));
    }
  }
}
