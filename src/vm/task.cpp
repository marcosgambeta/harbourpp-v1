...
#        if defined(JB_SP)
#           define HB_TASK_STACK_INIT(jmp, sp)   do { (jmp)[0].__jmpbuf[JB_SP] = (int) (sp); } while(false)
#        else
#           define HB_TASK_STACK_INIT(jmp, sp)   do { (jmp)[0].__jmpbuf[4] = (int) (sp); } while(false)
#        endif
#     elif defined(HB_OS_WIN)
#        define HB_TASK_STACK_INIT(jmp, sp)      do { (jmp)[7] = (unsigned) (sp); } while(false)
#     endif
#  endif
   ...
   while( pTask != s_currTask ) {
      if( pTask->state == TASK_RUNNING || pTask->state == TASK_INIT ) {
         ...
         if( pTask->state == TASK_SLEEPING && pTask == s_taskSleep ) {
            if( s_taskSleep->wakeup == HB_TASK_NO_DELAY || (s_taskSleep->wakeup != HB_TASK_INFINITE_DELAY && s_taskSleep->wakeup <= hb_taskTimeStop(0)) ) {
