 ...
 */
static bool hb_clsAddMsg(HB_USHORT uiClass, const char * szMessage, HB_USHORT uiType, HB_USHORT uiScope, PHB_ITEM pFunction, PHB_ITEM pInit)
{
   if( szMessage && uiClass && uiClass <= s_uiClasses ) {
      ...
      if( !pNewMeth->pMessage ) {
         ...
         bool fOverLoad = (pNewMeth->uiScope & HB_OO_CLSTP_OVERLOADED) || ((pNewMeth->uiScope & HB_OO_CLSTP_NONVIRTUAL) && pNewMeth->uiSprClass != uiClass);
 ...
 */
static HB_USHORT hb_clsNew(const char * szClassName, HB_USHORT uiDatas, PHB_ITEM pSuperArray, PHB_SYMB pClassFunc, bool fModuleFriendly)
{
   PMETHOD pMethod;
...
HB_FUNC( __CLSNEW )
{
   ...
   PHB_ITEM pSuperArray = hb_param(3, Harbour::Item::ANY);
   if( pSuperArray && HB_IS_NIL(pSuperArray) ) {
      pSuperArray = nullptr;
   }

   PHB_ITEM pClassFunc = hb_param(4, Harbour::Item::ANY);
   if( pClassFunc && HB_IS_NIL(pClassFunc) ) {
      pClassFunc = nullptr;
   }

   PHB_ITEM pModFriend = hb_param(5, Harbour::Item::ANY);
