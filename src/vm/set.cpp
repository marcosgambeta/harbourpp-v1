...
static const char * is_devicename(const char * szFileName)
{
   if( szFileName && *szFileName ) {
      ...
      const char * szDevices[] = {"NUL", "PRN", "CON", "LPT1", "LPT2", "LPT3", "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9"};
      int iSkip = 0;
...
HB_BOOL hb_setSetCentury(HB_BOOL new_century_setting)
{
   ...
   if( old_century_setting != new_century_setting ) {
      ...
      char * szNewFormat = static_cast<char*>(hb_xgrab(size + 1));
      ...
      switch( set_specifier ) {
         ...
         case HB_SET_PRINTFILE:
            if( HB_IS_STRING(pItem1) || HB_IS_NIL(pItem1) ) {
               ...
               open_handle(pSet, hb_itemGetCPtr(pItem1), set_logical(pItem2, false), set_specifier);
               fResult = true;
               if( set_specifier == HB_SET_PRINTFILE && pSet->HB_SET_PRINTFILE == nullptr ) {
                  pSet->HB_SET_PRINTFILE = hb_set_PRINTFILE_default();
               }
...
HB_WCHAR * hb_osStrU16Encode(const char * pszName)
{
   if( hb_vmIsReady() ) {
      ...
      if( cdp ) {
         ...
         HB_WCHAR * pszBufferW = static_cast<HB_WCHAR*>(hb_xgrab((nSize + 1) * sizeof(HB_WCHAR)));
...
char * hb_osStrU16Decode(const HB_WCHAR * pszNameW)
{
   if( hb_vmIsReady() ) {
      ...
      if( cdp ) {
         ...
         char * pszBuffer = static_cast<char*>(hb_xgrab(nSize + 1));
