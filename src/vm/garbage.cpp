...
#define HB_GARBAGE_SIZE                                                                                                \
  ((sizeof(HB_GARBAGE) + HB_ALLOC_ALIGNMENT - 1) - (sizeof(HB_GARBAGE) + HB_ALLOC_ALIGNMENT - 1) % HB_ALLOC_ALIGNMENT)
#else
...
{
  hb_gcItemRef(static_cast<HB_ITEM *>(Cargo));
}

static HB_GARBAGE_FUNC(hb_gcGripRelease)
{
  if (static_cast<HB_ITEM *>(Cargo)->isComplex()) {
    static_cast<HB_ITEM *>(Cargo)->clear();
  }
}

static const HB_GC_FUNCS s_gcGripFuncs = {hb_gcGripRelease, hb_gcGripMark};

HB_ITEM *hb_gcGripGet(HB_ITEM *pOrigin)
{
  PHB_GARBAGE pAlloc = HB_GARBAGE_NEW(sizeof(HB_ITEM));
  auto pItem = static_cast<HB_ITEM *>(HB_BLOCK_PTR(pAlloc));
...
void hb_gcGripDrop(HB_ITEM *pItem)
{
  hb_gcRefFree(pItem);
}
...
void hb_gcItemRef(HB_ITEM *pItem)
{
  while (pItem->isByRef()) {
    if (pItem->isEnum()) {
      return;
...
void hb_gcReleaseAll(void)
{
  if (s_pCurrBlock) {
    ...
    do {
      ...
      PHB_GARBAGE pDelete = s_pCurrBlock;
      hb_gcUnlink(&s_pCurrBlock, pDelete);
      HB_GC_AUTO_DEC();
      HB_GARBAGE_FREE(pDelete);
...
HB_FUNC(HB_GCSETAUTO)
{
  ...
  bool fSet = HB_ISNUM(1);

  uintptr_t nBlocks = fSet ? hb_parnint(1) * 1000 : 0;

  HB_GC_LOCK();
  uintptr_t nPrevBlocks = s_ulBlocksAuto;
  if (fSet) {
