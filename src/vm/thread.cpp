...
#elif defined(HB_ATOM_INC) && defined(HB_ATOM_DEC) && defined(HB_ATOM_GET) && defined(HB_ATOM_SET)
void hb_atomic_set(volatile HB_COUNTER *pCounter, HB_COUNTER value)
{
...
static HB_GARBAGE_FUNC(hb_threadDestructor)
{
  ...
  if (pThread->pWaitList.cond) {
    ...
    pThread->pWaitList.cond = static_cast<HANDLE>(0);
#endif
  }
#endif
}
...
static const HB_GC_FUNCS s_gcThreadFuncs = {hb_threadDestructor, hb_threadMark};
...
HB_CARGO_FUNC(hb_threadStartVM)
{
  ...
  if (ulPCount > 0) {
    ...
    if (fSend) {
      hb_vmSend(static_cast<uint16_t>(ulPCount - 1));
    } else {
      hb_vmProc(static_cast<uint16_t>(ulPCount - 1));
    }
  } else {
    hb_itemRelease(pThread->pParams);
    pThread->pParams = nullptr;
...
PHB_THREADSTATE hb_threadStateClone(HB_ULONG ulAttr, PHB_ITEM pParams)
{
  ...
  PHB_THREADSTATE pThread = hb_threadStateNew();
#if defined(HB_MT_VM)
...
HB_FUNC(HB_THREADSTART)
{
  ...
  while (pStart && pStart->isNumeric()) {
    ulAttr |= static_cast<HB_ULONG>(pStart->getNL());
...
#define HB_THREAD_WAIT_ALLOC 16
  ...
  if (HB_ISARRAY(1)) {
    ...
    auto iLen = static_cast<int32_t>(hb_arrayLen(pArray));
    int32_t i;
...
#else
          HB_CRITICAL_UNLOCK(pMutex->mutex);
          (void)HB_COND_WAIT(pMutex->cond_w); // TODO: C++ cast
          HB_CRITICAL_LOCK(pMutex->mutex);
#endif
          pMutex->waiters--;
        }
...
#else
          HB_CRITICAL_UNLOCK(pMutex->mutex);
          (void)HB_COND_TIMEDWAIT(pMutex->cond_w, ulMilliSec); // TODO: C++ cast
          HB_CRITICAL_LOCK(pMutex->mutex);
#endif
        }
#endif
...
#else
          HB_CRITICAL_UNLOCK(pSyncMutex->mutex);
          (void)HB_COND_WAIT(pSyncMutex->cond_l); // TODO: C++ cast
          HB_CRITICAL_LOCK(pSyncMutex->mutex);
#endif
        }
        pSyncMutex->lockers--;
...
#else
          HB_CRITICAL_UNLOCK(pMutex->mutex);
          (void)HB_COND_WAIT(pMutex->cond_l); // TODO: C++ cast
          HB_CRITICAL_LOCK(pMutex->mutex);
#endif
          pMutex->lockers--;
        }
  ...
  if (pMutex) {
    ...
    if (HB_THREAD_EQUAL(pMutex->owner, HB_THREAD_SELF())) {
      ...
      if (ulMilliSec && pMutex->lock_count != 0) {
        ...
        pMutex->lockers++;
        HB_CRITICAL_UNLOCK(pMutex->mutex);
        (void)HB_COND_TIMEDWAIT(pMutex->cond_l, ulMilliSec); // TODO: C++ cast
        HB_CRITICAL_LOCK(pMutex->mutex);
        pMutex->lockers--;
#endif
#endif
  ...
  if (pMutex) {
    ...
    if (!fWaiting || pMutex->waiters) {
      ...
      if (!fWaiting) {
        ...
        auto iLen = static_cast<int32_t>(hb_arrayLen(pMutex->events));
  ...
  if (pItem != nullptr) {
    ...
    if (pEval) {
      ...
      if (hb_threadMutexLock(pItem)) {
        ...
        hb_vmSend(static_cast<uint16_t>(iPCount - 2));
