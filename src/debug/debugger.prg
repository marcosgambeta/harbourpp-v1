...
CREATE CLASS HBDebugger
   ...
   METHOD TabWidth() INLINE ::nTabWidth := ::InputBox("Tab width", ::nTabWidth), ::oBrwText:nTabWidth := ::nTabWidth, ::oBrwText:RefreshAll()
...
METHOD PROCEDURE HBDebugger:OpenDebuggerWindow()
...
METHOD PROCEDURE HBDebugger:CloseDebuggerWindow()
...
METHOD PROCEDURE HBDebugger:Activate()
...
METHOD PROCEDURE HBDebugger:All()
...
METHOD PROCEDURE HBDebugger:BarDisplay()
   ...
   hb_Scroll(::nMaxRow, 0, ::nMaxRow, ::nMaxCol, NIL, NIL, cClrItem)
...
METHOD PROCEDURE HBDebugger:BuildBrowseStack()
...
METHOD PROCEDURE HBDebugger:BuildCommandWindow()
...
METHOD PROCEDURE HBDebugger:CallStackProcessKey(nKey)
...
METHOD PROCEDURE HBDebugger:CodeblockTrace()
...
METHOD PROCEDURE HBDebugger:CodeWindowProcessKey(nKey)
...
METHOD PROCEDURE HBDebugger:Colors()
...
METHOD PROCEDURE HBDebugger:CommandWindowDisplay(cLine, lCmd)
   ...
   hb_DispOutAt(--nRow, ::oWndCommand:nLeft + 1, PadR("> ", nSize), __dbgColors()[2])
   DO WHILE --nRow > ::oWndCommand:nTop
      hb_DispOutAt(nRow, ::oWndCommand:nLeft + 1, PadR(IIf(n > 0, ::aHistCommands[n--], ""), nSize), __dbgColors()[2])
   ENDDO

   RETURN

METHOD PROCEDURE HBDebugger:CommandWindowProcessKey(nKey)
   ...
   CASE K_ENTER
      ...
      IF !Empty(cCommand)
         IF (n := hb_AScan(::aLastCommands, cCommand, NIL, NIL, .T.)) > 0 .AND. n < Len(::aLastCommands)
         ...
         ::DoCommand(cCommand)
      ENDIF
      hb_DispOutAt(::oWndCommand:nBottom - 1, ::oWndCommand:nLeft + 1, PadR("> ", ::oWndCommand:nRight - ::oWndCommand:nLeft - 1), __dbgColors()[2])
...
METHOD HBDebugger:DoCommand(cCommand)
   ...
   CASE hb_LeftEqN("INPUT", cCommand, 4)
      IF Empty(cParams)
         cParams := AllTrim(::InputBox("File name", NIL, {|cFile|hb_FileExists(cFile) .OR. (__dbgAlert("File unavailable"), .F.)}))
         IF LastKey() == K_ESC
            cParams := ""
         ENDIF
      ENDIF
   ...
   CASE hb_LeftEq("OPTIONS", cCommand)
      ...
      CASE hb_LeftEq("LINENUMBERS", cParam1)
         ::LineNumbers()
      CASE hb_LeftEq("EXCHANGESCREENS", cParam1) .OR. hb_LeftEq("SWAPONINPUT", cParam1) .OR. hb_LeftEq("MENUBAR", cParam1)
         ::NotSupported()
   ...
   CASE hb_LeftEq("WINDOW", cCommand)
      ...
      CASE hb_LeftEq("MOVE", cParam1)
         ...
         ELSE
            ...
            ENDIF
            oWindow:Resize(Val(cParam2), n, oWindow:nBottom + Val(cParam2) - oWindow:nTop, oWindow:nRight + n - oWindow:nLeft)
            ::lWindowsAutoSized := .F.
         ENDIF
         ...
         ELSE
            IF Val(cParam2) >= 2 .AND. ;
              (n := At(" ", cParam2)) > 0 .AND. Val(SubStr(cParam2, n)) > 0
               oWindow := ::aWindows[::nCurrentWindow]
               oWindow:Resize(oWindow:nTop, oWindow:nLeft, Val(cParam2) - 1 + oWindow:nTop, Val(SubStr(cParam2, n)) - 1 + oWindow:nLeft)
               ::lWindowsAutoSized := .F.
            ENDIF
         ENDIF
      CASE hb_LeftEq("ZOOM", cParam1) .OR. hb_LeftEq("ICONIZE", cParam1) .OR. hb_LeftEq("TILE", cParam1)
         ::NotSupported()
      OTHERWISE
         cResult := "Command error"
      ENDCASE
   ...
   IF ::lActive
      ...
      hb_DispOutAt(::oWndCommand:nBottom - 1, ::oWndCommand:nLeft + 1, Space(::oWndCommand:nRight - ::oWndCommand:nLeft - 1), __dbgColors()[2])
      IF !Empty(cResult)
...
METHOD PROCEDURE HBDebugger:DoScript(cFileName)
...
METHOD PROCEDURE HBDebugger:EditColor(nColor, oBrwColors)
...
METHOD PROCEDURE HBDebugger:EditSet(nSet, oBrwSets)

   LOCAL cSet  := PadR(__dbgValToExp(Set(nSet)), oBrwSets:getColumn(2):Width)
...
METHOD PROCEDURE HBDebugger:EditVar(nVar)
...
METHOD PROCEDURE HBDebugger:Global()
...
METHOD PROCEDURE HBDebugger:Go()
...
METHOD PROCEDURE HBDebugger:GotoLine(nLine)

   ::oBrwText:GotoLine(nLine)

   RETURN

METHOD PROCEDURE HBDebugger:HandleEvent()
...
METHOD PROCEDURE HBDebugger:Hide()

   ::CloseDebuggerWindow()

   RETURN

METHOD PROCEDURE HBDebugger:HideCallStack()
   ...
   IF ::oWndStack != NIL
      ...
      ::oWndCode:Resize(NIL, NIL, NIL, ::oWndCode:nRight + 16)
      IF ::oWndVars != NIL
         ::oWndVars:Resize(NIL, NIL, NIL, ::oWndVars:nRight + 16)
      ENDIF
      IF ::oWndPnt != NIL
         ::oWndPnt:Resize(NIL, NIL, NIL, ::oWndPnt:nRight + 16)
      ENDIF
      DispEnd()
   ENDIF
...
METHOD PROCEDURE HBDebugger:HideVars()
   ...
   IF ::oWndPnt == NIL
      nTop := 1
   ELSE
      ::oWndPnt:Resize(1, NIL, ::oWndPnt:nBottom - (::oWndPnt:nTop - 1))
      ::oBrwPnt:Resize(2, NIL, ::oWndPnt:nBottom - 1)
...
METHOD HBDebugger:InputBox(cMsg, uValue, bValid, lEditable)
   ...
   ELSE
      ...
      hb_DispOutAt(nTop + 1, nLeft + 1, left(__dbgValToStr(uValue), nRight - nLeft - 1), __dbgColors()[2] + "," + __dbgColors()[5])
...
METHOD PROCEDURE HBDebugger:Inspect(uValue, cValueName)
...
METHOD HBDebugger:ListBox(cCaption, aItems)
   ...
   aColors := __dbgColors()
   n := __dbgAChoice(nTop + 1, nLeft + 1, nBottom - 1, nRight - 1, aItems, aColors[8] + "," + aColors[10])
   oWndList:Hide()

   RETURN n

METHOD PROCEDURE HBDebugger:LoadCallStack()
...
METHOD PROCEDURE HBDebugger:LoadColors()
...
METHOD PROCEDURE HBDebugger:LoadSettings()

   ::DoScript(::cSettingsFileName)

   RETURN

METHOD PROCEDURE HBDebugger:LoadVars() // updates monitored variables
...
METHOD PROCEDURE HBDebugger:Local()
...
METHOD PROCEDURE HBDebugger:MonoDisplay()
...
METHOD PROCEDURE HBDebugger:NextWindow()
...
METHOD PROCEDURE HBDebugger:Open(cFileName)
...
METHOD PROCEDURE HBDebugger:OSShell()
   ...
   SetCursor(SC_NONE)
   __dbgRestScreen(NIL, NIL, NIL, NIL, cImage)
   SetColor(cColors)

   RETURN

METHOD PROCEDURE HBDebugger:Quit()
...
METHOD PROCEDURE HBDebugger:PrevWindow()
...
METHOD PROCEDURE HBDebugger:Private()
...
METHOD PROCEDURE HBDebugger:Public()
...
METHOD PROCEDURE HBDebugger:RefreshVars()
...
METHOD PROCEDURE HBDebugger:RemoveWindow(oWnd)

   LOCAL n

   IF (n := hb_AScan(::aWindows, oWnd, NIL, NIL, .T.)) > 0
   ...
   ELSE
      ...
      IF oWindow2:lVisible
         IF oWindow:nTop < oWindow2:nTop
            ...
            oWindow2:Resize(oWindow:nBottom + 1, NIL, oWindow:nBottom + nTop)
         ELSE
            nTop := oWindow:nBottom - oWindow:nTop + 1
            oWindow:Resize(oWindow2:nBottom + 1, NIL, oWindow2:nBottom + nTop)
         ENDIF
...
METHOD PROCEDURE HBDebugger:RestoreAppScreen()
...
METHOD PROCEDURE HBDebugger:RestoreAppState()
...
METHOD PROCEDURE HBDebugger:RestoreSettings(cFileName)
...
METHOD PROCEDURE HBDebugger:SaveAppScreen()
   ...
   IF ::nMaxRow != MaxRow() .OR. ::nMaxCol != MaxCol()
      ...
      IF ::oWndStack != NIL
         nRight -= 16
         ::oWndStack:Resize(NIL, nRight + 1, ::nMaxRow - ::nCmdWndHight - 3, ::nMaxCol)
      ENDIF
      IF ::oWndVars != NIL
         ::oWndVars:Resize(NIL, NIL, NIL, nRight)
         nTop := Max(nTop, ::oWndVars:nBottom + 1)
      ENDIF
      IF ::oWndPnt != NIL
         ::oWndPnt:Resize(NIL, NIL, NIL, nRight)
...
METHOD PROCEDURE HBDebugger:SaveAppState()
...
METHOD PROCEDURE HBDebugger:SaveSettings(cFileName)
   ...
   FOR EACH aBreak IN __dbgGetBreakPoints(::pInfo)
      cInfo += "BP " + hb_ntos(aBreak[HB_DBG_BP_LINE]) + " " + aBreak[HB_DBG_BP_MODULE] + hb_eol()
   NEXT
...
METHOD PROCEDURE HBDebugger:ShowAllGlobals()
...
METHOD PROCEDURE HBDebugger:ShowAppScreen()
...
METHOD PROCEDURE HBDebugger:ShowCallStack()
...
METHOD PROCEDURE HBDebugger:ShowCodeLine(nProc)
      ...
      IF !Empty(cPrgName)
         ...
         IF !::ModuleMatch(cPrgName, ::cPrgName) .OR. ;
            ...
            IF !hb_FileExists(cPrgName)
               ::oBrwText := NIL
               ::oWndCode:Browser := NIL
               ::oWndCode:SetCaption(::aProcStack[nProc][HB_DBG_CS_MODULE] + "  File not found")
               ::oWndCode:Refresh()
               RETURN
            ENDIF
...
METHOD PROCEDURE HBDebugger:ShowHelp(cTopic)
...
METHOD PROCEDURE HBDebugger:ShowVars()
   ...
   ELSE
      ...
      ENDIF
      IF nBottom != ::oWndVars:nBottom
         ::oWndVars:Resize(NIL, NIL, nBottom)
         lRepaint := .T.
      ELSE
         IF ::oBrwVars != NIL
...
METHOD PROCEDURE HBDebugger:Static()
...
METHOD PROCEDURE HBDebugger:Step()
...
METHOD PROCEDURE HBDebugger:BreakPointToggle(nLine, cFileName)
...
METHOD HBDebugger:BreakPointList()
   ...
   FOR EACH aBreak IN __dbgGetBreakPoints(::pInfo)
      ...
      ::CommandWindowDisplay(hb_ntos(aBreak:__enumIndex() - 1) + ") " + cType, .F.)
   NEXT
...
METHOD HBDebugger:TracepointAdd(cExpr)
   ...
   IF cExpr == NIL
      cExpr := AllTrim(::InputBox("Enter Tracepoint", NIL, __dbgExprValidBlock()))
      IF LastKey() == K_ESC
         RETURN Self
      ENDIF
   ENDIF
...
METHOD PROCEDURE HBDebugger:ViewSets()
...
METHOD HBDebugger:WatchpointAdd(cExpr)
   ...
   IF cExpr == NIL
      cExpr := ::InputBox("Enter Watchpoint", NIL, __dbgExprValidBlock())
      IF LastKey() == K_ESC
         RETURN Self
      ENDIF
   ENDIF
...
METHOD PROCEDURE HBDebugger:WatchpointsHide()
...
METHOD PROCEDURE HBDebugger:WatchpointsShow()
   ...
   ELSE
      ...
      CASE Len(::aWatch) > ::oWndPnt:nBottom - ::oWndPnt:nTop - 1
         ...
         ::oWndPnt:Resize(NIL, NIL, ::oWndPnt:nTop + Min(Len(::aWatch) + 1, 4))
         lRepaint := .T.
      CASE Len(::aWatch) < ::oWndPnt:nBottom - ::oWndPnt:nTop - 1
         ::oWndPnt:Resize(NIL, NIL, ::oWndPnt:nTop + Len(::aWatch) + 1)
         lRepaint := .T.
      OTHERWISE
...
METHOD PROCEDURE HBDebugger:WatchpointList()
   ...
   FOR EACH aWatch IN ::aWatch
      ...
      ENDSWITCH
      ::CommandWindowDisplay(hb_ntos(aWatch:__enumIndex() - 1) + ") " + cType + " " + AllTrim(aWatch[WP_EXPR]), .F.)
   NEXT

   RETURN

METHOD PROCEDURE HBDebugger:WndVarsLButtonDown(nMRow, nMCol)
...
FUNCTION __dbgExprValidBlock(cType)
   ...
   IF HB_ISSTRING(cType)
      ...
      CASE "L" ; cTypeName := "logical"   ; EXIT
      CASE "D" ; cTypeName := "date"      ; EXIT
      CASE "T" ; cTypeName := "timestamp" ; EXIT
      CASE "S" ; cTypeName := "symbol"    ; EXIT
      CASE "A" ; cTypeName := "array"     ; EXIT
      CASE "H" ; cTypeName := "hash"      ; EXIT
      CASE "P" ; cTypeName := "pointer"
      ENDSWITCH
   ENDIF
